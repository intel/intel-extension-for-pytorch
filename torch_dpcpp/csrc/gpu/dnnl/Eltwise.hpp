...
void mkldnn_eltwise (at::Tensor& output, const at::Tensor& input, float alpha, float beta)
{
#if AT_SYCL_ENABLED()
...
template <algorithm alg_kind>
void mkldnn_eltwise_backward(char* diff_src, char* src, char* diff_dst, int32_t len, float alpha, float beta)
{
  ...
  sycl_set_mkldnn_buffer(src, src_usr_memory);

  auto diff_dst_memory  = memory({{{input_tz}, data_t, format_nchw}, engine});
  sycl_set_mkldnn_buffer(diff_dst, diff_dst_memory);

  auto diff_src_memory = memory({{{input_tz}, data_t, format_nchw}, engine});
  sycl_set_mkldnn_buffer(diff_src, diff_src_memory);
