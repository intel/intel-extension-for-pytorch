...
#include <ATen/dpcpp/detail/TensorInfo.h>
...
static SyclOffsetCal<IndexType> make_offset_calculator(const TensorIterator& iter, int n) {
...
void sycl_loops_kernel_impl(TensorIterator& iter, const func_t f) {
  ...
  AT_ASSERTM(traits::arity <= MAX_INPUT_TENSOR_NUM , "loops kernel for", traits::arity, " operands is not generated");
...
#define OFFSET_DEFINE(n) SyclOffsetCal<uint32_t> __off_##n = make_offset_calculator<uint32_t>(iter, n);
  ...
  sycl_queue.submit([&](cl::sycl::handler& cgh) {
    ...
    cgh.parallel_for<KernelName<op_type, arg0_t, arg1_t, arg2_t> >(
            ...
            [=](cl::sycl::nd_item<1> item){
              ...
              if (globalid < rng)
                output_ptr[SyclConvertToActualOffset(arg0_t, globalid * s0)] =
                        op( input1_ptr[SyclConvertToActualOffset(arg1_t, globalid * s1)],
...
void sycl_index_kernel(TensorIterator& iter, IntArrayRef index_size, IntArrayRef index_stride, const func_t f) {
  ...
  auto index_ptrs = at::detail::Array<char*, MAX_TENSORINFO_DIMS>((char*)in_data);
...
#define ACCESSOR_DEFINE(n) in_accessor_t in_acc_##n = in_accessor_t ( __cgh, index_ptrs[n]);
