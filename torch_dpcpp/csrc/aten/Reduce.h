...
#include <assert.h>
#include <ATen/ATen.h>
#include <ATen/dpcpp/Array.h>
#include <ATen/dpcpp/detail/OffsetCalculator.h>
...
struct ReduceConfig {
  ...
  ReduceConfig(int element_size_bytes, int num_outputs, int num_inputs, int num_total,int work_group_size)
...
struct ReduceOp {
  ...
  ReduceOp(ops_t ops, ReduceConfig config,InputCalculator input_calc, OutputCalculator output_calc,
           const void* src, void* dst0, void* dst1, void* buffer, int* semaphores, arg_t ident, int noutputs)
    : ops(ops)
    , ident(ident)
    , config(config)
  ...
  Array<scalar_t, vt0> load_inputs(const dp_global_ptr_pt<scalar_t> &data, index_t offset) const {
    ...
    if (input_calc.dims == 1) {
      return load_memory<vt0, index_t, scalar_t>(data, offset, end, config.step_input, [&](index_t idx) {
        return idx * stride;
      });
    } else {
      return load_memory<vt0, index_t, scalar_t>(data, offset, end, config.step_input, [&](index_t idx) {
  ...
  arg_t global_reduce(arg_t value, const dp_global_ptr_pt<out_scalar_t> &out0,
    ...
    if (should_store) {
      index_t offset = config.staging_memory_offset(item_id.get_group(1), item_id);
    ...
    if (is_last_block_done) {
      ...
      if (config.should_sg_reduce()) {
        index_t input_offset = item_id.get_local_id(0) + item_id.get_local_id(1) * item_id.get_local_range(0);
        index_t step = item_id.get_local_range(0) * item_id.get_local_range(1);
        for (; input_offset < static_cast<index_t>(config.wg_per_output); input_offset += step) {
  ...
  void run( const dp_global_ptr_pt<char> &input,
    ...
    if (config.should_sg_reduce() && (!should_wg_reduce || item_id.get_local_id(1) == 0)) {
      value = sub_group_reduce(value);
    }
...
inline void sycl_reduce_kernel(TensorIterator& iter, const ops_t& ops, ident_t ident=0) {
  ...
  auto config = ReduceConfig(sizeof(arg_t), num_outputs, inputs_per_output, iter.numel(), wg_size);
  ...
  if (config.values_per_thread() >= sgs_per_wg * 16 || config.values_per_thread() >= 256) {
