...
#include "torch_dpcpp/csrc/cpu/OPs.h"
...
#include "torch_dpcpp/csrc/utils.h"
...
void AtenIpexCPUDefault::backward(const at::Tensor & self, const at::Tensor & gradient, bool keep_graph, bool create_graph) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(gradient.layout() == c10::kStrided);
  TORCH_CHECK(gradient.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_gradient = bridge::fallbackToCPUTensor(gradient);
...
void AtenIpexCPUDefault::set_data(const at::Tensor & self, const at::Tensor & new_data) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(new_data.layout() == c10::kStrided);
  TORCH_CHECK(new_data.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_new_data = bridge::fallbackToCPUTensor(new_data);
...
at::Tensor AtenIpexCPUDefault::_masked_scale(const at::Tensor & self, const at::Tensor & mask, double scale) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(mask.layout() == c10::kStrided);
  TORCH_CHECK(mask.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_mask = bridge::fallbackToCPUTensor(mask);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::_sobol_engine_draw(const at::Tensor & quasi, int64_t n, const at::Tensor & sobolstate, int64_t dimension, int64_t num_generated, c10::optional<at::ScalarType> dtype) {
  TORCH_CHECK(quasi.layout() == c10::kStrided);
  TORCH_CHECK(quasi.is_contiguous());
  TORCH_CHECK(sobolstate.layout() == c10::kStrided);
  TORCH_CHECK(sobolstate.is_contiguous());
  auto&& _ipex_quasi = bridge::fallbackToCPUTensor(quasi);
  auto&& _ipex_sobolstate = bridge::fallbackToCPUTensor(sobolstate);
...
at::Tensor & AtenIpexCPUDefault::_sobol_engine_scramble_(at::Tensor & self, const at::Tensor & ltm, int64_t dimension) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(ltm.layout() == c10::kStrided);
  TORCH_CHECK(ltm.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_ltm = bridge::fallbackToCPUTensor(ltm);
...
at::Tensor AtenIpexCPUDefault::_reshape_from_tensor(const at::Tensor & self, const at::Tensor & shape) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(shape.layout() == c10::kStrided);
  TORCH_CHECK(shape.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_shape = bridge::fallbackToCPUTensor(shape);
...
at::Tensor AtenIpexCPUDefault::dropout(const at::Tensor & input, double p, bool train) {
  ...
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
...
at::Tensor & AtenIpexCPUDefault::abs_out(at::Tensor & out, const at::Tensor & self) {
  TORCH_CHECK(out.layout() == c10::kStrided);
  TORCH_CHECK(out.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_out = bridge::fallbackToCPUTensor(out);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor AtenIpexCPUDefault::add(const at::Tensor & self, const at::Tensor & other, at::Scalar alpha) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(other.layout() == c10::kStrided);
  TORCH_CHECK(other.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_other = bridge::fallbackToCPUTensor(other);
...
at::Tensor AtenIpexCPUDefault::addmv(const at::Tensor & self, const at::Tensor & mat, const at::Tensor & vec, at::Scalar beta, at::Scalar alpha) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(mat.layout() == c10::kStrided);
  TORCH_CHECK(mat.is_contiguous());
  TORCH_CHECK(vec.layout() == c10::kStrided);
  TORCH_CHECK(vec.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_mat = bridge::fallbackToCPUTensor(mat);
  auto&& _ipex_vec = bridge::fallbackToCPUTensor(vec);
...
at::Tensor AtenIpexCPUDefault::addr(const at::Tensor & self, const at::Tensor & vec1, const at::Tensor & vec2, at::Scalar beta, at::Scalar alpha) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(vec1.layout() == c10::kStrided);
  TORCH_CHECK(vec1.is_contiguous());
  TORCH_CHECK(vec2.layout() == c10::kStrided);
  TORCH_CHECK(vec2.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_vec1 = bridge::fallbackToCPUTensor(vec1);
  auto&& _ipex_vec2 = bridge::fallbackToCPUTensor(vec2);
...
at::Tensor AtenIpexCPUDefault::affine_grid_generator(const at::Tensor & theta, at::IntArrayRef size, bool align_corners) {
  TORCH_CHECK(theta.layout() == c10::kStrided);
  TORCH_CHECK(theta.is_contiguous());
  auto&& _ipex_theta = bridge::fallbackToCPUTensor(theta);
...
at::Tensor AtenIpexCPUDefault::affine_grid_generator_backward(const at::Tensor & grad, at::IntArrayRef size, bool align_corners) {
  TORCH_CHECK(grad.layout() == c10::kStrided);
  TORCH_CHECK(grad.is_contiguous());
  auto&& _ipex_grad = bridge::fallbackToCPUTensor(grad);
...
at::Tensor AtenIpexCPUDefault::_dim_arange(const at::Tensor & like, int64_t dim) {
  TORCH_CHECK(like.layout() == c10::kStrided);
  TORCH_CHECK(like.is_contiguous());
  auto&& _ipex_like = bridge::fallbackToCPUTensor(like);
...
at::Tensor AtenIpexCPUDefault::baddbmm(const at::Tensor & self, const at::Tensor & batch1, const at::Tensor & batch2, at::Scalar beta, at::Scalar alpha) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(batch1.layout() == c10::kStrided);
  TORCH_CHECK(batch1.is_contiguous());
  TORCH_CHECK(batch2.layout() == c10::kStrided);
  TORCH_CHECK(batch2.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_batch1 = bridge::fallbackToCPUTensor(batch1);
  auto&& _ipex_batch2 = bridge::fallbackToCPUTensor(batch2);
...
at::Tensor AtenIpexCPUDefault::batch_norm(const at::Tensor & input, const at::Tensor & weight, const at::Tensor & bias, const at::Tensor & running_mean, const at::Tensor & running_var, bool training, double momentum, double eps, bool cudnn_enabled) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(bias.layout() == c10::kStrided);
  TORCH_CHECK(bias.is_contiguous());
  TORCH_CHECK(running_mean.layout() == c10::kStrided);
  TORCH_CHECK(running_mean.is_contiguous());
  TORCH_CHECK(running_var.layout() == c10::kStrided);
  TORCH_CHECK(running_var.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_bias = bridge::fallbackToCPUTensor(bias);
  auto&& _ipex_running_mean = bridge::fallbackToCPUTensor(running_mean);
  auto&& _ipex_running_var = bridge::fallbackToCPUTensor(running_var);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::_batch_norm_impl_index_backward(int64_t impl_index, const at::Tensor & input, const at::Tensor & grad_output, const at::Tensor & weight, const at::Tensor & running_mean, const at::Tensor & running_var, const at::Tensor & save_mean, const at::Tensor & save_var_transform, bool train, double eps, std::array<bool,3> output_mask, const at::Tensor & reservedSpace) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(grad_output.layout() == c10::kStrided);
  ...
  TORCH_CHECK(running_var.layout() == c10::kStrided);
  TORCH_CHECK(running_var.is_contiguous());
  TORCH_CHECK(save_mean.layout() == c10::kStrided);
  TORCH_CHECK(save_mean.is_contiguous());
  TORCH_CHECK(save_var_transform.layout() == c10::kStrided);
  TORCH_CHECK(save_var_transform.is_contiguous());
  TORCH_CHECK(reservedSpace.layout() == c10::kStrided);
  TORCH_CHECK(reservedSpace.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_running_mean = bridge::fallbackToCPUTensor(running_mean);
  auto&& _ipex_running_var = bridge::fallbackToCPUTensor(running_var);
  auto&& _ipex_save_mean = bridge::fallbackToCPUTensor(save_mean);
  auto&& _ipex_save_var_transform = bridge::fallbackToCPUTensor(save_var_transform);
  auto&& _ipex_reservedSpace = bridge::fallbackToCPUTensor(reservedSpace);
...
at::Tensor & AtenIpexCPUDefault::bernoulli_(at::Tensor & self, const at::Tensor & p, at::Generator * generator) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(p.layout() == c10::kStrided);
  TORCH_CHECK(p.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_p = bridge::fallbackToCPUTensor(p);
...
at::Tensor AtenIpexCPUDefault::bilinear(const at::Tensor & input1, const at::Tensor & input2, const at::Tensor & weight, const at::Tensor & bias) {
  TORCH_CHECK(input1.layout() == c10::kStrided);
  TORCH_CHECK(input1.is_contiguous());
  TORCH_CHECK(input2.layout() == c10::kStrided);
  TORCH_CHECK(input2.is_contiguous());
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(bias.layout() == c10::kStrided);
  TORCH_CHECK(bias.is_contiguous());
  auto&& _ipex_input1 = bridge::fallbackToCPUTensor(input1);
  auto&& _ipex_input2 = bridge::fallbackToCPUTensor(input2);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_bias = bridge::fallbackToCPUTensor(bias);
...
at::Tensor AtenIpexCPUDefault::binary_cross_entropy_with_logits(const at::Tensor & self, const at::Tensor & target, const at::Tensor & weight, const at::Tensor & pos_weight, int64_t reduction) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(target.layout() == c10::kStrided);
  TORCH_CHECK(target.is_contiguous());
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(pos_weight.layout() == c10::kStrided);
  TORCH_CHECK(pos_weight.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_target = bridge::fallbackToCPUTensor(target);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_pos_weight = bridge::fallbackToCPUTensor(pos_weight);
...
at::Tensor AtenIpexCPUDefault::bincount(const at::Tensor & self, const at::Tensor & weights, int64_t minlength) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(weights.layout() == c10::kStrided);
  TORCH_CHECK(weights.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_weights = bridge::fallbackToCPUTensor(weights);
...
at::Tensor AtenIpexCPUDefault::bmm(const at::Tensor & self, const at::Tensor & mat2) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(mat2.layout() == c10::kStrided);
  TORCH_CHECK(mat2.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_mat2 = bridge::fallbackToCPUTensor(mat2);
...
std::vector<at::Tensor> AtenIpexCPUDefault::broadcast_tensors(at::TensorList tensors) {
  auto&& _ipex_tensors = bridge::fallbackToCPUTensorList(tensors);
...
at::Tensor AtenIpexCPUDefault::chain_matmul(at::TensorList matrices) {
  auto&& _ipex_matrices = bridge::fallbackToCPUTensorList(matrices);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::_convolution_double_backward(const at::Tensor & ggI, const at::Tensor & ggW, const at::Tensor & ggb, const at::Tensor & gO, const at::Tensor & weight, const at::Tensor & self, at::IntArrayRef stride, at::IntArrayRef padding, at::IntArrayRef dilation, bool transposed, at::IntArrayRef output_padding, int64_t groups, bool benchmark, bool deterministic, bool cudnn_enabled, std::array<bool,3> output_mask) {
  TORCH_CHECK(ggI.layout() == c10::kStrided);
  TORCH_CHECK(ggI.is_contiguous());
  TORCH_CHECK(ggW.layout() == c10::kStrided);
  TORCH_CHECK(ggW.is_contiguous());
  TORCH_CHECK(ggb.layout() == c10::kStrided);
  TORCH_CHECK(ggb.is_contiguous());
  TORCH_CHECK(gO.layout() == c10::kStrided);
  TORCH_CHECK(gO.is_contiguous());
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_ggI = bridge::fallbackToCPUTensor(ggI);
  auto&& _ipex_ggW = bridge::fallbackToCPUTensor(ggW);
  auto&& _ipex_ggb = bridge::fallbackToCPUTensor(ggb);
  auto&& _ipex_gO = bridge::fallbackToCPUTensor(gO);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor & AtenIpexCPUDefault::copy_(at::Tensor & self, const at::Tensor & src, bool non_blocking) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(src.layout() == c10::kStrided);
  TORCH_CHECK(src.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_src = bridge::fallbackToCPUTensor(src);
...
at::Tensor AtenIpexCPUDefault::_copy_from(const at::Tensor & self, const at::Tensor & dst, bool non_blocking) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(dst.layout() == c10::kStrided);
  TORCH_CHECK(dst.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_dst = bridge::fallbackToCPUTensor(dst);
...
at::Tensor AtenIpexCPUDefault::ctc_loss(const at::Tensor & log_probs, const at::Tensor & targets, at::IntArrayRef input_lengths, at::IntArrayRef target_lengths, int64_t blank, int64_t reduction, bool zero_infinity) {
  TORCH_CHECK(log_probs.layout() == c10::kStrided);
  TORCH_CHECK(log_probs.is_contiguous());
  TORCH_CHECK(targets.layout() == c10::kStrided);
  TORCH_CHECK(targets.is_contiguous());
  auto&& _ipex_log_probs = bridge::fallbackToCPUTensor(log_probs);
  auto&& _ipex_targets = bridge::fallbackToCPUTensor(targets);
...
at::Tensor AtenIpexCPUDefault::ctc_loss(const at::Tensor & log_probs, const at::Tensor & targets, const at::Tensor & input_lengths, const at::Tensor & target_lengths, int64_t blank, int64_t reduction, bool zero_infinity) {
  ...
  TORCH_CHECK(targets.layout() == c10::kStrided);
  TORCH_CHECK(targets.is_contiguous());
  TORCH_CHECK(input_lengths.layout() == c10::kStrided);
  TORCH_CHECK(input_lengths.is_contiguous());
  TORCH_CHECK(target_lengths.layout() == c10::kStrided);
  TORCH_CHECK(target_lengths.is_contiguous());
  auto&& _ipex_log_probs = bridge::fallbackToCPUTensor(log_probs);
  auto&& _ipex_targets = bridge::fallbackToCPUTensor(targets);
  auto&& _ipex_input_lengths = bridge::fallbackToCPUTensor(input_lengths);
  auto&& _ipex_target_lengths = bridge::fallbackToCPUTensor(target_lengths);
...
at::Tensor AtenIpexCPUDefault::_ctc_loss_backward(const at::Tensor & grad, const at::Tensor & log_probs, const at::Tensor & targets, at::IntArrayRef input_lengths, at::IntArrayRef target_lengths, const at::Tensor & neg_log_likelihood, const at::Tensor & log_alpha, int64_t blank, bool zero_infinity) {
  ...
  TORCH_CHECK(targets.layout() == c10::kStrided);
  TORCH_CHECK(targets.is_contiguous());
  TORCH_CHECK(neg_log_likelihood.layout() == c10::kStrided);
  TORCH_CHECK(neg_log_likelihood.is_contiguous());
  TORCH_CHECK(log_alpha.layout() == c10::kStrided);
  TORCH_CHECK(log_alpha.is_contiguous());
  auto&& _ipex_grad = bridge::fallbackToCPUTensor(grad);
  auto&& _ipex_log_probs = bridge::fallbackToCPUTensor(log_probs);
  auto&& _ipex_targets = bridge::fallbackToCPUTensor(targets);
  auto&& _ipex_neg_log_likelihood = bridge::fallbackToCPUTensor(neg_log_likelihood);
  auto&& _ipex_log_alpha = bridge::fallbackToCPUTensor(log_alpha);
...
at::Tensor AtenIpexCPUDefault::dot(const at::Tensor & self, const at::Tensor & tensor) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(tensor.layout() == c10::kStrided);
  TORCH_CHECK(tensor.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_tensor = bridge::fallbackToCPUTensor(tensor);
...
at::Tensor AtenIpexCPUDefault::embedding(const at::Tensor & weight, const at::Tensor & indices, int64_t padding_idx, bool scale_grad_by_freq, bool sparse) {
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(indices.layout() == c10::kStrided);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::embedding_bag(const at::Tensor & weight, const at::Tensor & indices, const at::Tensor & offsets, bool scale_grad_by_freq, int64_t mode, bool sparse, const at::Tensor & per_sample_weights) {
  ...
  TORCH_CHECK(indices.layout() == c10::kStrided);
  TORCH_CHECK(indices.is_contiguous());
  TORCH_CHECK(offsets.layout() == c10::kStrided);
  TORCH_CHECK(offsets.is_contiguous());
  TORCH_CHECK(per_sample_weights.layout() == c10::kStrided);
  TORCH_CHECK(per_sample_weights.is_contiguous());
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_indices = bridge::fallbackToCPUTensor(indices);
  auto&& _ipex_offsets = bridge::fallbackToCPUTensor(offsets);
  auto&& _ipex_per_sample_weights = bridge::fallbackToCPUTensor(per_sample_weights);
...
at::Tensor AtenIpexCPUDefault::_embedding_bag_backward(const at::Tensor & grad, const at::Tensor & indices, const at::Tensor & offsets, const at::Tensor & offset2bag, const at::Tensor & bag_size, const at::Tensor & maximum_indices, int64_t num_weights, bool scale_grad_by_freq, int64_t mode, bool sparse, const at::Tensor & per_sample_weights) {
  ...
  TORCH_CHECK(offsets.layout() == c10::kStrided);
  TORCH_CHECK(offsets.is_contiguous());
  TORCH_CHECK(offset2bag.layout() == c10::kStrided);
  TORCH_CHECK(offset2bag.is_contiguous());
  TORCH_CHECK(bag_size.layout() == c10::kStrided);
  TORCH_CHECK(bag_size.is_contiguous());
  TORCH_CHECK(maximum_indices.layout() == c10::kStrided);
  TORCH_CHECK(maximum_indices.is_contiguous());
  TORCH_CHECK(per_sample_weights.layout() == c10::kStrided);
  TORCH_CHECK(per_sample_weights.is_contiguous());
  auto&& _ipex_grad = bridge::fallbackToCPUTensor(grad);
  auto&& _ipex_indices = bridge::fallbackToCPUTensor(indices);
  auto&& _ipex_offsets = bridge::fallbackToCPUTensor(offsets);
  auto&& _ipex_offset2bag = bridge::fallbackToCPUTensor(offset2bag);
  auto&& _ipex_bag_size = bridge::fallbackToCPUTensor(bag_size);
  auto&& _ipex_maximum_indices = bridge::fallbackToCPUTensor(maximum_indices);
  auto&& _ipex_per_sample_weights = bridge::fallbackToCPUTensor(per_sample_weights);
...
at::Tensor AtenIpexCPUDefault::empty(at::IntArrayRef size, const at::TensorOptions & options, c10::optional<at::MemoryFormat> memory_format) {
  TORCH_CHECK(options.device().type() == at::DeviceType::DPCPP);
  at::TensorOptions _ipex_options = options.device(at::DeviceType::CPU);
  TORCH_CHECK(memory_format.value_or(c10::MemoryFormat::Contiguous) == c10::MemoryFormat::Contiguous);
...
at::Tensor AtenIpexCPUDefault::_empty_per_channel_affine_quantized(at::IntArrayRef size, const at::Tensor & scales, const at::Tensor & zero_points, int64_t axis, const at::TensorOptions & options, c10::optional<at::MemoryFormat> memory_format) {
  TORCH_CHECK(scales.layout() == c10::kStrided);
...
at::Tensor & AtenIpexCPUDefault::fill_(at::Tensor & self, const at::Tensor & value) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(value.layout() == c10::kStrided);
  TORCH_CHECK(value.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_value = bridge::fallbackToCPUTensor(value);
...
at::Tensor AtenIpexCPUDefault::grid_sampler(const at::Tensor & input, const at::Tensor & grid, int64_t interpolation_mode, int64_t padding_mode, bool align_corners) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(grid.layout() == c10::kStrided);
  TORCH_CHECK(grid.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_grid = bridge::fallbackToCPUTensor(grid);
...
at::Tensor AtenIpexCPUDefault::index(const at::Tensor & self, at::TensorList indices) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_indices = bridge::fallbackToCPUTensorList(indices);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor & AtenIpexCPUDefault::index_copy_(at::Tensor & self, int64_t dim, const at::Tensor & index, const at::Tensor & source) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(index.layout() == c10::kStrided);
  TORCH_CHECK(index.is_contiguous());
  TORCH_CHECK(source.layout() == c10::kStrided);
  TORCH_CHECK(source.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_index = bridge::fallbackToCPUTensor(index);
  auto&& _ipex_source = bridge::fallbackToCPUTensor(source);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::native_layer_norm_backward(const at::Tensor & grad_out, const at::Tensor & input, const at::Tensor & mean, const at::Tensor & rstd, const at::Tensor & weight, int64_t M, int64_t N, std::array<bool,3> output_mask) {
  TORCH_CHECK(grad_out.layout() == c10::kStrided);
  TORCH_CHECK(grad_out.is_contiguous());
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(mean.layout() == c10::kStrided);
  TORCH_CHECK(mean.is_contiguous());
  TORCH_CHECK(rstd.layout() == c10::kStrided);
  TORCH_CHECK(rstd.is_contiguous());
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  auto&& _ipex_grad_out = bridge::fallbackToCPUTensor(grad_out);
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_mean = bridge::fallbackToCPUTensor(mean);
  auto&& _ipex_rstd = bridge::fallbackToCPUTensor(rstd);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
...
at::Tensor AtenIpexCPUDefault::fbgemm_linear_int8_weight_fp32_activation(const at::Tensor & input, const at::Tensor & weight, const at::Tensor & packed, const at::Tensor & col_offsets, at::Scalar weight_scale, at::Scalar weight_zero_point, const at::Tensor & bias) {
  ...
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(packed.layout() == c10::kStrided);
  TORCH_CHECK(packed.is_contiguous());
  TORCH_CHECK(col_offsets.layout() == c10::kStrided);
  TORCH_CHECK(col_offsets.is_contiguous());
  TORCH_CHECK(bias.layout() == c10::kStrided);
  TORCH_CHECK(bias.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_packed = bridge::fallbackToCPUTensor(packed);
  auto&& _ipex_col_offsets = bridge::fallbackToCPUTensor(col_offsets);
  auto&& _ipex_bias = bridge::fallbackToCPUTensor(bias);
...
at::Tensor AtenIpexCPUDefault::fbgemm_linear_fp16_weight_fp32_activation(const at::Tensor & input, const at::Tensor & packed_weight, const at::Tensor & bias) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(packed_weight.layout() == c10::kStrided);
  TORCH_CHECK(packed_weight.is_contiguous());
  TORCH_CHECK(bias.layout() == c10::kStrided);
  TORCH_CHECK(bias.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_packed_weight = bridge::fallbackToCPUTensor(packed_weight);
  auto&& _ipex_bias = bridge::fallbackToCPUTensor(bias);
...
at::Tensor AtenIpexCPUDefault::_log_softmax_backward_data(const at::Tensor & grad_output, const at::Tensor & output, int64_t dim, const at::Tensor & self) {
  TORCH_CHECK(grad_output.layout() == c10::kStrided);
  TORCH_CHECK(grad_output.is_contiguous());
  TORCH_CHECK(output.layout() == c10::kStrided);
  TORCH_CHECK(output.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::max_out(at::Tensor & max, at::Tensor & max_values, const at::Tensor & self, int64_t dim, bool keepdim) {
  TORCH_CHECK(max.layout() == c10::kStrided);
  TORCH_CHECK(max.is_contiguous());
  TORCH_CHECK(max_values.layout() == c10::kStrided);
  TORCH_CHECK(max_values.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_max = bridge::fallbackToCPUTensor(max);
  auto&& _ipex_max_values = bridge::fallbackToCPUTensor(max_values);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::min_out(at::Tensor & min, at::Tensor & min_indices, const at::Tensor & self, int64_t dim, bool keepdim) {
  TORCH_CHECK(min.layout() == c10::kStrided);
  TORCH_CHECK(min.is_contiguous());
  TORCH_CHECK(min_indices.layout() == c10::kStrided);
  TORCH_CHECK(min_indices.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_min = bridge::fallbackToCPUTensor(min);
  auto&& _ipex_min_indices = bridge::fallbackToCPUTensor(min_indices);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::miopen_batch_norm_backward(const at::Tensor & input, const at::Tensor & grad_output, const at::Tensor & weight, const at::Tensor & running_mean, const at::Tensor & running_var, const at::Tensor & save_mean, const at::Tensor & save_var, double epsilon) {
  ...
  TORCH_CHECK(save_mean.layout() == c10::kStrided);
  TORCH_CHECK(save_mean.is_contiguous());
  TORCH_CHECK(save_var.layout() == c10::kStrided);
  TORCH_CHECK(save_var.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_running_mean = bridge::fallbackToCPUTensor(running_mean);
  auto&& _ipex_running_var = bridge::fallbackToCPUTensor(running_var);
  auto&& _ipex_save_mean = bridge::fallbackToCPUTensor(save_mean);
  auto&& _ipex_save_var = bridge::fallbackToCPUTensor(save_var);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::miopen_rnn(const at::Tensor & input, at::TensorList weight, int64_t weight_stride0, const at::Tensor & hx, const at::Tensor & cx, int64_t mode, int64_t hidden_size, int64_t num_layers, bool batch_first, double dropout, bool train, bool bidirectional, at::IntArrayRef batch_sizes, const at::Tensor & dropout_state) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  auto&& _ipex_weight = bridge::fallbackToCPUTensorList(weight);
  TORCH_CHECK(hx.layout() == c10::kStrided);
  TORCH_CHECK(hx.is_contiguous());
  TORCH_CHECK(cx.layout() == c10::kStrided);
  TORCH_CHECK(cx.is_contiguous());
  TORCH_CHECK(dropout_state.layout() == c10::kStrided);
  TORCH_CHECK(dropout_state.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_hx = bridge::fallbackToCPUTensor(hx);
  auto&& _ipex_cx = bridge::fallbackToCPUTensor(cx);
  auto&& _ipex_dropout_state = bridge::fallbackToCPUTensor(dropout_state);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor,std::vector<at::Tensor>> AtenIpexCPUDefault::miopen_rnn_backward(const at::Tensor & input, at::TensorList weight, int64_t weight_stride0, const at::Tensor & weight_buf, const at::Tensor & hx, const at::Tensor & cx, const at::Tensor & output, const at::Tensor & grad_output, const at::Tensor & grad_hy, const at::Tensor & grad_cy, int64_t mode, int64_t hidden_size, int64_t num_layers, bool batch_first, double dropout, bool train, bool bidirectional, at::IntArrayRef batch_sizes, const at::Tensor & dropout_state, const at::Tensor & reserve, std::array<bool,4> output_mask) {
  ...
  TORCH_CHECK(input.is_contiguous());
  auto&& _ipex_weight = bridge::fallbackToCPUTensorList(weight);
  TORCH_CHECK(weight_buf.layout() == c10::kStrided);
  TORCH_CHECK(weight_buf.is_contiguous());
  TORCH_CHECK(hx.layout() == c10::kStrided);
  TORCH_CHECK(hx.is_contiguous());
  TORCH_CHECK(cx.layout() == c10::kStrided);
  TORCH_CHECK(cx.is_contiguous());
  ...
  TORCH_CHECK(grad_output.layout() == c10::kStrided);
  TORCH_CHECK(grad_output.is_contiguous());
  TORCH_CHECK(grad_hy.layout() == c10::kStrided);
  TORCH_CHECK(grad_hy.is_contiguous());
  TORCH_CHECK(grad_cy.layout() == c10::kStrided);
  TORCH_CHECK(grad_cy.is_contiguous());
  TORCH_CHECK(dropout_state.layout() == c10::kStrided);
  TORCH_CHECK(dropout_state.is_contiguous());
  TORCH_CHECK(reserve.layout() == c10::kStrided);
  TORCH_CHECK(reserve.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_weight_buf = bridge::fallbackToCPUTensor(weight_buf);
  auto&& _ipex_hx = bridge::fallbackToCPUTensor(hx);
  auto&& _ipex_cx = bridge::fallbackToCPUTensor(cx);
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_grad_hy = bridge::fallbackToCPUTensor(grad_hy);
  auto&& _ipex_grad_cy = bridge::fallbackToCPUTensor(grad_cy);
  auto&& _ipex_dropout_state = bridge::fallbackToCPUTensor(dropout_state);
  auto&& _ipex_reserve = bridge::fallbackToCPUTensor(reserve);
...
at::Tensor AtenIpexCPUDefault::_sparse_mm(const at::Tensor & sparse, const at::Tensor & dense) {
  TORCH_CHECK(sparse.layout() == c10::kStrided);
  TORCH_CHECK(sparse.is_contiguous());
  TORCH_CHECK(dense.layout() == c10::kStrided);
  TORCH_CHECK(dense.is_contiguous());
  auto&& _ipex_sparse = bridge::fallbackToCPUTensor(sparse);
  auto&& _ipex_dense = bridge::fallbackToCPUTensor(dense);
...
at::Tensor AtenIpexCPUDefault::batch_norm_elemt(const at::Tensor & input, const at::Tensor & weight, const at::Tensor & bias, const at::Tensor & mean, const at::Tensor & invstd, double eps) {
  ...
  TORCH_CHECK(mean.layout() == c10::kStrided);
  TORCH_CHECK(mean.is_contiguous());
  TORCH_CHECK(invstd.layout() == c10::kStrided);
  TORCH_CHECK(invstd.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_bias = bridge::fallbackToCPUTensor(bias);
  auto&& _ipex_mean = bridge::fallbackToCPUTensor(mean);
  auto&& _ipex_invstd = bridge::fallbackToCPUTensor(invstd);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::native_batch_norm_backward(const at::Tensor & grad_out, const at::Tensor & input, const at::Tensor & weight, const at::Tensor & running_mean, const at::Tensor & running_var, const at::Tensor & save_mean, const at::Tensor & save_invstd, bool train, double eps, std::array<bool,3> output_mask) {
  ...
  TORCH_CHECK(save_mean.layout() == c10::kStrided);
  TORCH_CHECK(save_mean.is_contiguous());
  TORCH_CHECK(save_invstd.layout() == c10::kStrided);
  ...
  auto&& _ipex_running_var = bridge::fallbackToCPUTensor(running_var);
  auto&& _ipex_save_mean = bridge::fallbackToCPUTensor(save_mean);
  auto&& _ipex_save_invstd = bridge::fallbackToCPUTensor(save_invstd);
...
at::Tensor AtenIpexCPUDefault::batch_norm_backward_elemt(const at::Tensor & grad_out, const at::Tensor & input, const at::Tensor & mean, const at::Tensor & invstd, const at::Tensor & weight, const at::Tensor & mean_dy, const at::Tensor & mean_dy_xmu) {
  ...
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(mean_dy.layout() == c10::kStrided);
  TORCH_CHECK(mean_dy.is_contiguous());
  TORCH_CHECK(mean_dy_xmu.layout() == c10::kStrided);
  TORCH_CHECK(mean_dy_xmu.is_contiguous());
  auto&& _ipex_grad_out = bridge::fallbackToCPUTensor(grad_out);
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_mean = bridge::fallbackToCPUTensor(mean);
  auto&& _ipex_invstd = bridge::fallbackToCPUTensor(invstd);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_mean_dy = bridge::fallbackToCPUTensor(mean_dy);
  auto&& _ipex_mean_dy_xmu = bridge::fallbackToCPUTensor(mean_dy_xmu);
...
at::Tensor AtenIpexCPUDefault::pairwise_distance(const at::Tensor & x1, const at::Tensor & x2, double p, double eps, bool keepdim) {
  TORCH_CHECK(x1.layout() == c10::kStrided);
  TORCH_CHECK(x1.is_contiguous());
  TORCH_CHECK(x2.layout() == c10::kStrided);
  TORCH_CHECK(x2.is_contiguous());
  auto&& _ipex_x1 = bridge::fallbackToCPUTensor(x1);
  auto&& _ipex_x2 = bridge::fallbackToCPUTensor(x2);
...
at::Tensor AtenIpexCPUDefault::_cdist_backward(const at::Tensor & grad, const at::Tensor & x1, const at::Tensor & x2, double p, const at::Tensor & cdist) {
  ...
  TORCH_CHECK(x2.layout() == c10::kStrided);
  TORCH_CHECK(x2.is_contiguous());
  TORCH_CHECK(cdist.layout() == c10::kStrided);
  TORCH_CHECK(cdist.is_contiguous());
  auto&& _ipex_grad = bridge::fallbackToCPUTensor(grad);
  auto&& _ipex_x1 = bridge::fallbackToCPUTensor(x1);
  auto&& _ipex_x2 = bridge::fallbackToCPUTensor(x2);
  auto&& _ipex_cdist = bridge::fallbackToCPUTensor(cdist);
...
at::Tensor AtenIpexCPUDefault::_pdist_backward(const at::Tensor & grad, const at::Tensor & self, double p, const at::Tensor & pdist) {
  ...
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(pdist.layout() == c10::kStrided);
  TORCH_CHECK(pdist.is_contiguous());
  auto&& _ipex_grad = bridge::fallbackToCPUTensor(grad);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_pdist = bridge::fallbackToCPUTensor(pdist);
...
at::Tensor AtenIpexCPUDefault::repeat_interleave(const at::Tensor & repeats) {
  TORCH_CHECK(repeats.layout() == c10::kStrided);
  TORCH_CHECK(repeats.is_contiguous());
  auto&& _ipex_repeats = bridge::fallbackToCPUTensor(repeats);
...
at::Tensor AtenIpexCPUDefault::sspaddmm(const at::Tensor & self, const at::Tensor & mat1, const at::Tensor & mat2, at::Scalar beta, at::Scalar alpha) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(mat1.layout() == c10::kStrided);
  TORCH_CHECK(mat1.is_contiguous());
  TORCH_CHECK(mat2.layout() == c10::kStrided);
  TORCH_CHECK(mat2.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_mat1 = bridge::fallbackToCPUTensor(mat1);
  auto&& _ipex_mat2 = bridge::fallbackToCPUTensor(mat2);
...
at::Tensor AtenIpexCPUDefault::stft(const at::Tensor & self, int64_t n_fft, c10::optional<int64_t> hop_length, c10::optional<int64_t> win_length, const at::Tensor & window, bool normalized, bool onesided) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(window.layout() == c10::kStrided);
  TORCH_CHECK(window.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_window = bridge::fallbackToCPUTensor(window);
...
at::Tensor AtenIpexCPUDefault::trapz(const at::Tensor & y, const at::Tensor & x, int64_t dim) {
  TORCH_CHECK(y.layout() == c10::kStrided);
  TORCH_CHECK(y.is_contiguous());
  TORCH_CHECK(x.layout() == c10::kStrided);
  TORCH_CHECK(x.is_contiguous());
  auto&& _ipex_y = bridge::fallbackToCPUTensor(y);
  auto&& _ipex_x = bridge::fallbackToCPUTensor(x);
...
at::Tensor AtenIpexCPUDefault::_trilinear(const at::Tensor & i1, const at::Tensor & i2, const at::Tensor & i3, at::IntArrayRef expand1, at::IntArrayRef expand2, at::IntArrayRef expand3, at::IntArrayRef sumdim, int64_t unroll_dim) {
  TORCH_CHECK(i1.layout() == c10::kStrided);
  TORCH_CHECK(i1.is_contiguous());
  TORCH_CHECK(i2.layout() == c10::kStrided);
  TORCH_CHECK(i2.is_contiguous());
  TORCH_CHECK(i3.layout() == c10::kStrided);
  TORCH_CHECK(i3.is_contiguous());
  auto&& _ipex_i1 = bridge::fallbackToCPUTensor(i1);
  auto&& _ipex_i2 = bridge::fallbackToCPUTensor(i2);
  auto&& _ipex_i3 = bridge::fallbackToCPUTensor(i3);
...
at::Tensor AtenIpexCPUDefault::triplet_margin_loss(const at::Tensor & anchor, const at::Tensor & positive, const at::Tensor & negative, double margin, double p, double eps, bool swap, int64_t reduction) {
  TORCH_CHECK(anchor.layout() == c10::kStrided);
  TORCH_CHECK(anchor.is_contiguous());
  TORCH_CHECK(positive.layout() == c10::kStrided);
  TORCH_CHECK(positive.is_contiguous());
  TORCH_CHECK(negative.layout() == c10::kStrided);
  TORCH_CHECK(negative.is_contiguous());
  auto&& _ipex_anchor = bridge::fallbackToCPUTensor(anchor);
  auto&& _ipex_positive = bridge::fallbackToCPUTensor(positive);
  auto&& _ipex_negative = bridge::fallbackToCPUTensor(negative);
...
bool AtenIpexCPUDefault::_has_compatible_shallow_copy_type(const at::Tensor & self, const at::Tensor & from) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(from.layout() == c10::kStrided);
  TORCH_CHECK(from.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_from = bridge::fallbackToCPUTensor(from);
...
at::Tensor AtenIpexCPUDefault::where(const at::Tensor & condition, const at::Tensor & self, const at::Tensor & other) {
  TORCH_CHECK(condition.layout() == c10::kStrided);
  TORCH_CHECK(condition.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(other.layout() == c10::kStrided);
  TORCH_CHECK(other.is_contiguous());
  auto&& _ipex_condition = bridge::fallbackToCPUTensor(condition);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_other = bridge::fallbackToCPUTensor(other);
...
at::Tensor AtenIpexCPUDefault::norm_except_dim(const at::Tensor & v, int64_t pow, int64_t dim) {
  TORCH_CHECK(v.layout() == c10::kStrided);
  TORCH_CHECK(v.is_contiguous());
  auto&& _ipex_v = bridge::fallbackToCPUTensor(v);
...
at::Tensor AtenIpexCPUDefault::_weight_norm(const at::Tensor & v, const at::Tensor & g, int64_t dim) {
  TORCH_CHECK(v.layout() == c10::kStrided);
  TORCH_CHECK(v.is_contiguous());
  TORCH_CHECK(g.layout() == c10::kStrided);
  TORCH_CHECK(g.is_contiguous());
  auto&& _ipex_v = bridge::fallbackToCPUTensor(v);
  auto&& _ipex_g = bridge::fallbackToCPUTensor(g);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::_weight_norm_cuda_interface_backward(const at::Tensor & grad_w, const at::Tensor & saved_v, const at::Tensor & saved_g, const at::Tensor & saved_norms, int64_t dim) {
  TORCH_CHECK(grad_w.layout() == c10::kStrided);
  TORCH_CHECK(grad_w.is_contiguous());
  TORCH_CHECK(saved_v.layout() == c10::kStrided);
  TORCH_CHECK(saved_v.is_contiguous());
  TORCH_CHECK(saved_g.layout() == c10::kStrided);
  TORCH_CHECK(saved_g.is_contiguous());
  TORCH_CHECK(saved_norms.layout() == c10::kStrided);
  TORCH_CHECK(saved_norms.is_contiguous());
  auto&& _ipex_grad_w = bridge::fallbackToCPUTensor(grad_w);
  auto&& _ipex_saved_v = bridge::fallbackToCPUTensor(saved_v);
  auto&& _ipex_saved_g = bridge::fallbackToCPUTensor(saved_g);
  auto&& _ipex_saved_norms = bridge::fallbackToCPUTensor(saved_norms);
...
at::Tensor AtenIpexCPUDefault::_dirichlet_grad(const at::Tensor & x, const at::Tensor & alpha, const at::Tensor & total) {
  TORCH_CHECK(x.layout() == c10::kStrided);
  TORCH_CHECK(x.is_contiguous());
  TORCH_CHECK(alpha.layout() == c10::kStrided);
  TORCH_CHECK(alpha.is_contiguous());
  TORCH_CHECK(total.layout() == c10::kStrided);
  TORCH_CHECK(total.is_contiguous());
  auto&& _ipex_x = bridge::fallbackToCPUTensor(x);
  auto&& _ipex_alpha = bridge::fallbackToCPUTensor(alpha);
  auto&& _ipex_total = bridge::fallbackToCPUTensor(total);
...
at::Tensor & AtenIpexCPUDefault::resize_as_(at::Tensor & self, const at::Tensor & the_template) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(the_template.layout() == c10::kStrided);
  TORCH_CHECK(the_template.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_the_template = bridge::fallbackToCPUTensor(the_template);
...
at::Tensor AtenIpexCPUDefault::_make_per_channel_quantized_tensor(const at::Tensor & self, const at::Tensor & scale, const at::Tensor & zero_point, int64_t axis) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(scale.layout() == c10::kStrided);
  TORCH_CHECK(scale.is_contiguous());
  TORCH_CHECK(zero_point.layout() == c10::kStrided);
  TORCH_CHECK(zero_point.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_scale = bridge::fallbackToCPUTensor(scale);
  auto&& _ipex_zero_point = bridge::fallbackToCPUTensor(zero_point);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::_thnn_fused_lstm_cell(const at::Tensor & input_gates, const at::Tensor & hidden_gates, const at::Tensor & cx, const at::Tensor & input_bias, const at::Tensor & hidden_bias) {
  TORCH_CHECK(input_gates.layout() == c10::kStrided);
  TORCH_CHECK(input_gates.is_contiguous());
  TORCH_CHECK(hidden_gates.layout() == c10::kStrided);
  TORCH_CHECK(hidden_gates.is_contiguous());
  TORCH_CHECK(cx.layout() == c10::kStrided);
  TORCH_CHECK(cx.is_contiguous());
  TORCH_CHECK(input_bias.layout() == c10::kStrided);
  TORCH_CHECK(input_bias.is_contiguous());
  TORCH_CHECK(hidden_bias.layout() == c10::kStrided);
  TORCH_CHECK(hidden_bias.is_contiguous());
  auto&& _ipex_input_gates = bridge::fallbackToCPUTensor(input_gates);
  auto&& _ipex_hidden_gates = bridge::fallbackToCPUTensor(hidden_gates);
  auto&& _ipex_cx = bridge::fallbackToCPUTensor(cx);
  auto&& _ipex_input_bias = bridge::fallbackToCPUTensor(input_bias);
  auto&& _ipex_hidden_bias = bridge::fallbackToCPUTensor(hidden_bias);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::_thnn_fused_lstm_cell_backward(const at::Tensor & grad_hy, const at::Tensor & grad_cy, const at::Tensor & cx, const at::Tensor & cy, const at::Tensor & workspace, bool has_bias) {
  ...
  TORCH_CHECK(cx.layout() == c10::kStrided);
  TORCH_CHECK(cx.is_contiguous());
  TORCH_CHECK(cy.layout() == c10::kStrided);
  TORCH_CHECK(cy.is_contiguous());
  TORCH_CHECK(workspace.layout() == c10::kStrided);
  TORCH_CHECK(workspace.is_contiguous());
  auto&& _ipex_grad_hy = bridge::fallbackToCPUTensor(grad_hy);
  auto&& _ipex_grad_cy = bridge::fallbackToCPUTensor(grad_cy);
  auto&& _ipex_cx = bridge::fallbackToCPUTensor(cx);
  auto&& _ipex_cy = bridge::fallbackToCPUTensor(cy);
  auto&& _ipex_workspace = bridge::fallbackToCPUTensor(workspace);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::lstm(const at::Tensor & input, at::TensorList hx, at::TensorList params, bool has_biases, int64_t num_layers, double dropout, bool train, bool bidirectional, bool batch_first) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  auto&& _ipex_hx = bridge::fallbackToCPUTensorList(hx);
  auto&& _ipex_params = bridge::fallbackToCPUTensorList(params);
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
...
std::tuple<at::Tensor,at::Tensor,at::Tensor> AtenIpexCPUDefault::lstm(const at::Tensor & data, const at::Tensor & batch_sizes, at::TensorList hx, at::TensorList params, bool has_biases, int64_t num_layers, double dropout, bool train, bool bidirectional) {
  TORCH_CHECK(data.layout() == c10::kStrided);
  TORCH_CHECK(data.is_contiguous());
  TORCH_CHECK(batch_sizes.layout() == c10::kStrided);
  TORCH_CHECK(batch_sizes.is_contiguous());
  auto&& _ipex_hx = bridge::fallbackToCPUTensorList(hx);
  auto&& _ipex_params = bridge::fallbackToCPUTensorList(params);
  auto&& _ipex_data = bridge::fallbackToCPUTensor(data);
  auto&& _ipex_batch_sizes = bridge::fallbackToCPUTensor(batch_sizes);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::lstm_cell(const at::Tensor & input, at::TensorList hx, const at::Tensor & w_ih, const at::Tensor & w_hh, const at::Tensor & b_ih, const at::Tensor & b_hh) {
  ...
  TORCH_CHECK(input.is_contiguous());
  auto&& _ipex_hx = bridge::fallbackToCPUTensorList(hx);
  TORCH_CHECK(w_ih.layout() == c10::kStrided);
  TORCH_CHECK(w_ih.is_contiguous());
  TORCH_CHECK(w_hh.layout() == c10::kStrided);
  TORCH_CHECK(w_hh.is_contiguous());
  TORCH_CHECK(b_ih.layout() == c10::kStrided);
  TORCH_CHECK(b_ih.is_contiguous());
  TORCH_CHECK(b_hh.layout() == c10::kStrided);
  TORCH_CHECK(b_hh.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_w_ih = bridge::fallbackToCPUTensor(w_ih);
  auto&& _ipex_w_hh = bridge::fallbackToCPUTensor(w_hh);
  auto&& _ipex_b_ih = bridge::fallbackToCPUTensor(b_ih);
  auto&& _ipex_b_hh = bridge::fallbackToCPUTensor(b_hh);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::quantized_lstm_cell(const at::Tensor & input, at::TensorList hx, const at::Tensor & w_ih, const at::Tensor & w_hh, const at::Tensor & b_ih, const at::Tensor & b_hh, const at::Tensor & packed_ih, const at::Tensor & packed_hh, const at::Tensor & col_offsets_ih, const at::Tensor & col_offsets_hh, at::Scalar scale_ih, at::Scalar scale_hh, at::Scalar zero_point_ih, at::Scalar zero_point_hh) {
  ...
  TORCH_CHECK(b_hh.layout() == c10::kStrided);
  TORCH_CHECK(b_hh.is_contiguous());
  TORCH_CHECK(packed_ih.layout() == c10::kStrided);
  TORCH_CHECK(packed_ih.is_contiguous());
  TORCH_CHECK(packed_hh.layout() == c10::kStrided);
  TORCH_CHECK(packed_hh.is_contiguous());
  TORCH_CHECK(col_offsets_ih.layout() == c10::kStrided);
  TORCH_CHECK(col_offsets_ih.is_contiguous());
  TORCH_CHECK(col_offsets_hh.layout() == c10::kStrided);
  TORCH_CHECK(col_offsets_hh.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_w_ih = bridge::fallbackToCPUTensor(w_ih);
  auto&& _ipex_w_hh = bridge::fallbackToCPUTensor(w_hh);
  auto&& _ipex_b_ih = bridge::fallbackToCPUTensor(b_ih);
  auto&& _ipex_b_hh = bridge::fallbackToCPUTensor(b_hh);
  auto&& _ipex_packed_ih = bridge::fallbackToCPUTensor(packed_ih);
  auto&& _ipex_packed_hh = bridge::fallbackToCPUTensor(packed_hh);
  auto&& _ipex_col_offsets_ih = bridge::fallbackToCPUTensor(col_offsets_ih);
  auto&& _ipex_col_offsets_hh = bridge::fallbackToCPUTensor(col_offsets_hh);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::_pack_padded_sequence(const at::Tensor & input, const at::Tensor & lengths, bool batch_first) {
  TORCH_CHECK(input.layout() == c10::kStrided);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(lengths.layout() == c10::kStrided);
  TORCH_CHECK(lengths.is_contiguous());
  auto&& _ipex_input = bridge::fallbackToCPUTensor(input);
  auto&& _ipex_lengths = bridge::fallbackToCPUTensor(lengths);
...
at::Tensor & AtenIpexCPUDefault::pow_(at::Tensor & self, const at::Tensor & exponent) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(exponent.layout() == c10::kStrided);
  TORCH_CHECK(exponent.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_exponent = bridge::fallbackToCPUTensor(exponent);
...
at::Tensor & AtenIpexCPUDefault::lerp_(at::Tensor & self, const at::Tensor & end, at::Scalar weight) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(end.layout() == c10::kStrided);
  TORCH_CHECK(end.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_end = bridge::fallbackToCPUTensor(end);
...
at::Tensor & AtenIpexCPUDefault::addcdiv_(at::Tensor & self, const at::Tensor & tensor1, const at::Tensor & tensor2, at::Scalar value) {
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(tensor1.layout() == c10::kStrided);
  TORCH_CHECK(tensor1.is_contiguous());
  TORCH_CHECK(tensor2.layout() == c10::kStrided);
  TORCH_CHECK(tensor2.is_contiguous());
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_tensor1 = bridge::fallbackToCPUTensor(tensor1);
  auto&& _ipex_tensor2 = bridge::fallbackToCPUTensor(tensor2);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::lstsq_out(at::Tensor & X, at::Tensor & qr, const at::Tensor & self, const at::Tensor & A) {
  TORCH_CHECK(X.layout() == c10::kStrided);
  TORCH_CHECK(X.is_contiguous());
  TORCH_CHECK(qr.layout() == c10::kStrided);
  TORCH_CHECK(qr.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(A.layout() == c10::kStrided);
  TORCH_CHECK(A.is_contiguous());
  auto&& _ipex_X = bridge::fallbackToCPUTensor(X);
  auto&& _ipex_qr = bridge::fallbackToCPUTensor(qr);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_A = bridge::fallbackToCPUTensor(A);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::triangular_solve_out(at::Tensor & X, at::Tensor & M, const at::Tensor & self, const at::Tensor & A, bool upper, bool transpose, bool unitriangular) {
  TORCH_CHECK(X.layout() == c10::kStrided);
  TORCH_CHECK(X.is_contiguous());
  TORCH_CHECK(M.layout() == c10::kStrided);
  TORCH_CHECK(M.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(A.layout() == c10::kStrided);
  TORCH_CHECK(A.is_contiguous());
  auto&& _ipex_X = bridge::fallbackToCPUTensor(X);
  auto&& _ipex_M = bridge::fallbackToCPUTensor(M);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_A = bridge::fallbackToCPUTensor(A);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::symeig_out(at::Tensor & e, at::Tensor & V, const at::Tensor & self, bool eigenvectors, bool upper) {
  TORCH_CHECK(e.layout() == c10::kStrided);
  TORCH_CHECK(e.is_contiguous());
  TORCH_CHECK(V.layout() == c10::kStrided);
  TORCH_CHECK(V.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_e = bridge::fallbackToCPUTensor(e);
  auto&& _ipex_V = bridge::fallbackToCPUTensor(V);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor &,at::Tensor &,at::Tensor &> AtenIpexCPUDefault::svd_out(at::Tensor & U, at::Tensor & S, at::Tensor & V, const at::Tensor & self, bool some, bool compute_uv) {
  TORCH_CHECK(U.layout() == c10::kStrided);
  TORCH_CHECK(U.is_contiguous());
  TORCH_CHECK(S.layout() == c10::kStrided);
  TORCH_CHECK(S.is_contiguous());
  TORCH_CHECK(V.layout() == c10::kStrided);
  TORCH_CHECK(V.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_U = bridge::fallbackToCPUTensor(U);
  auto&& _ipex_S = bridge::fallbackToCPUTensor(S);
  auto&& _ipex_V = bridge::fallbackToCPUTensor(V);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::solve_out(at::Tensor & solution, at::Tensor & lu, const at::Tensor & self, const at::Tensor & A) {
  TORCH_CHECK(solution.layout() == c10::kStrided);
  TORCH_CHECK(solution.is_contiguous());
  TORCH_CHECK(lu.layout() == c10::kStrided);
  TORCH_CHECK(lu.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(A.layout() == c10::kStrided);
  TORCH_CHECK(A.is_contiguous());
  auto&& _ipex_solution = bridge::fallbackToCPUTensor(solution);
  auto&& _ipex_lu = bridge::fallbackToCPUTensor(lu);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_A = bridge::fallbackToCPUTensor(A);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::qr_out(at::Tensor & Q, at::Tensor & R, const at::Tensor & self, bool some) {
  TORCH_CHECK(Q.layout() == c10::kStrided);
  TORCH_CHECK(Q.is_contiguous());
  TORCH_CHECK(R.layout() == c10::kStrided);
  TORCH_CHECK(R.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_Q = bridge::fallbackToCPUTensor(Q);
  auto&& _ipex_R = bridge::fallbackToCPUTensor(R);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::geqrf_out(at::Tensor & a, at::Tensor & tau, const at::Tensor & self) {
  TORCH_CHECK(a.layout() == c10::kStrided);
  TORCH_CHECK(a.is_contiguous());
  TORCH_CHECK(tau.layout() == c10::kStrided);
  TORCH_CHECK(tau.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_a = bridge::fallbackToCPUTensor(a);
  auto&& _ipex_tau = bridge::fallbackToCPUTensor(tau);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor & AtenIpexCPUDefault::ormqr_out(at::Tensor & out, const at::Tensor & self, const at::Tensor & input2, const at::Tensor & input3, bool left, bool transpose) {
  ...
  TORCH_CHECK(input2.layout() == c10::kStrided);
  TORCH_CHECK(input2.is_contiguous());
  TORCH_CHECK(input3.layout() == c10::kStrided);
  TORCH_CHECK(input3.is_contiguous());
  auto&& _ipex_out = bridge::fallbackToCPUTensor(out);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_input2 = bridge::fallbackToCPUTensor(input2);
  auto&& _ipex_input3 = bridge::fallbackToCPUTensor(input3);
...
at::Tensor & AtenIpexCPUDefault::lu_solve_out(at::Tensor & out, const at::Tensor & self, const at::Tensor & LU_data, const at::Tensor & LU_pivots) {
  ...
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(LU_data.layout() == c10::kStrided);
  TORCH_CHECK(LU_data.is_contiguous());
  TORCH_CHECK(LU_pivots.layout() == c10::kStrided);
  TORCH_CHECK(LU_pivots.is_contiguous());
  auto&& _ipex_out = bridge::fallbackToCPUTensor(out);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_LU_data = bridge::fallbackToCPUTensor(LU_data);
  auto&& _ipex_LU_pivots = bridge::fallbackToCPUTensor(LU_pivots);
...
std::tuple<at::Tensor,at::Tensor> AtenIpexCPUDefault::_multinomial_alias_setup(const at::Tensor & probs) {
  TORCH_CHECK(probs.layout() == c10::kStrided);
  TORCH_CHECK(probs.is_contiguous());
  auto&& _ipex_probs = bridge::fallbackToCPUTensor(probs);
...
at::Tensor AtenIpexCPUDefault::_multinomial_alias_draw(const at::Tensor & J, const at::Tensor & q, int64_t num_samples, at::Generator * generator) {
  TORCH_CHECK(J.layout() == c10::kStrided);
  TORCH_CHECK(J.is_contiguous());
  TORCH_CHECK(q.layout() == c10::kStrided);
  TORCH_CHECK(q.is_contiguous());
  auto&& _ipex_J = bridge::fallbackToCPUTensor(J);
  auto&& _ipex_q = bridge::fallbackToCPUTensor(q);
...
at::Tensor & AtenIpexCPUDefault::normal_out(at::Tensor & out, double mean, const at::Tensor & std, at::Generator * generator) {
  TORCH_CHECK(out.layout() == c10::kStrided);
  TORCH_CHECK(out.is_contiguous());
  TORCH_CHECK(std.layout() == c10::kStrided);
  TORCH_CHECK(std.is_contiguous());
  auto&& _ipex_out = bridge::fallbackToCPUTensor(out);
  auto&& _ipex_std = bridge::fallbackToCPUTensor(std);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::_max_out(at::Tensor & max, at::Tensor & max_indices, const at::Tensor & self, int64_t dim, bool keepdim) {
  TORCH_CHECK(max.layout() == c10::kStrided);
  TORCH_CHECK(max.is_contiguous());
  TORCH_CHECK(max_indices.layout() == c10::kStrided);
  TORCH_CHECK(max_indices.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_max = bridge::fallbackToCPUTensor(max);
  auto&& _ipex_max_indices = bridge::fallbackToCPUTensor(max_indices);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor & AtenIpexCPUDefault::binary_cross_entropy_backward_out(at::Tensor & grad_input, const at::Tensor & grad_output, const at::Tensor & self, const at::Tensor & target, const at::Tensor & weight, int64_t reduction) {
  TORCH_CHECK(grad_input.layout() == c10::kStrided);
  ...
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  auto&& _ipex_grad_input = bridge::fallbackToCPUTensor(grad_input);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_target = bridge::fallbackToCPUTensor(target);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::multilabel_margin_loss_forward_out(at::Tensor & output, at::Tensor & is_target, const at::Tensor & self, const at::Tensor & target, int64_t reduction) {
  TORCH_CHECK(output.layout() == c10::kStrided);
  TORCH_CHECK(output.is_contiguous());
  TORCH_CHECK(is_target.layout() == c10::kStrided);
  TORCH_CHECK(is_target.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(target.layout() == c10::kStrided);
  TORCH_CHECK(target.is_contiguous());
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_is_target = bridge::fallbackToCPUTensor(is_target);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_target = bridge::fallbackToCPUTensor(target);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::nll_loss_forward_out(at::Tensor & output, at::Tensor & total_weight, const at::Tensor & self, const at::Tensor & target, const at::Tensor & weight, int64_t reduction, int64_t ignore_index) {
  TORCH_CHECK(output.layout() == c10::kStrided);
  TORCH_CHECK(output.is_contiguous());
  TORCH_CHECK(total_weight.layout() == c10::kStrided);
  TORCH_CHECK(total_weight.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(target.layout() == c10::kStrided);
  TORCH_CHECK(target.is_contiguous());
  ...
  TORCH_CHECK(weight.is_contiguous());
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_total_weight = bridge::fallbackToCPUTensor(total_weight);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_target = bridge::fallbackToCPUTensor(target);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::log_sigmoid_forward_out(at::Tensor & output, at::Tensor & buffer, const at::Tensor & self) {
  TORCH_CHECK(output.layout() == c10::kStrided);
  TORCH_CHECK(output.is_contiguous());
  TORCH_CHECK(buffer.layout() == c10::kStrided);
  TORCH_CHECK(buffer.is_contiguous());
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_buffer = bridge::fallbackToCPUTensor(buffer);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
...
at::Tensor & AtenIpexCPUDefault::rrelu_with_noise_out(at::Tensor & out, const at::Tensor & self, const at::Tensor & noise, at::Scalar lower, at::Scalar upper, bool training, at::Generator * generator) {
  ...
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(noise.layout() == c10::kStrided);
  TORCH_CHECK(noise.is_contiguous());
  auto&& _ipex_out = bridge::fallbackToCPUTensor(out);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_noise = bridge::fallbackToCPUTensor(noise);
...
std::tuple<at::Tensor &,at::Tensor &> AtenIpexCPUDefault::fractional_max_pool2d_out(at::Tensor & output, at::Tensor & indices, const at::Tensor & self, at::IntArrayRef kernel_size, at::IntArrayRef output_size, const at::Tensor & random_samples) {
  ...
  TORCH_CHECK(self.layout() == c10::kStrided);
  TORCH_CHECK(self.is_contiguous());
  TORCH_CHECK(random_samples.layout() == c10::kStrided);
  TORCH_CHECK(random_samples.is_contiguous());
  auto&& _ipex_output = bridge::fallbackToCPUTensor(output);
  auto&& _ipex_indices = bridge::fallbackToCPUTensor(indices);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_random_samples = bridge::fallbackToCPUTensor(random_samples);
...
std::tuple<at::Tensor &,at::Tensor &,at::Tensor &> AtenIpexCPUDefault::slow_conv_transpose2d_backward_out(at::Tensor & grad_input, at::Tensor & grad_weight, at::Tensor & grad_bias, const at::Tensor & grad_output, const at::Tensor & self, const at::Tensor & weight, at::IntArrayRef kernel_size, at::IntArrayRef stride, at::IntArrayRef padding, at::IntArrayRef output_padding, at::IntArrayRef dilation, const at::Tensor & columns, const at::Tensor & ones) {
  TORCH_CHECK(grad_input.layout() == c10::kStrided);
  TORCH_CHECK(grad_input.is_contiguous());
  TORCH_CHECK(grad_weight.layout() == c10::kStrided);
  TORCH_CHECK(grad_weight.is_contiguous());
  TORCH_CHECK(grad_bias.layout() == c10::kStrided);
  ...
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(columns.layout() == c10::kStrided);
  TORCH_CHECK(columns.is_contiguous());
  TORCH_CHECK(ones.layout() == c10::kStrided);
  TORCH_CHECK(ones.is_contiguous());
  auto&& _ipex_grad_input = bridge::fallbackToCPUTensor(grad_input);
  auto&& _ipex_grad_weight = bridge::fallbackToCPUTensor(grad_weight);
  auto&& _ipex_grad_bias = bridge::fallbackToCPUTensor(grad_bias);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_columns = bridge::fallbackToCPUTensor(columns);
  auto&& _ipex_ones = bridge::fallbackToCPUTensor(ones);
...
std::tuple<at::Tensor &,at::Tensor &,at::Tensor &> AtenIpexCPUDefault::slow_conv_transpose3d_backward_out(at::Tensor & grad_input, at::Tensor & grad_weight, at::Tensor & grad_bias, const at::Tensor & grad_output, const at::Tensor & self, const at::Tensor & weight, at::IntArrayRef kernel_size, at::IntArrayRef stride, at::IntArrayRef padding, at::IntArrayRef output_padding, at::IntArrayRef dilation, const at::Tensor & finput, const at::Tensor & fgrad_input) {
  ...
  TORCH_CHECK(weight.layout() == c10::kStrided);
  TORCH_CHECK(weight.is_contiguous());
  TORCH_CHECK(finput.layout() == c10::kStrided);
  TORCH_CHECK(finput.is_contiguous());
  TORCH_CHECK(fgrad_input.layout() == c10::kStrided);
  TORCH_CHECK(fgrad_input.is_contiguous());
  auto&& _ipex_grad_input = bridge::fallbackToCPUTensor(grad_input);
  auto&& _ipex_grad_weight = bridge::fallbackToCPUTensor(grad_weight);
  auto&& _ipex_grad_bias = bridge::fallbackToCPUTensor(grad_bias);
  auto&& _ipex_grad_output = bridge::fallbackToCPUTensor(grad_output);
  auto&& _ipex_self = bridge::fallbackToCPUTensor(self);
  auto&& _ipex_weight = bridge::fallbackToCPUTensor(weight);
  auto&& _ipex_finput = bridge::fallbackToCPUTensor(finput);
  auto&& _ipex_fgrad_input = bridge::fallbackToCPUTensor(fgrad_input);
