...
::
   ...
   >>> python -m intel_pytorch_extension.launch --throughput_performance script.py args
    ...
    def launch(self, args):
        ...
        for i in range(args.ninstances):
           ...
           if not os.path.exists(path):
               os.makedirs(path)
           log_name = args.log_file_prefix + "_instance#{}_cores_".format(i) + cur_process_cores.replace(',','_') + ".log" 
...
def add_multi_instance_params(parser):
    ...
    group.add_argument("--log_path", metavar='\b', default="./", type=str,
                         help="The log file path, defualt path is './'")
...
def add_kmp_iomp_params(parser): 
    ...
    group.add_argument("--disable_iomp", action='store_true', default=False,
                        help="By default, we use Intel OpenMP and libiomp.so will be add to LD_PRELOAD") 
