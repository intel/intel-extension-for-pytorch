...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  m.def("lamb_fused_step(Tensor(a!) param, Tensor(b!) exp_avg, Tensor(c!) exp_avg_sq, Tensor grad, Tensor trail, int step, float beta1, float beta2, float lr, float weight_decay, float eps) -> (Tensor(a!), Tensor(b!), Tensor(c!))", torch_ipex::cpu::lamb_fused_step);
}
