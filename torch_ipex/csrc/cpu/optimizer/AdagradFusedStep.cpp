...
template <>
void adagrad_fused_step_kernel<at::BFloat16>(
    const at::Tensor& param,
    const at::Tensor& grad,
    const at::Tensor& state_sum,
    const at::Tensor& param2,
    ...
    double lr_decay,
    double eps) {
  TORCH_CHECK(param.scalar_type() == at::kBFloat16,
  ...
  TORCH_CHECK(state_sum.scalar_type() == at::kFloat,
      "adagrad_fused_step_kernel: expect stats_sum to be float32");
  TORCH_CHECK(param2.scalar_type() == at::kBFloat16,
  ...
  at::parallel_for(0, param.numel(), grain_size, [&](int64_t begin, int64_t end) {
    ...
    for (; d < size - (size % bVec::size()); d += bVec::size()) {
      ...
      std::tie(param_fvec, param_fvec2) = bf16::pack_bfloat16_float(param_bvec, param2_bvec);
      ...
      std::tie(param_bvec, param2_bvec) = bf16::unpack_float_bfloat16(param_fvec, param_fvec2);
    ...
    for (; d < size; d++) {
      ...
      std::tie(param_ptr[d], param2_ptr[d]) = bf16::unpack_float_bfloat16(param_val);
    }
  });
}
...
std::tuple<at::Tensor, at::Tensor> adagrad_fused_step(
    ...
    RECORD_FUNCTION("torch_ipex::adagrad_fused_step", std::vector<c10::IValue>({}));
#endif
  ...
  TORCH_CHECK(param_.sizes() == grad_.sizes(),
      ...
      param_.sizes(), "; grad_ sizes: ", grad_.sizes());
...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  m.def("adagrad_fused_step(Tensor(a!) param, Tensor grad, Tensor(b!) state_sum, Tensor trail, int step, float lr, float weight_decay, float lr_decay, float eps) -> (Tensor(a!), Tensor(b!))", torch_ipex::cpu::adagrad_fused_step);
}
