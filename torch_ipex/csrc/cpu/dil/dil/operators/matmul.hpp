#ifndef DIL_OPERATORS_INNER_PRODUCT_MATMUL_HPP
#define DIL_OPERATORS_INNER_PRODUCT_MATMUL_HPP
...
private:
 ...
 static void compute_impl(const tensor& src,
                          ...
                          const lowp_kind alowp_kind = u8s8,
                          const engine& aengine = engine::cpu_engine()) {
   DIL_ENFORCE(src.ndims() == weights.ndims(), "Invalid dims in src or weights");
   ...
   if (!weights_scales_in.empty()) {
     ...
     DIL_ENFORCE(src_zero_point_size == 1 && dst_zero_point_size == 1, 
     ...
     auto bias_scales_in =
         bias.has_scale() ? bias.get_scale() : DIL_DEF_SCALE;
     ...
     // in DNNL.
     DIL_ENFORCE(weights.get_data_type() == data_type::f32 ||
     ...
     if (with_bias) {
       DIL_ENFORCE(bias.get_data_type() == data_type::f32 ||
