...
struct lstm_forward : public dnnl::lstm_forward {
  ...
  static void compute(const dims& output_sizes,
    ...
    auto aprop = prop_kind::forward_training;
    ...
    auto expected_src_iter_c = src_iter_c.reorder_if_differ_in(pd.src_iter_c_desc());
    ...
    super(pd).execute(stream::default_stream(),
                      {{DNNL_ARG_SRC_LAYER, expected_src_layer},
                       {DNNL_ARG_SRC_ITER, expected_src_iter},
                       {DNNL_ARG_SRC_ITER_C, expected_src_iter_c},
...
struct lstm_backward : public dnnl::lstm_backward {
  ...
  static void compute(const tensor& src_layer,
    ...
    auto expected_dst_layer = dst_layer.reorder_if_differ_in(pd.dst_layer_desc());
    auto expected_dst_iter_c = dst_iter_c.reorder_if_differ_in(pd.dst_iter_c_desc());
    auto expected_dst_iter = dst_iter.reorder_if_differ_in(pd.dst_iter_desc());

    auto expected_diff_dst_layer = diff_dst_layer.reorder_if_differ_in(pd.diff_dst_layer_desc());
    auto expected_diff_dst_iter_c = diff_dst_iter_c.reorder_if_differ_in(pd.diff_dst_iter_c_desc());
    auto expected_diff_dst_iter = diff_dst_iter.reorder_if_differ_in(pd.diff_dst_iter_desc());
    ...
    super(pd).execute(stream::default_stream(),
                      {{DNNL_ARG_SRC_LAYER, expected_src_layer},
                       ...
                       {DNNL_ARG_DST_LAYER, expected_dst_layer},
                       {DNNL_ARG_DST_ITER, expected_dst_iter},
                       {DNNL_ARG_DST_ITER_C, expected_dst_iter_c},
                       ...
                       {DNNL_ARG_DIFF_DST_LAYER, expected_diff_dst_layer},
                       {DNNL_ARG_DIFF_DST_ITER, expected_diff_dst_iter},
                       {DNNL_ARG_DIFF_DST_ITER_C, expected_diff_dst_iter_c},
