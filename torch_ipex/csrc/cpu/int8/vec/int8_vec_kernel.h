...
static inline __attribute__((always_inline)) void zero_ker(int32_t *out,
                                                           int64_t len) {
  int64_t i;
...
static inline __attribute__((always_inline)) void zero_ker(int8_t *out,
                                                           int64_t len) {
  int64_t i;
  __m512i zero_512 = _mm512_setzero_si512();
#pragma unroll(4)
...
move_ker(int64_t *out, const int64_t *in, int64_t len) {
  int64_t i;
#pragma unroll(4)
  for (i = 0; i < len - 7; i += 8) {
...
static inline __attribute__((always_inline)) void
move_ker(int16_t *out, const int16_t *in, int64_t len) {
  int64_t i;
#pragma unroll(2)
...
static inline __attribute__((always_inline)) void
move_ker(unsigned char *out, const unsigned char *in, int64_t len) {
  int64_t i;
#pragma unroll(2)
  for (i = 0; i < len - 63; i += 64) {
    auto in0 = _mm512_loadu_si512(in + i);
...
static inline __attribute__((always_inline)) void
move_ker(bool *out, const bool *in, int64_t len) {
  int64_t i;
#pragma unroll(2)
  for (i = 0; i < len - 63; i += 64) {
    auto in0 = _mm512_loadu_si512(in + i);
...
static inline __attribute__((always_inline)) void
move_ker(int8_t *out, const int8_t *in, int64_t len) {
  int64_t i;
#pragma unroll(2)
  for (i = 0; i < len - 63; i += 64) {
    auto in0 = _mm512_loadu_si512(in + i);
...
static inline __attribute__((always_inline)) void
scale_and_store_int8_128(void *out, const __m128i *in, __m512 scale) {
...
static inline __attribute__((always_inline)) void
scale_and_store_int8_128(void *out, const void *in, __m512 &scale) {
...
static inline void scale_and_store_int8_64(void *out, const void *in,
                                           __m512 &scale) {
  auto in0_0_32i = _mm512_cvtepi8_epi32(_mm_loadu_si128((__m128i *)in));
  auto in0_1_32i = _mm512_cvtepi8_epi32(_mm_loadu_si128((__m128i *)(in + 16)));
  auto in0_2_32i = _mm512_cvtepi8_epi32(_mm_loadu_si128((__m128i *)(in + 32)));
...
static inline void scale_and_store_int8_32(void *out, const void *in,
                                           __m512 &scale) {
  auto in0_0_32i = _mm512_cvtepi8_epi32(_mm_loadu_si128((__m128i *)in));
  auto in0_1_32i = _mm512_cvtepi8_epi32(_mm_loadu_si128((__m128i *)(in + 16)));
  auto in0_0_32f = _mm512_cvt_roundepi32_ps(
...
static inline void scale_and_store_int8_16(void *out, const void *in,
                                           __m512 &scale) {
...
static inline void scale_and_store_int8_maskz_16(void *out, const void *in,
                                                 __m512 &scale, __mmask8 mask) {
...
static inline __attribute__((always_inline)) void
scale_and_move_ker_128(int8_t *out, const int8_t *in, float scale) {
...
static inline __attribute__((always_inline)) void
scale_and_move_ker_128_m512i(int8_t *out, const __m512i *in, float scale) {
  if (std::abs(scale - 1.0) < 0.0005) {
    move_ker(out, in, 2);
  } else {
    __m512 scale_vec512 = _mm512_set1_ps(scale);
...
static inline void scale_and_move_ker(int8_t *out, const int8_t *in,
                                      float scale, int64_t len) {
  int64_t i;
  /*
    for (i = 0; i < len; i ++) {
       int32_t out_i = (int32_t)((float)in[i] * scale);
       out[i] = (out_f >= 127 ? (int8_t)127 : out_f <= -127 ? (int8_t)-127 :
    (int8_t)(int32_t)out_f);
    }
  */
  __m512 scale_vec512 = _mm512_set1_ps(scale);
  for (i = 0; i < len - 127; i += 128) {
    scale_and_store_int8_128((void *)(out + i), (const void *)(in + i),
                             scale_vec512);
  }
  if ((len - i) > 63) {
    scale_and_store_int8_64((void *)(out + i), (const void *)(in + i),
                            scale_vec512);
    i += 64;
  }
  if ((len - i) > 31) {
    scale_and_store_int8_32((void *)(out + i), (const void *)(in + i),
                            scale_vec512);
    i += 32;
  }
  if ((len - i) > 15) {
    scale_and_store_int8_16((void *)(out + i), (const void *)(in + i),
                            scale_vec512);
    i += 16;
  }
  if (i < len) {
...
static inline __attribute__((always_inline)) void
mul_and_sum_s8x128_to_s32x16(__m512i &out, const int8_t *a, const int8_t *b) {
...
static inline __attribute__((always_inline)) void
load_s8x128_to_s16x128(__m512i *out_s16x4, const int8_t *in) {
...
static inline __attribute__((always_inline)) void
load_s8x128x2_to_s16x128x2(__m512i *out_s16x8, const int8_t *in0,
                           const int8_t *in1) {
...
static inline __attribute__((always_inline)) void
mul_and_sum_s16x128_to_s32x16(__m512i &out, const __m512i *a16x4,
                              const __m512i *b16x4) {
...
static inline __attribute__((always_inline)) void
mul_and_sum_s8x128x2_to_s32x16x2(__m512i &out0, __m512i &out1, const int8_t *a0,
                                 const int8_t *b0, const int8_t *a1,
                                 const int8_t *b1) {
...
static inline __attribute__((always_inline)) void
mul_and_sum_s16x128x2_to_s32x16x2(__m512i &out0, __m512i &out1,
...
reduce_add_s32x16x16(__m512i *acc_sums) {
...
static inline __attribute__((always_inline)) void
reduce_add_s32x16x16_with_scales(int8_t *outs, __m512i *acc_sums,
                                 const __m512 &scales) {
  auto l1 = reduce_add_s32x16x16(acc_sums);
  auto l1_f = _mm512_cvtepi32_ps(l1);
  l1_f = _mm512_mul_round_ps(l1_f, scales,
...
static inline __attribute__((always_inline)) void
reduce_add_s32x16x16x4_with_scales(int8_t *outs, __m512i *acc_sums,
  ...
  l1_f = _mm512_mul_round_ps(l1_f, scales[0],
                             (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
  l2_f = _mm512_mul_round_ps(l2_f, scales[1],
                             (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
  l3_f = _mm512_mul_round_ps(l3_f, scales[2],
                             (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
  l4_f = _mm512_mul_round_ps(l4_f, scales[3],
                             (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
  l1 = _mm512_cvtps_epi32(l1_f);
...
static inline __attribute__((always_inline)) void
reduce_add_s32x16x16_with_scales_and_mask_store(int8_t *outs, __mmask16 mask,
                                                __m512i *acc_sums,
                                                const __m512 &scales) {
  auto l1 = reduce_add_s32x16x16(acc_sums);
  auto l1_f = _mm512_cvtepi32_ps(l1);
...
static inline __attribute__((always_inline)) int32_t _scale_int32(int32_t value,
                                                                  float scale) {
  auto v_simd = _mm_setzero_ps();
  auto s_simd = _mm_set1_ps(scale);
  v_simd = _mm_cvt_si2ss(v_simd, value);
  v_simd = _mm_mul_round_ss(v_simd, s_simd,
                            (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
...
_dot_s8s8_scale_s32s8(const int8_t *a, const int8_t *b, size_t len,
                      float scale) {
  int32_t c = 0;
  int64_t i;
  for (i = 0; i < len - 127; i += 128) {
