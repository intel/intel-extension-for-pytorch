...
template <>
inline void prefix_sum<int64_t>(const int64_t *src, int64_t *dst, int64_t init,
                                int64_t n) {
  int64_t i;
...
static inline void cumsum_lastdim_kernel(at::Tensor &result,
                                         const at::Tensor &self, int64_t dim) {
  const auto input_ndim = self.dim();
  TORCH_CHECK(dim == input_ndim - 1, "cumsum_lastdim_kernel: expect dim to be ",
              input_ndim - 1, " got ", dim);
  TORCH_CHECK(
...
bool cumsum_fast_path(const at::Tensor &self, const at::Tensor &result,
...
public:
  static at::Tensor _forward(at::Tensor &result, const at::Tensor &self,
                             int64_t dim, c10::optional<at::ScalarType> dtype) {
...
at::Tensor AtenIpexTypeExt::cumsum(at::Tensor &result, const at::Tensor &self,
                                   int64_t dim,
...
at::Tensor &cumsum_(at::Tensor &self, int64_t dim,
                    c10::optional<at::ScalarType> dtype) {
  AtenIpexTypeExt::cumsum(self, self, dim, dtype);
  return self;
}

at::Tensor &cumsum_out(const at::Tensor &self, int64_t dim,
                       c10::optional<at::ScalarType> dtype,
                       at::Tensor &result) {
  AtenIpexTypeExt::cumsum(result, self.toType(result.scalar_type()), dim,
                          dtype);
  return result;
}
...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  m.def("cumsum(Tensor self, int dim, *, ScalarType? dtype=None) -> Tensor",
        torch_ipex::cumsum);
  m.def("cumsum_(Tensor(a!) self, int dim, *, ScalarType? dtype=None) -> "
        "Tensor(a!)",
        torch_ipex::cumsum_);
  m.def("cumsum.out(Tensor self, int dim, *, ScalarType? dtype=None, "
        "Tensor(a!) out) -> Tensor(a!)",
        torch_ipex::cumsum_out);
}
