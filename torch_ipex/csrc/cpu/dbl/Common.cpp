...
dil::tensor dil_tensor_from_dil_buffer(const at::Tensor& tensor) {
  ...
  if (dil_buffer.is_public_format() &&
    ...
    if (dil_buffer.has_conv_params()) {
      result.copy_conv_params(dil_buffer);
    }
    if (dil_buffer.has_inner_product_params()) {
      result.copy_inner_product_params(dil_buffer);
    }
...
std::vector<std::vector<float>> get_int8_scales(const at::TensorList& inputs, bool uint8_used, const int64_t ops_id) {
  IPEX_CHECK(check_auto_mix_int8_fp32(), "Need enable auto mix_int8 _p32 to query int8 scales");
  IPEX_CHECK(!check_int8_calibration(), "Should query int8 scales after calibration");
