...
public:
  ...
  inline void add_indictors() {
    num_ops_id = 0;
    auto Qmin = -128; // -128 for int8, and 0 for uint8
    for (auto i = 0; i < observers_.size(); i++) {
    std::vector<float> scales, zero_points;
      for (auto j = 0; j < observers_[i].mins.size(); j++) {
        scales.push_back((observers_[i].maxs[j] - observers_[i].mins[j]) / 255);
        zero_points.push_back(Qmin - std::round(observers_[i].mins[j] / scales[j]));
      }
  ...
  inline std::tuple<std::vector<float>, std::vector<float>> get_indictor_scales(bool uint8_used) {
    ...
    num_ops_id++;
    return std::make_tuple(indicators_[num_ops_id].scales, indicators_[num_ops_id].zero_points);
  }
