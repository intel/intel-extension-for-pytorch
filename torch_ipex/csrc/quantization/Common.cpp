...
void insert_or_updata_observer(const at::TensorList& inputs,
  ...
  if (weight_granularity == "per_channel") {
    for (auto k = 0; k < weights.size(); k++) {
      ...
      for (int l = 0; l < w.size(0); l++) {
        min_max_values.push_back({w[l].min().item<float>(), w[l].max().item<float>()});
      }
    ...
    for (auto k = 0; k < weights.size(); k++) {
      std::vector<std::vector<float>> min_max_values;
      min_max_values.push_back({weights[k].min().item<float>(), weights[k].max().item<float>()});
      weights_min_max_values.push_back(min_max_values);
    }
  }
  ...
  Int8OptConfig::get_config().insert_or_updata_observer(
      op_name, inputs_min_max_values, weights_min_max_values, outputs_min_max_values, ops_id,
