...
at::Tensor conv2d(const at::Tensor &input, const at::Tensor &weight, const c10::optional<at::Tensor> &bias, at::IntArrayRef stride, at::IntArrayRef padding, at::IntArrayRef dilation, int64_t groups) {
  if (torch_ipex::check_int8_calibration()) {
    ...
    std::tie(inputs_quantized, outputs_quantized) = torch_ipex::get_int8_insert_quantized_status(num_ops_id);
    auto conv_x = input;
    auto conv_w = weight;
...
at::Tensor add_tensor(const at::Tensor &input, const at::Tensor &other, const at::Scalar &alpha) {
  ...
  std::vector<std::vector<float>> scales = torch_ipex::get_int8_scales({false, false}, {false}, num_ops_id);
  std::vector<bool> inputs_quantized, outputs_quantized;
  std::tie(inputs_quantized, outputs_quantized) = torch_ipex::get_int8_insert_quantized_status(num_ops_id);
  auto add_x1 = input;
  auto add_x2 = other;
  ...
  if (inputs_quantized[0]) {
    // add quantize and dequantize for input and weight.
    auto input_q = at::quantize_per_tensor(input, scales[0][0], 0, at::kQInt8);
    add_x1 = input_q.dequantize();
  }

  if (inputs_quantized[1]) {
    // add quantize and dequantize for input and weight.
    auto input_q2 = at::quantize_per_tensor(other, scales[0][1], 0, at::kQInt8);
