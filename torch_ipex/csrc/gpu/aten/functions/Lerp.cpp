...
struct TensorLerpScaleOp {
  ...
  void operator()(T &out, T &a, T &b) const {
    ...
    ) : Numerics<T>::sub(b, Numerics<T>::mul(Numerics<T>::sub(b, a), Numerics<T>::sub(1, w)));
  }
...
struct TensorLerpOp {
  void operator()(T &out, T &a, T &b, T &weight) const {
    ...
    Numerics<T>::sub(b, Numerics<T>::mul(Numerics<T>::sub(b, a), Numerics<T>::sub(1, weight)));
  }
};

template <typename scalar_t>
void lerp(at::Tensor& ret, const at::Tensor& self, const at::Tensor& end, const at::Tensor& weight) {
  DPCPP_tensor_apply4<scalar_t, scalar_t, scalar_t, scalar_t>(ret, self, end, weight, TensorLerpOp<scalar_t>());
}

template <typename scalar_t>
void lerp(at::Tensor& ret, const at::Tensor& self, const at::Tensor& end, scalar_t weight_val) {
  DPCPP_tensor_apply3<scalar_t, scalar_t, scalar_t>(ret, self, end, TensorLerpScaleOp<scalar_t>(weight_val));
}

} //namespace impl

  
Tensor & lerp_out(Tensor & out, const Tensor & self,
