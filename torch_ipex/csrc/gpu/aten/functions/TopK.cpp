...
void gatherTopK(TensorInfo<T, IndexType> input,
  ...
  auto cgf = DP_Q_CGF(cgh) {
    ...
    auto kfn = DP_Q_KFN(DP::nd_item<1> item_id) {
      ...
      IndexType numIterations = RoundUp(inputSliceSize, (IndexType)item_id.get_local_range(0));
      IndexType writeIndexStart = 0;
      for (IndexType i = local_id; i < numIterations; i += item_id.get_local_range(0)) {
        bool inRange = (i < inputSliceSize);
        T v = inRange ?
              inputSliceStart[i * inputWithinSliceStride] :
