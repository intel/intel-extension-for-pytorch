...
DP_DEVICE void inclusivePrefixScan(T *smem, BinaryOp binop, const DP::nd_item<1> &item_id) {
  // Reduce step ("upsweep")
...
DP_DEVICE void inclusivePrefixScan(const dp_local_acc_t<T>& smem, T in, T* out,
    BinaryFunction binop, const DP::nd_item<1> &item_id) {
  // FIXME: this is slow
  int threadIdx = item_id.get_local_id(0);
  smem[threadIdx] = in;
  item_id.barrier(dp_local_fence);
...
template <typename T, class BinaryFunction>
DP_DEVICE void exclusivePrefixScan(const dp_local_acc_t<T>&smem, T in, T* out,
    T* carry, BinaryFunction binop, const DP::nd_item<1> &item_id) {
