...
DPCPP_DEVICE void inclusivePrefixScan(T *smem, BinaryOp binop, const DPCPP::nd_item<1> &item_id) {
  // Reduce step ("upsweep")
...
DPCPP_DEVICE void inclusivePrefixScan(const dpcpp_local_acc_t<T>& smem, T in, T* out,
    BinaryFunction binop, const DPCPP::nd_item<1> &item_id) {
  // FIXME: this is slow
  int threadIdx = item_id.get_local_id(0);
  smem[threadIdx] = in;
  item_id.barrier(dpcpp_local_fence);
...
template <typename T, class BinaryFunction>
DPCPP_DEVICE void exclusivePrefixScan(const dpcpp_local_acc_t<T>&smem, T in, T* out,
    T* carry, BinaryFunction binop, const DPCPP::nd_item<1> &item_id) {
