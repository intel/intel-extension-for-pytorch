...
template <typename _Inclusive, typename _NDItemId, typename _GlobalIdx, typename _Size, typename _AccLocal,
        typename _InAcc, typename _OutAcc, typename _Tp, typename _Fp,
        typename _BinaryOp, typename _UnaryOp>
void
...
struct scan
{
  ...
  template <typename _NDItemId, typename _GlobalIdx, typename _Size, typename _AccLocal, typename _InAcc, typename _OutAcc>
  void
  operator()(const _NDItemId __item_id, const _GlobalIdx __global_idx, const _Size __n, const _AccLocal& __local_mem,
             const _InAcc& __input, const _OutAcc& __result, _Tp __init) const
  {
    auto __local_idx = __item_id.get_local_id(0);
    ...
    write_to_global<_Inclusive>(__item_id, __global_idx, __n, __local_mem, __input, __result,
            __old_init, __f, __bin_op, __unary_op);
  }
};
...
struct reduce
{
  ...
  _Tp
  operator()(const _NDItemId __item_id, const _GlobalIdx __global_idx, const _Size __n, const _AccLocal& __local_mem) const
  {
    auto __local_idx = __item_id.get_local_id(0);
    auto __group_size = __item_id.get_local_range().size();
...
struct transform_init
{
  ...
  void
  operator()(const _NDItemId __item_id, const _GlobalIdx __global_idx, const _Acc& __acc,
             _Size __n, const _AccLocal& __local_mem) const
  {
    auto __local_idx = __item_id.get_local_id(0);
    auto __global_range_size = __item_id.get_global_range().size();
    auto __n_iter = __n / __global_range_size;
                        ...
                        _Transform __brick_transform, _Reduce __brick_reduce, _Scan __brick_scan)
{
  ...
  auto cgf_2 = DP_Q_CGF(cgh) {
    ...
    auto kfn = DP_Q_KFN(DP::nd_item</*dim=*/1> item_id) {
      ...
      __brick_transform(item_id, __global_idx, __acc_ptr, __n, __transform_local);
      ...
      // 3. local reduce
      auto __local_reduce = __brick_reduce(item_id, __global_idx, __n, __reduce_local_mem);
                  ...
                  TensorInfo<OutputType, IndexType>& output, IndexType num_elements, _CreateMaskOp __create_mask_op, _CopyByMaskOp __copy_by_mask_op)
{
  ...
  return parallel_transform_scan(queue, input, output,  num_elements, __reduce_op, IndexType{0},
          transform_init<_ReduceOp, _CreateMaskOp>{__reduce_op, __create_mask_op},
          reduce<_ReduceOp, IndexType>{__reduce_op},
          scan</*inclusive*/ std::true_type, _ReduceOp, _GetMaskOp, _CopyByMaskOp, IndexType>
                  {__reduce_op, __get_mask_op, __copy_by_mask_op});
}
...
private:
  ...
  idx_functor(const TensorInfo<T, IndexType>& t_info, c10::guts::index_sequence<I...>) : dims(t_info.dims),
                                                                                         sz{t_info.sizes[I]...},
                                                                                         st{t_info.strides[I]...} {}
};
             ...
             Predicate __pred, CopyMaskOp __copy_by_mask_op)
{
  ...
  auto __result = pattern_scan_copy(queue, input, output, num_elements, __create_mask_op, __copy_by_mask_op);
