...
static inline void dpcpp_set_mkldnn_buffer(void *vptr, mkldnn::memory &memory) {
  ...
  if (dpcppGetBufferMap().get_offset(vptr) == 0) {
    // if offset is 0, which means this dpcpp_buffer is exact the corresponding one for this vptr, we can safely set it to mkl-dnn dpcpp API
    auto buffer = dpcppGetBufferMap().get_buffer(vptr);
    memory.template set_sycl_buffer<uint8_t, 1>(buffer);
  } else {
    // Currently, memory offset can't have representation in dpcpp buffer.It's difficult to handle
    ...
    AT_ERROR("the offset of dpcpp buffer is not 0. We don't support this case.");
  #if 0
...
struct GpuStreamManager {
  ...
  stream get_stream(int device_index = 0) {
    ...
    return mkldnn::stream(GpuEngineManager::Instance().get_engine({kDPCPP, current_device()}),
