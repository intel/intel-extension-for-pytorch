...
uint64_t DPCPPGenerator::current_seed() const { return seed_; }
...
DPCPPGenerator::philox_engine_inputs(uint64_t increment) {
  uint64_t offset = this->philox_offset_per_thread_;
  this->philox_offset_per_thread_ += increment;
  return std::make_pair(this->seed_, offset);
}

DeviceType DPCPPGenerator::device_type() { return DeviceType::DPCPP; }
