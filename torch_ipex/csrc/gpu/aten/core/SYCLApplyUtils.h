...
inline static void apply(const detail::TensorInfo<scalar, IndexType> &a,
  ...
  ApplyOp1<Op, scalar, IndexType, ADims, with_offset, remaining_steps - 1, const IndexType, Offsets...>::apply(
      a, op, a_acc, n, linearIndex + 1, aOffsets..., aOffset);
}
};
...
inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
  ...
  ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, with_offset, remaining_steps - 1, const IndexType, Offsets...>::apply(
      a, b, op, a_acc, b_acc, n, linearIndex + 1, aOffsets..., aOffset, bOffsets..., bOffset);
}
};
...
template <typename Op,
         ...
         int BDims,
         typename Offset>
struct ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, false, 0, Offset> {
  inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                           const detail::TensorInfo<scalar2, IndexType> &b,
                           const Op &op,
                           const c10::sycl::SYCLAccessor<write_mode> &a_acc,
...
template <typename Op,
         ...
         int BDims,
         typename Offset>
struct ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, true, 0, Offset> {
  inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                           const detail::TensorInfo<scalar2, IndexType> &b,
                           const Op &op,
                           const c10::sycl::SYCLAccessor<write_mode> &a_acc,
...
template <typename Op,
         ...
         bool with_offset,
         typename... Offsets>
struct ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, with_offset, 0, Offsets...> {
inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                         const detail::TensorInfo<scalar2, IndexType> &b,
                         const Op &op,
                         const c10::sycl::SYCLAccessor<write_mode> &a_acc,
...
inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
  ...
  ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims, remaining_steps - 1, const IndexType, Offsets...>::apply(
    a, b, c, op, a_acc, b_acc, c_acc, n, linearIndex + 1, aOffsets..., aOffset, bOffsets..., bOffset, cOffsets..., cOffset);
}
};
...
template <typename Op,
      ...
      int CDims,
          typename Offset>
struct ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims, 0, Offset> {
  inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                           const detail::TensorInfo<scalar2, IndexType> &b,
               const detail::TensorInfo<scalar3, IndexType> &c,
                           const Op &op,
...
template <typename Op,
      ...
      int CDims,
          typename... Offsets>
struct ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims, 0, Offsets...> {
inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                         const detail::TensorInfo<scalar2, IndexType> &b,
             const detail::TensorInfo<scalar3, IndexType> &c,
                         const Op &op,
  ...
  inline static void apply(
    ...
    ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType,
        ADims, BDims, CDims, DDims, remaining_steps - 1, const IndexType, Offsets...>::apply(
            a, b, c, d, op, a_acc, b_acc, c_acc, d_acc, n, linearIndex + 1,
            aOffsets..., aOffset, bOffsets..., bOffset, cOffsets..., cOffset, dOffsets..., dOffset);
  }
};
...
template <typename Op,
          ...
          int ADims, int BDims, int CDims, int DDims,
          typename Offset>
struct ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims, CDims, DDims, 0, Offset> {
  inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                           const detail::TensorInfo<scalar2, IndexType> &b,
               const detail::TensorInfo<scalar3, IndexType> &c,
                           const detail::TensorInfo<scalar3, IndexType> &d,
...
template <typename Op,
          ...
          int ADims, int BDims, int CDims, int DDims,
          typename... Offsets>
struct ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims, CDims, DDims, 0, Offsets...> {
  inline static void apply(const detail::TensorInfo<scalar1, IndexType> &a,
                           const detail::TensorInfo<scalar2, IndexType> &b,
               const detail::TensorInfo<scalar3, IndexType> &c,
                           const detail::TensorInfo<scalar3, IndexType> &d,
    ...
    auto c_ptr = c_acc.template get_pointer<scalar3>();
    auto d_ptr = d_acc.template get_pointer<scalar4>();
    op(n, a_ptr[aOffsets]..., b_ptr[bOffsets]..., c_ptr[cOffsets]..., d_ptr[dOffsets]...);
  }
};
...
#define HANDLE_A_CASE(TYPE, A, B) {     \
  ...
  if (detail::canUse32BitIndexMath(dst) &&
      detail::canUse32BitIndexMath(src)) {
...
#define HANDLE_A_CASE(TYPE, A, B, C) {      \
  ...
  if (detail::canUse32BitIndexMath(dst) &&
      detail::canUse32BitIndexMath(src1) &&
 ...
 //   HANDLE_A_CASE(unsigned int, dstInfo.dims, src1Info.dims, src2Info.dims);
  } else {
    detail::TensorInfo<scalar1, uint64_t> dstInfo =
      detail::getTensorInfo<scalar1, uint64_t>(dst);
...
#define HANDLE_CASE(TYPE, A, B, C, D)                                  \
  kernelPointwiseApply4<Op,                                            \
                        ...
                        scalar4,                                       \
                        TYPE, A, B, C, D, step>                        \
    (dstInfo, src1Info, src2Info, src3Info, static_cast<TYPE>(totalElements), op);
...
#define HANDLE_A_CASE(TYPE, A, B, C, D) {       \
  ...
  if (detail::canUse32BitIndexMath(dst) &&
    ...
    if (dstInfo.dims == 1 && src1Info.dims == 1 && src2Info.dims == 1 && src3Info.dims == 1) {
      HANDLE_CASE(unsigned int, 1, 1, 1, 1);
    } else {
      HANDLE_CASE(unsigned int, -1, -1, -1, -1);
    }

    // HANDLE_CASE(uint64_t, dstInfo.dims, src1Info.dims, src2Info.dims, src3Info.dims);
  } else {
    detail::TensorInfo<scalar1, uint64_t> dstInfo =
      detail::getTensorInfo<scalar1, uint64_t>(dst);
