...
#define IMPLEMENT_POINTWISE_FUNC_(NAME, CFUNC, REAL)                           \
  template <typename scalar_t> struct Tensor_##NAME##_##REAL##_Op {            \
    ...
    inline void operator()(scalar_t &v) const { v = CFUNC(v); }                \
                                                                               ...
                                                                               \
  template <typename scalar_t> void NAME(Tensor &self_, const Tensor &src) {   \
...
#define IMPLEMENT_POINTWISE_CALLABLE_(NAME, APPLY_NUM, APPLY_NUM_EXT,          \
                                      CALLABLE, CALLABLE_ARGS_NUM)             \
...
#define IPEX_OUT_INPLACE_UNARY_FUNC_OPS(op, func, real, types)                 \
  IPEX_FUNC_OPS(op##_out, func, real, types)                                   \
                                                                               \
  Tensor &op##_(Tensor &self) { return at::op##_out(self, self); }
...
#define IPEX_CALLABLE_OPS(op, callable, types, oprand_num, arg_num,            \
                          callable_args_num)                                   \
  namespace impl {                                                             \
  IMPLEMENT_POINTWISE_##oprand_num##_CALLABLE_##callable_args_num(op,          \
                                                                  callable)    \
  }                                                                            \
                                                                               \
...
template <typename T> struct TensorATan2Op {
  void operator()(T &out, T &a, T &b) const { out = Numerics<T>::atan2(a, b); }
};
...
template <typename T> struct TensorSigmoidGradOp {
  void operator()(T &gradInput, T &output, T &gradOutput) const {
    gradInput = gradOutput * (1.f - output) * output;
  }
};

template <> struct TensorSigmoidGradOp<at::Half> {
  void operator()(at::Half &gradInput, at::Half &output,
                  at::Half &gradOutput) const {
 ...
 */
template <typename T> struct TensorDigammaOp {
  void operator()(T &out, T &in) const {

    using compute_type =
        typename std::conditional<std::is_same<T, at::Half>::value, float,
                                  T>::type;
...
template <typename T> struct TensorErfinvOp {
  void operator()(T &out, T &in) const {
    ...
    static const double PI_f64 = 3.14159265358979323846;
    static const compute_type a[4] = {0.886226899, -1.645349621, 0.914624893,
                                      -0.140543331};
    static const compute_type b[4] = {-2.118377725, 1.442710462, -0.329097515,
                                      0.012229801};
    static const compute_type c[4] = {-1.970840454, -1.624906493, 3.429567803,
                                      1.641345311};
    ...
    if (DPCPP::fabs(x) == 1.0) {
      out = scalar_cast<T>((DPCPP::copysign(1.0, scalar_cast<double>(x))) *
    ...
    if (DPCPP::fabs(x) <= 0.7) {
      ...
      out = scalar_cast<T>(scalar_cast<compute_type>(
                               DPCPP::copysign(1.0, scalar_cast<double>(x))) *
                           num / dem);
    }
    out = out -
          scalar_cast<T>((DPCPP::erf(scalar_cast<double>(out)) - x) /
  ...
  void operator()(T &v) const {
    ...
    if (DPCPP::fabs(x) == 1.0) {
      v = scalar_cast<T>((DPCPP::copysign(1.0, scalar_cast<double>(x))) *
                         (scalar_cast<double>(INFINITY)));
      return;
    }
    if (DPCPP::fabs(x) <= 0.7) {
      ...
      num = ((c[3] * z + c[2]) * z + c[1]) * z + c[0];
      dem = (d[1] * z + d[0]) * z + scalar_cast<compute_type>(1.0);
      v = scalar_cast<T>(scalar_cast<compute_type>(
                             DPCPP::copysign(1.0, scalar_cast<double>(x))) *
                         num / dem);
    }
    v = v - scalar_cast<T>((DPCPP::erf(scalar_cast<double>(v)) - x) /
                           ((2.0 / DPCPP::sqrt(PI_f64)) * DPCPP::exp(-x * x)));
    v = v - scalar_cast<T>((DPCPP::erf(scalar_cast<double>(v)) - x) /
                           ((2.0 / DPCPP::sqrt(PI_f64)) * DPCPP::exp(-x * x)));
    return;
...
template <> struct TensorSignOp<bool> {
  void operator()(bool &out, bool &in) const { out = in; }
...
template <typename T> struct TensorAddCMulOp {
  ...
  void operator()(T &out, T &in1, T &in2) const {
    out = Numerics<T>::add(out,
                           Numerics<T>::mul(val, Numerics<T>::mul(in1, in2)));
  }
...
template <typename T> struct TensorAddCDivOp {
  ...
  void operator()(T &out, T &in1, T &in2) const {
    out = Numerics<T>::add(out,
                           Numerics<T>::mul(val, Numerics<T>::div(in1, in2)));
  }
...
template <> struct TensorCRemainderOp<at::Half> {
...
template <typename T> struct TensorCFmodOp {
  void operator()(T &out, T &in) const { out = out % in; }

  void operator()(T &out, T &in1, T &in2) const { out = in1 % in2; }
};

template <> struct TensorCFmodOp<float> {
  void operator()(float &out, float &in) const { out = DPCPP::fmod(out, in); }
...
template <> struct TensorCFmodOp<double> {
  void operator()(double &out, double &in) const { out = DPCPP::fmod(out, in); }
...
template <> struct TensorCFmodOp<at::Half> {
  void operator()(at::Half &out, at::Half &in) const {
...
template <typename T> struct TensorCPowOp {
  void operator()(T &out, T &in) const { out = Numerics<T>::pow(out, in); }
...
template <> struct TensorCPowOp<float> {
  void operator()(float &out, float &in) const { out = DPCPP::pow(out, in); }
...
template <> struct TensorCPowOp<double> {
  void operator()(double &out, double &in) const { out = DPCPP::pow(out, in); }
...
template <typename T> struct TensorPowOp<T, -1> {
  TensorPowOp(T v) : val(v) {}
  void operator()(T &out, T &in) const { out = Numerics<T>::cinv(in); }

  void operator()(T &v) const { v = Numerics<T>::cinv(v); }

  const T val;
};

template <typename T> struct TensorPowOp<T, -2> {
  TensorPowOp(T v) : val(v) {}
  void operator()(T &out, T &in) const {
    T square = Numerics<T>::mul(in, in);
...
template <typename T> struct TensorTPowOp {
  ...
  void operator()(T &out, T &in) const { out = Numerics<T>::pow(val, in); }

  void operator()(T &v) const { v = Numerics<T>::pow(val, v); }
