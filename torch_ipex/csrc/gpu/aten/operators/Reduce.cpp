...
void std_var_kernel_impl(TensorIterator& iter, bool unbiased, bool take_sqrt) {
  ...
  dpcpp_reduce_kernel<scalar_t, scalar_t, 2>(iter, WelfordOps<scalar_t, scalar_t, int32_t, float, std::pair<scalar_t, scalar_t>> { unbiased, take_sqrt }, WelfordData<scalar_t, int32_t, float> {});
}
...
void std_var_kernel_impl<at::Half>(TensorIterator& iter, bool unbiased, bool take_sqrt) {
  // reducing unrolling factor to 2 for welford kernel
  // This is necessary to lower register usage that leads to register spills.
  dpcpp_reduce_kernel<at::Half, at::Half, 2>(iter, WelfordOps<at::Half, float, int32_t, float, std::pair<at::Half, at::Half>> { unbiased, take_sqrt }, WelfordData<float, int32_t, float> {});
}

template <typename scalar_t, typename acc_t=scalar_t, typename out_t=scalar_t>
void sum_kernel_impl(TensorIterator& iter) {
  dpcpp_reduce_kernel<scalar_t, out_t>(iter, func_wrapper<out_t>(add_kernel<acc_t>()));
}
...
void min_kernel_impl(TensorIterator& iter) {
  dpcpp_reduce_kernel<scalar_t, out_t>(iter, func_wrapper<scalar_t>(min_kernel<scalar_t>()), Numerics<scalar_t>::upper_bound());
}

template <typename scalar_t, typename acc_t=scalar_t, typename out_t=scalar_t>
void max_kernel_impl(TensorIterator& iter) {
  dpcpp_reduce_kernel<scalar_t, out_t>(iter, func_wrapper<scalar_t>(max_kernel<scalar_t>()), Numerics<scalar_t>::lower_bound());
}
...
static void norm_kernel_impl(TensorIterator& iter, Scalar val) {
  ...
  if (p == static_cast<float>(0)) {
    ...
    dpcpp_reduce_kernel<scalar_t, out_t>(iter, AbsMaxOps<acc_t>(), std::numeric_limits<acc_t>::min());
  } else if (p == static_cast<float>(-INFINITY)) {
    dpcpp_reduce_kernel<scalar_t, out_t>(iter, AbsMinOps<acc_t>(), std::numeric_limits<acc_t>::max());
  } else {
    dpcpp_reduce_kernel<scalar_t, out_t>(iter, NormOps<acc_t>{ acc_t(p) }, 0);
  }
}

static void std_var_kernel_dpcpp(TensorIterator& iter, bool unbiased, bool take_sqrt) {
...
void and_kernel_dpcpp(TensorIterator& iter) {
  dpcpp_reduce_kernel<uint8_t, uint8_t>(iter, func_wrapper<uint8_t> (and_kernel<uint8_t>()), true);
}

void or_kernel_dpcpp(TensorIterator& iter) {
  dpcpp_reduce_kernel<uint8_t, uint8_t>(iter, func_wrapper<uint8_t> (or_kernel<uint8_t>()), false);
}

} // namespace impl

Tensor & sum_out(Tensor & result, const Tensor & self,
    IntArrayRef dim, bool keepdim, c10::optional<at::ScalarType> opt_dtype) {
