...
Tensor& elu_out(Tensor& out, const Tensor& self, Scalar alpha, Scalar scale, Scalar input_scale) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(iter.dtype(), "elu", [&]() {
    ...
    dpcpp_kernel_for_tensor_iter<DPCPP_K(SyclOpElu)>(iter, [=](scalar_t x) -> scalar_t {
      x = x <= 0 ? (Numerics<scalar_t>::exp(x * negiptocoef) - 1) * negcoef : x * poscoef;
      return x;
    });
  });
...
Tensor& elu_backward_out(Tensor& grad_input, const Tensor& grad_output, Scalar alpha, Scalar scale, Scalar input_scale, const Tensor& output) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(iter.dtype(), "elu_backward", [&]() {
    ...
    dpcpp_kernel_for_tensor_iter<DPCPP_K(SyclOpEluBackward)>(iter, [=](scalar_t grad_output, scalar_t output) -> scalar_t {
      if (output <= 0)
...
Tensor elu_backward(const Tensor& grad_output, Scalar alpha, Scalar scale, Scalar input_scale, const Tensor & output) {
  ...
  return at::AtenIpexTypeDPCPP::elu_backward_out(grad_input, grad_output, alpha, scale, input_scale, output);
}
