...
using ACC_1D_RW_G = accessor<buffer_data_type_t, 1, access::mode::read_write,
                             DPCPP::access::target::global_buffer>;
using ACC_1D_W_G = accessor<buffer_data_type_t, 1, access::mode::write,
                            DPCPP::access::target::global_buffer>;
...
template <typename META_TYPE> class MTRandomEngine {
public:
  MTRandomEngine(ACC_1D_W_G ptr, int64_t ele_num, uint64_t seed, META_TYPE min,
                 META_TYPE max)
      : m_ptr(ptr) {
    m_ele_num = ele_num;
    m_seed = seed;
  ...
  void generate_random_numbers(const int &global_linear_id,
                               const int &seed_offset, uint32_t *mt) {
...
public:
  HalfRandomFiller(ACC_1D_W_G ptr, int64_t ele_num, uint64_t seed, float min,
                   float max)
...
public:
  FloatRandomFiller(ACC_1D_W_G ptr, int64_t ele_num, uint64_t seed, float min,
                    float max)
...
public:
  DoubleRandomFiller(ACC_1D_W_G ptr, int64_t ele_num, uint64_t seed, double min,
                     double max)
...
template <typename META_TYPE> class NormalRandomFiller {
public:
  NormalRandomFiller(ACC_1D_RW_G ptr, int64_t compute_num, double stdv,
                     double mean)
      : m_ptr(ptr) {
