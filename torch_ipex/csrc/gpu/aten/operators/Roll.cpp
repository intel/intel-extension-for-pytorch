...
template <typename scalar_t> class roll_dpcpp_ker {};

template <typename scalar_t>
void roll_dpcpp_kernel(const Tensor &in_tensor, Tensor &out_tensor, int64_t N,
                       int64_t roll_dim, int64_t start, int64_t size,
                       int64_t stride, int64_t total_dims) {
...
Tensor roll_dpcpp(const Tensor &self, IntArrayRef shifts, IntArrayRef dims) {
  ...
  AT_DISPATCH_FLOATING_TYPES(in_tensor.scalar_type(), "roll_dpcpp", [&] {
    roll_dpcpp_kernel<scalar_t>(in_tensor, out_tensor, N, dim, start, size,
                                in_tensor.stride(dim), total_dims);
  });
  return out_tensor;
}
