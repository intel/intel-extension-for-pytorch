...
std::tuple<Tensor &, Tensor &> _min_out(Tensor &min, Tensor &min_indices,
  ...
  AT_DISPATCH_ALL_TYPES(min.scalar_type(), "_min_out", [&] {
    ...
    reduceDimIndex<scalar_t, int64_t>(min, min_indices, self, dim, keepdim,
                                      init, MinValuePair<scalar_t, int64_t>());
  });
...
std::tuple<Tensor &, Tensor &> min_out(Tensor &min, Tensor &min_values,
                                       const Tensor &self, int64_t dim,
                                       bool keepdim) {
  return AtenIpexTypeDPCPP::_min_out(min, min_values, self, dim, keepdim);
}

std::tuple<Tensor &, Tensor &> _max_out(Tensor &max, Tensor &max_indices,
                                        const Tensor &self, int64_t dim,
                                        bool keepdim) {
  AT_DISPATCH_ALL_TYPES(max.scalar_type(), "_max_out", [&] {
    ...
    reduceDimIndex<scalar_t, int64_t>(max, max_indices, self, dim, keepdim,
                                      init, MaxValuePair<scalar_t, int64_t>());
  });
...
std::tuple<Tensor &, Tensor &> max_out(Tensor &max, Tensor &max_values,
                                       const Tensor &self, int64_t dim,
                                       bool keepdim) {
