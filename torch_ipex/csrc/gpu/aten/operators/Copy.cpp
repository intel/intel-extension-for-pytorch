...
template <typename T> struct inter_copy_type { using type = T; };

template <> struct inter_copy_type<uint8_t> { using type = int64_t; };

template <typename T>
using inter_copy_type_t = typename inter_copy_type<T>::type;

template <typename dst_T, typename src_T> class copy_functor {
public:
  copy_functor() {}
...
template <typename dst_T, typename src_T> struct CopyOp {
...
void copy_device_to_device(TensorIterator &iter, bool non_blocking) {
  ...
  if (memcpy_eligible) {
    dpcppMemcpyAsync(iter.data_ptr(0), iter.data_ptr(1),
                     numel * iter.element_size(0), DeviceToDevice);
  } else {
...
void copy_from_cpu(TensorIterator &iter, bool non_blocking) {
  ...
  dpcppMemcpy(dst_contig.data_ptr(), src_contig.data_ptr(),
              src.numel() * src.dtype().itemsize(), HostToDevice);
  AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::Bool,
                             src.scalar_type(), "copy_from_cpu", [&]() {
...
void copy_to_cpu(TensorIterator &iter, bool non_blocking) {
  ...
  dpcppMemcpy(dst_contig.data_ptr(), src_contig.data_ptr(),
              src.numel() * src.dtype().itemsize(), DeviceToHost);
...
void copy_from_cpu_async_(TensorIterator &iter) {
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::Bool, src.scalar_type(),
      "copy_from_cpu_async", [&]() {
        dpcppMemcpyAsync(dst.data_ptr<scalar_t>(), src.data_ptr<scalar_t>(),
                         src.numel() * sizeof(scalar_t), HostToDevice);
      });
}
...
void copy_to_cpu_async_(TensorIterator &iter) {
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      "copy_to_cpu_async", [&]() {
        dpcppMemcpyAsync(dst.data_ptr<scalar_t>(), src.data_ptr<scalar_t>(),
                         src.numel() * sizeof(scalar_t), DeviceToHost);
      });
}
                               ...
                               if (dst.device().type() == at::kDPCPP &&
                                 ...
                                 if (dst.device().type() == at::kDPCPP) {
                                   if (std::is_same<dst_T, scalar_t>::value) {
                                     ...
                                     // then copy to device
                                     Tensor srcf = at::empty_like(
                                         src, src.options().dtype(dst.dtype()));
                                     // DispatchStub is not exposed by torch
                                   ...
                                   if (std::is_same<dst_T, scalar_t>::value) {
                                     ...
                                     Tensor srcf = at::empty_like(
                                         src, dst.options().dtype(src.dtype()));
                                     BUILD_TENSOR_ITER(srcf, src, iter1)
...
void copy_kernel_dpcpp(TensorIterator &iter, bool non_blocking) {
  AT_DISPATCH_ALL_TYPES_AND2(
      ScalarType::Half, ScalarType::Bool, iter.tensor(0).scalar_type(),
      "_copy__dpcpp", [&]() { _copy__dpcpp<scalar_t>(iter, non_blocking); });
}
