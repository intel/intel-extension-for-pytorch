  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_FLOATING_TYPES(r.scalar_type(), "linspace_dpcpp", [&]() {
      ...
      auto cgf = DPCPP_Q_CGF(cgh) {
        auto acc =
            DPCPPAccessor<dpcpp_discard_w_mode>(cgh, r.data_ptr<scalar_t>());
...
Tensor& arange_dpcpp_out(
  ...
  AT_DISPATCH_ALL_TYPES_AND(
      at::ScalarType::Half, result.scalar_type(), "arange_dpcpp", [&]() {
        ...
        // float32 on gpu for a float32 scalar_t,
        // but double on cpu for the same,
        // and the effective output size starts differing on CPU vs GPU because
        // of precision issues, which
        // we dont want.
        // the corner-case we do want to take into account is int64_t, which has
