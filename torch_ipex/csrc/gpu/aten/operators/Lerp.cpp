...
template <typename T> struct TensorLerpOp {
  void operator()(T &out, T &a, T &b, T &weight) const {
    out = (weight < 0.5)
              ...
              : Numerics<T>::sub(b,
                                 Numerics<T>::mul(Numerics<T>::sub(b, a),
                                                  Numerics<T>::sub(1, weight)));
  }
};

template <typename scalar_t>
void lerp(at::Tensor &ret, const at::Tensor &self, const at::Tensor &end,
...
Tensor &lerp_out(Tensor &out, const Tensor &self, const Tensor &end,
                 const Tensor &weight) {
...
Tensor &lerp_(Tensor &self, const Tensor &end, const Tensor &weight) {
  ...
  TORCH_CHECK(b_self.sizes() == self.sizes(), "output with shape ",
              self.sizes(), " doesn't match the broadcast shape ",
              b_self.sizes());
  TORCH_CHECK(
      weight.dim() <= std::max(self.dim(), end.dim()),
      "weight should be of dimension max(self.dim(), end.dim()) or lesser");
