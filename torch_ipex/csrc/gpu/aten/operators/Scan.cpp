...
template <typename T, typename BinOp> class scanthrust_dpcpp_ker {};

template <typename T, typename BinOp> class scanOuterDim_dpcpp_kernel {};

template <typename T, typename BinOp> class scanInnerDim_dpcpp_kernel {};
...
void scanOuterDim(Tensor &tgt, Tensor &src, int dimension, scalar_t init,
                  BinaryOp binary_op) {
...
template <typename scalar_t, class BinaryFunction>
void scanInnermostDim(Tensor &tgt, Tensor &src, scalar_t init,
...
template <typename scalar_t, class BinaryFunction>
typename std::enable_if<!IS_HALF(scalar_t), void>::type
scanDim(Tensor &self_, const Tensor &src_, int dimension, scalar_t init,
        BinaryFunction binary_op) {
  int ndim = src_.dim() == 0 ? 1 : src_.dim();
  TORCH_CHECK(dimension >= 0 && dimension < ndim, "dimension ", dimension,
              " out of range");
