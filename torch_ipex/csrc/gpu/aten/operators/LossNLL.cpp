...
void ClassNLLCriterion_updateOutput(const Tensor &input, const Tensor &target,
                                    Tensor &output, const Tensor &weights,
                                    Tensor &total_weight, int64_t reduction,
                                    int64_t ignore_index) {
  TORCH_CHECK(input.dim() > 0 && input.dim() <= 2,
              "input tensor should be 1D or 2D");
  TORCH_CHECK(target.dim() == 1,
              "1D target tensor expected, multi-target not supported");
  TORCH_CHECK(input.size(0) == target.size(0), "size mismatch (got input: ",
              input.sizes(), ", target: ", target.sizes(), ")")
  ...
  TORCH_CHECK(!weights.defined() || weights.numel() == n_classes,
              "weight tensor should be defined either for all ", n_classes,
  ...
  if (reduction == at::Reduction::None && n_dims == 2) {
    ...
    auto cgf = DPCPP_Q_CGF(cgh) {
      ...
      auto weights_acc =
          has_weights
              ? DPCPPAccessor<dpcpp_r_mode>(cgh,
...
void ClassNLLCriterion_updateGradInput(
    const Tensor &input, const Tensor &target, const Tensor &gradOutput,
    Tensor &gradInput, int64_t reduction, const Tensor &weights,
    const Tensor &total_weight, int64_t ignore_index) {
  TORCH_CHECK(target.dim() == 1,
              "1D target tensor expected, multi-target not supported");
  ...
  TORCH_CHECK(!weights.defined() || weights.numel() == input.size(-1),
...
nll_loss_forward_out(Tensor &output, Tensor &total_weight, const Tensor &self,
                     const Tensor &target, const Tensor &weight,
                     int64_t reduction, int64_t ignore_index) {
  AT_DISPATCH_ALL_TYPES(
      self.scalar_type(), "ClassNLLCriterion_updateOutput", [&]() {
        impl::ClassNLLCriterion_updateOutput<scalar_t>(self, target, output,
                                                       weight, total_weight,
...
nll_loss_forward(const Tensor &self, const Tensor &target, const Tensor &weight,
                 int64_t reduction, int64_t ignore_index) {
...
Tensor &nll_loss_backward_out(Tensor &grad_input, const Tensor &grad_output,
  ...
  AT_DISPATCH_ALL_TYPES(self.scalar_type(), "ClassNLLCriterion_updateGradInput",
                        [&]() {
                          impl::ClassNLLCriterion_updateGradInput<scalar_t>(
                              self, target, grad_output, grad_input, reduction,
                              weight, total_weight, ignore_index);
                        });
  return grad_input;
}

Tensor nll_loss_backward(const Tensor &grad_output, const Tensor &self,
                         const Tensor &target, const Tensor &weight,
                         int64_t reduction, int64_t ignore_index,
                         const Tensor &total_weight) {
