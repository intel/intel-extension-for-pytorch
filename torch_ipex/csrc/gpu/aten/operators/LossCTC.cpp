...
template <typename...> class ctc_loss_alpha_kernel {};
template <typename...> class ctc_loss_beta_kernel {};
...
void
ctc_loss_log_alpha_kernel(Tensor& log_alpha,
                          const Tensor& log_probs, const Tensor& input_lengths, int64_t __max_input_length,
                          const Tensor& targets, const Tensor& target_lengths, int64_t __max_target_length,
                          Tensor& neg_log_likelihood,
                          const Tensor& tg_batch_offsets, int64_t __tg_target_stride,
                          int64_t __batch_size, int64_t __BLANK) {
  ...
  auto cgf = DPCPP_Q_CGF(cgh) {
    ...
    auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<2> item_id)  {
      ...
      scalar_t* neg_log_likelihood_data = neg_log_likelihood_acc.template get_pointer<scalar_t>();
      int64_t* input_lengths_data = input_lengths_acc.template get_pointer<int64_t>();
      int64_t* target_lengths_data = target_lengths_acc.template get_pointer<int64_t>();
      int64_t* tg_batch_offsets_data = tg_batch_offsets_acc.template get_pointer<int64_t>();

      // bookkeeping
      int64_t b = item_id.get_local_id(1) + item_id.get_group(1) * item_id.get_local_range(1);
      ...
      if (b < batch_size) {
        ...
        for (int64_t s = intra_batch_id; s < 2*max_target_length+1; s += intra_batch_size) {
          scalar_t la;
          switch (s) {
            case 0:
      ...
      for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += intra_batch_size) {
        ...
        for (int64_t t=1; t < max_input_length; t++) {
          ...
          if ((b < batch_size) && (t < input_length) && (s < 2 * target_length + 1)) {
            ...
            log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s] = tmp +
              lamax +
      ...
      if (intra_batch_id == 0) {
        ...
        if (target_length == 0) {
          ...
          neg_log_likelihood_data[b] = -log_alpha_data[la_batch_offset + la_input_stride*(input_length-1) /*+ la_target_stride * 0*/];
        } else {
...
template<typename scalar_t, typename target_t>
void
ctc_loss_backward_log_beta_kernel(Tensor& log_beta,
                                  const Tensor& log_probs, const Tensor& input_lengths, int64_t __max_input_length,
                                  const Tensor& targets, const Tensor& target_lengths, int64_t __max_target_length,
                                  const Tensor& tg_batch_offsets, int64_t __tg_target_stride,
                                  int64_t __batch_size, int64_t __BLANK) {
  ...
  auto cgf = DPCPP_Q_CGF(cgh) {
    ...
    auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<2> item_id)  {
      ...
      if (b < batch_size) {
        ...
        int64_t last_s_offset = 2*max_target_length - (2*max_target_length % intra_batch_size);
        for (int64_t s = last_s_offset + intra_batch_id; s >= 0; s -= intra_batch_size) {
          scalar_t lb;
      ...
      // go backward in s
      for (int64_t block_s = 2*max_target_length - (2*max_target_length % intra_batch_size); block_s >= 0; block_s -= intra_batch_size) {
        int64_t s = intra_batch_id + block_s;
        ...
        for (int64_t t=max_input_length-2; t>=0; t--) {
          ...
          if ((b < batch_size) && (t < input_length - 1) && (s < 2 * target_length + 1)) {
            ...
            scalar_t lb = Numerics<scalar_t>::log(Numerics<scalar_t>::exp(lb1-lbmax)+Numerics<scalar_t>::exp(lb2-lbmax)+Numerics<scalar_t>::exp(lb3-lbmax))+lbmax
...
template<typename scalar_t, typename target_t>
void
ctc_loss_backward_collect_nonblank_kernel(Tensor& gradient,
                                          const Tensor& grad_out, int64_t grad_out_batch_stride,
                                          const Tensor& log_alpha, const Tensor& log_beta,
                                          const Tensor& log_probs, const Tensor& input_lengths, int64_t max_input_length,
                                          const Tensor& targets, const Tensor& target_lengths, int64_t max_target_length,
                                          const Tensor& neg_log_likelihood,
                                          const Tensor& tg_batch_offsets, int64_t tg_target_stride,
  ...
  auto cgf = DPCPP_Q_CGF(cgh) {
    ...
    auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<2> item_id)  {
      ...
      int64_t b = item_id.get_local_id(1) + item_id.get_group(1) * item_id.get_local_range(1);
      int64_t s = item_id.get_local_id(0) + item_id.get_group(0) * item_id.get_local_range(0);
      ...
      for (int64_t t = 0; t < input_length; t++) {
        ...
        atomicAdd(&gradient_data[gr_batch_offset + t * gr_input_stride + gr_char_stride * target],
                  -Numerics<scalar_t>::exp(log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * (s*2+1)]
...
template<typename scalar_t, typename target_t>
void
ctc_loss_backward_collect_kernel(Tensor& gradient,
                                 const Tensor& grad_out, int64_t grad_out_batch_stride,
                                 const Tensor& log_alpha, const Tensor& log_beta,
                                 const Tensor& log_probs, const Tensor& input_lengths, int64_t max_input_length,
                                 const Tensor& targets, const Tensor& target_lengths, int64_t max_target_length,
  ...
  auto cgf = DPCPP_Q_CGF(cgh) {
    ...
    auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<2> item_id)  {
      ...
      int64_t b = item_id.get_local_id(1) + item_id.get_group(1) * item_id.get_local_range(1);
      int64_t t = item_id.get_local_id(0) + item_id.get_group(0) * item_id.get_local_range(0);
      ...
      for (int s = 0; s < 2*max_target_length+1; s++) {
        if (s < 2 * target_length + 1) { // if target_length == 0, s == 0
          ...
          if (lcab == neginf) {
            ...
            lcab = Numerics<scalar_t>::log(Numerics<scalar_t>::exp(lcab-max)+Numerics<scalar_t>::exp(log_alpha_beta-max))+max;
          }
        }
      }
      ...
      for (int64_t c = 0; c < num_labels; c++) {
        ...
        if (t < input_length && (! zero_infinity || nll != INFINITY)) {
          ...
          res = (Numerics<scalar_t>::exp(lp)-Numerics<scalar_t>::exp(res + nll - lp)) * gr;
        }
        else {
          res = 0.;
        }
...
std::tuple<Tensor, Tensor> ctc_loss_template(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK) {
  ...
  ctc_loss_log_alpha_kernel<scalar_t, target_t>(log_alpha,
    log_probs, input_lengths_t, log_probs.size(0),
    targets, target_lengths_t, max_target_length,
    neg_log_likelihood,
    tg_batch_offsets, tg_target_stride,
    batch_size, BLANK);
...
template<typename scalar_t, ScalarType target_scalar_type>
Tensor ctc_loss_backward_template(const Tensor& grad_out, const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths,
  ...
  ctc_loss_backward_log_beta_kernel<scalar_t, target_t>(log_beta,
     log_probs, input_lengths_t, log_probs.size(0),
     targets, target_lengths_t, max_target_length,
     tg_batch_offsets, tg_target_stride,
     batch_size, BLANK);
