...
template <typename IndexType, int Dim> class fill_slice_dpcpp_ker {};

template <typename IndexType, int Dim>
void fillSliceWithIndex(TensorInfo<int64_t, IndexType> out,
                        IndexType totalSlices, IndexType sliceSize,
...
Tensor &fill_(Tensor &self, Scalar value) { return fill_out(self, value); }
...
Tensor &zero_(Tensor &self) { return at::AtenIpexTypeDPCPP::fill_(self, 0); }
...
Tensor &fill_slice_with_index(Tensor &t, int dim) {
  ...
  TORCH_CHECK(t.scalar_type() == at::kLong || t.scalar_type() == at::kInt,
              "non integer tensor");
...
#define FILL_INDEX(T, DIM)                                                     \
  impl::fillSliceWithIndex<T, DIM>(info, numSlices, sliceSize,                 \
