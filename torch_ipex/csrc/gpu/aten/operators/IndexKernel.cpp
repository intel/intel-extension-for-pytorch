...
static void index_kernel_dpcpp(TensorIterator &iter, IntArrayRef index_size,
  ...
  AT_DISPATCH_ALL_TYPES(iter.dtype(), "index", [&] {
    ...
    dpcpp_index_kernel<DPCPP_K(index_kernel, scalar_t)>(
        ...
        [](dpcpp_global_ptr_pt<char> out_data,
           dpcpp_global_ptr_pt<char> in_data, int64_t offset) {
...
static void index_put_kernel_dpcpp(TensorIterator &iter, IntArrayRef index_size,
                                   IntArrayRef index_stride, bool accumulate) {
  if (accumulate) {
