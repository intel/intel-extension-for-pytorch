...
at::Tensor bmm_forward_cpu(
  ...
  return bmm_kernel_stub(kCPU, out, mat1, mat2, is_vnni, scale, false, false, block_size_n());
}

at::Tensor convert_weight_packed(at::Tensor& weight, bool use_tuned_block_n=false) {
  RECORD_FUNCTION("ipex::convert_weight_packed", c10::ArrayRef<c10::IValue>({})); 
...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  ...
  m.impl("moe_gate_bmm_forward", c10::DispatchKey::CPU, torch_ipex::cpu::moe_gate_bmm_forward);
  m.def("convert_weight_packed(Tensor weight, bool use_tuned_block_n) -> (Tensor)");
  m.impl(
