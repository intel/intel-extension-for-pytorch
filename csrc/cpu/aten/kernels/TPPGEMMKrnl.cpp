...
at::Tensor tpp_linear_bias_kernel_impl(
  ...
  auto dt = t_wt.dtype();
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_bias<float>(t_in_, t_wt, t_bias, t_out);
  } else if (dt == at::kBFloat16) {
    torch_ipex::tpp::tpp_linear_bias<at::BFloat16>(t_in_, t_wt, t_bias, t_out);
  } else {
    AT_ASSERT(
        0,
...
at::Tensor tpp_linear_nobias_kernel_impl(
  ...
  auto dt = t_wt.dtype();
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_no_bias<float>(t_in_, t_wt, t_out);
  } else if (dt == at::kBFloat16) {
    torch_ipex::tpp::tpp_linear_no_bias<at::BFloat16>(t_in_, t_wt, t_out);
  } else {
    AT_ASSERT(
        0,
        "TPP does not support current weight dtype %s:%d\n",
...
at::Tensor tpp_linear_gelu_kernel_impl(
  ...
  if (dt == at::kFloat) {
    if (algorithm == "none") {
      torch_ipex::tpp::tpp_linear_gelu<float>(t_in_, t_wt, t_bias, t_out);
    } else { // tanh
      torch_ipex::tpp::tpp_linear_gelu_tanh<float>(t_in_, t_wt, t_bias, t_out);
    }
  } else if (dt == at::kBFloat16) {
    if (algorithm == "none") {
      torch_ipex::tpp::tpp_linear_gelu<at::BFloat16>(t_in_, t_wt, t_bias, t_out);
    } else { // tanh
      torch_ipex::tpp::tpp_linear_gelu_tanh<at::BFloat16>(
          t_in_, t_wt, t_bias, t_out);
    }
  } else {
    AT_ASSERT(
        0,
...
at::Tensor tpp_fused_gate_up_proj_kernel_impl(
  ...
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_fused_gate_up_proj<float>(
        t_in_, t_wt_gate, t_bias_gate, t_wt_up, t_bias_up, t_out);
  } else if (dt == at::kBFloat16) {
...
at::Tensor tpp_linear_silu_kernel_impl(
  ...
  auto dt = t_wt.dtype();
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_silu<float>(t_in_, t_wt, t_bias, t_out);
  } else if (dt == at::kBFloat16) {
    torch_ipex::tpp::tpp_linear_silu<at::BFloat16>(t_in_, t_wt, t_bias, t_out);
  } else {
    AT_ASSERT(
        0,
        "TPP does not support current weight dtype %s:%d\n",
...
at::Tensor tpp_linear_relu_kernel_impl(
  ...
  auto dt = t_wt.dtype();
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_relu<float>(t_in_, t_wt, t_bias, t_out);
  } else if (dt == at::kBFloat16) {
    torch_ipex::tpp::tpp_linear_relu<at::BFloat16>(t_in_, t_wt, t_bias, t_out);
  } else {
    AT_ASSERT(
        0,
        "TPP does not support current weight dtype %s:%d\n",
...
at::Tensor tpp_linear_add_add_kernel_impl(
  ...
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_add_add<float>(
        t_in_, t_in1_, t_in2_, t_wt, t_bias, t_out, scale);
  } else if (dt == at::kBFloat16) {
...
at::Tensor tpp_linear_add_kernel_impl(
  ...
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_add<float>(
        t_in_, t_in1_, t_wt, t_bias, t_out, scale);
  } else if (dt == at::kBFloat16) {
...
at::Tensor tpp_linear_mul_kernel_impl(
  ...
  auto dt = t_wt.dtype();
  if (dt == at::kFloat) {
    torch_ipex::tpp::tpp_linear_mul<float>(t_in_, t_in1_, t_wt, t_bias, t_out);
  } else if (dt == at::kBFloat16) {
    torch_ipex::tpp::tpp_linear_mul<at::BFloat16>(
        t_in_, t_in1_, t_wt, t_bias, t_out);
  } else {
    AT_ASSERT(
        0,
        "TPP does not support current weight dtype %s:%d\n",
