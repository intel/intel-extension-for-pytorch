...
template <typename T>
std::tuple<at::Tensor, at::Tensor> ApplyDeepseekROPEV2Kernel(
    at::Tensor& q,
    at::Tensor& k_pe,
    at::Tensor& t_emb_pos,
    at::Tensor& t_pos,
...
rotary_position_embedding_deepseek_kernel_impl(
  ...
  if (q.scalar_type() == at::kFloat) {
    ...
    TORCH_CHECK(
        false,
        "rotary_position_embedding_deepseek_kernel_impl: unsupported '",
        q.scalar_type(),
        "'");
    return std::make_tuple(at::Tensor(), at::Tensor(), at::Tensor());
  }
...
rotary_position_embedding_deepseek_v2_kernel_impl(
  ...
  t_pos = t_pos.contiguous();
  if (q.scalar_type() == at::kFloat) {
    return ApplyDeepseekROPEV2Kernel<float>(
        q, k_pe, t_emb_pos, t_pos, N, H, offset, rotary_dim);
  } else if (q.scalar_type() == at::kBFloat16) {
    return ApplyDeepseekROPEV2Kernel<at::BFloat16>(
        q, k_pe, t_emb_pos, t_pos, N, H, offset, rotary_dim);
  } else if (q.scalar_type() == at::kHalf) {
    return ApplyDeepseekROPEV2Kernel<at::Half>(
        q, k_pe, t_emb_pos, t_pos, N, H, offset, rotary_dim);
  } else {
    TORCH_CHECK(
        false,
        "rotary_position_embedding_deepseek_v2_kernel_impl: unsupported '",
        q.scalar_type(),
        "'");
