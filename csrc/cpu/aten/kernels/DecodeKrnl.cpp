...
struct tinygemm_kernel_nt_opt {
  static inline void apply(
    ...
    for (int m = 0; m < BLOCK_M; ++m) {
      for (int n = 0; n < BLOCK_N; ++n) {
        float sum = 0.f;
        TORCH_CHECK(n < max_tokens, "token index out of scope!");
        for (int k = 0; k < K; ++k) {
          sum += scale * static_cast<float>(A[m * lda + k]) *
...
struct tinygemm_kernel_nt_opt<at::BFloat16, BLOCK_M, BLOCK_N> {
  static inline void apply(
      const at::BFloat16* __restrict__ A,
      const at::BFloat16* __restrict__ B,
      float* __restrict__ C,
    ...
    auto compute = [&](auto i, int k) {
      ...
      if constexpr (row == 0) {
        if constexpr (col + 1 < COLS) {
          _mm_prefetch(B + (col + 1) * ldb + k, _MM_HINT_T0);
        }
        TORCH_CHECK(col < max_tokens, "token index out of scope!");
        vb[col] = (__m512bh)(_mm512_loadu_si512(B + col * ldb + k));
      }
      vc[i] = _mm512_dpbf16_ps(vc[i], va, vb[col]);
    };
    // for remainder
    auto compute2 = [&](auto i, int k, __mmask32 mask) {
      ...
      if constexpr (row == 0) {
        TORCH_CHECK(col < max_tokens, "token index out of scope!");
        vb[col] = (__m512bh)(_mm512_maskz_loadu_epi16(mask, B + col * ldb + k));
      }
      vc[i] = _mm512_dpbf16_ps(vc[i], va, vb[col]);
    };
    int k = 0;
...
#define LAUNCH_TINYGEMM_KERNEL_NT(MB_SIZE, NB_SIZE)               \
  tinygemm_kernel_nt<scalar_t, index_t, MB_SIZE, NB_SIZE>::apply( \
      ...
      K,                                                          \
      max_tokens);
#define LAUNCH_TINYGEMM_KERNEL_NT_OPT(MB_SIZE, NB_SIZE)      \
  tinygemm_kernel_nt_opt<scalar_t, MB_SIZE, NB_SIZE>::apply( \
      A + mb_start * lda,                                    \
      B + nb_start * ldb,                                    \
      C + mb_start * ldc + nb_start,                         \
      scale,                                                 \
...
template <typename scalar_t>
inline void tinygemm_kernel_nn_scalar_opt(
    const float* __restrict__ A,
    const scalar_t* __restrict__ B,
    float* __restrict__ C,
    const float* __restrict__ scale,
  ...
  for (int m = 0; m < M; ++m) {
    for (int n = 0; n < N; ++n) {
      C[m * ldc + n] *= scale[m];
      for (int k = 0; k < K; ++k) {
        TORCH_CHECK(k < max_tokens, "token index out of scope!");
        C[m * ldc + n] += A[m * lda + k] * static_cast<float>(B[k * ldb + n]);
      }
    }
  }
}
...
struct tinygemm_kernel_nn_opt {
  static inline void apply(
      ...
      int K,
      int max_tokens) {
    tinygemm_kernel_nn_scalar_opt(
        A, B, C, scale, BLOCK_M, BLOCK_N, K, lda, ldb, ldc, max_tokens);
  }
};
#if defined(CPU_CAPABILITY_AVX512_BF16)
template <typename index_t, int BLOCK_M, int BLOCK_N>
...
template <int BLOCK_M, int BLOCK_N>
struct tinygemm_kernel_nn_opt<at::BFloat16, BLOCK_M, BLOCK_N> {
  static inline void apply(
      const float* __restrict__ A,
      const at::BFloat16* __restrict__ B,
      float* __restrict__ C,
...
#pragma GCC diagnostic pop
    ...
    auto compute = [&](auto i, int k) {
      ...
      if constexpr (row == 0) {
        if (k + 1 < K) {
          _mm_prefetch(B + (k + 1) * ldb + col * 16, _MM_HINT_T0);
        }
        TORCH_CHECK(k < max_tokens, "token index out of scope!");
        // for COLS = 2, 4, 6, 8 use 512 bit load
        // for COLS = 1, 3, 5, 7 use 256 bit load
        if constexpr (COLS % 2 == 0) {
          if constexpr (col % 2 == 0) {
            __m512i b16 = _mm512_loadu_si512(
                reinterpret_cast<const __m512i*>(B + k * ldb + col * 16));
            vb[col + 0] = CVT_BF16_TO_FP32(_mm512_extracti32x8_epi32(b16, 0));
            vb[col + 1] = CVT_BF16_TO_FP32(_mm512_extracti32x8_epi32(b16, 1));
          }
        } else {
...
#define LAUNCH_TINYGEMM_KERNEL_NN(MB_SIZE, NB_SIZE)               \
  tinygemm_kernel_nn<scalar_t, index_t, MB_SIZE, NB_SIZE>::apply( \
      ...
      K,                                                          \
      max_tokens);
#define LAUNCH_TINYGEMM_KERNEL_NN_OPT(MB_SIZE, NB_SIZE)      \
  tinygemm_kernel_nn_opt<scalar_t, MB_SIZE, NB_SIZE>::apply( \
      A + mb_start * lda,                                    \
      B + nb_start,                                          \
      C + mb_start * ldc + nb_start,                         \
      scale + mb_start,                                      \
...
void gemm_kernel_nt(
  ...
  if (M == 1) {
    ...
    for (int nb = 0; nb < NB; ++nb) {
      ...
      switch (nb_size) {
        case 1:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 1);
          break;
        case 2:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 2);
          break;
        case 3:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 3);
          break;
        case 4:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 4);
          break;
        case 5:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 5);
          break;
        case 6:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 6);
          break;
        case 7:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 7);
          break;
        case 8:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(1, 8);
          break;
        default:
          TORCH_CHECK(false, "Unexpected block size, 1x", "nb_size");
      }
  ...
  for (int mb = 0; mb < MB; ++mb) {
    ...
    for (int nb = 0; nb < NB; ++nb) {
      ...
      switch (mb_size << 4 | nb_size) {
        ...
        // mb_size = 2
        case 0x21:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 1);
          break;
        case 0x22:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 2);
          break;
        case 0x23:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 3);
          break;
        case 0x24:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 4);
          break;
        case 0x25:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 5);
          break;
        case 0x26:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(2, 6);
          break;
        // mb_size = 3
        case 0x31:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 1);
          break;
        case 0x32:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 2);
          break;
        case 0x33:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 3);
          break;
        case 0x34:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 4);
          break;
        case 0x35:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 5);
          break;
        case 0x36:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(3, 6);
          break;
        // mb_size = 4
        case 0x41:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 1);
          break;
        case 0x42:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 2);
          break;
        case 0x43:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 3);
          break;
        case 0x44:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 4);
          break;
        case 0x45:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 5);
          break;
        case 0x46:
          LAUNCH_TINYGEMM_KERNEL_NT_OPT(4, 6);
          break;
        default:
          TORCH_CHECK(
              false, "Unexpected block size, ", mb_size, "x", "nb_size");
...
void gemm_kernel_nn(
  ...
  if ((N & (kVecSize - 1)) != 0) {
    tinygemm_kernel_nn_scalar_opt(
        A, B, C, scale, M, N, K, lda, ldb, ldc, max_tokens);
    return;
  }
  // pattern: 1-8-8
  if (M == 1) {
    ...
    for (int nb = 0; nb < NB; ++nb) {
      ...
      switch (nb_size >> 4) {
        case 1:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 16);
          break;
        case 2:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 32);
          break;
        case 3:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 48);
          break;
        case 4:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 64);
          break;
        case 5:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 80);
          break;
        case 6:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 96);
          break;
        case 7:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 112);
          break;
        case 8:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(1, 128);
          break;
        default:
          TORCH_CHECK(false, "Unexpected block size, 1x", "nb_size");
      }
  ...
  for (int mb = 0; mb < MB; ++mb) {
    ...
    for (int nb = 0; nb < NB; ++nb) {
      ...
      switch (mb_size << 4 | nb_size >> 4) {
        ...
        // mb_size = 2
        case 0x21:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 16);
          break;
        case 0x22:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 32);
          break;
        case 0x23:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 48);
          break;
        case 0x24:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 64);
          break;
        case 0x25:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 80);
          break;
        case 0x26:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(2, 96);
          break;
        // mb_size = 3
        case 0x31:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 16);
          break;
        case 0x32:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 32);
          break;
        case 0x33:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 48);
          break;
        case 0x34:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 64);
          break;
        case 0x35:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 80);
          break;
        case 0x36:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(3, 96);
          break;
        // mb_size = 4
        case 0x41:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 16);
          break;
        case 0x42:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 32);
          break;
        case 0x43:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 48);
          break;
        case 0x44:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 64);
          break;
        case 0x45:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 80);
          break;
        case 0x46:
          LAUNCH_TINYGEMM_KERNEL_NN_OPT(4, 96);
          break;
        default:
          TORCH_CHECK(
              false, "Unexpected block size, ", mb_size, "x", "nb_size");
...
template <typename scalar_t>
void decode_attention_opt_kernel_impl(
    scalar_t* __restrict__ output,
    float* __restrict__ attn_logits,
    const scalar_t* __restrict__ query,
    const scalar_t* __restrict__ kv_cache,
...
template <typename scalar_t>
void decode_attention_grouped_opt_kernel_impl(
    scalar_t* __restrict__ output,
    float* __restrict__ attn_logits,
    const scalar_t* __restrict__ query,
    const scalar_t* __restrict__ kv_cache,
  ...
  const int stride_q0 = num_heads * head_size;
  const int stride_q1 = head_size;
  const int stride_kv0 = num_heads_kv * head_size * batches;
  const int stride_kv1 = num_heads_kv * head_size;
  const int stride_kv2 = head_size;
  const int stride_l0 = num_heads * num_kv_splits * (head_size_v + 1);
  const int stride_l1 = num_kv_splits * (head_size_v + 1);
  const int stride_l2 = head_size_v + 1;
  const bool has_logit_cap = logit_cap > 0;
...
// output: [bs*cur_len, num_heads, head_size_v]
...
// kv_cache: [max_positions, beam_batch, kv_num_heads, head_size]
// attn_logits: [bs, num_heads, num_kv_splits, head_size_v + 1]
at::Tensor decode_attention_opt(
    at::Tensor& query,
    at::Tensor& output,
    at::Tensor& kv_cache,
    at::Tensor& attn_logits,
    ...
    double logit_cap,
    int64_t offset) {
  RECORD_FUNCTION("ipex::decode_attention_opt", c10::ArrayRef<c10::IValue>({}));
  CHECK_INPUT(query);
  CHECK_INPUT(kv_cache);
  CHECK_DIM(3, query);
  CHECK_DIM(4, kv_cache);
  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(
      query.scalar_type(), "decode_attention_kernel", [&] {
        if (num_heads == num_heads_kv) {
          // MHA
          decode_attention_opt_kernel_impl<scalar_t>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
              kv_cache.data_ptr<scalar_t>(),
          ...
          // GQA/MQA/MLA
          decode_attention_grouped_opt_kernel_impl<scalar_t>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
              kv_cache.data_ptr<scalar_t>(),
...
IPEX_REGISTER_DISPATCH(decode_attention_opt_kernel_stub, &decode_attention_opt);
