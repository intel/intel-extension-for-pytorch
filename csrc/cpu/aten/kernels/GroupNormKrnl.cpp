...
void GroupNormInputBackward(
  ...
  at::parallel_for(0, N * G, 1, [=](int64_t start, int64_t end) {
    ...
    for (const auto i : c10::irange(start, end)) {
      ...
      const PT c2 = (db_val * PT(mean[i]) - ds_val) * PT(rstd[i]) *
          PT(rstd[i]) * PT(rstd[i]) * s;
...
inline void ApplyInputGradientsChannelsLastColMov(
  ...
  for (; d < D / K * K; d += K) {
    for (const auto m : c10::irange(HxW)) {
      ...
      fVec c1_0 = fVec(*rstd) * (gamma_null ? fVec(1) : fVec::loadu(gamma + d));
      fVec c1_1 = fVec(*rstd) *
  ...
  if (D - d > 0) {
    fVec c1_0 = fVec(*rstd) *
        (gamma_null
             ? fVec(1)
             : fVec::loadu(
                   gamma + d, (D - d) > fVec::size() ? fVec::size() : (D - d)));
    fVec c1_1 = fVec(*rstd) *
        (gamma_null ? fVec(1)
                    : fVec::loadu(
                          gamma + d + fVec::size(),
  ...
  if (HxW < feature_map_threshold) {
    ...
    at::parallel_for(0, N * G, 1, [=](int64_t begin, int64_t end) {
      ...
      for (const auto i : c10::irange(begin, end)) {
        ...
        const PT c2 = (db_gamma * mean_data[i] - ds_gamma) * rstd_data[i] *
            rstd_data[i] * rstd_data[i] * s;
        const PT c3 = -c2 * mean_data[i] - db_gamma * rstd_data[i] * s;
        ApplyInputGradientsChannelsLastColMov<T, PT>(
    ...
    at::Tensor tmp_buffer = at::empty(
        {N, 2 * G}, X.options().dtype(c10::CppTypeToScalarType<PT>::value));
      ...
      at::parallel_for(0, N * HxW, 1, [&](int64_t begin, int64_t end) {
        ...
        for (const auto i : c10::irange(begin, end)) {
          for (const auto g : c10::irange(G)) {
            ...
            const PT c2 = (db_val * (*mean_ptr) - ds_val) * (*rstd_ptr) *
                (*rstd_ptr) * (*rstd_ptr) * s;
            const PT c3 = -c2 * (*mean_ptr) - db_val * (*rstd_ptr) * s;
            ApplyInputGradientsChannelsLastRowMov<T, PT>(
                dY_ptr, X_ptr, dX_ptr, rstd_ptr, gamma_ptr, c2, c3, HxW, C, D);
          }
