...
void _add_and_compute_rmsnorm(
  ...
  for (i = 0; i <= size - 16; i += 16) {
    auto vec_input = _loadu(a_ptr + i);
    auto vec_input1 = _loadu(b_ptr + i);
    vec_input = vec_input + vec_input1;
    auto vec_gamma = _mm512_set1_ps(1.0);
    if (gamma_ptr) {
      vec_gamma = _loadu(gamma_ptr + i);
    }
  ...
  if (i < size) {
    __mmask16 mask = (1 << (size - i)) - 1;
    auto vec_input = _maskz_loadu(a_ptr + i, mask);
    auto vec_input1 = _maskz_loadu(b_ptr + i, mask);
    vec_input = vec_input + vec_input1;
    auto vec_gamma = _mm512_set1_ps(1.0);
    if (gamma_ptr) {
      vec_gamma = _maskz_loadu(gamma_ptr + i, mask);
...
template <typename T, typename T1>
void _add_back_and_compute_rmsnorm(
    const T* a_ptr,
    T* b_ptr,
    const int& size,
    float eps,
  ...
  if (i < size) {
    ...
    auto vec_b = _maskz_loadu(b_ptr + i, mask);
    vec_a = vec_a + vec_b;
    _mask_store_data(b_ptr + i, vec_a, mask);
    auto s = vec_a * vec_a;
    vec_acc_pow += s;
  }
  float var_val = _mm512_reduce_add_ps(vec_acc_pow) / static_cast<float>(size);
  ...
  if (i < size) {
    __mmask16 mask = (1 << (size - i)) - 1;
    auto vec_input = _maskz_loadu(b_ptr + i, mask);
    auto vec_gamma = _mm512_set1_ps(1.0);
    if (gamma_ptr) {
      vec_gamma = _maskz_loadu(gamma_ptr + i, mask);
    }
