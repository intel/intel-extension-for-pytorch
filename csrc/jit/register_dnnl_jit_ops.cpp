    ...
    op(
        {
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    ...
                    auto result = torch::jit::xpu::matmul_add(
                        (std::move(peek(stack, 0, 4))).toTensor(),
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto result = torch::jit::xpu::trans_matmul(
                        (std::move(peek(stack, 0, 4))).toTensor(),
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto result = torch::jit::xpu::t_matmul(
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto accumu = (std::move(peek(stack, 2, 4))).toTensor();
                    auto result = torch::jit::xpu::t_matmul_add(
                        (std::move(peek(stack, 0, 4))).toTensor(),
                        (std::move(peek(stack, 1, 4))).toTensor(),
                        accumu,
                        (std::move(peek(stack, 3, 4))).toScalar());
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto accumul1 = (std::move(peek(stack, 2, 6))).toTensor();
                    auto accumul2 = (std::move(peek(stack, 4, 6))).toTensor();
                    auto result = torch::jit::xpu::t_matmul_add_add(
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto result = torch::jit::xpu::trans_matmul_div(
                        (std::move(peek(stack, 0, 5))).toTensor(),
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto result = torch::jit::xpu::linear_relu(
                        (std::move(peek(stack, 0, 3))).toTensor(),
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    auto result = torch::jit::xpu::linear_sigmoid(
                        (std::move(peek(stack, 0, 3))).toTensor(),
                        (std::move(peek(stack, 1, 3))).toTensor(),
                        toOptionalTensor(std::move(peek(stack, 2, 3))));
                    drop(stack, 3);
            ...
            Operator(
                ...
                [](const Node* node) -> Operation {
                  return [](Stack& stack) {
                    at::Tensor accumu = std::move(peek(stack, 3, 5)).toTensor();
                    at::Tensor bias =
                        toOptionalTensor(std::move(peek(stack, 2, 5)));
                    auto result = torch::jit::xpu::linear_add(
                        (std::move(peek(stack, 0, 5))).toTensor(),
                        (std::move(peek(stack, 1, 5))).toTensor(),
                        bias,
                        accumu,
