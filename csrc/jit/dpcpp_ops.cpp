...
at::Tensor matmul_sum(
    ...
    const at::Tensor& m2,
    at::Scalar alpha) {
  RECORD_FUNCTION("matmul_sum", std::vector<c10::IValue>({m1, m2, accumu}));
...
at::Tensor trans_matmul_scale_sum(
    ...
    at::Scalar alpha) {
  RECORD_FUNCTION(
      "trans_matmul_scale_sum", std::vector<c10::IValue>({tensor1, tensor2}));
  ...
  if ((dim_tensor1 >= 1 && dim_tensor2 >= 1) &&
    ...
    if (accumu.defined() && accumu.sizes().vec() == output_shape) {
      output = at::_unsafe_view(
          at::AtenIpexTypeXPU::trans_baddbmm_out(
              accumu,
              accumu,
              tensor1_expanded,
              tensor2_expanded,
