...
static inline Tensor sum(Tensor& output,
  ...
  memory::desc tar_desc = tar_ctx.is_plain() ?
      memory::desc(get_onednn_dims(tar),
  ...
  for (int i = 1; i < inputs.size(); i++) {
    ...
    memory::desc cur_desc = cur_ctx.is_plain() ?
        memory::desc(get_onednn_dims(cur),
