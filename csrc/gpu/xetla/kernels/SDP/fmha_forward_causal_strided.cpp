...
class fmha_forward_causal_strided_t {
 ...
 public:
  ...
  inline KERNEL_FUNC void operator()(
    ...
    uint32_t endF = (startF + kBr) > args.uF ? args.uF : (startF + kBr); // 1
...
void fmha_forward_causal_strided(
    ...
    uint32_t alibi_padded_block_size,
    uint32_t attn_mask_padded_block_size) {
  TORCH_CHECK(head_size <= 256, "IPEX FSDP requires head_size <= 256 ...");

  if (num_queries <= 8) {
    if (head_size <= 128) {
      if (num_keys > 64) {
        CALL_IMPL_FUNC(fmha_policy_f8_t1024_h128);
      } else {
        CALL_IMPL_FUNC(fmha_policy_f8_t64_h128);
      }
    } else if (head_size <= 256) {
      if (num_keys > 64) {
        CALL_IMPL_FUNC(fmha_policy_f8_t1024_h256);
      } else {
        CALL_IMPL_FUNC(fmha_policy_f8_t64_h256);
      }
    }
  } else {
    if (head_size <= 64) {
