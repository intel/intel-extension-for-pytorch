...
void causal_conv1d_fwd_sycl(ConvParamsBase& params, sycl::queue stream);

template <typename input_t, typename weight_t>
void causal_conv1d_update_sycl(ConvParamsBase& params, sycl::queue stream);
...
void causal_conv1d_fn(
  ...
  if (has_initial_state.has_value()) {
    ...
    TORCH_CHECK(has_initial_state_.is_xpu());
  ...
  if (query_start_loc.has_value()) {
    ...
    TORCH_CHECK(query_start_loc_.is_xpu());
  }

  if (cache_indices.has_value()) {
    auto cache_indices_ = cache_indices.value();
    TORCH_CHECK(cache_indices_.scalar_type() == at::ScalarType::Int);
    TORCH_CHECK(cache_indices_.is_xpu());
  ...
  if (conv_states.has_value()) {
    ...
    TORCH_CHECK(conv_states_.is_xpu());
  ...
  DISPATCH_WTYPE_ITYPE_FLOAT_AND_HALF_AND_BF16(
      x.scalar_type(), "causal_conv1d_fn", [&] {
        causal_conv1d_fwd_sycl<input_t, weight_t>(params, stream);
      });
}
...
void causal_conv1d_update(
  ...
  DISPATCH_WTYPE_ITYPE_FLOAT_AND_HALF_AND_BF16(
      x.scalar_type(), "causal_conv1d_update", [&] {
        causal_conv1d_update_sycl<input_t, weight_t>(params, stream);
      });
}
...
struct Causal_conv1d_fwd_kernel_traits {
  ...
  static_assert(sizeof(vec_t) == kNBytes * kNElts);
  ...
  //     BlockStore<input_t, kNThreads, kNElts,
...
class causal_conv1d_fwd_kernel {
 private:
  ConvParamsBase params;

 public:
  causal_conv1d_fwd_kernel(ConvParamsBase params_) : params(params_) {}
  ...
  void operator()(sycl::nd_item<2> item) const {
    ...
    // constexpr bool kIsVecLoad = false;
    ...
    // Ktraits::BlockLoadT::TempStorage&>(smem_); auto& smem_load_vec =
    // reinterpret_cast<typename Ktraits::BlockLoadVecT::TempStorage&>(smem_);
    // auto& smem_store = reinterpret_cast<typename
    // Ktraits::BlockStoreT::TempStorage&>(smem_); auto& smem_store_vec =
    // reinterpret_cast<typename Ktraits::BlockStoreVecT::TempStorage&>(smem_);
    //
    // TODO: Use shared memory for exchange.
    auto& smem_exchange =
        *sycl::ext::oneapi::group_local_memory_for_overwrite<vec_t[kNThreads]>(
            item.get_group());

    const bool kVarlen = params.query_start_loc_ptr != nullptr;
    const int tidx = item.get_local_linear_id();
    const int batch_id = item.get_group(0);
    const int channel_id = item.get_group(1);
...
#pragma unroll
    ...
    for (int chunk = 0; chunk < n_chunks; ++chunk) {
      ...
      if constexpr (kIsVecLoad) {
        // typename
        // Ktraits::BlockLoadVecT(smem_load_vec).Load(reinterpret_cast<vec_t*>(x),
        // *reinterpret_cast<vec_t (*)[1]>(&x_vals_load[kNElts]), (seqlen -
        // chunk * kChunkSize) / kNElts);

        if (tidx * kNElts < (seqlen - chunk * kChunkSize)) {
          auto x_vec = reinterpret_cast<vec_t*>(x);
          auto x_vals_load_vec =
              reinterpret_cast<vec_t(*)[1]>(&x_vals_load[kNElts]);

          x_vals_load_vec[0][0] = x_vec[tidx];
        }
      } else {
        group_barrier(item.get_group());
        // typename Ktraits::BlockLoadT(smem_load).Load(x,
        // *reinterpret_cast<input_t (*)[kNElts]>(&x_vals_load[kNElts]), seqlen
        // - chunk * kChunkSize);
        for (int i = 0; i < kNElts; ++i) {
          auto x_vals_load_elts =
              reinterpret_cast<input_t(*)[kNElts]>(&x_vals_load[kNElts]);
          if (tidx * kNElts + i < seqlen - chunk * kChunkSize) {
            x_vals_load_elts[0][i] = x[tidx * kNElts + i];
          }
        }
      }
      x += kChunkSize;
...
#pragma unroll
        for (int i = 0; i < kNElts; ++i) {
          out_vals[i] = out_vals[i] / (1 + sycl::exp(-out_vals[i]));
        }
      }
...
#pragma unroll
      ...
      if constexpr (kIsVecLoad) {
        // typename
        // Ktraits::BlockStoreVecT(smem_store_vec).Store(reinterpret_cast<vec_t*>(out),
        // reinterpret_cast<vec_t (&)[1]>(out_vals_store), (seqlen - chunk *
        // kChunkSize) / kNElts);
        if (tidx * kNElts < (seqlen - chunk * kChunkSize)) {
          auto out_vec = reinterpret_cast<vec_t*>(out);
          auto out_vals_store_vec =
              reinterpret_cast<vec_t(&)[1]>(out_vals_store);

          out_vec[tidx] = out_vals_store_vec[0];
        }
      } else {
        ...
        for (int i = 0; i < kNElts; ++i) {
          if (tidx * kNElts + i < seqlen - chunk * kChunkSize) {
            out[tidx * kNElts + i] = out_vals_store[i];
          }
        }
      }
      out += kChunkSize;
      ...
      // in case the final state is separated between the last "smem_exchange"
      // and and the one before it (chunk = n_chunks - 1 and chunk = n_chunks -
      // 2), (which occurs when `final_state_position` is a non-positivie index)
      // we load the correct data from smem_exchange from both chunks, the last
      // chunk iteration and the one before it
      if (conv_states != nullptr && final_state_position < 0 &&
          seqlen > kWidth) {
...
#pragma unroll
        ...
        if ((offset + kWidth - 2) >= kNElts && (last_thread + 1 < kNThreads)) {
          // In case last_thread == kNThreads - 1, accessing last_thread + 1
          // will result in a illegal access error on H100. Therefore, we access
          // last_thread + 1, only if the final state data sits there
...
void causal_conv1d_fwd_launch(ConvParamsBase& params, sycl::queue stream) {
  ...
  BOOL_SWITCH(params.seqlen % kNElts == 0 && !kVarlen, kIsVecLoad, [&] {
    ...
    sycl::range<2> grid(params.batch, params.dim);
    sycl::range<2> block(1, Ktraits::kNThreads);

    stream.submit([&](sycl::handler& cgh) {
      causal_conv1d_fwd_kernel<Ktraits> kernel(params);
...
template void causal_conv1d_fwd_sycl<float, float>(
    ConvParamsBase& params,
    sycl::queue stream);
template void causal_conv1d_fwd_sycl<at::Half, at::Half>(
    ConvParamsBase& params,
    sycl::queue stream);
template void causal_conv1d_fwd_sycl<at::BFloat16, at::BFloat16>(
    ConvParamsBase& params,
    sycl::queue stream);
...
class causal_conv1d_update_kernel {
 private:
  ConvParamsBase params;

 public:
  causal_conv1d_update_kernel(ConvParamsBase params_) : params(params_) {}
  void operator()(sycl::nd_item<2> item) const {
    constexpr int kWidth = Ktraits::kWidth;
    constexpr int kNThreads = Ktraits::kNThreads;
    using input_t = typename Ktraits::input_t;
    ...
    const int tidx = item.get_local_linear_id();
    const int batch_id = item.get_group(0);
    const int channel_id = item.get_group(1) * kNThreads + tidx;
    if (channel_id >= params.dim)
      return;

    input_t* x = reinterpret_cast<input_t*>(params.x_ptr) +
        batch_id * params.x_batch_stride + channel_id * params.x_c_stride;

    // If params.conv_state_batch_indices is set, then the conv state is
    // gathered from the conv state tensor along the batch axis. Otherwise, the
    // conv state coordinate is the same as the batch id.
    ...
    weight_t* weight = reinterpret_cast<weight_t*>(params.weight_ptr) +
        channel_id * params.weight_c_stride;
    input_t* out = reinterpret_cast<input_t*>(params.out_ptr) +
        batch_id * params.out_batch_stride + channel_id * params.out_c_stride;
    float bias_val = params.bias_ptr == nullptr
        ? 0.f
        : float(reinterpret_cast<weight_t*>(params.bias_ptr)[channel_id]);
...
#pragma unroll
      for (int i = 0; i < kWidth - 1; ++i) {
        input_t state_val = conv_state
            [(state_len - (kWidth - 1) + i) * params.conv_state_l_stride];
        if (i < advance_len + (kWidth - 1) &&
            state_len - advance_len - (kWidth - 1) + i >= 0) {
          conv_state
              [(state_len - advance_len - (kWidth - 1) + i) *
    ...
    for (int i = 0; i < params.seqlen; ++i) {
      ...
      if constexpr (!kIsCircularBuffer) {
        if (i < advance_len && state_len - advance_len + i >= 0) {
          conv_state
              [(state_len - advance_len + i) * params.conv_state_l_stride] =
                  x_val;
        }
      } else {
...
#pragma unroll
      ...
      if (params.silu_activation) {
        out_val = out_val / (1 + sycl::exp(-out_val));
      }
...
template <int kNThreads, int kWidth, typename input_t, typename weight_t>
void causal_conv1d_update_launch(ConvParamsBase& params, sycl::queue stream) {
  using Ktraits =
      Causal_conv1d_update_kernel_traits<kNThreads, kWidth, input_t, weight_t>;
  // dim3 grid(params.batch, (params.dim + kNThreads - 1) / kNThreads);
  sycl::range<2> grid(params.batch, (params.dim + kNThreads - 1) / kNThreads);
  sycl::range<2> block(1, kNThreads);

  if (params.cache_seqlens == nullptr) {
    stream.submit([&](sycl::handler& cgh) {
      auto kernel = causal_conv1d_update_kernel<Ktraits, false>(params);

      cgh.parallel_for(sycl::nd_range<2>(grid * block, block), kernel);
    });
  } else {
    stream.submit([&](sycl::handler& cgh) {
      auto kernel = causal_conv1d_update_kernel<Ktraits, true>(params);
...
template void causal_conv1d_update_sycl<float, float>(
    ConvParamsBase& params,
    sycl::queue stream);
template void causal_conv1d_update_sycl<at::Half, at::Half>(
    ConvParamsBase& params,
    sycl::queue stream);
template void causal_conv1d_update_sycl<at::BFloat16, at::BFloat16>(
    ConvParamsBase& params,
    sycl::queue stream);
} // namespace AtenIpexTypeXPU
} // namespace at
...
IPEX_LIBRARY_FRAGMENT() {
  IPEX_OP_REGISTER_DISPATCH(
      "causal_conv1d_fn.xpu",
      at::AtenIpexTypeXPU::causal_conv1d_fn,
      c10::DispatchKey::XPU);

  IPEX_OP_REGISTER_DISPATCH(
      "causal_conv1d_update.xpu",
      at::AtenIpexTypeXPU::causal_conv1d_update,
      c10::DispatchKey::XPU);
}
} // namespace
