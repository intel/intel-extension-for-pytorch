...
struct Selective_Scan_fwd_kernel_traits {
  ...
  // // using BlockScanT = cub::BlockScan<scan_t, kNThreads,
  // cub::BLOCK_SCAN_RAKING>; using BlockScanT = cub::BlockScan<scan_t,
  // kNThreads, cub::BLOCK_SCAN_WARP_SCANS>; static constexpr int kSmemIOSize =
  // custom_max({sizeof(typename BlockLoadT::TempStorage),
  //                                              sizeof(typename
  //                                              BlockLoadVecT::TempStorage),
  //                                              (int(kIsVariableB) +
  //                                              int(kIsVariableC)) *
  //                                              sizeof(typename
  //                                              BlockLoadWeightT::TempStorage),
  //                                              (int(kIsVariableB) +
  //                                              int(kIsVariableC)) *
  //                                              sizeof(typename
  //                                              BlockLoadWeightVecT::TempStorage),
  //                                              sizeof(typename
  //                                              BlockStoreT::TempStorage),
  //                                              sizeof(typename
  //                                              BlockStoreVecT::TempStorage)});
  // static constexpr int kSmemSize = kSmemIOSize + sizeof(typename
  // BlockScanT::TempStorage);
};
...
class selective_scan_fwd_kernel {
 ...
 public:
  selective_scan_fwd_kernel(SSMParamsBase params_) : params(params_) {}
  ...
  void operator()(sycl::nd_item<2> item) const {
    ...
    // TODO: dynamic shared memory in sycl?
    // Shared memory.
    // extern __shared__ char smem_[];
    // cast to lvalue reference of expected type
    // char *smem_loadstorescan = smem_ + 2 * MAX_DSTATE * sizeof(weight_t);
    // auto& smem_load = reinterpret_cast<typename
    // BlockLoadT::TempStorage&>(smem_ + 2 * MAX_DSTATE * sizeof(weight_t));
    // auto& smem_load = reinterpret_cast<typename
    // BlockLoadT::TempStorage&>(smem_loadstorescan); auto& smem_load =
    // reinterpret_cast<typename Ktraits::BlockLoadT::TempStorage&>(smem_);
    // auto& smem_load_weight = reinterpret_cast<typename
    // Ktraits::BlockLoadWeightT::TempStorage&>(smem_); auto& smem_load_weight1
    // = *reinterpret_cast<typename
    // Ktraits::BlockLoadWeightT::TempStorage*>(smem_ + sizeof(typename
    // Ktraits::BlockLoadWeightT::TempStorage)); auto& smem_store =
    // reinterpret_cast<typename Ktraits::BlockStoreT::TempStorage&>(smem_);
    // auto& smem_scan = *reinterpret_cast<typename
    // Ktraits::BlockScanT::TempStorage*>(smem_ + Ktraits::kSmemIOSize);
    ...
    // scan_t *smem_running_prefix = reinterpret_cast<scan_t *>(smem_ +
    // Ktraits::kSmemSize);
    auto& smem_running_prefix =
        *sycl::ext::oneapi::group_local_memory_for_overwrite<
            scan_t[MAX_DSTATE * kNRows]>(item.get_group());

    const int batch_id = item.get_group(0);
    const int dim_id = item.get_group(1);
    ...
    input_t* u = reinterpret_cast<input_t*>(params.u_ptr) +
        sequence_start_index * params.u_batch_stride +
        dim_id * kNRows * params.u_d_stride;
    input_t* delta = reinterpret_cast<input_t*>(params.delta_ptr) +
        sequence_start_index * params.delta_batch_stride +
    ...
    input_t* ssm_states = reinterpret_cast<input_t*>(params.ssm_states_ptr) +
        (cache_index * params.dim + dim_id * kNRows) * params.dstate;
...
#pragma unroll
      for (int r = 0; r < kNRows; ++r) {
        delta_bias[r] = reinterpret_cast<float*>(
            params.delta_bias_ptr)[dim_id * kNRows + r];
      }
    }

    // for (int state_idx = item.get_local_id(1); state_idx < params.dstate;
    // state_idx += blockDim.x) {
    ...
    auto blocked_offset = item.get_local_id(1) * kNItems;
...
#pragma unroll
      for (int r = 0; r < kNRows; ++r) {
        ...
        // TODO: add shared memory to get better performance
        // load_input<Ktraits>(u + r * params.u_d_stride, u_vals[r], smem_load,
        // seqlen - chunk * kChunkSize); if constexpr (!kDirectIO) {
        // __syncthreads(); } load_input<Ktraits>(delta + r *
        // params.delta_d_stride, delta_vals_load[r], smem_load, seqlen - chunk
        // * kChunkSize);
        for (int i = 0; i < kNItems; ++i) {
          if (blocked_offset + i >= seqlen - chunk * kChunkSize) {
            u_vals[r][i] = 0;
            delta_vals_load[r][i] = 0;
            continue;
          }
          u_vals[r][i] = u[r * params.u_d_stride + blocked_offset + i];
          delta_vals_load[r][i] =
              delta[r * params.delta_d_stride + blocked_offset + i];
        }
      }
      u += kChunkSize;
      delta += kChunkSize;
        ...
        for (int i = 0; i < kNItems; ++i) {
          ...
          if (params.delta_softplus) {
            delta_vals[r][i] = delta_vals[r][i] <= 20.f
                ? sycl::log1p(sycl::exp(delta_vals[r][i]))
                : delta_vals[r][i];
          }
...
#pragma unroll
        for (int r = 0; r < kNRows; ++r) {
          ...
          // Multiply the real part of A with LOG2E so we can use exp2f instead
          // of expf.
        ...
        if constexpr (kIsVariableB) {
          // load_weight<Ktraits>(Bvar + state_idx * params.B_dstate_stride,
          // B_vals,
          ...
          for (int i = 0; i < kNItems; ++i) {
            ...
            B_vals[i] =
                Bvar[state_idx * params.B_dstate_stride + blocked_offset + i];
          }
...
#pragma unroll
        ...
        if constexpr (kIsVariableC) {
          // auto &smem_load_weight_C = !kIsVariableB ? smem_load_weight :
          // smem_load_weight1; load_weight<Ktraits>(Cvar + state_idx *
          // params.C_dstate_stride, C_vals,
          ...
          for (int i = 0; i < kNItems; ++i) {
            ...
            C_vals[i] =
                Cvar[state_idx * params.C_dstate_stride + blocked_offset + i];
          }
          if constexpr (!kIsVariableB) {
#pragma unroll
            for (int r = 0; r < kNRows; ++r) {
...
#pragma unroll
          for (int i = 0; i < kNItems; ++i) {
            thread_data[i] = sycl::float2(
                sycl::exp2(delta_vals[r][i] * A_val[r]),
                !kIsVariableB ? delta_u_vals[r][i]
                              : B_vals[i] * delta_u_vals[r][i]);

            if (seqlen % (kNItems * kNThreads) !=
                0) { // So that the last state is correct
              if (item.get_local_id(1) * kNItems + i >=
          ...
          scan_t running_prefix = chunk > 0
              ? smem_running_prefix[state_idx + r * MAX_DSTATE]
              : sycl::float2(
                    1.0,
                    has_initial_state ? float(ssm_states[state_idx]) : 0.0);

          SSMScanPrefixCallbackOp<weight_t> prefix_op(running_prefix);
          sycl_scan::inclusive_scan_over_group(
              item, thread_data, thread_data, SSMScanOp<weight_t>(), prefix_op);

          // There's a syncthreads in the scan op, so we don't need to sync
          // here. Unless there's only 1 warp, but then it's the same thread (0)
          // reading and writing.
...
#pragma unroll
      for (int r = 0; r < kNRows; ++r) {
         ...
         * smem_store, seqlen - chunk * kChunkSize); */
        for (int i = 0; i < kNItems; ++i) {
          if (blocked_offset + i >= seqlen - chunk * kChunkSize) {
            continue;
          }
          out[r * params.out_d_stride + blocked_offset + i] = out_vals[r][i];
        }
      }

      if constexpr (kHasZ) {
        input_t* z = reinterpret_cast<input_t*>(params.z_ptr) +
            sequence_start_index * params.z_batch_stride +
            dim_id * kNRows * params.z_d_stride + chunk * kChunkSize;
        input_t* out_z = reinterpret_cast<input_t*>(params.out_z_ptr) +
            sequence_start_index * params.out_z_batch_stride +
...
#pragma unroll
        for (int r = 0; r < kNRows; ++r) {
          ...
          for (int i = 0; i < kNItems; ++i) {
            ...
            z_vals[i] = z[r * params.z_d_stride + blocked_offset + i];
          }
#pragma unroll
          for (int i = 0; i < kNItems; ++i) {
          ...
          // __syncthreads();
          // store_output<Ktraits>(out_z + r * params.out_z_d_stride,
          // out_vals[r], smem_store, seqlen - chunk * kChunkSize);
          for (int i = 0; i < kNItems; ++i) {
            if (blocked_offset + i >= seqlen - chunk * kChunkSize) {
              continue;
            }
            out_z[r * params.out_z_d_stride + blocked_offset + i] =
                out_vals[r][i];
          }
        }
      }
...
void selective_scan_fwd_launch(SSMParamsBase& params, sycl::queue stream) {
  // Only kNRows == 1 is tested for now, which ofc doesn't differ from
  // previously when we had each block processing 1 row.
  constexpr int kNRows = 1;
  // kIsVariableB, kIsVariableC and kHasZ are all set to True to reduce binary
  // size
  ...
  BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
    BOOL_SWITCH(params.query_start_loc_ptr != nullptr, kVarlen, [&] {
      using Ktraits = Selective_Scan_fwd_kernel_traits<
          ...
          input_t,
          weight_t>;
      // constexpr int kSmemSize = Ktraits::kSmemSize + kNRows * MAX_DSTATE *
      // sizeof(typename Ktraits::scan_t); dim3 grid(params.batch, params.dim /
      // kNRows);
      /* auto kernel = &selective_scan_fwd_kernel<Ktraits>; */
      // TODO: is there a constrant for sycl kernel?
      ...
      // TODO: how to set dynamic shared memory in sycl?
      // kernel<<<grid, Ktraits::kNThreads, kSmemSize, stream>>>(params);
      // C10_CUDA_KERNEL_LAUNCH_CHECK();
      sycl::range<2> grid(params.batch, params.dim / kNRows);
      sycl::range<2> block(1, Ktraits::kNThreads);
      stream.submit([&](sycl::handler& cgh) {
        selective_scan_fwd_kernel<Ktraits> kernel(params);
        cgh.parallel_for(sycl::nd_range<2>(grid * block, block), kernel);
      });
    });
  });
}
...
void selective_scan_fwd_sycl(SSMParamsBase& params, sycl::queue stream) {
...
template void selective_scan_fwd_sycl<at::BFloat16, float>(
    SSMParamsBase& params,
    sycl::queue stream);
template void selective_scan_fwd_sycl<at::Half, float>(
    SSMParamsBase& params,
    sycl::queue stream);
template void selective_scan_fwd_sycl<float, float>(
    SSMParamsBase& params,
    sycl::queue stream);
...
void set_ssm_params_fwd(
  ...
  params.delta_bias_ptr =
      delta_bias.has_value() ? delta_bias.value().data_ptr() : nullptr;
...
void selective_scan_fwd(
  ...
  TORCH_CHECK(u.device().is_xpu());
  TORCH_CHECK(delta.device().is_xpu());
  TORCH_CHECK(A.device().is_xpu());
  TORCH_CHECK(B.device().is_xpu());
  TORCH_CHECK(C.device().is_xpu());
  ...
  TORCH_CHECK(
      dstate <= 256, "selective_scan only supports state dimension <= 256");
  ...
  if (delta_bias_.has_value()) {
    ...
    TORCH_CHECK(delta_bias.device().is_xpu());
  ...
  if (has_initial_state.has_value()) {
    ...
    TORCH_CHECK(has_initial_state_.device().is_xpu());
  ...
  if (query_start_loc.has_value()) {
    ...
    TORCH_CHECK(query_start_loc_.device().is_xpu());
  }

  if (cache_indices.has_value()) {
    auto cache_indices_ = cache_indices.value();
    TORCH_CHECK(cache_indices_.scalar_type() == at::ScalarType::Int);
    TORCH_CHECK(cache_indices_.device().is_xpu());
  ...
  TORCH_CHECK(
      has_z, "has_z = False is disabled in favor of reduced binary size")
  z = z_.value();
  TORCH_CHECK(z.scalar_type() == input_type);
  TORCH_CHECK(z.device().is_xpu());
  ...
  TORCH_CHECK(ssm_states.device().is_xpu());
  ...
  //
  // auto device_id = at::xpu::current_device();
  // c10::xpu::XPUGuard device_guard(device_id);
  ...
  DISPATCH_WTYPE_ITYPE_FLOAT_AND_HALF_AND_BF16(
      u.scalar_type(), "selective_scan_fwd", [&] {
        selective_scan_fwd_sycl<input_t, weight_t>(params, stream);
      });
}
...
IPEX_LIBRARY_FRAGMENT() {
  IPEX_OP_REGISTER_DISPATCH(
      "selective_scan_fn.xpu",
      at::AtenIpexTypeXPU::selective_scan_fwd,
      c10::DispatchKey::XPU);
}
} // namespace
