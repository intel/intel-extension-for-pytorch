...
DS_D_INLINE void partitioned_block(
    sycl::group<3>& tb,
    sycl::sub_group& warp,
    float& val);
...
DS_D_INLINE int _warp_rank() {
  ...
  const int thread_rank = item_ct1.get_local_id(2) +
...
template <>
DS_D_INLINE sycl::half element<ROpType::Add>(
    const sycl::half lhs,
    const sycl::half rhs) {
  return lhs + rhs;
}
...
template <>
DS_D_INLINE sycl::half element<ROpType::Max>(
    const sycl::half lhs,
    const sycl::half rhs) {
...
template <>
DS_D_INLINE sycl::half element<ROpType::Min>(
    const sycl::half lhs,
    const sycl::half rhs) {
  return (lhs < rhs) ? lhs : rhs;
}
...
template <>
DS_D_INLINE sycl::half2 element<ROpType::Add>(
    const sycl::half2 lhs,
    const sycl::half2 rhs) {
  return lhs + rhs;
}

template <>
DS_D_INLINE sycl::half2 element<ROpType::Max>(
    const sycl::half2 lhs,
    const sycl::half2 rhs) {
  sycl::half2 ret_val;
...
template <>
DS_D_INLINE sycl::half2 element<ROpType::Min>(
    const sycl::half2 lhs,
    const sycl::half2 rhs) {
  sycl::half2 ret_val;
  ret_val.x() = (lhs.x() < rhs.x()) ? lhs.x() : rhs.x();
...
template <>
DS_D_INLINE int32_t
element<ROpType::Add>(const int32_t lhs, const int32_t rhs) {
  return lhs + rhs;
}

template <>
DS_D_INLINE int32_t
element<ROpType::Max>(const int32_t lhs, const int32_t rhs) {
  return (lhs > rhs) ? lhs : rhs;
}

template <>
DS_D_INLINE int32_t
element<ROpType::Min>(const int32_t lhs, const int32_t rhs) {
  return (lhs < rhs) ? lhs : rhs;
}

template <>
DS_D_INLINE uint32_t
element<ROpType::Add>(const uint32_t lhs, const uint32_t rhs) {
  return lhs + rhs;
}

template <>
DS_D_INLINE uint32_t
element<ROpType::Max>(const uint32_t lhs, const uint32_t rhs) {
  return (lhs > rhs) ? lhs : rhs;
}

template <>
DS_D_INLINE uint32_t
element<ROpType::Min>(const uint32_t lhs, const uint32_t rhs) {
  return (lhs < rhs) ? lhs : rhs;
}

template <>
DS_D_INLINE int64_t
element<ROpType::Add>(const int64_t lhs, const int64_t rhs) {
  return lhs + rhs;
}

template <>
DS_D_INLINE int64_t
element<ROpType::Max>(const int64_t lhs, const int64_t rhs) {
  return (lhs > rhs) ? lhs : rhs;
}

template <>
DS_D_INLINE int64_t
element<ROpType::Min>(const int64_t lhs, const int64_t rhs) {
  return (lhs < rhs) ? lhs : rhs;
}
...
DS_D_INLINE void _warp(sycl::sub_group& warp, T* data) {
  ...
  auto reduce_width_ = tb.get_local_range(2) < reduce_width
      ? tb.get_local_range(2)
      : reduce_width;
#pragma unroll
...
DS_D_INLINE void _block(
  ...
  T* reduce_buffer = *sycl::ext::oneapi::group_local_memory_for_overwrite<
      T[max_warps * elems]>(sycl::ext::oneapi::experimental::this_group<3>());
  ...
  if (total_warps != 1) {
    if (sycl::ext::oneapi::experimental::this_sub_group()
            .get_local_linear_id() == 0) {
#pragma unroll
      for (int i = 0; i < elems; i++) {
        reduce_buffer[_warp_rank() * elems + i] = data[i];
      }
    }
    ...
    if (_warp_rank() == 0) {
      if (sycl::ext::oneapi::experimental::this_sub_group()
              .get_local_linear_id() < running_warps) {
#pragma unroll
        for (int i = 0; i < elems; i++) {
          data[i] = reduce_buffer
...
#pragma unroll
    for (int i = 0; i < elems; i++) {
      data[i] = reduce_buffer[_warp_rank() * elems + i];
    }
  }
}
...
/*
Arg-reduce is a specialization of the above. We only support this with a single
reduction parameter. This only works for max/min reductions.
*/
...
idx_reduce(sycl::group<3>& tb, sycl::sub_group& warp, float val, int idx) {
  ...
  // The way floating point compare works is normally to perform a sign
  // comparison and if they match, then do a comparison of the rest of the bits
  // as unsigned integers. Since we are bundling these, that means for negative
  // values we need to reverse the sort order, which we can do with an XOR.
  if (val < 0) {
    *res_as_int ^= 0x7fffffff00000000;
  }

  _block<int64_t, warpBound, Op>(tb, warp, res_as_int);

  // Sign bit is preserved, so we can check if we need to invert the mantissa
  // back
  if (res.val < 0) {
    *res_as_int ^= 0x7fffffff00000000;
  }
