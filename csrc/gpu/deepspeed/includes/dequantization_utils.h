...
DS_D_INLINE void to_global(
    sycl::half* global_output,
    const int8_t* data,
...
dequantized data       sycl::half* or sycl::half2* data         -  Pointer to
quantized input data.            int8_t* Params       -  Parameters for
quantization.                Params<qType, numBits>
*/
template <int numBits, Type qType>
DS_D_INLINE void chunk(
    sycl::half2* local_output,
    const int8_t* data,
...
#pragma unroll
  for (int i = 0; i < iters; i++) {
    if constexpr (num_elems_packed == 1) {
      local_output[i] = q_params.template dequant<T>(data[i]);
    } else {
      auto accessible_data = *(PackedInt4*)(&data[i]);
      local_output[2 * i] = q_params.template dequant<T>(accessible_data.low);
      local_output[2 * i + 1] =
          q_params.template dequant<T>(accessible_data.high);
    }
  }
}
