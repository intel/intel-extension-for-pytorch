...
#ifndef __RNG_UTILS_H__
#define __RNG_UTILS_H__
 ...
 public:
  ...
  template <
      ...
      typename std::enable_if<
          !std::is_same_v<T, oneapi::mkl::rng::device::mcg59<1>>>::type* =
          nullptr>
  rng_generator(
      std::uint64_t seed,
  ...
  auto generate(distr_params_t... distr_params) {
    static_assert(
        vec_size == 1 || vec_size == 2 || vec_size == 4,
        "vec_size is not supported.");
    static_assert(
        std::disjunction_v<
            std::
                is_same<distr_t, oneapi::mkl::rng::device::bits<std::uint32_t>>,
            std::is_same<
                distr_t,
                oneapi::mkl::rng::device::uniform_bits<std::uint32_t>>,
    ...
    if constexpr (std::is_same_v<
                      distr_t,
                      oneapi::mkl::rng::device::lognormal<float>>) {
      return generate_vec<vec_size>(
          _distr_lognormal_float, distr_params..., 0.0f, 1.0f);
    }
    if constexpr (std::is_same_v<
                      distr_t,
                      oneapi::mkl::rng::device::lognormal<double>>) {
      return generate_vec<vec_size>(
          _distr_lognormal_double, distr_params..., 0.0, 1.0);
    }
    if constexpr (std::is_same_v<
                      distr_t,
                      oneapi::mkl::rng::device::poisson<std::uint32_t>>) {
...
class rng_generator_base {
 public:
  ...
  virtual inline void generate_uniform_bits(
      unsigned int* output,
      std::int64_t n) = 0;
  ...
  virtual inline void generate_lognormal(
      float* output,
      std::int64_t n,
      float m,
      float s) = 0;
  ...
  virtual inline void generate_gaussian(
      float* output,
      std::int64_t n,
      float mean,
      float stddev) = 0;
  ...
  virtual inline void generate_poisson(
      unsigned int* output,
      std::int64_t n,
      double lambda) = 0;
 ...
 protected:
  sycl::queue* _queue{&at::xpu::getCurrentXPUStream().queue()};
...
class rng_generator : public rng_generator_base {
 public:
  ...
  /// \param output The pointer of the first random number.
  /// \param n The number of random numbers.
  inline void generate_uniform_bits(
      unsigned long long* output,
      std::int64_t n) {
#ifndef __INTEL_MKL__
    throw std::runtime_error(
  ...
  /// \param m Mean of associated normal distribution
  /// \param s Standard deviation of associated normal distribution.
  inline void generate_lognormal(
      float* output,
      std::int64_t n,
      float m,
      float s) {
 ...
 private:
  static inline engine_t create_engine(
      sycl::queue* queue,
  ...
  void generate(
    ...
    oneapi::mkl::rng::generate(distr_t(distr_params...), _engine, n, output);
  }
  engine_t _engine{};
};
} // namespace detail
