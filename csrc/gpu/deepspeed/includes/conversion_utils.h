...
// Note: there is a CVT instruction for sycl::half -> double, but there's no
// inline interface for passing a single half value
template <>
DS_D_INLINE double to(sycl::half val) {
  return to<double>(sycl::vec<sycl::half, 1>{val}
                        .convert<float, sycl::rounding_mode::automatic>()[0]);
}
template <>
DS_D_INLINE double to(int64_t val) {
  return sycl::vec<long long, 1>{val}
...
DS_D_INLINE double to(uint64_t val) {
  return sycl::vec<unsigned long long, 1>{val}
      .convert<double, sycl::rounding_mode::rte>()[0];
}
template <>
DS_D_INLINE double to(uint32_t val) {
  return sycl::vec<unsigned int, 1>{val}
      .convert<double, sycl::rounding_mode::rte>()[0];
}
template <>
...
DS_D_INLINE sycl::half to(int32_t val) {
  return sycl::vec<int, 1>{val}
      .convert<sycl::half, sycl::rounding_mode::rte>()[0];
}
template <>
DS_D_INLINE sycl::half to(int16_t val) {
  return sycl::vec<short, 1>{val}
      .convert<sycl::half, sycl::rounding_mode::rte>()[0];
}
template <>
...
DS_D_INLINE sycl::half to(uint16_t val) {
  return sycl::vec<unsigned short, 1>{val}
      .convert<sycl::half, sycl::rounding_mode::rte>()[0];
}
template <>
...
DS_D_INLINE sycl::marray<sycl::ext::oneapi::bfloat16, 2> to(sycl::half2 val) {
  return to<sycl::marray<sycl::ext::oneapi::bfloat16, 2>>(
      to<sycl::float2>(val));
}
#endif
...
DS_D_INLINE int64_t to(double val) {
  return sycl::vec<double, 1>{val}
      .convert<long long, sycl::rounding_mode::rte>()[0];
}
template <>
...
DS_D_INLINE uint32_t to(sycl::ext::oneapi::bfloat16 val) {
  return sycl::vec<float, 1>(val)
      .convert<unsigned, sycl::rounding_mode::rte>()[0];
}
#endif
