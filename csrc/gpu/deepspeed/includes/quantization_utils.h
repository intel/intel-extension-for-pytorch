...
class Params {
  ...
  */
  ...
  template <typename T>
  DS_D_INLINE T dequant(int8_t val);
...
class Params<Type::Symmetric, numBits> {
 public:
  ...
  DS_D_INLINE int8_t quantize(sycl::half val) {
    ...
    data_i32 = sycl::min(sycl::max(data_i32, q_min), q_max);
...
class Params<Type::Asymmetric, numBits> {
 public:
  ...
  DS_D_INLINE void store(float* params, int group_index) {
    ...
    const float store_scale = 1 / scale;
    mem_access::store_global<sizeof(float)>(
        params + 2 * group_index, &store_scale);
    mem_access::store_global<sizeof(float)>(
        params + 2 * group_index + 1, &offset);
  }
...
class GroupStats<Type::Asymmetric> {
  ...
  */
  ...
  DS_D_INLINE Params<Type::Asymmetric, numBits> get_params(
    ...
    reduce::partitioned_block<rop::Max, rop::Min, threads_per_group>(
        tb, warp, max, min);
...
store quantized data.    int8_t* data         -  Pointer to input data.
sycl::half* Params       -  Parameters for quantization. Params<qType, numBits>
*/
template <int numBits, Type qType>
DS_D_INLINE void _chunk(
    int8_t* local_output,
...
Type::Asymmetric Function Arguments : local_output -  Pointer to local memory to
store quantized data.    int8_t* data         -  Pointer to input data.
sycl::half2* Params       -  Parameters for quantization. Params<qType, numBits>
*/
template <int numBits, Type qType>
DS_D_INLINE void _chunk(
    int8_t* local_output,
...
/*
The main loop of the kernel that quantizes array in local memory of sycl::half2
type input data, when Quantization parameters are pre-computed. Template
Arguments : qType       -   Type of quantization to perform. Type::Symmetric or
Type::Asymmetric numBits     -   Number of bits in quantized element.        int
: 8 or 4 numChunks   -   Number of chunks(16 bytes of Input data).   int : 8 or
4 Function Arguments : local_buffer    -   Pointer memory with input half2 data
to be quantized. scales          -   Pointer to output scales. offsets         -
Pointer to output offsets. output_data     -   Pointer to output data.
...
/*
The main loop of the kernel that quantizes array in local memory of sycl::half2
type input data. This function computes quantization parameters for each group.
Template Arguments :
...
DS_D_INLINE void local_array(
  ...
  // Indexing offsets
  const int block_num =
      (tb.get_group_id()[2] * max_threads / threads_per_group) +
      tb.get_local_id()[1];
  ...
  const int stride = sycl::ext::oneapi::experimental::this_group<3>()
...
DS_D_INLINE void local_array(
  ...
  quantize::
      local_array<qType, numBits, numChunks, threads_per_group, max_threads>(
          tb,
          warp,
          local_buffer,
          global_params,
