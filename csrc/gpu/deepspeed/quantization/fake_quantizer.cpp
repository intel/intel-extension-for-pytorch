...
class fake_quantize_kernel<sycl::half> {
 ...
 public:
  fake_quantize_kernel(sycl::half* vals, int group_size, int num_bits)
      : vals(vals), group_size(group_size), num_bits(num_bits) {}
...
#pragma unroll
      for (int i = 0; i < vals_per_access; i++) {
        if (abs((float)data[i]) > max)
          max = abs((float)data[i]);
      }
    }
...
#pragma unroll
    ...
    auto& partialMax =
        *sycl::ext::oneapi::group_local_memory_for_overwrite<float[WARP_SIZE]>(
...
class fake_quantize_kernel<float> {
 ...
 public:
  fake_quantize_kernel(float* vals, int group_size, int num_bits)
      : vals(vals), group_size(group_size), num_bits(num_bits) {}
...
#pragma unroll
      for (int i = 0; i < vals_per_access; i++) {
        if (sycl::fabs(data[i]) > max)
          max = sycl::fabs(data[i]);
      }
    }
...
template void launch_fake_quantize_kernel(
    float* vals,
    int total_count,
    int group_num,
    int num_bits,
...
class sr_fake_quantize_kernel<sycl::half> {
 ...
 public:
  ...
  void operator()(sycl::nd_item<3>) const {
    ...
    auto idx = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
    ...
    ds::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>
        state;
    state = ds::rng::device::rng_generator<
        oneapi::mkl::rng::device::philox4x32x10<1>>(
        seed.first, {seed.second, (unsigned long)idx * 4});
    ...
    if (group_index < total_count) {
      ...
      while (tid < token_size) {
        ...
        if (abs((float)data_f[0].x()) > max)
          max = abs((float)data_f[0].x());
        if (abs((float)data_f[0].y()) > max)
          max = abs((float)data_f[0].y());
        if (abs((float)data_f[1].x()) > max)
          max = abs((float)data_f[1].x());
        if (abs((float)data_f[1].y()) > max)
          max = abs((float)data_f[1].y());
...
#pragma unroll
      ...
      auto& partialMax = *sycl::ext::oneapi::group_local_memory_for_overwrite<
          float[WARP_SIZE]>(sycl::ext::oneapi::experimental::this_group<3>());
...
#pragma unroll
      ...
      for (int i = 0; i < reg_count; i++) {
        int token_index =
            i * item_ct1.get_local_range(2) + item_ct1.get_local_id(2);
        if (token_index < token_size) {
          ...
          // Stochastic rounding
          sycl::float4 rand =
              state.generate<oneapi::mkl::rng::device::uniform<float>, 4>();

          float q_error[4];
          q_error[0] = abs(data_f[0].x() - (q_data_int[0].x() / q_scale_val)) *
              q_scale_val;
          q_error[1] = abs(data_f[0].y() - (q_data_int[0].y() / q_scale_val)) *
              q_scale_val;
          q_error[2] = abs(data_f[1].x() - (q_data_int[1].x() / q_scale_val)) *
              q_scale_val;
          q_error[3] = abs(data_f[1].y() - (q_data_int[1].y() / q_scale_val)) *
              q_scale_val;

          q_data_int[0].x() =
              (rand.x() < q_error[0] && q_data_int[0].x() > low_q &&
               q_data_int[0].x() < high_q)
              ? (q_data_int[0].x() + (data_f[0].x() > 0 ? 1 : -1))
              : q_data_int[0].x();
          q_data_int[0].y() =
              (rand.y() < q_error[1] && q_data_int[0].y() > low_q &&
               q_data_int[0].y() < high_q)
              ? (q_data_int[0].y() + (data_f[0].y() > 0 ? 1 : -1))
              : q_data_int[0].y();
          q_data_int[1].x() =
              (rand.w() < q_error[2] && q_data_int[1].x() > low_q &&
               q_data_int[1].x() < high_q)
              ? (q_data_int[1].x() + (data_f[1].x() > 0 ? 1 : -1))
              : q_data_int[1].x();
          q_data_int[1].y() =
              (rand.z() < q_error[3] && q_data_int[1].y() > low_q &&
...
class sr_fake_quantize_kernel<float> {
 ...
 public:
  ...
  void operator()(sycl::nd_item<3>) const {
    ...
    state = ds::rng::device::rng_generator<
        oneapi::mkl::rng::device::philox4x32x10<1>>(
        seed.first, {(unsigned long)idx, seed.second * 4});
    ...
    if (group_index < total_count) {
      ...
      while (tid < token_size) {
        ...
        if (sycl::fabs(data[reg_count].x()) > max)
          max = sycl::fabs(data[reg_count].x());
        if (sycl::fabs(data[reg_count].y()) > max)
          max = sycl::fabs(data[reg_count].y());
        if (sycl::fabs(data[reg_count].z()) > max)
          max = sycl::fabs(data[reg_count].z());
        if (sycl::fabs(data[reg_count].w()) > max)
          max = sycl::fabs(data[reg_count].w());
...
#pragma unroll
      ...
      for (int i = 0; i < reg_count; i++) {
        ...
        if (group_index < token_size) {
          ...
          float q_error[4];
          q_error[0] = sycl::fabs(q_data.x() - (q_data_int.x() / q_scale_val)) *
              q_scale_val;
          q_error[1] = sycl::fabs(q_data.y() - (q_data_int.y() / q_scale_val)) *
              q_scale_val;
          q_error[2] = sycl::fabs(q_data.w() - (q_data_int.w() / q_scale_val)) *
              q_scale_val;
          q_error[3] = sycl::fabs(q_data.z() - (q_data_int.z() / q_scale_val)) *
              q_scale_val;
...
template <typename T>
void launch_sr_fake_quantize_kernel(
    T* vals,
    int total_count,
    int group_num,
    int num_bits,
  ...
  std::pair<uint64_t, uint64_t> seed =
      TrainingContext::Instance().IncrementOffset(inc);

  ds::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
  sr_fake_quantize_kernel<T> fn(
      vals, (total_count / group_num) / 4, group_num, num_bits, seed);
  stream->parallel_for(sycl::nd_range<3>(grid_dim * block_dim, block_dim), fn);
}
template void launch_sr_fake_quantize_kernel(
    float* vals,
    int total_count,
    int group_num,
    int num_bits,
...
class fake_quantize_kernel_asym<sycl::half> {
 ...
 public:
  fake_quantize_kernel_asym(sycl::half* vals, int group_size, int num_bits)
      : vals(vals), group_size(group_size), num_bits(num_bits) {}
  ...
  void operator()(sycl::nd_item<3>) const {
...

    {
      ...
      while (group_index < group_size && reg_count < MAX_REG) {
        ...
        if (((float)data_h[0]) > max)
          max = (float)data_h[0];
        if (((float)data_h[1]) > max)
          max = (float)data_h[1];
        if (((float)data_h[2]) > max)
          max = (float)data_h[2];
        if (((float)data_h[3]) > max)
          max = (float)data_h[3];

        if (((float)data_h[0]) < min)
          min = (float)data_h[0];
        if (((float)data_h[1]) < min)
          min = (float)data_h[1];
        if (((float)data_h[2]) < min)
          min = (float)data_h[2];
        if (((float)data_h[3]) < min)
          min = (float)data_h[3];
...
#pragma unroll
      ...
      auto& partialMax = *sycl::ext::oneapi::group_local_memory_for_overwrite<
          float[WARP_SIZE]>(sycl::ext::oneapi::experimental::this_group<3>());
      auto& partialMin = *sycl::ext::oneapi::group_local_memory_for_overwrite<
          float[WARP_SIZE]>(sycl::ext::oneapi::experimental::this_group<3>());
...
class fake_quantize_kernel_asym<float> {
 ...
 public:
  fake_quantize_kernel_asym(float* vals, int group_size, int num_bits)
      : vals(vals), group_size(group_size), num_bits(num_bits) {}
...
void launch_fake_quantize_kernel_asym(
  ...
  ds::has_capability_or_fail(stream->get_device(), {sycl::aspect::fp16});
  fake_quantize_kernel_asym<T> fn(
      vals, (total_count / group_num) / 4, num_bits);
  stream->parallel_for(sycl::nd_range<3>(grid_dim * block_dim, block_dim), fn);
}

template void launch_fake_quantize_kernel_asym(
    float* vals,
    int total_count,
    int group_num,
    int num_bits,
...
void sr_fake_quantize_kernel_asym(
    sycl::half* vals,
    int token_size,
    int token_num,
    int num_bits,
  ...
  if (group_index < total_count) {
    ...
    while (tid < token_size) {
      ...
      if (((float)data_f[0].x()) > max)
        max = (float)data_f[0].x();
      if (((float)data_f[0].y()) > max)
        max = (float)data_f[0].y();
      if (((float)data_f[1].x()) > max)
        max = (float)data_f[1].x();
      if (((float)data_f[1].y()) > max)
        max = (float)data_f[1].y();

      if (((float)data_f[0].x()) < min)
        min = (float)data_f[0].x();
      if (((float)data_f[0].y()) < min)
        min = (float)data_f[0].y();
      if (((float)data_f[1].x()) < min)
        min = (float)data_f[1].x();
      if (((float)data_f[1].y()) < min)
        min = (float)data_f[1].y();
...
#pragma unroll
    ...
    for (int i = 0; i < reg_count; i++) {
      ...
      if (token_index < token_size) {
        ...
        q_data_int[0].x() =
            (rand.x() < q_error[0] && q_data_int[0].x() < high_q)
            ? (q_data_int[0].x() + 1)
            : q_data_int[0].x();
        q_data_int[0].y() =
            (rand.y() < q_error[1] && q_data_int[0].y() < high_q)
            ? (q_data_int[0].y() + 1)
            : q_data_int[0].y();
        q_data_int[1].x() =
            (rand.w() < q_error[2] && q_data_int[1].x() < high_q)
            ? (q_data_int[1].x() + 1)
            : q_data_int[1].x();
        q_data_int[1].y() =
            (rand.z() < q_error[3] && q_data_int[1].y() < high_q)
            ? (q_data_int[1].y() + 1)
            : q_data_int[1].y();
...
#pragma unroll
    ...
    for (int i = 0; i < reg_count; i++) {
      ...
      if (group_index < token_size) {
        ...
        float q_error[4];
        q_error[0] =
            sycl::fabs(q_data.x() - ((q_data_int.x() * q_scale_val) + min)) /
            q_scale_val;
        q_error[1] =
            sycl::fabs(q_data.y() - ((q_data_int.y() * q_scale_val) + min)) /
            q_scale_val;
        q_error[2] =
            sycl::fabs(q_data.w() - ((q_data_int.w() * q_scale_val) + min)) /
            q_scale_val;
        q_error[3] =
            sycl::fabs(q_data.z() - ((q_data_int.z() * q_scale_val) + min)) /
            q_scale_val;
...
template <typename T>
void launch_sr_fake_quantize_kernel_asym(
    T* vals,
    int total_count,
    int group_num,
    int num_bits,
...
template void launch_sr_fake_quantize_kernel_asym(
    float* vals,
    int total_count,
    int group_num,
    int num_bits,
