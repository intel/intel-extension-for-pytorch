...
bool is_profiler_enabled() {
  return torch_ipex::xpu::dpcpp::is_profiler_enabled();
}

void profiler_record(std::string name, sycl::event& event) {
  return torch_ipex::xpu::dpcpp::dpcpp_log(name, event);
}
...
void profiler_record(
    ...
    sycl::event& start_event,
    sycl::event& end_event) {
  torch_ipex::xpu::dpcpp::dpcpp_log(name, start_event, end_event);
}
