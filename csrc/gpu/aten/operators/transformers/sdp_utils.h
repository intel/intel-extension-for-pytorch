...
inline bool check_for_seq_len_1_nested_tensor(sdp_params params, bool debug) {
  ...
  if (!num_head_dims.has_value()) {
    ...
    if (debug) {
      TORCH_WARN(
          "Memory efficient attention does not support ragged num_head_dims");
    }
    return false;
  }
  ...
  for (const auto i : c10::irange(n_tensors)) {
    if (sizes_ptr[(i * size_tensor_stride) + 1] <= 1) {
      if (debug) {
        TORCH_WARN(
            "Memory efficient attention does not support sequence_length <= 1");
      }
      return false;
    }
  }
...
inline bool check_for_nested_inputs(sdp_params params, bool debug) {
  if (params.query.is_nested() || params.key.is_nested() ||
    ...
    if (debug) {
      TORCH_WARN(
          "We are not enabling nested Tensors for Flash Attention because of xpu memory errors.");
    }
    return false;
  }
  return true;
...
inline bool check_requires_grad_and_nested(sdp_params params, bool debug) {
  ...
  if (!check_for_nested_inputs(params, false) &&
      !check_requires_grad(params, false)) {
    if (debug) {
      TORCH_WARN(
...
inline bool check_equal_batch_size_and_num_heads(
    sdp_params params,
    bool debug) {
...
inline bool check_requires_grad_and_head_dim_gt64( //_and_sm_ge86(
    sdp_params params,
    bool debug) {
...
inline bool use_flash_attention(sdp_params params, bool debug) {
  constexpr std::array<bool (*)(sdp_params, bool), 9> constraints{
      {check_runtime_disabled_flash,
       ...
       check_for_attn_mask,
       check_head_dim_size,
       check_requires_grad_and_head_dim_gt64,
  ...
  static const std::array<at::ScalarType, 1> default_flash_dtypes{at::kFloat};
