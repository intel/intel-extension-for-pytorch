...
using torch_ipex::xpu::oneDNN::Attr;
...
struct DeformInterpolateKernelFunctor {
    ...
    if (idx < out_size) {
      ...
      for (int j = 0; j < k_h; ++j) {
        for (int k = 0; k < k_w; ++k) {
          ...
          int local_mask_idx = ((j * k_w + k) * o_h + o_h_idx) * o_w + o_w_idx;
  ...
  DeformInterpolateKernelFunctor(
      scalar_t* input_,
      scalar_t* out_,
      scalar_t* offset_,
      scalar_t* mask_,
      ...
      const int pad_h_,
      const int pad_w_,
      const int dilation_h_,
      const int dilation_w_,
      const int deformable_group_,
      int channel_per_group_,
      int64_t out_size_,
      int64_t glb_size_)
      : input(input_),
        out(out_),
        ...
        deformable_group(deformable_group_),
        channel_per_group(channel_per_group_),
        out_size(out_size_),
        glb_size(glb_size_) {}
...
void deform_interpolate(
  ...
  auto cgf = DPCPP_Q_CGF(_cgh) {
    DeformInterpolateKernelFunctor<scalar_t> kfn(
        input,
        out,
        offset,
        mask,
