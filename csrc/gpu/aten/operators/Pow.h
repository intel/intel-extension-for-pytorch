...
#else
template <typename Base_type, typename Exp_type>
static inline Base_type powI_(Base_type base, Exp_type exp) {
  return ::pow(base, exp);
}
#endif

template <typename T>
static inline std::enable_if_t<std::is_integral<T>::value, T> powI_(
    T base,
    T exp) {
...
template <typename T>
static inline c10::complex<T> powI_(c10::complex<T> base, c10::complex<T> exp) {
