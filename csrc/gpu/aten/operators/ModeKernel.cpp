    ...
    auto cgf = DPCPP_Q_CGF(cgh) {
      ...
      auto kfn = DPCPP_Q_KFN(sycl::nd_item<1> item) {
        mode_fused_impl(
            ...
            values_info,
            indices_info,
            (ModeOpHelper*)slm_helper.get_pointer().get(),
            (ModeOpValueIndex<scalar_t>*)slm_value_indice.get_pointer().get(),
            (std::byte*)sort_scratch.get_pointer().get(),
