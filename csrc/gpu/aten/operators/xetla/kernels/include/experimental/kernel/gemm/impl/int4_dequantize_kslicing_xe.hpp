...
class int4_gemm_universal_t {
 ...
 public:
  ...
  static bool can_implement(arguments_t& args) {
    ...
    // check for int4 packing, following packing conversion along k dimension
...
inclusive_prefix_sum_int4(xetla_vector<T, N> src) {
  return src;
}
template <typename T, int N, int Start>
...
inclusive_prefix_sum_int4(xetla_vector<T, N> src) {
  ...
  return inclusive_prefix_sum_int4<T, N, Start * 2>(dst);
}
template <typename gemm_t_, typename epilogue_t_>
class int4_group_gemm_universal_t {
  using gemm_t = gemm_t_;
  using epilogue_t = epilogue_t_;
  using gemm_args_t = typename gemm_t::arguments_t;
  using epilogue_args_t = typename epilogue_t::arguments_t;
  ...
  static constexpr int PERSISTANT_SG_NUMS = 20 * 32;
 ...
 public:
  ...
  static sycl::range<3> get_group_range(sycl::range<3>& local_range) {
    ...
    if (PERSISTANT_SG_NUMS > local_size) {
      group_size = PERSISTANT_SG_NUMS / local_size;
    }
    assert(group_size * local_size == PERSISTANT_SG_NUMS);
  ...
  __XETLA_API KERNEL_FUNC void operator()(
    ...
    for (int i = 0; i < args.expert_num; i += load_expert_num) {
      ...
      xetla_vector<int, load_expert_num> cumsum_rows_for_experts =
          inclusive_prefix_sum_int4<int, load_expert_num, 1>(rows_for_experts);

      xetla_vector<int, load_expert_num> cumsum_tiles_for_experts =
          inclusive_prefix_sum_int4<int, load_expert_num, 1>(
