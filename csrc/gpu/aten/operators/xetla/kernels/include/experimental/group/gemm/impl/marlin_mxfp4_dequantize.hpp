...
template <
    ...
    gpu_arch arch_tag_>
class gemm_t<
    compute_policy_mxfp4_dequantize<
        compute_attr_,
        perf_tuning_knob_,
        dtype_scale_,
        dequant_s_,
 ...
 public:
  ...
  using compute_policy = compute_policy_mxfp4_dequantize<
      compute_attr_,
      perf_tuning_knob_,
      dtype_scale_,
      dequant_s_,
  ...
  static constexpr uint16_t dst_bias = 251;
  static constexpr uint16_t fast_cvt_u16 = 0x7E80;
  static constexpr sycl::ext::oneapi::bfloat16 fast_cvt_bf16 =
      sycl::bit_cast<sycl::ext::oneapi::bfloat16>(fast_cvt_u16);
...
#pragma unroll
      for (uint32_t j = 0; j < num_block_x; ++j) {
        ...
        xetla_vector<uint16_t, matB_acc_t::block_elems> src_elems_u16;
        auto l_in =
            src_elems_u16.template select<matB_t::block_elems * 4, 2>(0);
        auto h_in =
            src_elems_u16.template select<matB_t::block_elems * 4, 2>(1);
        l_in = matB_blk & 0x07;
        h_in = (matB_blk & 0x70) >> 4;

        xetla_vector<uint16_t, matB_acc_t::block_elems> s_u16;
        auto s0 = s_u16.template select<matB_t::block_elems * 4, 2>(0);
        auto s1 = s_u16.template select<matB_t::block_elems * 4, 2>(1);
        s0 = matB_blk & 0x08;
        s1 = (matB_blk & 0x80) >> 4;

        xetla_vector<uint16_t, matB_acc_t::block_elems> mask_gt1_u16 =
            xetla_vector<uint16_t, matB_acc_t::block_elems>(src_elems_u16 > 1);
        xetla_vector<uint16_t, matB_acc_t::block_elems> mask_nonzero_u16 =
            xetla_vector<uint16_t, matB_acc_t::block_elems>(src_elems_u16 != 0);
        src_elems_u16 =
            (((src_elems_u16 + dst_bias + mask_gt1_u16) * mask_nonzero_u16)
             << 6) |
            (s_u16 << 12);
        auto src_elems_bf16 =
            src_elems_u16.xetla_format<sycl::ext::oneapi::bfloat16>();
        ...
        xetla_vector<uint16_t, scale_t::block_size_x> scale_u16 = scale_vec
            << 7;
        auto scale_bf16 =
            (scale_u16.xetla_format<sycl::ext::oneapi::bfloat16>());
...
#pragma unroll
            for (uint32_t m = 0; m < vnni_rows; ++m) {
              ...
              dst = src_elems_bf16.xetla_select<block_size_x_b, pack_ratio>(
...
#pragma unroll
      for (uint32_t j = 0; j < num_block_x; ++j) {
        ...
        // 01 | 23
        // 45 | 67
        xetla_vector<uint8_t, matB_t::block_elems * 4> u8_matB_blk;
        for (uint32_t k = 0; k < packed_block_size_y; ++k) {
          u8_matB_blk.template select<2 * block_size_x_b, 1>(
              k * 4 * block_size_x_b) =
        ...
        //  911 | 911 | ... | 911
        //  1315| 1315| ... | 1315
        xetla_vector<uint16_t, matB_t::block_elems* 4> u16_lo_matB =
            (u8_matB_blk & 0x0f) << 12;
        xetla_vector<uint16_t, matB_t::block_elems* 4> u16_hi_matB =
            (u8_matB_blk & 0xf0) << 8;

        xetla_vector<int16_t, matB_t::block_elems* 4> i16_lo_matB =
            u16_lo_matB.xetla_format<int16_t>() >> 6;
        xetla_vector<int16_t, matB_t::block_elems* 4> i16_hi_matB =
            u16_hi_matB.xetla_format<int16_t>() >> 6;

        u16_lo_matB = i16_lo_matB.xetla_format<uint16_t>() & 0x81C0;
        u16_hi_matB = i16_hi_matB.xetla_format<uint16_t>() & 0x81C0;

        xetla_vector<sycl::ext::oneapi::bfloat16, matB_t::block_elems* 4>
            bf16_lo_matB =
                u16_lo_matB.xetla_format<sycl::ext::oneapi::bfloat16>() *
            fast_cvt_bf16;
        xetla_vector<sycl::ext::oneapi::bfloat16, matB_t::block_elems* 4>
            bf16_hi_matB =
                u16_hi_matB.xetla_format<sycl::ext::oneapi::bfloat16>() *
            fast_cvt_bf16;
        ...
        xetla_vector<uint16_t, scale_t::block_size_x* vnni_rows> scale_u16 =
            scale_vec_vnni << 7;
        auto scale_bf16 =
            (scale_u16.xetla_format<sycl::ext::oneapi::bfloat16>());
...
#pragma unroll
        for (uint32_t k = 0; k < packed_block_size_y; ++k) {
          ...
          lo_dst =
              bf16_lo_matB.template select<4 * block_size_x_b, 1>(src_offset);
          hi_dst =
              bf16_hi_matB.template select<4 * block_size_x_b, 1>(src_offset);
        }
