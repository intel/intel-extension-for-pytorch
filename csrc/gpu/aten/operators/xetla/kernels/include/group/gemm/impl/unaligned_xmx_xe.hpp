...
class gemm_t<
 ...
 public:
  ...
  static constexpr bool lda_align =
      (mem_desc_a_t::alignment_in_bytes % load_store_attr::alignment_in_bytes ==
       0);
  static constexpr bool ldb_align =
      (mem_desc_b_t::alignment_in_bytes % load_store_attr::alignment_in_bytes ==
       0);
 ...
 private:
  ...
  using matA_payload_t = subgroup::mem_payload_t<
      ...
      is_local_a      ? msg_type::scatter
          : lda_align ? msg_type::block_2d
  ...
  using matB_payload_t = subgroup::mem_payload_t<
      ...
      is_local_b      ? msg_type::scatter
          : ldb_align ? msg_type::block_2d
                      : msg_type::unaligned_2d,
      arch_tag>;
 ...
 public:
  ...
  static constexpr uint32_t slm_base_b = slm_size_a * num_cyclic;
  ...
  inline void sync_arrive_wait() {
    if constexpr (arch_has_named_barrier<arch_tag>) {
      if constexpr (wg_size_x > 1) {
        nbarrier_a.arrive_wait();
      }
      if constexpr (wg_size_y > 1) {
        nbarrier_b.arrive_wait();
      }
    } else if constexpr (wg_size > 1) {
