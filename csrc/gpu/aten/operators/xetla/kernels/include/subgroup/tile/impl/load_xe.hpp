...
tile_load(tile_t& tile, payload_t& payload) {
  ...
  using check_load =
      typename subgroup::check_load<gpu_arch::Xe, dtype, load_dtype>::
          template global_2d<payload_t::mem_transform, tile_desc::block_size_x>;
...
#pragma unroll
    for (int j = 0; j < num_block_x; j += arr_len) {
      ...
      xetla_vector<dtype, tmp_size> reg_tmp;
#pragma unroll
      for (int ii = 0; ii < remained_size_y / remained_ld_blk_size_y; ++ii) {
        constexpr uint32_t load_elems =
...
tile_load(tile_t& tile, payload_t& payload) {
  ...
  using load_dtype = typename payload_t::mem_dtype;
  using check_load =
      typename subgroup::check_load<gpu_arch::Xe, dtype, load_dtype>::global_1d;
...
tile_load(tile_t& tile, payload_t& payload, oob_check_tag tag = {}) {
  ...
  using load_dtype = typename payload_t::mem_dtype;
  using check_load =
      typename subgroup::check_load<gpu_arch::Dg2, dtype, load_dtype>::
          template global_2d<payload_t::mem_transform, tile_desc::block_size_x>;
...
tile_load(tile_t& tile, payload_t& payload, oob_check_tag tag = {}) {
  ...
  using tile_desc = typename payload_t::tile_desc;
  using load_dtype = typename payload_t::mem_dtype;
  using check_load = typename subgroup::
      check_load<gpu_arch::Xe, dtype, load_dtype>::template unaligned_2d<
          payload_t::mem_transform,
...
tile_load(tile_t& tile, payload_t& payload) {
  ...
  using load_dtype = typename payload_t::mem_dtype;
  using check_load = typename subgroup::
      check_load<gpu_arch::Xe, dtype, load_dtype>::template local_scatter<
          payload_t::memory_layout,
          payload_t::tile_desc::block_size_x,
          payload_t::tile_bytes,
          payload_t::min_bytes,
          payload_t::block_bytes,
          payload_t::num_channel_x,
          payload_t::num_channel>;
...
tile_load(tile_t& tile, payload_t& payload) {
  ...
  using load_dtype = typename payload_t::mem_dtype;
  using check_load =
      typename subgroup::check_load<gpu_arch::Xe, dtype, load_dtype>::local_1d;
