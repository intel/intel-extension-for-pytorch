...
struct tile_op_arg_helper_t {
  ...
  inline tile_op_args_t get_args() const {
    return args;
  }
  inline void set_args(const tile_op_args_t& new_args) {
    args = new_args;
  }
};
...
struct chained_tile_op_arg_t<idx, curr_args_t, remain_args_t...>
  ...
  inline chained_tile_op_arg_t(
      curr_args_t curr_args,
      remain_args_t... remain_args)
...
struct chained_tile_op_t {
  ...
  __XETLA_API KERNEL_FUNC void operator()(
    ...
    if constexpr (list_size == 0) {
      ...
      chained_tile_op(matAcc, coord, args_helper, slm_base, nbarrier_base);
    }
  }
 ...
 private:
  ...
  struct chained_tile_op_helper<curr_tile_op_t, remain_tile_op_t...> {
    ...
    static constexpr int curr_idx = list_size - sizeof...(remain_tile_op_t) - 1;
    template <typename matAcc_t, typename coord_t>
    __XETLA_API KERNEL_FUNC void operator()(
        matAcc_t& matAcc,
        const coord_t& coord,
      ...
      if constexpr (sizeof...(remain_tile_op_t) > 0) {
        ...
        remain_tile_op(matAcc, coord, args_helper, slm_base, nbarrier_base);
      }
    }
  };
};
