...
function(configure_ SCALAR_TS ARCH AOT_DEVLIST QUANT_MODE WG_M WG_N SG_M SG_N SG_K SLM_KS L3_KS SYNC_FREQ STAGES)
    ...
    foreach(SCALAR_T ${SCALAR_TS})
        ...
        add_library_with_options("xetla_${SUFFIX}" ${IS_GEMM} "${AOT_DEVLIST}" "${CMAKE_CURRENT_BINARY_DIR}/GEMM_int4_${SUFFIX}.cpp")
...
foreach(QUANT_MODE ${L_QUANT_MODE})
    ...
    elseif(ARCH STREQUAL "xe_hpg")
        configure_("fp16;bf16" ${arch_tag} "${aot_tag}" ${QUANT_MODE} 1   1   1  1  256  1 1 0 0)
        configure_("fp16;bf16" ${arch_tag} "${aot_tag}" ${QUANT_MODE} 1   1   1  1  512  1 1 0 0)

    elseif(ARCH STREQUAL "xe_hpc")
        configure_("fp16;bf16" ${arch_tag} "${aot_tag}" ${QUANT_MODE} 1   1   1  1  256  1 1 1 3)
        configure_("fp16;bf16" ${arch_tag} "${aot_tag}" ${QUANT_MODE} 1   1   1  1  512  1 1 1 3)
