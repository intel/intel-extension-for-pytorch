...
void fmha_forward_causal_impl(
  ...
  using fmha_forward_op_t =
      fmha_forward_causal_t<fmha_policy, T, kUseBias, kIsCausal, kIsTraining>;
...
#define CALL_IMPL_FUNC(P)                                                 \
  fmha::fmha_forward_causal_impl<P, T, kUseBias, kIsCausal, kIsTraining>( \
      q,                                                                  \
      query,                                                              \
      key,                                                                \
      value,                                                              \
...
void fmha_forward_op_causal(
    ...
    const float scale) {
  using T = sycl::half;
  fmha_forward_causal<T, false, true, false>(
      q,
      (T*)query,
      (T*)key,
      (T*)value,
