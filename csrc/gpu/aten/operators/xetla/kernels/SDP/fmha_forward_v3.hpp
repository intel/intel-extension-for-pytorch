...
class fmha_forward_v3_t {
 ...
 private:
  ...
  // ------------------- // Slm and nbarrier // ------------------- //
  static constexpr uint32_t slm_size_Qi = kHeadPerKv * kBr * kHm * sizeof(scalar_t);
  static constexpr uint32_t slm_size_Pij = kHeadPerKv * kBr * kBc * sizeof(scalar_t);
  static constexpr uint32_t slm_size_softmax = kHeadPerKv * (
  ...
  struct context_t {
    ...
    inline void init_context(nd_item<3>& item, arguments_t& args) {
      ...
      for(uint32_t i = 0; i < kHeadPerKv; i++) {
        mem_desc_Qi_L[i].init(Qi_slm + i * kBr * kHm * sizeof(scalar_t), {kHm, kBr, kHm}, {0, 0});
        mem_desc_Pij_L[i].init(Pij_slm + i * kBr * kBc * sizeof(scalar_t), {kBc, kBr, kBc}, {0, 0});
      }
    }
    ...
    inline void update_context(
      ...
      //   // bias per head
      //   int32_t offset =
      //       (batch_id * args.bias_strideB + head_id_start * args.bias_strideN) /
      //       args.uMT;
      //   int32_t start_y =
  ...
  /// # [4 *[Br,H]] x [H,Bc] = [Br,Bc]
  inline void gemm_Sij(std::array<matAccSij_t, kHeadPerKv>& matAccSij_s, arguments_t& args) {
  ...
  inline void apply_mask(
    ...
    if constexpr (kUseAlibi && kVarlen) {
      float _alibi_slopes = reinterpret_cast<float*>(
          args.A_ptr)[ctx.batch_id * args.uAT + (ctx.head_id_kv * kHeadPerKv + iter)];
  ...
  inline void softmax_fwd(
    ...
    uint32_t reducer_slm =
        softmax_slm + (iter * wg_size + ctx.sg_idy * wg_size_x) * kSgBr * sizeof(accum_t);
    ...
    // correct old l
    ctx.softmax_l[iter] *= xetla_exp<accum_t, kSgBr>(ctx.softmax_m[iter] - m_new);
    // compute Pij
  ...
  inline void preload_Qi([[maybe_unused]] arguments_t& args, uint32_t iter = 0) {
...
public:
  ...
  inline static constexpr uint32_t get_barrier_count() {
    ...
    constexpr uint32_t count =
        std::max(barrier_count_Sij, barrier_count_Oi);
    static_assert(
  ...
  inline KERNEL_FUNC void operator()(nd_item<3>& item, arguments_t& args) {
    ...
    for (uint32_t startT = 0; startT < args.uT; startT += kBc) {
      ...
      for (uint32_t i = 0; i < kHeadPerKv; i++) {
        ...
        // apply mask
        apply_mask(item, matAccSij, args, startF + seqlen_diff, startT, i);
        // softmax
        dp_mask_tile_t mask_in;
        softmax_fwd(matAccSij, matAccOi, mask_in, args, i);
      }
      // compute Oi
