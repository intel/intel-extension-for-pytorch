...
class fmha_forward_strided_t {
  ...
  static constexpr uint32_t kQueryBlock = fmha_policy::wg_tile0_m; // 64
  static constexpr uint32_t kKeyBlock = fmha_policy::wg_tile0_n; // 64
 ...
 public:
  struct arguments_t {
    // Input tensors
    scalar_t* query_ptr; // [B, N, F, H] -> [B, F, N, H]
    scalar_t* key_ptr; // [B, N, T, H] -> [B, T, N, H]
    scalar_t* value_ptr; // [B, N, T, H] -> [B, T, N, H]
    ...
    inline void init_coords(xetla_exec_item<3>& ei) {
      ...
      startF = batch_id * uF + block_id * kQueryBlock; // wg_tile_m0
      // Same reason like above
      boundaryF = (batch_id + 1) * uF;
      // key (T)
      // TODO: fix this
      startT = batch_id * uT; // kv_seq_len
      boundaryT = (batch_id + 1) * uT;
      // k-dimension also needs to indicate
      startH = head_id * uH;
 ...
 private:
  struct gemm0_t {
    ...
    */
    ...
    inline KERNEL_FUNC void operator()(arguments_t* args, uint32_t sg_id) {
      ...
      // query x key.T = score  # [bF,H] x [H,bT] = [bF,bT]
      mem_desc_a_t mem_desc_a( // q
          args->query_ptr, // [bs, sl, hn, hd]
          {args->uH * args->uN, boundary_m, args->uH * args->uN},
          {args->startH, args->startF});
      mem_desc_b_t mem_desc_b(
          args->key_ptr,
          {boundary_n, args->uH * args->uN, args->uH * args->uN},
  ...
  struct gemm1_t {
    ...
    */
    ...
    // Number of threads used in gemm1
    static constexpr uint32_t wg_size_x = tile_shape::wg_size_x;
    static constexpr uint32_t wg_size_y = tile_shape::wg_size_y;
    static constexpr uint32_t wg_size = wg_size_x * wg_size_y;
    ...
    inline KERNEL_FUNC void operator()(
      ...
      mem_desc_b_t mem_desc_b(
          args->value_ptr,
          {args->uH * args->uN, boundary_k, args->uH * args->uN},
          {args->startH, args->startT});
...
class FmhaForwardStridedKernel;

template <typename fmha_forward_op_t, typename T>
struct FmhaForwardImplStridedKernelFunctor {
  ...
  FmhaForwardImplStridedKernelFunctor(
      T* query_,
      T* key_,
      T* value_,
      T* bias_,
      uint8_t* dropout_mask_,
      float dropout_prob_,
      T* out_,
...
template <typename fmha_policy, typename T, bool kUseBias, bool kUseDropout>
void fmha_forward_impl_strided(
    sycl::queue& q,
    T* query,
    T* key,
    T* value,
  ...
  using fmha_forward_op_t =
      fmha_forward_strided_t<fmha_policy, T, kUseBias, kUseDropout>;

  sycl::nd_range<3> NdRange =
      fmha_forward_op_t::get_nd_range(num_batches * num_heads, num_queries);

  auto cgf = DPCPP_Q_CGF(cgh) {
    FmhaForwardImplStridedKernelFunctor<fmha_forward_op_t, T> kfn(
        query,
        key,
        value,
        bias,
...
#define CALL_IMPL_FUNC(P)                                 \
  fmha_forward_impl_strided<P, T, kUseBias, kUseDropout>( \
      q,                                                  \
      query,                                              \
      key,                                                \
      value,                                              \
