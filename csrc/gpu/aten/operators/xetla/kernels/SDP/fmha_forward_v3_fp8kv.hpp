...
class fmha_forward_v3_fp8kv_t {
 public:
  ...
  struct arguments_t {
    ...
    uint8_t* K_ptr; // [num_blocks, block_size, num_heads_kv, head_size]
    uint8_t* V_ptr; // [num_blocks, block_size, num_heads_kv, head_size]
    scalar_t* A_ptr =
 ...
 private:
  ...
  using compute_policy_BrBc = group::
      compute_policy_fp8_dequantize<comp_attr, knobs, fp8_t, false, arch_tag>;
  // TODO: add k slicing
  ...
  using mem_desc_Kj_T_t =
      mem_mask_desc_t<uint8_t, mem_layout::col_major, mem_space::global>;
  ...
  using gemm_Sij_t = group::gqa_fp8_t<
      compute_policy_BrBc,
      tile_shape_BrBc,
      mem_desc_Qi_L_t,
      mem_desc_Kj_T_t,
  ...
  inline void gemm_Sij(
    ...
    gemm_args_t gemm_args(
        ctx.mem_desc_Qi_L, ctx.mem_desc_Kj_T, loop_count, args.fp8_scale);
    gemm(ctx.g, matAccSij_s, gemm_args);

    // Multiply by softmax scaling factor
    // bmm * alpha
    for (auto& matAccSij : matAccSij_s) {
      matAccSij.reg *= (args.sm_scale * args.fp8_scale);
      if (args.softcap > 0.0) {
  ...
  using gemm_Oi_t = group::gqa_fp8_t<
      compute_policy_BrBm,
      tile_shape_BrHm,
      mem_desc_Pij_L_t,
      mem_desc_Vj_t,
  ...
  inline void gemm_Oi(
    ...
    gemm_Oi_t gemm;
    gemm_args_t gemm_args(
        ctx.mem_desc_Pij_L, ctx.mem_desc_Vj, loop_count, args.fp8_scale);
    gemm(ctx.g, matAccOi_s, gemm_args);

    for (auto& matAccOi : matAccOi_s) {
      matAccOi.reg *= args.fp8_scale;
    }
  }
