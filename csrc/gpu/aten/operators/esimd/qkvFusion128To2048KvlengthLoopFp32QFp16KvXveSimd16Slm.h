ESIMD_INLINE void qkvFusion128To2048KvlengthLoopFp32QFp16KvXveSimd16Slm_ipex(
  ...
  unsigned int offsetVBase = (h * 128 + vv * 16 + hh * 8 * 128 * 32) * sizeof(fp16);
  ...
  qq.template bit_cast_view<unsigned char>().template select<128, 1>(128) =
    ...
    __ESIMD_ENS::lsc_data_size::default_size,
    __ESIMD_ENS::cache_hint::cached,
    __ESIMD_ENS::cache_hint::cached>((unsigned char*)qState + offsetQ + 128);
...
#pragma unroll
    for (int k = 0; k < 8; k++)
    {
      vvState.template bit_cast_view<unsigned char>().template select<32, 1>(32*k) =
          __ESIMD_ENS::lsc_block_load<
          unsigned char,
          32,
          __ESIMD_ENS::lsc_data_size::default_size,
          __ESIMD_ENS::cache_hint::cached,
          __ESIMD_ENS::cache_hint::cached>((unsigned char*)vState + offsetV_temp);
...
#pragma unroll
    ...
    //slm_scalar_store(outputSoftmaxOffsetSlm + localLinearId * sizeof(float), softMax);
    slm_block_store<float, 1>(outputSoftmaxOffsetSlm + localLinearId * sizeof(float), softMax);
// #pragma unroll
...
#pragma unroll
  ...
  if (localLinearId < 8) {
    ...
    __ESIMD_ENS::lsc_block_store<
      ...
      __ESIMD_ENS::cache_hint::write_back>((fp16*)out + outputOffset, kvCacheOut_fp16);
  }
}
