...
template <
    ...
    typename vec_t>
struct LaunchWeightNormReduceKernelFunctor {
  ...
  LaunchWeightNormReduceKernelFunctor(
      ScalarTypeInfo iinfo_,
      AccTypeInfo oinfo_,
      BatchKernelConfig cfg_,
      bool need_squre_,
      bool is_final_,
      dpcpp_local_acc_t<accscalar_t> shared_)
      : iinfo(iinfo_),
        oinfo(oinfo_),
        cfg(cfg_),
        need_squre(need_squre_),
        is_final(is_final_),
        shared(shared_) {}
 ...
 private:
  ...
  bool need_squre;
  bool is_final;
  dpcpp_local_acc_t<accscalar_t> shared;
};
...
struct SegmentWeightNormKernelFunctor {
  void operator()(sycl::nd_item<2> item) const {
    ...
    v_off = IndexToOffset<scalar_t, int64_t>::get(
        w_lid, vinfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);

    w_off = IndexToOffset<scalar_t, int64_t>::get(
        w_lid, winfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);

    g_off = IndexToOffset<scalar_t, int64_t>::get(
        n_lid, ginfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);
  ...
  SegmentWeightNormKernelFunctor(
      ...
      AccTypeInfo ninfo_,
      int dim_after_collapse_,
      BatchKernelConfig cfg_)
      : vinfo(vinfo_),
        ginfo(ginfo_),
        winfo(winfo_),
        ninfo(ninfo_),
        dim_after_collapse(dim_after_collapse_),
        cfg(cfg_) {}
...
template <
    ...
    typename accscalar_t,
    typename vec_t>
struct WeightNormKernelFunctor {
  void operator()(sycl::nd_item<2> item) const {
    auto id = cfg.get_item_desc(item);
  ...
  WeightNormKernelFunctor(
      ...
      : vinfo(vinfo_),
        ...
        cfg(cfg_),
        wg_size(wg_size_),
        batch_wg_range(batch_wg_range_),
        shared(shared_) {}
...
template <
    ...
    typename accscalar_t,
    typename vec_t>
struct LaunchWeightNormBackwardReduceKernelFunctor {
  void operator()(sycl::nd_item<2> item) const {
    auto id = cfg.get_item_desc(item);
  ...
  LaunchWeightNormBackwardReduceKernelFunctor(
...
struct SegmentWeightNormBackwardKernelFunctor {
  void operator()(sycl::nd_item<2> item) const {
    ...
    v_off = IndexToOffset<scalar_t, int64_t>::get(
        gv_lid, vinfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);
    ...
    g_off = IndexToOffset<scalar_t, int64_t>::get(
        gg_lid, ginfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);
...
template <
    ...
    typename accscalar_t,
    typename vec_t>
struct WeightNormBackwardKernelFunctor {
  void operator()(sycl::nd_item<2> item) const {
    auto id = cfg.get_item_desc(item);
    ...
    gg_off = IndexToOffset<scalar_t, int64_t>::get(
        n_lid, gginfo, IndexToOffset<scalar_t, int64_t>::NON_STRICT_CONTIGUOUS);
