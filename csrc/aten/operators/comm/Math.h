...
static inline scalar_t zeta(scalar_t _x, scalar_t _q) {
  ...
  bool q_is_integer = q == DPCPP::floor(q);
...
static inline scalar_t calc_digamma(scalar_t in) {
  ...
  if (x < 0) {
    ...
    result = static_cast<accscalar_t>(- PI_f64 / DPCPP::tan(PI_f64 * static_cast<double>(x)));
    x = 1 - x;
  }
  ...
  return static_cast<scalar_t>(DPCPP::log(x) - (static_cast<accscalar_t>(0.5) / x) - y + result);
}
...
template <typename scalar_t>
static inline scalar_t calc_polygamma(int n, scalar_t x) {
  // already blocked if n <= 1
  return ((n % 2) ? 1.0 : -1.0) * DPCPP::exp(std::lgamma(static_cast<scalar_t>(n) + 1.0)) * zeta(static_cast<scalar_t>(n + 1), x);
}
...
static inline C10_HOST_DEVICE scalar_t calc_i0(scalar_t _x) {
  ...
  accscalar_t x = DPCPP::abs(static_cast<accscalar_t>(_x));
  ...
  if (x <= 8.0) {
    ...
    return static_cast<scalar_t>(DPCPP::exp(x) * chbevl(y, A, 30));
  }

  return static_cast<scalar_t>(DPCPP::exp(x) * chbevl(static_cast<accscalar_t>(32.0 / x - 2.0), B, 25) / DPCPP::sqrt(x));
}
