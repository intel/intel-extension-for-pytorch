...
std::tuple<Tensor, Tensor, Tensor> gru_forward_layer(
  ...
  auto weight_ih = at::cat({gates_i[1], gates_i[0], gates_i[2]}, /*gates*/0).t().contiguous();

  std::vector<at::Tensor> gates_h = weight2.chunk(3, /*gates*/0);
  auto weight_hh = at::cat({gates_h[1], gates_h[0], gates_h[2]}, /*gates*/0).t().contiguous();
  ...
  if (has_biases) {
    ...
    bias = at::cat({b1[1] + b2[1], b1[0] + b2[0], b1[2], b2[2]}, /*output_channels*/0);
  } else {
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor> gru_backward_layer(
  ...
  xpu::oneDNN::gru_backward(input, hx, weight_ih, weight_hh, bias, output, hy, workspace, grad_y, grad_hy, grad_input, grad_hx,
    grad_weight_ih, grad_weight_hh, grad_bias, reverse, hidden_size, has_biases, train, bidirectional);

  std::vector<at::Tensor> grad_w_1 = grad_weight_ih.permute({0, 1, 3, 4, 2}).reshape(weight1.sizes()).chunk(3, /*gates*/0);
  auto grad_w_ih = at::cat({grad_w_1[1], grad_w_1[0], grad_w_1[2]}, /*gates*/0);
  std::vector<at::Tensor> grad_w_2 = grad_weight_hh.permute({0, 1, 3, 4, 2}).reshape(weight2.sizes()).chunk(3, /*gates*/0);
  auto grad_w_hh = at::cat({grad_w_2[1], grad_w_2[0], grad_w_2[2]}, /*gates*/0);
  std::vector<at::Tensor> grad_b = grad_bias.reshape(bias.sizes()).chunk(4, /*output_channels*/0);
  auto grad_b1 = at::cat({grad_b[1], grad_b[0], grad_b[2]}, /*output_channels*/0);
  auto grad_b2 = at::cat({grad_b[1], grad_b[0], grad_b[3]}, /*output_channels*/0);
  auto grad_hx_ = grad_hx.reshape(hx.sizes());
  return std::make_tuple(std::move(grad_input), std::move(grad_w_ih), std::move(grad_w_hh), std::move(grad_b1), std::move(grad_b2), std::move(grad_hx));
}

class GRUFunction : public Function<GRUFunction> {
 public:
  static variable_list forward(
      AutogradContext *ctx, Variable input, Variable w1, Variable w2, Variable w3, Variable w4,
      Variable hx, bool reverse, int64_t hidden_size, int64_t num_layers, bool has_biases, bool train, bool bidirectional) {
    ...
    auto result_ = gru_forward_layer(input, w1, w2, w3, w4, hx, reverse, hidden_size, num_layers, has_biases, train, bidirectional);
  ...
  static variable_list backward(AutogradContext *ctx, variable_list grad_outputs) {
    ...
    auto result_ = gru_backward_layer(input, w1, w2, w3, w4, hx, output, hy, workspace, grad_output, grad_hy, reverse, hidden_size, num_layers, has_biases, train, bidirectional);
    variable_list result = {std::get<0>(result_), std::get<1>(result_), std::get<2>(result_), std::get<3>(result_), std::get<4>(result_), std::get<5>(result_), Tensor(), Tensor(), Tensor(), Tensor(), Tensor(), Tensor()};
    return result;
  }
};
...
std::vector<at::Tensor> rnn_layer(
  ...
  if (weights.size() == 4) {
    ...
    variable_list output = GRUFunction::apply(input_v, weight_0, weight_1, weight_2, weight_3, hx_v, reverse, hidden_size, num_layers, true, train, bidirectional);
    ...
    variable_list output = GRUFunction::apply(input_v, weight_0, weight_1, weight_2, weight_3, hx_v, reverse,  hidden_size, num_layers, false, train, bidirectional);
    return {output[0], output[1]};
  }
}

std::tuple<Tensor,Tensor> gru_xpu(const Tensor & input_,
    const Tensor & hx_,
    TensorList params,
    bool has_biases,
    int64_t num_layers,
...
//  TORCH_CHECK(!train || dropout == 0.0, "onednn_rnn doesn't support dropout");
  ...
  at::MatrixRef<at::Tensor> weights{params, static_cast<size_t>(has_biases ? 4 : 2)};
  ...
  for (int64_t layer = 0; layer < num_layers; layer++) {
    for (int64_t direction = 0; direction < num_directions; direction++) {
      ...
      auto outputs = rnn_layer(layer_input, layer_weights, layer_hx, reverse, hidden_size, num_layers, train, bidirectional);
