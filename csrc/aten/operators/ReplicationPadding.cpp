...
void replication_pad_forward_kernel2d(
    ...
    int o0, int o1, int o2, int o3, int i0, int i1, int i2, int i3) {
    ...
    auto cgf = DPCPP_Q_CGF(cgh) {
        auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<3> item) {
            int outputPointId = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
            int plane = item.get_group(1);
            int batch = item.get_group(2);
            if (outputPointId >= o2 * o3) return;
            ...
            int inputPointX = imin(imax(padL, outputPointX), i3 + padL - 1) - oStartX + iStartX;
            int inputPointY = imin(imax(padT, outputPointY), i2 + padT - 1) - oStartY + iStartY;
            
            size_t in_ = ((batch*i1 + plane)*i2 + inputPointY)*i3 + inputPointX;
            scalar_t valueToCopy = input[in_];
            size_t out_ = ((batch*o1 + plane)*o2 + outputPointY)*o3 + outputPointX;
            output[out_] = valueToCopy;
        };
        cgh.parallel_for<DPCPP_K(ReplicationPad2d, scalar_t)>(DPCPP::nd_range<3>(
            DPCPP::range<3>(CeilDiv(outputPlaneSize, workgroup_size)*workgroup_size, o1, o0), 
...
{
    TORCH_CHECK(xpu::dpcpp::detail::canUse32BitIndexMath(input), 
    ...
    TORCH_CHECK(input.numel() && (numInputDims == 3 || numInputDims == 4),
        "non-empty 3D or 4D (batch mode) tensor expected for input, but got: ", input)
    ...
    IPEX_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        input.scalar_type(), "replication_pad2d_out_template", [&] {
...
void replication_pad_backward_kernel(
    ...
    int go0, int go1, int go2, int go3, 
    int gi0, int gi1, int gi2, int gi3)
{
    ...
    auto cgf = DPCPP_Q_CGF(cgh) {
        auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<3> item) {
            ...
            int plane = item.get_group(1);
            int batch = item.get_group(2);
            if (outputPointId >= go2 * go3) return;
            ...
            int oStartX = imax(0, padL);
            int oStartY = imax(0, padT);
            int inputPointX = imin(imax(padL, outputPointX), gi3 + padL - 1) - oStartX + iStartX;
            int inputPointY = imin(imax(padT, outputPointY), gi2 + padT - 1) - oStartY + iStartY;
            size_t go_ = ((batch*go1 + plane)*go2 + outputPointY)*go3 + outputPointX;
            scalar_t valueToCopy = gradOutput[go_];
            size_t gi_ = ((batch*gi1 + plane)*gi2 + inputPointY)*gi3 + inputPointX;
            atomicAdd((dpcpp_global_ptr_pt<scalar_t>)&gradInput[gi_], valueToCopy);
            
        };
        cgh.parallel_for<DPCPP_K(ReplicationPad2dBackward, scalar_t)>(DPCPP::nd_range<3>(
            DPCPP::range<3>(CeilDiv(outputPlaneSize, workgroup_size)*workgroup_size, go1, go0), 
            DPCPP::range<3>(workgroup_size, 1, 1)
        ), kfn);
    };
    DPCPP_Q_ASYNC_SUBMIT(queue, cgf);
    ...
    IntArrayRef paddingSize)
{
    TORCH_CHECK(xpu::dpcpp::detail::canUse32BitIndexMath(input),
        "input tensor must fit into 32-bit index math");
    TORCH_CHECK(xpu::dpcpp::detail::canUse32BitIndexMath(gradOutput),
    ...
    IPEX_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        input.scalar_type(), "replication_pad2d_backward_out_template", [&] {
        ...
        replication_pad_backward_kernel<scalar_t>(
            ...
            padT, padB, padL, padR, 
            go0, go1, go2, go3, gi0, gi1, gi2, gi3);
    });
}
...
Tensor& replication_pad2d_out(
    ...
    IntArrayRef paddingSize)
{
  impl::replication_pad2d_out_template(
      output, input, paddingSize);
  return output;
}

Tensor replication_pad2d(
    const Tensor& input,
    IntArrayRef paddingSize)
{
