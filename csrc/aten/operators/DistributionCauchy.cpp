...
void cauchy_kernel(TensorIterator& iter, double median_, double sigma_, c10::optional<Generator> generator) {
  auto gen = get_generator_or_default<xpu::dpcpp::DPCPPGeneratorImpl>(
      generator, xpu::dpcpp::detail::getDefaultDPCPPGenerator());
  IPEX_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "cauchy_dpcpp", [&] {
    ...
    auto cauchy_func = [median, sigma] (accscalar_t rand) {
      return static_cast<scalar_t>(median + sigma * at::tan(static_cast<accscalar_t>(M_PI) * (rand - static_cast<accscalar_t>(0.5))));
    };
...
void cauchy_mkl_kernel(Tensor& self,
  ...
  std::initializer_list<std::uint64_t> seed = { rng_engine_inputs.first, 0, rng_engine_inputs.second };
