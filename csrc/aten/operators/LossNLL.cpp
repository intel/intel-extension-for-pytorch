...
void spatial_class_nll_criterion_update_output_kernel(
  ...
  auto cgf = DPCPP_Q_CGF(cgh) {
    ...
    auto kfn = DPCPP_Q_KFN(DPCPP::nd_item<1> item_id) {
      ...
      if (local_idx == 0) {
        atomicAdd((dpcpp_global_ptr_pt<scalar_t>)total_weight_ptr, ScalarConvert<accscalar_t, scalar_t>::to(partial_weight[0]));
        atomicAdd((dpcpp_global_ptr_pt<scalar_t>)out_ptr, ScalarConvert<accscalar_t, scalar_t>::to(partial_sums[0]));
      }
    };
...
Tensor& nll_loss2d_backward_out(
  ...
  if (weight.defined()) {
    ...
    TORCH_CHECK(IsOnSameDevice({self, target, grad_input, total_weight}),
                "Some of weight/gradient/input tensors are located on different GPUs. Please move them to a single one.");
  }
  ...
  if (reduction == at::Reduction::None) {
    ...
    IPEX_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::BFloat16, self.scalar_type(), "nll_loss2d_dpcpp_backward", [&](){
...
std::tuple<Tensor&,Tensor&> nll_loss2d_forward_out(
  ...
  if (weight.defined()) {
    TORCH_CHECK(IsOnSameDevice({self, target, weight, output, total_weight}),
      "Some of weight/gradient/input tensors are located on different GPUs. Please move them to a single one.");
  }
  else {
    TORCH_CHECK(IsOnSameDevice({self, target, output, total_weight}),
      "Some of weight/gradient/input tensors are located on different GPUs. Please move them to a single one.");
  }
  ...
  if (count != 0) {
    ...
    if (reduction == at::Reduction::None) {
      ...
      IPEX_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::BFloat16, self.scalar_type(), "nll_loss2d_dpcpp_forward", [&](){
