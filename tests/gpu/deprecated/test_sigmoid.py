...
class TestNNMethod(TestCase):
    def test_sigmoid(self, dtype=torch.float):
        ...
        print(cpu_res)
        cpu_res.backward(
            torch.tensor([[0.5, 1.0, 1.5], [2.0, 2.5, 3.0]], device=cpu_device)
        )
        ...
        dpcpp_res.backward(
            torch.tensor([[0.5, 1.0, 1.5], [2.0, 2.5, 3.0]], device=dpcpp_device)
        )
