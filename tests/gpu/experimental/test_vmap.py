...
class TestVmapOperators(Namespace.TestVmapBase):
    ...
    def test_binary_pointwise_ops(self):
        ...
        for op, getter in cases:
            ...
            # number = get_number(getter)
            # self._test_unary(lambda t: op(t, number), getter, device='xpu')
            # self._test_unary(lambda t: op(number, t), getter, device='xpu')
            # self._test_unary(lambda t: op(t, torch.tensor(number)), getter, device='xpu')
