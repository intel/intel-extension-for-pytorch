...
from torch.testing._internal.common_utils import TEST_WITH_ROCM, run_tests, slowTest
from torch.testing._internal.common_device_type import instantiate_device_type_tests, dtypes, dtypesIfCPU, onlyCPU
...
class TestSortAndSelect(TestCase):
    ...
    @onlyXPU
    @dtypes(torch.uint8)
    @largeTensorTestXPU('200GB')  # Unfortunately 80GB A100 is not large enough
    ...
    @onlyOnCPUAndXPU
    @dtypesIfXPU(*(get_all_dtypes(include_complex=False,
                                   include_bool=False,
                                   include_half=False,
