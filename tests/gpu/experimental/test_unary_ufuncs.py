...
from torch.testing._internal.common_device_type import (
    instantiate_device_type_tests, ops, dtypes, onlyCPU, precisionOverride, dtypesIfCPU, OpDTypes)
...
class TestUnaryUfuncs(TestCase):
    ...
    def test_complex_edge_values(self, device, dtype):
        ...
        # Skip on Windows, as XPU acos  returns conjugate value
    ...
    @dtypesIfXPU(torch.float, torch.double, torch.bfloat16)
    @dtypes(torch.float, torch.double)
    ...
    @dtypes(*(get_all_fp_dtypes(include_half=False) +
              get_all_complex_dtypes()))
    @dtypesIfXPU(*(get_all_fp_dtypes(include_half=True) +
                    get_all_complex_dtypes()))
