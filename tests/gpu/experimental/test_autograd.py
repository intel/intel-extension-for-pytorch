...
from torch.testing._internal.common_utils import (run_tests, suppress_warnings, slowTest,
                                                  IS_WINDOWS, IS_MACOS, TEST_WITH_ROCM, disable_gc,
...
from torch.testing._internal.common_device_type import (instantiate_device_type_tests,
                                                        onlyCPU, dtypes, deviceCountAtLeast,
                                                        skipMeta)
...
from common.pytorch_test_base import XPUMemoryLeakCheck
...
"""
    ...
    @unittest.skipIf(not TEST_XPU, "test requires CUDA")
...
class TestAutogradFunctional(TestCase):
    ...
    @unittest.skipIf(not TEST_XPU, "test requires CUDA")
    def test_construct_standard_basis_for_xpu(self):
        test_cases = [
            (torch.randn(2), torch.randn(3, device='xpu')),
            (torch.randn(3, device='xpu'), torch.randn(2)),
        ]
...
class TestAutogradDeviceType(TestCase):
    ...
    def test_reentrant_parent_error_on_cpu(self, device):
        before = XPUMemoryLeakCheck.get_xpu_memory_usage()
        ...
        after = XPUMemoryLeakCheck.get_xpu_memory_usage()
        start = time.time()
