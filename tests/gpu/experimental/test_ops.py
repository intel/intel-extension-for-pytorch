...
from torch.testing._internal.common_utils import \
    (is_iterable_of_tensors, run_tests, IS_SANDCASTLE, clone_input_helper,
...
from torch.testing._internal.common_device_type import \
    (deviceCountAtLeast, instantiate_device_type_tests, ops, OpDTypes)
...
class TestCommon(TestCase):
    ...
    # Validates that each OpInfo works correctly on different XPU devices
    @onlyXPU
    ...
    def test_out(self, device, dtype, op):
        ...
        # NOTE: only checks on the CPU and XPU device types since some
        #   device types don't have storage
        if self.device_type == 'cpu' or self.device_type == 'xpu':
            if isinstance(out, torch.Tensor):
