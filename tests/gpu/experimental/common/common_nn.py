...
from .pytorch_test_base import TestCase, TEST_XPU
...
def fractional_max_pool2d_no_batch_dim_test(test_case, use_random_samples):
    if use_random_samples:
        ...
        # can not check xpu because there RNG is different between cpu and xpu
        if test_case == "ratio":
            return dict(
                constructor=lambda: nn.FractionalMaxPool2d(2, output_ratio=0.5),
                cpp_constructor_args="""torch::nn::FractionalMaxPool2dOptions(2)
                                        .output_ratio(0.5)""",
                input_size=(3, 5, 7),
                reference_fn=single_batch_reference_fn,
            ...
            return dict(
                constructor=lambda: nn.FractionalMaxPool2d((2, 3), output_size=(4, 3)),
                cpp_constructor_args="""torch::nn::FractionalMaxPool2dOptions({2, 3})
                                        .output_size(std::vector<int64_t>({4, 3}))""",
                input_size=(3, 7, 6),
                reference_fn=single_batch_reference_fn,
...
def fractional_max_pool3d_no_batch_dim_test(test_case, use_random_samples):
    if use_random_samples:
        ...
        if test_case == "ratio":
            return dict(
                constructor=lambda: nn.FractionalMaxPool3d(2, output_ratio=0.5),
                cpp_constructor_args="""torch::nn::FractionalMaxPool3dOptions(2)
                                        .output_ratio(0.5)""",
                input_size=(4, 5, 5, 5),
                reference_fn=single_batch_reference_fn,
...
new_module_tests = [
    ...
    dict(
        ...
        input_fn=lambda: torch.rand(
            2, 3, 8, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
    ...
    dict(
        ...
        cpp_constructor_args="torch::nn::ReflectionPad2dOptions({1, 2, 3, 4})",
        input_fn=lambda: torch.rand(
            2, 3, 8, 8, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
    ...
    dict(
        ...
        cpp_constructor_args="torch::nn::ReflectionPad3dOptions({1, 2, 0, 2, 1, 2})",
        input_fn=lambda: torch.rand(
            2, 3, 8, 8, 8, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
    ...
    dict(
        ...
        cpp_constructor_args="torch::nn::ReplicationPad1dOptions({1, 2})",
        input_fn=lambda: torch.rand(
            2, 3, 4, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
    ...
    dict(
        ...
        cpp_constructor_args="torch::nn::ReplicationPad2dOptions({1, 2, 3, 4})",
        input_fn=lambda: torch.rand(
            2, 3, 4, 4, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
    ...
    dict(
        ...
        cpp_constructor_args="torch::nn::ConstantPad3dOptions({1, 2, 3, 4, 1, 0}, 2.)",
        input_fn=lambda: torch.rand(
            2, 3, 4, 4, 5, dtype=torch.complex128, requires_grad=True
        ),
        skip_half=True,
        desc="complex",
    ),
...
class ModuleTest(TestBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.should_test_xpu = kwargs.get("test_xpu", True)
        self.should_test_pickle = kwargs.get("pickle", True)
        self.check_gradgrad = kwargs.get("check_gradgrad", True)
        self.FIXME_no_xpu_gradgrad_comparison = kwargs.get(
            "FIXME_no_xpu_gradgrad_comparison", False
        )
    ...
    def test_xpu(self, test_case):
        if not TEST_CUDA or not self.should_test_xpu:
        ...
        gpu_module = self.constructor(*self.constructor_args).float().xpu()
        ...
        if self.check_gradgrad and not self.FIXME_no_xpu_gradgrad_comparison:
...
class NewModuleTest(InputVariableMixin, ModuleTest):  # type: ignore[misc]
    ...
    def _do_test(self, test_case, module, input):
        ...
        if all(isinstance(t, torch.LongTensor) for t in input_tuple) and TEST_CUDA:
            # check that xpu() moves module parameters to correct GPU device,
            # and that float() casts parameters correctly
            input_tuple = tuple(t.xpu() for t in input_tuple)
            module.float().xpu()
            module(*input_tuple)
            assert_module_parameters_are(torch.xpu.FloatTensor, 0)  # type: ignore[attr-defined]

            if torch.xpu.device_count() > 1:
                input_tuple = tuple(t.xpu(1) for t in input_tuple)
                module.xpu(1)
                with torch.xpu.device(1):
                    module(*input_tuple)
                assert_module_parameters_are(torch.xpu.FloatTensor, 1)  # type: ignore[attr-defined]
        else:
            ...
            if TEST_CUDA and self.should_test_xpu:
                # check that xpu() moves module parameters to correct GPU device,
                # and that float() casts parameters correctly

                # to GPU0
                input_tuple = tuple(to_single(t).xpu() for t in input_tuple)
                module.float().xpu()
                module(*input_tuple)
                assert_module_parameters_are(torch.xpu.FloatTensor, 0)  # type: ignore[attr-defined]
                ...
                if not self.skip_double:
                    # test double()
                    input_tuple = tuple(to_double(t).xpu() for t in input_tuple)
                    module.double().xpu()
                    module(*input_tuple)
                    assert_module_parameters_are(torch.xpu.DoubleTensor, 0)  # type: ignore[attr-defined]

                # test half()
                if not self.skip_half:
                    input_tuple = tuple(to_half(t).xpu() for t in input_tuple)
                    module.half().xpu()
                    module(*input_tuple)
                    assert_module_parameters_are(torch.xpu.HalfTensor, 0)  # type: ignore[attr-defined]
...
class CriterionTest(InputVariableMixin, TestBase):  # type: ignore[misc]
    ...
    def test_xpu(self, test_case, dtype, extra_args=None):
