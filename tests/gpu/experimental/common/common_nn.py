...
from torch.testing._internal.common_utils import to_gpu, freeze_rng_state, is_iterable, \
...
new_module_tests = [
    ...
    dict(
        ...
        input_size=(2, 128),  # trigger the last-dim algo in XPU
    ...
    dict(
        constructor=wrap_functional(F.softmax, dim=1),
        cpp_options_args='F::SoftmaxFuncOptions(1)',
        input_size=(2, 128, 2, 2),  # trigger special case of spatial XPU algo
...
class ModuleTest(TestBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.FIXME_no_xpu_gradgrad_comparison = \
            kwargs.get('FIXME_no_xpu_gradgrad_comparison', False)
    ...
    def test_xpu(self, test_case):
        if not TEST_XPU or not self.should_test_xpu:
...
class NewModuleTest(InputVariableMixin, ModuleTest):  # type: ignore[misc]
    ...
    def _do_test(self, test_case, module, input):
        ...
        if all(isinstance(t, torch.LongTensor) for t in input_tuple) and TEST_XPU:
            ...
            if TEST_XPU and self.should_test_xpu:
                # check that xpu() moves module parameters to correct GPU device,
                # and that float() casts parameters correctly
