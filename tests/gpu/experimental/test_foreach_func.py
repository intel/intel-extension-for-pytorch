...
from torch.testing._internal.common_utils import run_tests, TEST_WITH_ROCM, TEST_WITH_SLOW
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, dtypes, skipMeta, ops, precisionOverride)
from torch.testing._internal.common_dtype import (
    _dispatch_dtypes, floating_types, floating_types_and, complex_types, floating_and_complex_types,
    floating_and_complex_types_and, all_types_and_complex_and, all_types_and, all_types_and_complex, integral_types_and,
    all_types, double_types,)
from torch.testing._internal.common_methods_invocations import \
    (foreach_unary_op_db, foreach_binary_op_db, foreach_pointwise_op_db, foreach_minmax_op_db, ForeachFuncInfo, sample_inputs_foreach)
from torch.testing._internal.common_dtype import (
...
class TestForeach(TestCase):
    ...
    def _test_binary_op_tensorlists(self, device, dtype, opinfo, N, is_fastpath, disable_fastpath):
        ...
        # `xpuLaunchKernel` will be equal to `N`. For assert in `ForeachFuncWrapper` to pass,
        # we pass `is_fastpath and disable_fastpath` to `_binary_test`'s argument of is_fastpath.
        # as n_expected_xpuLaunchKernels is N if disable_fastpath.
        inputs = [
            opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath),
            [
    ...
    @skipMeta
    @unittest.skipIf(not torch.xpu.is_available(), "XPU not found")
    @dtypes(*get_all_dtypes())
    @ops(foreach_binary_op_db)
    def test_binary_op_list_slow_path(self, device, dtype, op):
        # note(mkozuki): why `n_expected_xpuLaunchKernels=0`?
        # In this test, foreach functions don't go through fast path,
        # but as there is only one tensor in each list of tensors,
        # `xpuLaunchKernel` is 1 so ForeachFuncWrapper internal assert fails.
    ...
    # but tensors of the same index are on the same device, e.g., ['xpu', 'cpu].
    @onlyXPU
    @ops(foreach_unary_op_db)
