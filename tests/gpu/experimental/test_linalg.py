...
from common.pytorch_test_base import tf32_on_and_off
...
class TestLinalg(TestCase):
    def setUp(self):
        ...
        # FixMe: xpu needs API for tf32 /w enabled
    ...
    @skipCPUIfNoLapack
    @onlyOnCPUAndXPU   # TODO: XLA doesn't raise exception
    @dtypes(torch.float32, torch.float64, torch.complex64, torch.complex128)
    ...
    @dtypesIfXPU(torch.cfloat, torch.cdouble,
                  *get_all_fp_dtypes(include_half=True, include_bfloat16=True))
    ...
    def test_lu(self, device, dtype):
        ...
        def run_test(device, pivot):
            def run_subtest(matrix_size, batches, device, pivot, singular=False, a=None):
                ...
                if self.device_type == 'xpu':
                    # lu without pivoting is implemented only for xpu device
...
'''.format(tol, eq_err, eq_err_general, iters1, eq_err_scipy, eq_err_general_scipy, iters2, m, k, niter))
    ...
    @dtypesIfXPU(*get_all_complex_dtypes(),
                  *get_all_fp_dtypes(include_bfloat16=True, include_half=True))
    ...
    @dtypesIfXPU(*get_all_fp_dtypes(include_bfloat16=True))
    @dtypes(torch.float, torch.double)
    ...
    @dtypesIfXPU(*get_all_complex_dtypes(),
                  *get_all_fp_dtypes(include_bfloat16=True))
    @dtypes(*get_all_complex_dtypes(), *get_all_fp_dtypes())
    ...
    @dtypes(torch.float, torch.double)
    @dtypesIfXPU(*([torch.float, torch.double] + get_all_complex_dtypes()))
