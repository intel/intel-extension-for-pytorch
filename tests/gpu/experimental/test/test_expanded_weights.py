...
class TestExpandedWeightHelperFunction(TestCase):
    ...
    def test_forward_helper_failure_args(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "do not support inputs that are also ExpandedWeights."
        ):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "requires a Tensor as the first input"
        ):
            (expanded_args, expanded_kwargs) = standard_kwargs(
                ("bias",), (3, weight, bias)
            )
            forward_helper(nn.functional.linear, expanded_args, expanded_kwargs)
        with self.assertRaisesRegex(
            RuntimeError, "requires a batch dimension but got an input of size 0"
        ):
            (expanded_args, expanded_kwargs) = standard_kwargs(
            ...
            forward_helper(nn.functional.linear, expanded_args, expanded_kwargs)
        with self.assertRaisesRegex(
            RuntimeError, "0 is not a valid batch size for Expanded Weights"
        ):
            (expanded_args, expanded_kwargs) = standard_kwargs(
    ...
    def test_set_grad_sample_if_exists_failure(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "does not support a mixture of ExpandedWeight parameters and normal Parameters",
        ):
...
class TestExpandedWeightFunctional(TestCase):
    ...
    def test_expanded_weight_error(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Expanded Weights encountered but cannot handle function"
        ):
            torch.add(
                sample_input,
    ...
    def _test_conv_model(
        self, model, input_size, num_dim, device, loss_reduction="sum"
    ):
        batch_size = 32
        input_ending = [input_size] * num_dim
        input = torch.randn([batch_size, 3] + input_ending, device=device)
        return self._test_model(
            partial(model, num_dim=num_dim), batch_size, input, device, loss_reduction
        )
    ...
    def test_group_norm_error(self, device):
        ...
        inp = torch.randn(N, C)
        with self.assertRaisesRegex(
            RuntimeError, "Expected number of channels in input to be divisible"
        ):
            F.group_norm(inp, 2)
...
class TestExpandedWeightModule(TestCase):
    def _do_test(self, module, input):
        ...
        with freeze_rng_state():
            actual_res = call_for_per_sample_grads(module, loss_reduction="sum")(
                input
            ).sum()
            actual_res.backward()
            actual_grads = []
    ...
    def test_per_sample_api_failing(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "Batch size passed must be None or an integer"
        ):
        ...
        module = nn.Linear(10, 10)
        with self.assertRaisesRegex(
            RuntimeError, "Expected loss_reduction argument to be sum or mean"
        ):
...
for test_param in supported_tests:
    ...
    if TEST_XPU and test.test_xpu:
        setattr(
            ...
            lambda self, test=test: test.test_context_manager(self, "xpu"),
        )
