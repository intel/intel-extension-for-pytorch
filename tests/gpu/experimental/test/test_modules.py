...
import torch
from torch.testing._internal.common_device_type import instantiate_device_type_tests, onlyCUDA, toleranceOverride, tol, skipMeta
from torch.testing._internal.common_modules import module_db, modules, TrainEvalMode
from torch.testing._internal.common_utils import TestCase, run_tests, freeze_rng_state, mock_wrapper, get_tensors_from, gradcheck, gradgradcheck, skipIfMps
...
class TestModule(TestCase):
    ...
    def _assert_module_parameters_and_buffer_are(self, module, device, dtype):
        ...
        def _check_module(items, name, device=device, dtype=dtype):
            for (item_name, item) in items:
                self.assertEqual(item.device, device, f'{name} {item_name} is on device {item.device} instead of the expected device {device}')
                if item.dtype.is_floating_point:
                    self.assertEqual(item.dtype, dtype, f'{name} {item_name} is of dtype {item.dtype} instead of the expected dtype {dtype}')
    ...
    def test_forward(self, device, dtype, module_info, training):
        ...
        module_inputs = module_info.module_inputs_func(module_info, device=device, dtype=dtype, requires_grad=False, training=training)
        dtype_to_method_caller = {torch.float32: methodcaller('float'), torch.float64: methodcaller('double')}
    ...
    def test_factory_kwargs(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            ...
            if not module_creates_params_or_buffers:
                continue
            kwargs.update({'device': device, 'dtype': dtype})
            if issubclass(module_info.module_cls, torch.nn.modules.lazy.LazyModuleMixin):
                ...
                with patch.object(torch.nn.UninitializedParameter, '__new__', uninit_param_new):
                    ...
                    with patch.object(torch.nn.UninitializedBuffer, '__new__', uninit_buffer_new):
                        m = module_cls(*args, **kwargs)
                        m.train(training)
                        uninit_param_new.mock.assert_has_calls([call(device=device, dtype=dtype) for _ in uninit_param_new.mock.mock_calls])
                        uninit_buffer_new.mock.assert_has_calls([call(device=device, dtype=dtype) for _ in uninit_buffer_new.mock.mock_calls])
            else:
                m = module_cls(*args, **kwargs)
                m.train(training)
                self._assert_module_parameters_and_buffer_are(m, device, dtype)
    ...
    def test_multiple_device_transfer(self, device, dtype, module_info, training):
        module_cls = module_info.module_cls
        module_inputs_device = module_info.module_inputs_func(module_info, device=device, dtype=dtype, requires_grad=False, training=training)
        module_inputs_cpu = module_info.module_inputs_func(module_info, device='cpu', dtype=dtype, requires_grad=False, training=training)
        for (module_input_device, module_input_cpu) in zip(module_inputs_device, module_inputs_cpu):
            ...
            with freeze_rng_state():
                ...
                if torch.xpu.device_count() >= 2:
                    ...
                    self._assert_module_parameters_and_buffer_are(m, torch.device('xpu:1'), dtype)
    ...
    @skipMeta
    @modules([module_info for module_info in module_db if 'inplace' in signature(module_info.module_cls).parameters])
        ...
        for module_input in module_inputs:
            ...
            with freeze_rng_state():
                ...
                if isinstance(default_output, torch.Tensor):
                    ...
                    grad_output = tuple((self._traverse_obj(o, lambda o: o.clone().detach_().normal_()) for o in default_output))
    ...
    def _test_gradients_helper(self, device, dtype, module_info, training, check):
        ...
        if torch.device(device).type == 'xpu' and torch.backends.cudnn.enabled:
    ...
    @onlyCUDA
    @toleranceOverride({torch.float32: tol(0.05, 0), torch.float64: tol(0.0004, 0)})
    @modules(module_db)
    def test_cpu_gpu_parity(self, device, dtype, module_info, training):
        rnn_modules = set([torch.nn.RNN, torch.nn.GRU, torch.nn.LSTM])
        if module_info.module_cls in rnn_modules and (not training) and ('xpu' in device) and torch.backends.cudnn.enabled:
            return
        module_cls = module_info.module_cls
        module_inputs_cpu = module_info.module_inputs_func(module_info, device='cpu', dtype=dtype, requires_grad=True, training=training)
    ...
    def test_memory_format(self, device, dtype, module_info, training):
        ...
        def _get_mem_formats(channels_last=False, channels_last_3d=False):
            if channels_last:
                return ([torch.contiguous_format, torch.channels_last], [torch.preserve_format, torch.contiguous_format, torch.channels_last])
            elif channels_last_3d:
                return ([torch.contiguous_format, torch.channels_last_3d], [torch.preserve_format, torch.contiguous_format, torch.channels_last_3d])
            else:
                return ([torch.contiguous_format], [torch.preserve_format, torch.contiguous_format])
        ...
        def _check_out_mem_format(output, input_mem_format, module_mem_format):
            ...
            def inner_check_out_mem_format(output):
                d = output.dim()
                if d == 4 and (input_mem_format == torch.channels_last or (module_mem_format == torch.channels_last and module_memformat_affects_out)):
        ...
        for module_input in module_inputs:
            ...
            with freeze_rng_state():
                ...
                for input_mem_format in input_mem_formats:
                    module_input.forward_input.args = _to_mem_format(input_mem_format, module_input.forward_input.args)
                    module_input.forward_input.kwargs = _to_mem_format(input_mem_format, module_input.forward_input.kwargs)
        ...
        for module_input in module_inputs:
            ...
            try:
                ...
                if "'training'" in str(e):
                    self.assertTrue(module_info.train_and_eval_differ, f'The ModuleInfo entry for {module_info.name} has train_and_eval_differ=False, but the training mode was found to affect the forward pass. Consider setting train_and_eval_differ=True for this ModuleInfo entry.')
                else:
                    raise e
