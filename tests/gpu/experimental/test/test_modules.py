                ...
                with patch.object(torch.nn.Module, "register_buffer", register_buffer):
                    ...
                    for mock in [parameter_new.mock, register_buffer.mock]:
                        for call_args, call_kwargs in mock.call_args_list:
                            ...
                            if len(call_tensors) > 0 and (
                                not constructor_tensors.intersection(call_tensors)
                            ):
    ...
    def test_non_contiguous_tensors(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            ...
            for (in_args, in_kwargs), g_out in product(inputs, grads):
                ...
                self.assertEqual(
                    input_args_grad, default_input_args_grad, atol=0.0001, rtol=0
                )
                self.assertEqual(
                    input_kwargs_grad, default_input_kwargs_grad, atol=0.0001, rtol=0
                )
            ...
            try:
                ...
                if "'training'" in str(e):
                    self.assertTrue(
                        ...
                        f"The ModuleInfo entry for {module_info.name} has train_and_eval_differ=False, but the training mode was found to affect the forward pass. Consider setting train_and_eval_differ=True for this ModuleInfo entry.",
                    )
                else:
                    raise e
