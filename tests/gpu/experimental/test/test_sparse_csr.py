...
_sparse_compressed_ops = list(
    filter(
        lambda op: op.supports_sparse_csr
...
class TestSparseCompressed(TestCase):
    ...
    def _generate_small_inputs_utils(self, layout, device=None, dtype=None):
        ...
        def values(
            lst, basedim=0, blocksize=(1, 1), densesize=(), device=device, dtype=dtype
        ):
            max_val = (
            ...
            def stretch_values(value, bdim, values_item_shape):
                ...
                if isinstance(value, list) and bdim >= 0:
                    return [
                        stretch_values(item, bdim - 1, values_item_shape)
                        for item in value
                    ]
                new_value = functools.reduce(
                    lambda x, dims: [copy.deepcopy(x) for _ in range(dims)],
                    reversed(values_item_shape),
                    None,
                )
                for p in itertools.product(*map(list, map(range, values_item_shape))):
                    row = functools.reduce(
                        lambda x, i: x.__getitem__(i), p[:-1], new_value
                    )
                    row[p[-1]] = list_add(
                        value, sum([i * 10**d for (d, i) in enumerate(p)])
                    )
                return new_value
            ...
            if not lst:
                return torch.tensor(lst, device=device, dtype=dtype).reshape(
                    0, *values_item_shape
                )
    ...
    def _generate_small_inputs(
        ...
        yield (
            torch.tensor([0, 2, 4], device=device, dtype=index_dtype),
            torch.tensor([0, 1, 0, 2], device=device, dtype=index_dtype),
            values([1, 2, 3, 4], 0, (2, 1)),
            shape((2, 3), 0, (2, 1)),
        )
        yield (
            torch.tensor([0], device=device, dtype=index_dtype),
        ...
        if enable_batched:
            yield (
                torch.tensor([[0, 2, 4], [0, 3, 4]], device=device, dtype=index_dtype),
                torch.tensor(
                    [[0, 1, 0, 1], [0, 1, 2, 0]], device=device, dtype=index_dtype
                ),
    ...
    def test_sparse_compressed_constructor(
        ...
        if (
            input_kind == "list"
            and shape_and_device_inference
            and (torch.device(device).type == "xpu")
        ):
        ...
        for index_dtype in [torch.int32, torch.int64]:
            for expected_device in expected_devices:
                for (
                    ...
                    values,
                    size,
                ) in self._generate_small_inputs(
    ...
    def test_empty(self, layout, device, dtype):
        ...
        for m, n, b in itertools.product(ns, ns, batch_shapes):
            ...
            self.assertEqual(
                compressed_indices_mth(result).shape, (*b, shape[compressed_dim] + 1)
            )
    ...
    def test_consistency(self, layout, device, dtype, op):
        ...
        if layout is torch.sparse_bsc:
            self.skipTest(
                f"test requires conversion from Strided layout to {layout} layout"
            )
    ...
    def test_empty_like(self, layout, layout2, device, dtype):
        for (
            ...
            values,
            size,
        ) in self._generate_small_inputs(layout):
            sparse = torch.sparse_compressed_tensor(
                compressed_indices,
                plain_indices,
                values,
    ...
    def _generate_invalid_input(self, layout, device):
        ...
        yield (
            "compressed/plain_indices mismatch of dimensionalites",
            tensor([[0, 2, 4]]),
            tensor([0, 1, 0, 2]),
            values([1, 2, 3, 4]),
            shape((2, 3)),
        ...
        if layout in {torch.sparse_csr, torch.sparse_csc}:
            yield (
                "indices and values mismatch of dimensionalites",
                tensor([[0, 2, 4]]),
                tensor([[0, 1, 0, 2]]),
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "values must have dimensionality > sum of batch and block dimensionalities \\(=1 \\+ 0\\) but got 1",
            )
        else:
            yield (
                "indices and values mismatch of dimensionalites",
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "values must have dimensionality > sum of batch and block dimensionalities \\(=1 \\+ 2\\) but got 3",
            )
        yield (
            "invalid size",
            tensor([0, 2, 4]),
            ...
            values([1, 2, 3, 4]),
            (2,),
            "tensor dimensionality must be sum of batch, base, and dense dimensionalites \\(=0 \\+ 2 \\+ 0\\) but got 1",
        )
        yield (
            "invalid batchsize",
            tensor([[0, 2, 4]]),
            ...
            values([[1, 2, 3, 4]]),
            shape((2, 2, 3), 1),
            "all batch dimensions of compressed_indices \\(=\\[1\\]\\), plain_indices \\(=\\[1\\]\\), and values \\(=\\[1\\]\\) must be equal to tensor batch dimensions \\(=\\[2\\]\\)",
        )
        if layout is torch.sparse_bsr:
            yield (
                "invalid blocksize",
                ...
                shape((2, 3)),
                "tensor shape\\[1\\] \\(=3\\) must be divisible with blocksize\\[1\\] \\(=2\\) as defined by values shape",
            )
        if layout is torch.sparse_bsc:
            yield (
                "invalid blocksize",
        ...
        yield (
            ...
            values([1, 2, 3, 4]),
            shape((2, 3)),
            "compressed_indices.shape\\[-1\\] must be equal to the number of compressed_indices_names \\+ 1 \\(=3\\), but got 4",
        )
        yield (
            "invalid compressed_indices shape",
            tensor([0, 2, 4]),
            ...
            values([1, 2, 3, 4]),
            shape((2, 3)),
            "plain_indices.shape\\[-1\\] must be equal to nnz \\(=4\\) as defined by values.shape\\[0\\], but got 5",
        )
        yield (
            ...
            values([1, 2, 3, 4]),
            shape((2, 3)),
            "compressed_indices and plain_indices must have the same dtype, bot got Int and Long, respectively",
        )
        yield (
            ...
            values([1, 2, 3, 4]),
            shape((2, 3)),
            "compressed_indices and plain_indices dtype must be Int or Long, but got Short",
        )
        if torch.device(device).type == "cpu":
            yield (
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "`compressed_indices\\[..., 0\\] == 0` is not satisfied.",
            )
            yield (
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "`compressed_indices\\[..., -1\\] == nnz` is not satisfied.",
            )
            yield (
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "0 <= compressed_indices\\[..., 1:\\] - compressed_indices\\[..., :\\-1\\] <= plain_dim` is not satisfied.",
            )
            yield (
                "invalid compressed_indices.diff(dim=-1)",
                tensor([0, 5, 4]),
            ...
            yield (
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "`0 <= plain_indices < plain_dim` is not satisfied.",
            )
            yield (
            ...
            yield (
                ...
                values([1, 2, 3, 4]),
                shape((2, 3)),
                "`plain_indices\\[..., compressed_indices\\[..., i - 1\\]:compressed_indices\\[..., i\\]\\] for all i = 1, ..., compressed_dim are sorted and distinct along the last dimension values` is not satisfied.",
            )
        if TEST_XPU and torch.device(device).type == "cpu":
            yield (
                ...
                values([1, 2, 3, 4], device="xpu"),
                shape((2, 3)),
                "device of compressed_indices \\(=cpu\\) must match device of values \\(=xpu:0\\)",
            )
            yield (
                "compressed_indices and values mismatch of device",
                torch.tensor([0, 2, 4], device="xpu"),
                torch.tensor([0, 1, 0, 1]),
                values([1, 2, 3, 4]),
                shape((2, 3)),
        ...
        if (
            ...
            yield (
                ...
                torch.tensor([0, 2, 4], device="xpu:0"),
                torch.tensor([0, 1, 0, 1], device="xpu:0"),
                values([1, 2, 3, 4], device="xpu:1"),
                shape((2, 3)),
                "device of compressed_indices \\(=xpu:0\\) must match device of values \\(=xpu:1\\)",
            )
            yield (
                "compressed_indices and values mismatch of device index",
                torch.tensor([0, 2, 4], device="xpu:0"),
                torch.tensor([0, 1, 0, 1], device="xpu:1"),
                values([1, 2, 3, 4], device="xpu:0"),
                shape((2, 3)),
                "Expected all tensors to be on the same device, but found at least two devices, xpu:0 and xpu:1!",
            )
...
class TestSparseCSR(TestCase):
    ...
    def test_select(self, device, dtype, index_dtype, layout):
        ...
        sparse_hybrid = self.genSparseCompressedTensor(
            shape + (4, 2),
            nnz,
            device=device,
            layout=layout,
        ...
        self.assertEqual(
            expected_sparse_hybrid_dense_selected, sparse_hybrid_dense_selected
        )
    ...
    def test_resize_as_sparse_compressed(self, device, dtype, layout):
        ...
        for index_dtype in [torch.int32, torch.int64]:
            ...
            _check_resize_b_as_a(b, a)
            if torch.device(device).type == "xpu" and layout not in (
                torch.sparse_bsc,
                torch.sparse_bsr,
            ):
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                '"resize_as_sparse_compressed_: src " expected sparse compressed tensor layout',
            ):
                b.resize_as_sparse_(a_strided)
            b_strided = b.to_dense()
            with self.assertRaisesRegex(
                RuntimeError,
                '"resize_as_sparse_compressed_: self " expected sparse compressed tensor layout',
            ):
                b_strided.resize_as_sparse_(a)
            with self.assertRaisesRegex(
                RuntimeError,
                "resize_as_sparse_compressed_tensor_: self and src must have the same layout",
            ):
    ...
    def test_csr_to_block_csr_errors(self, device, dtype):
        for index_dtype in [torch.int32, torch.int64]:
            ...
            with self.assertRaisesRegex(
                RuntimeError, "size \\(16, 16\\) with block size \\(5, 5\\)"
            ):
    ...
    @onlyCUDA
    @unittest.skipIf(
        not (CUDA11OrLater or TEST_WITH_ROCM), "Only CUDA 11+ is supported"
    )
    ...
    @precisionOverride(
        {
            torch.float32: 0.001,
            torch.complex64: 0.001,
            torch.float64: 1e-05,
            torch.complex128: 1e-05,
            torch.float16: 0.001,
    ...
    def test_block_addmm(self, device, dtype, index_dtype, block_size, noncontiguous):
        ...
        def ref_sp_numpy(c, a, b, alpha=None, beta=None, out=None):
            ...
            res = _npref_block_addmm_addmv(
                *map(lambda t: prep_input(t), (c, a, b)), alpha, beta
            )
            if out is not None:
                out.copy_(res)
                return out
        ...
        for m, n, k in itertools.product([2, 5], repeat=3):
            ...
            a = torch._sparse_bsr_tensor_unsafe(
                a.crow_indices(),
                a.col_indices(),
                a_data,
    ...
    def test_block_addmv(self, device, dtype, index_dtype, block_size, noncontiguous):
        for m, k in itertools.product([2, 5], repeat=2):
            ...
            self.run_test_block_addmm_addmv(
                torch.addmv, c, a, b, dtype=dtype, device=device
            )
    ...
    def test_sampled_addmm_autograd(self, device, dtype):
        ...
        for sample, dense_covector in zip(samples, [True, False]):
            ...
            (c1, a1, b1) = map(
                lambda x: x.detach().to_dense().requires_grad_(True), [c, a, b]
            )
            dense_output = (
    ...
    def test_sampled_addmm_errors(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "self.shape\\[-2\\] must match mat1.shape\\[-2\\]"
        ):
            torch.sparse.sampled_addmm(b_sparse, a, a)
        b = make_tensor((2, 3), dtype=dtype, device=device)
        b_sparse = b.to_sparse_csr()
        with self.assertRaisesRegex(
            RuntimeError, "self.shape\\[-1\\] must match mat2.shape\\[-1\\]"
        ):
            torch.sparse.sampled_addmm(b_sparse, a, a)
        a = make_tensor((2, 2), dtype=dtype, device=device)
        a_sparse = a.to_sparse_csr()
        with self.assertRaisesRegex(
            RuntimeError, "Expected mat1 to have strided layout"
        ):
            torch.sparse.sampled_addmm(a_sparse, a_sparse, a_sparse)
        with self.assertRaisesRegex(
            RuntimeError, "Expected mat2 to have strided layout"
        ):
    ...
    def test_transpose(self, device, dtype, layout):
        ...
        def _check_layout_invariants(transpose):
            ...
            if transpose.layout in (torch.sparse_bsr, torch.sparse_bsc):
                ...
                self.assertTrue(
                    transpose.values().is_contiguous()
                    or transpose.values()
                    .transpose(-2 - n_dense, -1 - n_dense)
                    .is_contiguous()
                )
                torch._validate_sparse_compressed_tensor_args(
                    compressed_indices,
                    plain_indices,
                    transpose.values().contiguous(),
                    transpose.shape,
                    transpose.layout,
                )
            else:
            ...
            if expect_error:
                ...
                if b.layout in block_layouts:
                    for block_layout in block_layouts:
                        with self.assertRaisesRegex(
                            RuntimeError, "blocksize does not match the blocksize"
                        ):
    ...
    def test_dense_to_from_sparse_compressed(self, device, hybrid, batched, layout):
        ...
        """
        supports_batched_from_sparse = (
            torch.sparse_bsr,
            torch.sparse_bsc,
            torch.sparse_csr,
            torch.sparse_csc,
        )
        supports_batched_to_sparse = (
            torch.sparse_bsr,
            torch.sparse_bsc,
            torch.sparse_csr,
            torch.sparse_csc,
        ...
        def _check_batched(
            ...
            for batch_index in np.ndindex(batch_shape):
                ...
                dense_matrix_pt = self._convert_to_layout(
                    dense_matrix, layout, blocksize
                )
        ...
        if hybrid:
            expect_to_layout_support = (
                expect_to_layout_support and layout in supports_hybrid_to_sparse
            )
            expect_from_layout_support = (
                expect_from_layout_support and layout in supports_hybrid_from_sparse
            )
            check_content = _check_hybrid_matrix
        if batched:
            expect_to_layout_support = (
                expect_to_layout_support and layout in supports_batched_to_sparse
            )
            expect_from_layout_support = (
                expect_from_layout_support and layout in supports_batched_from_sparse
            )
        ...
        if hybrid:
            ...
            blocksize = blocksizes[0]
            sparse_hybrid = self.genSparseCompressedTensor(
                batch_shape + sparse_shape + hybrid_shape,
                nnz=4,
                layout=layout,
                device=device,
                dtype=torch.float,
