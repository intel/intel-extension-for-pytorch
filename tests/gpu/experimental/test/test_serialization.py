...
from torch.testing._internal.common_utils import TestCase, IS_WINDOWS, TEST_DILL, run_tests, download_file, BytesIOContext, TemporaryFileName, parametrize, instantiate_parametrized_tests
...
class SerializationMixin(object):
    ...
    def test_serialization_zipfile_utils(self):
        data = {'a': b'12039810948234589', 'b': b'1239081209484958', 'c/d': b'94589480984058'}
    ...
    def _test_serialization(self, weights_only):
        ...
        with TemporaryFileName() as fname:
            ...
            self._test_serialization_assert(b, c)
        serialized = b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9\x03.\x80\x02}q\x01(U\x10protocol_versionq\x02M\xe9\x03U\ntype_sizesq\x03}q\x04(U\x03intq\x05K\x04U\x05shortq\x06K\x02U\x04longq\x07K\x04uU\rlittle_endianq\x08\x88u.\x80\x02]q\x01(U\x0e\xc5\xbc\xc4\x85\xc4\x85\xc3\xb3\xc5\xbc\xc4\x85\xc5\xbcq\x02ctorch._utils\n_rebuild_tensor_v2\nq\x03((U\x07storageq\x04ctorch\nFloatStorage\nq\x05U\x0845640624q\x06U\x03cpuq\x07\x8a\x01\x01NtQK\x00K\x01\x85K\x01\x85\x89NtRq\x08K\x02e.\x80\x02]q\x01U\x0845640624q\x02a.\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
    ...
    @unittest.skipIf(not TEST_DILL or HAS_DILL_AT_LEAST_0_3_1, '"dill" not found or is correct version')
    ...
    @unittest.skipIf(not TEST_DILL or not HAS_DILL_AT_LEAST_0_3_1, '"dill" not found or not correct version')
    ...
    def test_serialization_sparse_invalid(self):
        ...
        class TensorSerializationSpoofer(object):
            ...
            def __reduce_ex__(self, proto):
                ...
                return (torch._utils._rebuild_sparse_tensor, (self.tensor.layout, (invalid_indices, self.tensor._values(), self.tensor.size())))
        with tempfile.NamedTemporaryFile() as f:
    ...
    def test_serialization_sparse_csr_invalid(self):
        ...
        class TensorSerializationSpoofer(object):
            ...
            def __reduce_ex__(self, proto):
                ...
                return (torch._utils._rebuild_sparse_tensor, (self.tensor.layout, (invalid_crow_indices, self.tensor.col_indices(), self.tensor.values(), self.tensor.size())))
        with tempfile.NamedTemporaryFile() as f:
            torch.save({'spoofed': TensorSerializationSpoofer(x)}, f)
            f.seek(0)
            with self.assertRaisesRegex(RuntimeError, 'rebuilding sparse tensor for layout torch.sparse_csr'):
                y = torch.load(f)

    def test_serialize_device(self):
        device_str = ['cpu', 'cpu:0', 'xpu', 'xpu:0']
    ...
    def _test_serialization_backwards_compat(self, weights_only):
        ...
        class OldTensorBase(object):
            ...
            def __getstate__(self):
                return (self.new_tensor.storage(), self.new_tensor.storage_offset(), tuple(self.new_tensor.size()), self.new_tensor.stride())
    ...
    def test_serialization_map_location(self):
        ...
        cpu_map_locations = [map_location, {'xpu:0': 'cpu'}, 'cpu', torch.device('cpu')]
        gpu_0_map_locations = [{'xpu:0': 'xpu:0'}, 'xpu', 'xpu:0', torch.device('xpu'), torch.device('xpu', 0)]
        gpu_last_map_locations = ['xpu:{}'.format(torch.xpu.device_count() - 1)]
        ...
        if torch.xpu.is_available():
            check_map_locations(gpu_0_map_locations, torch.xpu.FloatTensor, torch.device('xpu', 0))
            check_map_locations(gpu_last_map_locations, torch.xpu.FloatTensor, torch.device('xpu', torch.xpu.device_count() - 1))

    @unittest.skipIf(torch.cuda.is_available(), 'Testing torch.load on CPU-only machine')
    def test_load_nonexistent_device(self):
        serialized = b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9\x03.\x80\x02}q\x00(X\x10\x00\x00\x00protocol_versionq\x01M\xe9\x03X\r\x00\x00\x00little_endianq\x02\x88X\n\x00\x00\x00type_sizesq\x03}q\x04(X\x05\x00\x00\x00shortq\x05K\x02X\x03\x00\x00\x00intq\x06K\x04X\x04\x00\x00\x00longq\x07K\x04uu.\x80\x02ctorch._utils\n_rebuild_tensor_v2\nq\x00((X\x07\x00\x00\x00storageq\x01ctorch\nFloatStorage\nq\x02X\x0e\x00\x00\x0094919395964320q\x03X\x06\x00\x00\x00xpu:0q\x04K\x02Ntq\x05QK\x00K\x02\x85q\x06K\x01\x85q\x07\x89Ntq\x08Rq\t.\x80\x02]q\x00X\x0e\x00\x00\x0094919395964320q\x01a.\x02\x00\x00\x00\x00\x00\x00\x00\xbb\x1f\x82\xbe\xea\x81\xd1>'
        buf = io.BytesIO(serialized)
        error_msg = 'Attempting to deserialize object on a XPU device'
    ...
    def test_serialization_filelike_missing_attrs(self):
        mocks = [('no readinto', lambda x: FilelikeMock(x)), ('has readinto', lambda x: FilelikeMock(x, has_readinto=True)), ('no fileno', lambda x: FilelikeMock(x, has_fileno=False))]
    ...
    def test_serialization_filelike_stress(self):
        ...
        self._test_serialization_filelike(a, lambda x: FilelikeMock(x, has_readinto=False), 'read() stress test')
        self._test_serialization_filelike(a, lambda x: FilelikeMock(x, has_readinto=True), 'readinto() stress test')
    ...
    def test_serialization_storage_slice(self):
        serialized = b'\x80\x02\x8a\nl\xfc\x9cF\xf9 j\xa8P\x19.\x80\x02M\xe9\x03.\x80\x02}q\x00(X\n\x00\x00\x00type_sizesq\x01}q\x02(X\x03\x00\x00\x00intq\x03K\x04X\x05\x00\x00\x00shortq\x04K\x02X\x04\x00\x00\x00longq\x05K\x04uX\x10\x00\x00\x00protocol_versionq\x06M\xe9\x03X\r\x00\x00\x00little_endianq\x07\x88u.\x80\x02(X\x07\x00\x00\x00storageq\x00ctorch\nFloatStorage\nq\x01X\x0e\x00\x00\x0094279043900432q\x02X\x03\x00\x00\x00cpuq\x03K\x02X\x0e\x00\x00\x0094279029750368q\x04K\x00K\x01\x87q\x05tq\x06Q(h\x00h\x01X\x0e\x00\x00\x0094279043900432q\x07h\x03K\x02X\x0e\x00\x00\x0094279029750432q\x08K\x01K\x01\x87q\ttq\nQ\x86q\x0b.\x80\x02]q\x00X\x0e\x00\x00\x0094279043900432q\x01a.\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
        buf = io.BytesIO(serialized)
    ...
    def test_load_error_msg(self):
        expected_err_msg = '.*You can only torch.load from a file that is seekable. ' + 'Please pre-load the data into a buffer like io.BytesIO and ' + 'try to load from it instead.'
    ...
    def test_save_different_dtype_unallocated(self):
        ...
        for (device, dtype) in product(devices, all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool)):
    ...
    def test_save_different_dtype_error(self):
        error_msg = 'Cannot save multiple tensors or storages that view the same data as different types'
        devices = ['cpu']
        if torch.xpu.is_available():
            devices.append('xpu')
