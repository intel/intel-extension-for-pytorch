...
def skip_helper_for_fft(device, dtype):
    ...
    if not SM53OrLater:
        raise unittest.SkipTest(
            "half and complex32 are only supported on XPU device with SM>53"
        )
...
class TestFFT(TestCase):
    ...
    def _test_fft_ifft_rfft_irfft(self, device, dtype):
        ...
        def _test_real(sizes, signal_ndim, prepro_fn=lambda x: x):
            ...
            for norm in (None, "ortho"):
                ...
                self.assertEqual(
                    x_complex, rec, atol=1e-08, rtol=0, msg="fft and ifft (from real)"
                )
    ...
    def test_cufft_plan_cache(self, devices, dtype):
        ...
        with plan_cache_max_size(
            devices[0], max(1, torch.backends.xpu.cufft_plan_cache.size - 10)
        ):
        ...
        if len(devices) > 1:
            ...
            with plan_cache_max_size(devices[0], 10):
                with plan_cache_max_size(devices[1], 11):
                    self.assertEqual(
                        torch.backends.xpu.cufft_plan_cache[0].max_size, 10
                    )
                    self.assertEqual(
                        torch.backends.xpu.cufft_plan_cache[1].max_size, 11
                    )
                    self.assertEqual(torch.backends.xpu.cufft_plan_cache.max_size, 10)
                    with torch.xpu.device(devices[1]):
                        self.assertEqual(
                            torch.backends.xpu.cufft_plan_cache.max_size, 11
                        )
                        with torch.xpu.device(devices[0]):
                            self.assertEqual(
                                torch.backends.xpu.cufft_plan_cache.max_size, 10
                            )
    ...
    def test_batch_istft(self, device):
        ...
        self.assertEqual(
            i_original.repeat(1, 1), i_single, atol=1e-06, rtol=0, exact_dtype=True
        )
        self.assertEqual(
            i_original.repeat(4, 1), i_multi, atol=1e-06, rtol=0, exact_dtype=True
        )
