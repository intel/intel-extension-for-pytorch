...
class TestUnaryUfuncs(TestCase):
        ...
        if low is not None:
            ...
            for epsilon in eps:
                ...
                self.assertEqual(
                    result.item(),
                    float("nan"),
                    msg="input of {0} outside lower domain boundary {1} produced {2}, not nan!".format(
        ...
        if high is not None:
            ...
            for epsilon in eps:
                ...
                self.assertEqual(
                    result.item(),
                    float("nan"),
                    msg="input of {0} outside upper domain boundary {1} produced {2}, not nan!".format(
    ...
    def _test_reference_numerics(self, dtype, op, tensors, equal_nan=True):
        ...
        for t in tensors:
            ...
            if t.numel() < 10:
                msg = "Failed to produce expected results! Input tensor was {0}, torch result is {1}, and reference result is {2}.".format(
                    t, actual, expected
                )
            else:
                msg = None
    ...
    def test_frexp_assert_raises(self, device):
        ...
        for dtype in invalid_input_dtypes:
            ...
            with self.assertRaisesRegex(
                RuntimeError, "torch\\.frexp\\(\\) only supports floating-point dtypes"
            ):
                torch.frexp(input)
        for dtype in floating_types_and(torch.half):
            ...
            for mantissa_dtype in dtypes:
                ...
                with self.assertRaisesRegex(
                    RuntimeError,
                    "torch\\.frexp\\(\\) expects mantissa to have dtype .+ but got .+",
                ):
            ...
            for exponent_dtype in dtypes:
                ...
                with self.assertRaisesRegex(
                    RuntimeError,
                    "torch\\.frexp\\(\\) expects exponent to have int dtype but got .+",
                ):
                    torch.frexp(input, out=(mantissa, exponent))

    def test_polygamma_neg(self, device):
        with self.assertRaisesRegex(
            RuntimeError, "polygamma\\(n, x\\) does not support negative n\\."
        ):
    ...
    @precisionOverride(
        {torch.bfloat16: 0.01, torch.float: 0.0002, torch.double: 0.0002}
    )
    @dtypes(torch.float, torch.double, torch.bfloat16)
