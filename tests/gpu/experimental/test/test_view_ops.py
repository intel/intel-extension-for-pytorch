...
class TestViewOps(TestCase):
            ...
            for view_dtype, np_view_dtype in dtypes.items():
                ...
                if not equal_element_size and a.dim() == 0:
                    with self.assertRaisesRegex(
                        RuntimeError, "self.dim\\(\\) cannot be 0"
                    ):
                        a.view(view_dtype)
                    continue
                if not equal_element_size and a.stride(-1) != 1:
                    with self.assertRaisesRegex(
                        RuntimeError, "self.stride\\(-1\\) must be 1"
                    ):
                        a.view(view_dtype)
                    continue
    ...
    def test_view_dtype_upsize_errors(self, device, dtype):
        ...
        for view_dtype in all_types_and_complex_and(
            ...
            with self.assertRaisesRegex(
                RuntimeError, f"self.size\\(-1\\) must be divisible by {size_ratio}"
            ):
                a.view(view_dtype)
            with self.assertRaisesRegex(
                RuntimeError,
                f"self.storage_offset\\(\\) must be divisible by {size_ratio}",
            ):
                a[:, :, 1:].view(view_dtype)
            a = make_tensor(
            ...
            with self.assertRaisesRegex(
                RuntimeError, f"self.stride\\(1\\) must be divisible by {size_ratio}"
            ):
                a.view(view_dtype)
...
class TestOldViewOps(TestCase):
    ...
    def test_view(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "because the unspecified dimension size -1 can be any value"
        ):
            empty.view(-1, 0)
        with self.assertRaisesRegex(
            RuntimeError, "because the unspecified dimension size -1 can be any value"
    ...
    def test_tensor_split_errors(self, device):
        S = 10
        test_cases = [
            [(S,), 10, 1, IndexError, "Dimension out of range", IndexError],
            [
                (),
                10,
                0,
                RuntimeError,
                "tensor_split expected at least a 1-dimensional tensor, "
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "tensor_split expected tensor_indices_or_sections to have dtype of long, but got Float",
        ):
