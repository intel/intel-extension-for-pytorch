...
from torch.testing._internal.common_utils import TestCase, run_tests, suppress_warnings, gradcheck, gradgradcheck, numpy_to_torch_dtype_dict, skipIfTorchDynamo
from torch.testing._internal.common_device_type import instantiate_device_type_tests, onlyCPU, dtypes, onlyNativeDeviceTypes, skipMeta
from torch.testing._internal.common_dtype import all_types_and_complex_and, complex_types, all_types_and, floating_and_complex_types_and
...
class TestViewOps(TestCase):
        ...
        for a in generate_inputs():
            ...
            for (view_dtype, np_view_dtype) in dtypes.items():
                ...
                if not equal_element_size and a.dim() == 0:
                    with self.assertRaisesRegex(RuntimeError, 'self.dim\\(\\) cannot be 0'):
                        a.view(view_dtype)
                    continue
                if not equal_element_size and a.stride(-1) != 1:
                    with self.assertRaisesRegex(RuntimeError, 'self.stride\\(-1\\) must be 1'):
                        a.view(view_dtype)
                    continue
    ...
    def test_view_dtype_upsize_errors(self, device, dtype):
        ...
        for view_dtype in all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool):
            ...
            with self.assertRaisesRegex(RuntimeError, f'self.size\\(-1\\) must be divisible by {size_ratio}'):
                a.view(view_dtype)
            with self.assertRaisesRegex(RuntimeError, f'self.storage_offset\\(\\) must be divisible by {size_ratio}'):
                a[:, :, 1:].view(view_dtype)
            a = make_tensor((4, 4, size_ratio), dtype=dtype, device=device, low=-5, high=5)
            a = a.as_strided((4, 4, size_ratio), (size_ratio, 1, 1))
            with self.assertRaisesRegex(RuntimeError, f'self.stride\\(1\\) must be divisible by {size_ratio}'):
                a.view(view_dtype)
    ...
    def test_view_as_complex(self, device):
        ...
        def fn(contiguous_input=True, dim0=0, dim1=1):
            ...
            if input.size()[-1] != 2:
                self.assertRaisesRegex(RuntimeError, 'Tensor must have a last dimension of size 2', lambda : torch.view_as_complex(input))
                return
            if input.stride()[-1] != 1:
                self.assertRaisesRegex(RuntimeError, 'Tensor must have a last dimension with stride 1', lambda : torch.view_as_complex(input))
                return
        ...
        self.assertRaisesRegex(RuntimeError, 'Tensor must have a stride divisible by 2 for all but last dimension', lambda : torch.view_as_complex(t))
        x = torch.tensor([], device=device)
        self.assertRaisesRegex(RuntimeError, 'Tensor must have a last dimension of size 2', lambda : torch.view_as_complex(x))
        z = torch.tensor(2.0)
        self.assertRaisesRegex(RuntimeError, 'Input tensor must have one or more dimensions', lambda : torch.view_as_complex(z))
        y = x.reshape(0, 2)
    ...
    def test_unbind(self):
        ...
        for i in range(3):
            ...
            g_expected = torch.stack([gi if j == i else torch.zeros_like(gi) for j in range(3)], dim=0)
...
class TestOldViewOps(TestCase):
    ...
    def test_ravel(self, device):
        ...
        tensors = [torch.randn(5, 5, 5, 5, device=device), torch._empty_affine_quantized([5, 5, 5, 5], scale=2, zero_point=3, dtype=torch.quint8, device=device)]
        _test_ravel(tensors, 625)
        tensors = [torch.randn(0, 2, 3, device=device), torch.randn(3, 0, 2, device=device), torch._empty_affine_quantized([0, 2, 3], scale=2, zero_point=3, dtype=torch.quint8, device=device), torch._empty_affine_quantized([3, 0, 2], scale=2, zero_point=3, dtype=torch.quint8, device=device)]
        _test_ravel(tensors, 0)
        tensors = [torch.randn(5, 5, device=device), torch._empty_affine_quantized([5, 5], scale=2, zero_point=3, dtype=torch.quint8, device=device)]
                ...
                for random_negative in [True, False]:
                    for (src_dim, dst_dim) in permutations(range(nd), r=2):
                        ...
                        if random_negative and random_prob > 0.66:
                            ...
                            src_dim = src_dim - nd
                            dst_dim = dst_dim - nd
                        partial_map = {torch.swapdims: partial(torch.swapdims, dim0=src_dim, dim1=dst_dim), torch.swapaxes: partial(torch.swapaxes, axis0=src_dim, axis1=dst_dim), torch.transpose: partial(torch.transpose, dim0=src_dim, dim1=dst_dim)}
                        torch_fn = partial_map[fn]
                        np_fn = partial(np.swapaxes, axis1=src_dim, axis2=dst_dim)
                        self.compare_with_numpy(torch_fn, np_fn, x, device=None, dtype=None)
            x = torch.randn(2, 3, 5, 7, 11)
            partial_map = {torch.swapdims: partial(torch.swapdims, dim0=0, dim1=0), torch.swapaxes: partial(torch.swapaxes, axis0=0, axis1=0), torch.transpose: partial(torch.transpose, dim0=0, dim1=0)}
            torch_fn = partial_map[fn]
    ...
    def test_view(self, device):
        ...
        tensor = torch.empty(1, 1, device=device).expand(3, 4)
        contig_tensor = tensor.clone()
    ...
    def test_tensor_split_sections(self, device, dtype):
        input_sizes = [(0,), (10,), (10, 0), (0, 10), (4, 10), (12, 3)]
    ...
    def test_tensor_split_indices(self, device, dtype):
        input_sizes = [(0,), (10,), (10, 0), (0, 10), (4, 10), (12, 3)]
        indices_args = [(), (0,), (3,), (10,), (-1,), (-10,), (2, -1), (3, 4, 10), (0, -1, 0, 10), (1, 5, 2, 8)]
        for input_size in input_sizes:
            a_base = make_tensor(input_size, dtype=dtype, device=device, low=-9, high=9)
            for a in [a_base, a_base.t()] if a_base.dim() > 2 else [a_base]:
                a_n = a.cpu().numpy()
    ...
    def test_tensor_split_errors(self, device):
        S = 10
        test_cases = [[(S,), 10, 1, IndexError, 'Dimension out of range', IndexError], [(), 10, 0, RuntimeError, 'tensor_split expected at least a 1-dimensional tensor, ' + 'but got a tensor with 0 dims', IndexError], [(S,), (10,), 1, IndexError, 'Dimension out of range', IndexError], [(), (10,), 0, RuntimeError, 'tensor_split expected at least a 1-dimensional tensor, ' + 'but got a tensor with 0 dims', IndexError], [(S,), 0, 0, RuntimeError, 'number of sections must be larger than 0, got 0', ValueError], [(S,), -1, 0, RuntimeError, 'number of sections must be larger than 0, got -1', ValueError]]
        ...
        with self.assertRaisesRegex(RuntimeError, 'tensor_split expected tensor_indices_or_sections to have dtype of long, but got Float'):
            torch.tensor_split(a, torch.tensor(1.1), dim)
        with self.assertRaisesRegex(RuntimeError, 'tensor_split expected tensor_indices_or_sections to be a' + ' zero-dimensional or one-dimensional tensor, but got a tensor with 2 dims'):
