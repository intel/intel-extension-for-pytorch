...
from torch.testing._internal.common_device_type import instantiate_device_type_tests, dtypes, onlyCUDA, skipMeta, ops
from torch.testing._internal.common_methods_invocations import foreach_unary_op_db, foreach_binary_op_db, foreach_pointwise_op_db, foreach_minmax_op_db, foreach_reduce_op_db
from torch.testing._internal.common_dtype import all_types_and_complex_and, all_types_and, integral_types, complex_types, floating_types_and, floating_types, integral_types_and
N_values = [20, 23] if not TEST_WITH_SLOW else [23, 30, 300]
Scalars = (random.randint(1, 10), 1.0 - random.random(), True, complex(1.0 - random.random(), 1.0 - random.random()))
from common.pytorch_test_base import TestCase, dtypesIfXPU, TEST_XPU, TEST_MULTIGPU, largeTensorTest

def getScalarLists(N):
    return (('int', [random.randint(0, 9) + 1 for _ in range(N)]), ('float', [1.0 - random.random() for _ in range(N)]), ('complex', [complex(1.0 - random.random(), 1.0 - random.random()) for _ in range(N)]), ('bool', [True for _ in range(N)]), ('mixed', [1, 2.0, 3.0 + 4.5j] + [3.0 for _ in range(N - 3)]), ('mixed', [True, 1, 2.0, 3.0 + 4.5j] + [3.0 for _ in range(N - 4)]))
...
class ForeachFuncWrapper:
    ...
    def __call__(self, inputs, is_xpu, is_fastpath, **kwargs):
        actual = None
        if is_xpu and torch.autograd.kineto_available() and (torch.profiler.ProfilerActivity.XPU in torch.profiler.supported_activities()):
            with torch.profiler.profile(activities=(torch.profiler.ProfilerActivity.CPU,)) as p:
                actual = self.func(*inputs, **kwargs)
            for e in p.key_averages():
                if e.key == 'xpuLaunchKernel':
                    if is_fastpath:
                        assert e.count == self.n_expected_xpuLaunchKernels
                    else:
                        assert e.count > self.n_expected_xpuLaunchKernels
        else:
            actual = self.func(*inputs, **kwargs)
...
class TestForeach(TestCase):
    ...
    def _get_funcs(self, op, n_expected_xpuLaunchKernels: int):
        return (ForeachFuncWrapper(op.method_variant, n_expected_xpuLaunchKernels), RegularFuncWrapper(op.ref), ForeachFuncWrapper(op.inplace_variant, n_expected_xpuLaunchKernels), RegularFuncWrapper(op.ref_inplace))
    ...
    def _binary_test(self, dtype, op, ref, inputs, is_fastpath, is_inplace, *, alpha=None):
        ...
        if alpha is not None:
            ...
            ref_inputs = inputs
            try:
                actual = op(inputs, self.is_xpu, is_fastpath, **kwargs)
            except RuntimeError as e:
                with self.assertRaisesRegex(type(e), re.escape(str(e))):
    ...
    def _test_binary_op_tensorlists(self, device, dtype, opinfo, N, is_fastpath, disable_fastpath):
        n_expected_xpuLaunchKernels = N if disable_fastpath else 1
        (op, ref, inplace_op, inplace_ref) = self._get_funcs(opinfo, n_expected_xpuLaunchKernels)
        inputs = [opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath), opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath)]
        ...
        inputs = [opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath), [make_tensor((N - i, 1), device=device, dtype=dtype, noncontiguous=not is_fastpath) for i in range(N)]]
        self._binary_test(dtype, op, ref, inputs, is_fastpath and disable_fastpath, is_inplace=False)
        self._binary_test(dtype, inplace_op, inplace_ref, inputs, is_fastpath and disable_fastpath, is_inplace=True)
    ...
    def _pointwise_test(self, dtype, op, ref, inputs, is_fastpath, is_inplace, *, values=None):
        ...
        if values is not None:
            try:
                actual = op(inputs + [values], self.is_xpu, is_fastpath)
            except RuntimeError as e:
                with self.assertRaisesRegex(type(e), re.escape(str(e))):
    ...
    def _test_pointwise_op(self, device, dtype, opinfo, N, is_fastpath, disable_fastpath, *, values=None):
        n_expected_xpuLaunchKernels = N if disable_fastpath else 1
        (op, ref, inplace_op, inplace_ref) = self._get_funcs(opinfo, n_expected_xpuLaunchKernels)
        inputs = [opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath), opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath), opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath)]
        ...
        inputs = [opinfo.sample_inputs(device, dtype, N, noncontiguous=not is_fastpath, same_size=True), [make_tensor((N - i, 1), device=device, dtype=dtype, noncontiguous=not is_fastpath) for i in range(N)], [make_tensor((1, N - i), device=device, dtype=dtype, noncontiguous=not is_fastpath) for i in range(N)]]
        self._pointwise_test(dtype, op, ref, inputs, is_fastpath and disable_fastpath, is_inplace=False, values=values)
        self._pointwise_test(dtype, inplace_op, inplace_ref, inputs, is_fastpath and disable_fastpath, is_inplace=True, values=values)
    ...
    def test_pointwise_op_fastpath(self, device, dtype, op):
        ...
        for N in N_values:
            ...
            for (_, scalarlist) in getScalarLists(N):
                self._test_pointwise_op(device, dtype, op, N, True, disable_fastpath, values=scalarlist)
    ...
    def test_pointwise_op_slowpath(self, device, dtype, op):
        for N in N_values:
            ...
            for (_, scalarlist) in getScalarLists(N):
                self._test_pointwise_op(device, dtype, op, N, False, False, values=scalarlist)
    ...
    def _minmax_test(self, opinfo, inputs, is_fastpath, n_expected_xpuLaunchKernels):
        (op, ref, inplace_op, _) = self._get_funcs(opinfo, n_expected_xpuLaunchKernels)
        expected = ref(inputs)
        self.assertEqual(expected, op(inputs, self.is_xpu, is_fastpath))
        inplace_inputs = [[t.clone() for t in inputs[0]], inputs[1]]
        inplace_op(inplace_inputs, self.is_xpu, is_fastpath)
    ...
    @ops(foreach_minmax_op_db, dtypes=all_types_and(torch.half, torch.bfloat16, torch.bool))
    ...
    def _reduce_test(self, opinfo, inputs, ord, is_fastpath, n_expected_xpuLaunchKernels):
        (op, ref, _, _) = self._get_funcs(opinfo, n_expected_xpuLaunchKernels)
        self.assertEqual(ref(inputs, ord=ord), op(inputs, self.is_xpu, is_fastpath, ord=ord))
    ...
    def test_reduce_fastpath(self, device, dtype, op):
        for (N, ord) in itertools.product(N_values, (0, 1, 2, -1, -2)):
            if ord in (1, 2) and dtype in floating_types_and(torch.half, torch.bfloat16):
                n_expected_xpuLaunchKernels = 3
            else:
                n_expected_xpuLaunchKernels = N
            inputs = (op.sample_inputs(device, dtype, N, noncontiguous=False),)
            self._reduce_test(op, inputs, ord, True, n_expected_xpuLaunchKernels)
    ...
    def test_binary_op_list_error_cases(self, device, dtype, op):
        ...
        if self.device_type == 'xpu' and torch.xpu.device_count() > 1:
            tensor1 = torch.zeros(10, 10, device='xpu:0', dtype=dtype)
            tensor2 = torch.ones(10, 10, device='xpu:1', dtype=dtype)
    ...
    def test_binary_op_list_slow_path(self, device, dtype, op):
        (foreach_op, native_op, foreach_op_, native_op_) = self._get_funcs(op, n_expected_xpuLaunchKernels=0)
    ...
    def test_binary_op_tensors_on_different_devices(self, device, dtype, op):
        _xpu_tensors = op.sample_inputs(device, dtype, 2, same_size=True)
        _cpu_tensors = op.sample_inputs('cpu', dtype, 2, same_size=True)
        (tensors1, tensors2) = list((tensors for tensors in zip(_xpu_tensors, _cpu_tensors)))
    ...
    def test_pointwise_op_tensors_on_different_devices(self, device, dtype, op):
        _xpu_tensors = op.sample_inputs(device, dtype, 3, same_size=True)
        _cpu_tensors = op.sample_inputs('cpu', dtype, 3, same_size=True)
        (tensors1, tensors2, tensors3) = list((tensors for tensors in zip(_xpu_tensors, _cpu_tensors)))
        (foreach_op, foreach_op_, native_op) = (op.method_variant, op.inplace_variant, op.ref)
        actual = foreach_op(tensors1, tensors2, tensors3)
        expected = [native_op(*_xpu_tensors), native_op(*_cpu_tensors)]
        self.assertEqual(expected, actual)
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ...
        actual = fn(inputs, is_xpu=True, is_fastpath=True, ord=ord)
