...
class TestForeach(TestCase):
        ...
        if alpha is not None:
            ...
            try:
                ...
                if dtype in (torch.float16, torch.bfloat16) and TEST_WITH_ROCM:
                    self.assertEqual(
                        expected, actual, atol=0.001, rtol=default_tolerances(dtype)[0]
                    )
                else:
    ...
    def _test_binary_op_tensorlists(
        ...
        (op, ref, inplace_op, inplace_ref) = self._get_funcs(
            opinfo, n_expected_xpuLaunchKernels
        )
        inputs = [
    ...
    def _reduce_test(
        self, opinfo, inputs, ord, is_fastpath, n_expected_xpuLaunchKernels
    ):
        (op, ref, _, _) = self._get_funcs(opinfo, n_expected_xpuLaunchKernels)
        self.assertEqual(
            ref(inputs, ord=ord), op(inputs, self.is_xpu, is_fastpath, ord=ord)
        )
    ...
    def test_binary_op_list_slow_path(self, device, dtype, op):
        (foreach_op, native_op, foreach_op_, native_op_) = self._get_funcs(
            op, n_expected_xpuLaunchKernels=0
        )
    ...
    def test_binary_op_tensors_on_different_devices(self, device, dtype, op):
        ...
        (tensors1, tensors2) = list(
            (tensors for tensors in zip(_xpu_tensors, _cpu_tensors))
        )
    ...
    def test_foreach_l2_large_value_input(self, device, dtype, op):
        ...
        inputs = (
            [
                t * scaler
                for t in op.sample_inputs(device, dtype, N, noncontiguous=False, low=1)
            ],
        )
