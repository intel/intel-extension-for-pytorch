...
class TestBinaryUfuncs(TestCase):
    ...
    def _test_reference_numerics(self, dtype, op, gen, equal_nan=True):
        ...
        for sample in gen:
            ...
            if _numel(l) <= 100 and _numel(r) <= 100:
                msg = "Failed to produce expected results! Input lhs tensor was {0}, rhs tensor was {1}, torch result is {2}, and reference result is {3}.".format(
                    l, r, actual, expected
                )
            else:
                msg = None
    ...
    def test_div_rounding_modes(self, device, dtype):
        ...
        rounding_unsupported = (
            dtype == torch.half
            and device != "xpu"
            or (dtype == torch.bfloat16 and device != "cpu")
        )
            ...
            for fn, inplace_fn, ref_fn in fns_list:
                ...
                if torch.is_tensor(x):
                    ...
                    try:
                        ...
                        self.assertRegex(
                            str(e),
                            "result type (Half|Float|Double) can't be cast to the desired output type (Byte|Char|Short|Int|Long)",
                        )
    ...
    def test_add(self, device):
        ...
        expected = torch.tensor(
            [2.4 + 3.5j, 4.2 + 5.3j], dtype=torch.complex64, device=device
        )
        self.assertEqual(res, expected)
        res = torch.add(m1, m2, alpha=complex(0.1, 0.2))
        expected = torch.tensor(
            [1.4 + 4.3j, 3.6 + 5.7j], dtype=torch.complex64, device=device
        )
        self.assertEqual(res, expected)
        res = torch.add(m1, m2, alpha=2)
        expected = torch.tensor(
            [10.0 + 13j, 8.0 + 11j], dtype=torch.complex64, device=device
        )
        self.assertEqual(res, expected)
        ...
        self.assertRaisesRegex(
            RuntimeError,
            "result type ComplexFloat can't be cast to the desired output type Double",
    ...
    def test_cumulative_trapezoid(self, device):
        ...
        def test_dx(sizes, dim, dx, device):
            ...
            self.assertEqual(expected.shape, actual.shape)
            self.assertEqual(
                expected, actual, exact_dtype=False, atol=0.0001, rtol=0.0001
            )
        ...
        def test_x(sizes, dim, x, device):
            ...
            self.assertEqual(expected.shape, actual.shape)
            self.assertEqual(
                expected, actual.cpu(), exact_dtype=False, atol=0.0001, rtol=0.0001
            )
    ...
    def test_float_power_exceptions(self, device):
        ...
        test_cases = (
            ...
            (torch.tensor([-1j, 0j, 1j, 1.0 + 1j, -1.0 - 1.5j], device=device), 2.0),
        )
