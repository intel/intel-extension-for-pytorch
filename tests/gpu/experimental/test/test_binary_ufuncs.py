...
from torch.testing._internal.common_utils import TestCase, slowTest, iter_indices, TEST_WITH_ASAN, run_tests, gradcheck, torch_to_numpy_dtype_dict, numpy_to_torch_dtype_dict, TEST_SCIPY, set_default_dtype
from torch.testing._internal.common_device_type import expectedFailureMeta, instantiate_device_type_tests, onlyCUDA, onlyCPU, dtypes, dtypesIfCUDA, dtypesIfCPU, deviceCountAtLeast, precisionOverride, onlyNativeDeviceTypes, skipIf, ops, OpDTypes, skipMeta
from torch.testing import make_tensor
from torch.testing._internal.common_dtype import all_types_and_complex_and, all_types_and, integral_types, complex_types, integral_types_and, floating_types_and, floating_and_complex_types, get_all_math_dtypes
...
class TestBinaryUfuncs(TestCase):
    ...
    def _test_reference_numerics(self, dtype, op, gen, equal_nan=True):
        ...
        for sample in gen:
            ...
            if _numel(l) <= 100 and _numel(r) <= 100:
                msg = 'Failed to produce expected results! Input lhs tensor was {0}, rhs tensor was {1}, torch result is {2}, and reference result is {3}.'.format(l, r, actual, expected)
            else:
                msg = None
            exact_dtype = True
    ...
    binary_ufuncs_with_references = list(filter(lambda op: op.ref is not None and op.ref is not None, binary_ufuncs))
    ...
    @unittest.skipIf(TEST_WITH_ASAN, 'Skipped under ASAN')
    @ops(binary_ufuncs_with_references, allowed_dtypes=(torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32, torch.float64, torch.complex64, torch.complex128))
    ...
    @unittest.skipIf(TEST_WITH_ASAN, 'Skipped under ASAN')
    @ops(binary_ufuncs_with_references, allowed_dtypes=(torch.float16, torch.bfloat16, torch.float32, torch.float64, torch.complex64, torch.complex128))
    ...
    def test_scalar_support(self, device, dtype, op):
        ...
        self._test_reference_numerics(dtype, op, gen, equal_nan=True)
        gen = generate_elementwise_binary_with_scalar_and_type_promotion_samples(op, device=device, dtype=dtype)
        self._test_reference_numerics(dtype, op, gen, equal_nan=True)
    ...
    def test_type_promotion(self, device, op):
        ...
        make_lhs = partial(make_tensor, (5,), device=device, **op.lhs_make_tensor_kwargs)
        make_rhs = partial(make_tensor, (5,), device=device, **op.rhs_make_tensor_kwargs)
        make_lhs_scalar_tensor = partial(make_tensor, (), device='cpu', **op.lhs_make_tensor_kwargs)
        make_rhs_scalar_tensor = partial(make_tensor, (), device='cpu', **op.rhs_make_tensor_kwargs)
        ...
        if _supported((torch.float64, torch.complex64)):
            ...
            expected_dtype = torch.complex128 if not op.always_returns_bool else torch.bool
            self.assertEqual(result.dtype, expected_dtype)
        if _supported((torch.int64, torch.float32)) and op.supports_rhs_python_scalar:
            ...
            if _supported((torch.float64,)):
                ...
                expected_dtype = torch.float64 if not op.always_returns_bool else torch.bool
                self.assertEqual(result.dtype, expected_dtype)
        if _supported((torch.float32, torch.complex64)) and op.supports_rhs_python_scalar:
            lhs_f32 = make_lhs(dtype=torch.float32)
            rhs_c_scalar = complex(1, 1)
            result = op(lhs_f32, rhs_c_scalar)
            expected_dtype = torch.complex64 if not op.always_returns_bool else torch.bool
            self.assertEqual(result.dtype, expected_dtype)
        ...
        if _supported((torch.float32, torch.float64)) and op.supports_rhs_python_scalar:
            ...
            expected_dtype = torch.float32 if not op.always_returns_bool else torch.bool
            self.assertEqual(result.dtype, expected_dtype)
        if _supported((torch.complex64, torch.complex128)) and op.supports_rhs_python_scalar:
            lhs_c64 = make_lhs(dtype=torch.complex64)
            rhs_c128_scalar_tensor = make_rhs_scalar_tensor(dtype=torch.complex128)
    ...
    def test_not_broadcastable(self, device, dtype, op):
        for (shape_lhs, shape_rhs) in (((2,), (3,)), ((3, 1), (2, 1)), ((1, 3, 2), (3,)), ((3, 1, 2), (2, 1, 2))):
            ...
            try:
                ...
                continue
            msg = f'On {device}, torch.{op.name} broadcasts inputs shapes {shape_lhs} and {shape_rhs} into {broadcasted_shape}, although they are not broadcastable.'
    ...
    def test_add_broadcast_empty(self, device):
        ...
        self.assertEqual(torch.randn(0, 7, 0, 6, 5, 0, 7, device=device), torch.randn(0, 7, 0, 6, 5, 0, 1, device=device) + torch.randn(1, 1, 5, 1, 7, device=device))
    ...
    def test_div_rounding_modes(self, device, dtype):
        ...
        if dtype not in (torch.bfloat16, torch.half):
            ...
            self.assertEqual(d_floor * b + torch.remainder(a.float(), b.float()), a, exact_dtype=False)
        d_trunc = torch.divide(a, b, rounding_mode='trunc')
        rounding_unsupported = dtype == torch.half and device != 'xpu' or (dtype == torch.bfloat16 and device != 'cpu')
    ...
    def test_div_rounding_nonfinite(self, device, dtype):
        num = torch.tensor([1.0, -1.0, 0, 0.1, -0.1, np.pi, -np.pi, np.inf, -np.inf, np.nan], dtype=dtype)
        denom = num[num != 0]
        ...
        for (mode, np_ref) in ((None, np.true_divide), ('floor', np.floor_divide)):
            ...
            self.assertEqual(actual, torch.from_numpy(expect), exact_device=False, exact_dtype=exact_dtype)
    ...
    def test_divide_by_zero_rounding(self, device, dtype):
        a = torch.tensor([1.0, -1.0, 0, 0.1, -0.1, np.pi, -np.pi, np.inf, -np.inf, np.nan], dtype=dtype)
        exact_dtype = dtype != torch.bfloat16
        if exact_dtype:
            an = a.cpu().numpy()
        else:
    ...
    def test_div_rounding_numpy(self, device, dtype):
        ...
        if exact_dtype:
            ...
            (an, bn) = (a.float().cpu().numpy(), b.float().cpu().numpy())
        for (mode, np_ref) in ((None, np.true_divide), ('floor', np.floor_divide), ('trunc', lambda a, b: np.trunc(np.true_divide(a, b)).astype(a.dtype))):
            with np.errstate(all='ignore'):
    ...
    def binary_check_input_output_mem_overlap(self, op, device, expected_failure=False):
        ...
        self.unary_check_input_output_mem_overlap(data, sz, lambda input, out: op(other, input, out=out), expected_failure=expected_failure)
        self.unary_check_input_output_mem_overlap(data, sz, lambda input, out: op(input, other, out=out), expected_failure=expected_failure)

    @dtypes(torch.double)
    def test_binary_op_mem_overlap(self, device, dtype):
        ops = [('add', True, True, 'cpu'), ('add', True, True, 'xpu'), ('mul', True, True, 'cpu'), ('mul', True, True, 'xpu'), ('sub', True, True, 'cpu'), ('sub', True, True, 'xpu'), ('div', True, True, 'cpu'), ('div', True, True, 'xpu'), ('pow', True, True, 'cpu'), ('pow', True, True, 'xpu'), ('fmod', True, True, 'cpu'), ('fmod', True, True, 'xpu'), ('atan2', True, True, 'cpu'), ('atan2', True, True, 'xpu'), ('hypot', True, True, 'cpu'), ('hypot', True, True, 'xpu'), ('igamma', True, True, 'cpu'), ('igamma', True, True, 'xpu'), ('igammac', True, True, 'cpu'), ('igammac', True, True, 'xpu'), ('nextafter', True, True, 'cpu'), ('nextafter', True, True, 'xpu'), ('le', True, True, 'cpu'), ('le', True, True, 'xpu'), ('lt', True, True, 'cpu'), ('lt', True, True, 'xpu'), ('ge', True, True, 'cpu'), ('ge', True, True, 'xpu'), ('gt', True, True, 'cpu'), ('gt', True, True, 'xpu'), ('eq', True, True, 'cpu'), ('eq', True, True, 'xpu'), ('ne', True, True, 'cpu'), ('ne', True, True, 'xpu'), ('logical_and', True, True, 'cpu'), ('logical_and', True, True, 'xpu'), ('logical_or', True, True, 'cpu'), ('logical_or', True, True, 'xpu'), ('logical_xor', True, True, 'cpu'), ('logical_xor', True, True, 'xpu')]
        for (fn, has_input_output_mem_overlap_check, has_internal_mem_overlap_check, dev) in ops:
            if dev != device:
                continue
            out_op = getattr(torch, fn)
            inplace_op = getattr(torch.Tensor, fn + '_')
            self.check_internal_mem_overlap(inplace_op, 2, dtype, device, expected_failure=not has_internal_mem_overlap_check)
            self.binary_check_input_output_mem_overlap(out_op, device, expected_failure=not has_input_output_mem_overlap_check)
    ...
    def _do_pow_for_exponents(self, m1, exponents, pow_fn, atol):
        for num in exponents:
            if isinstance(num, int) and num < 0 and (not m1.is_floating_point()) and (not m1.is_complex()):
                with self.assertRaisesRegex(RuntimeError, 'Integers to negative integer powers are not allowed\\.'):
                    torch.pow(m1[4], num)
            else:
    ...
    def test_pow(self, device, dtype):
        ...
        complex_exponents = [-2.5j, -1j, 0j, 1j, 2.5j, 1.0 + 1j, -1.0 - 1.5j, 3.3j]
        if m1.is_complex():
            self._do_pow_for_exponents(m1, exponents + complex_exponents, pow, 0.001)
        else:
    ...
    def _test_pow(self, base, exponent, np_exponent=None):
        ...
        try:
            ...
            test_cases = [lambda : base.pow(exponent), lambda : base.pow_(exponent), lambda : torch.pow(base, exponent), lambda : torch.pow(base, exponent, out=out)]
            ...
            if isinstance(base, torch.Tensor):
                ...
                actual = base.clone()
                if isinstance(exponent, torch.Tensor) and base.dim() == 0 and (base.device.type == 'cpu') and (exponent.device.type == 'xpu'):
    ...
    def test_int_and_float_pow(self, device):
        ...
        def _test_int_and_float_pow(dt, low, high, dev):
            test_cases = (((4, 4), 0, (4, 1)), ((3, 1), 4, (3, 1)), ((2,), 4, (1,)), ((1,), 2, ()), ((513, 513), 4, (513,)), ((5, 5, 5), 5, (5,)), ((), 2, ()))
            for (base_shape, exp_scalar, exp_shape) in test_cases:
                ...
                base_tensor = make_tensor(base_shape, dtype=dt, device=dev, low=low, high=high, noncontiguous=True)
                if dt in [torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64]:
                    exp_tensor = make_tensor(exp_shape, dtype=dt, device=dev, low=0, high=high, noncontiguous=True)
                else:
                    exp_tensor = make_tensor(exp_shape, dtype=dt, device=dev, low=low, high=high, noncontiguous=True)
                self._test_pow(base_tensor, exp_scalar)
                self._test_pow(base_tensor, exp_tensor)
    ...
    def test_pow_inplace_resizing_exception(self, device):
        test_cases = (((), (3,)), ((2,), (2, 1)), ((2, 1), (2, 2)), ((2, 2), (2, 1, 1)))
        test_inputs = list(((make_tensor(base_size, dtype=torch.float64, device=device, high=10.0, low=0.0), make_tensor(exp_size, dtype=torch.float64, device=device, high=10.0, low=0.0)) for (base_size, exp_size) in test_cases))
        for (base, exponent) in test_inputs:
            regex = "doesn't match the broadcast shape"
            self.assertRaisesRegex(RuntimeError, regex, base.pow_, exponent)

    def test_int_tensor_pow_neg_ints(self, device):
        ints = [torch.iinfo(torch.int32).min, -3, -2, -1, 0, 1, 2, 3, torch.iinfo(torch.int32).max]
    ...
    def test_float_scalar_pow_float_tensor(self, device, dtype):
        ...
        exponent_shapes = ((1,), (2, 2), (2, 1), (2, 2, 2))
        tensors = list((make_tensor(shape, dtype=dtype, device=device, low=0) for shape in exponent_shapes))
    ...
    @onlyCUDA
    def test_xpu_tensor_pow_scalar_tensor(self, device):
        xpu_tensors = [torch.randn((3, 3), device=device), torch.tensor(3.0, device=device)]
        scalar_tensors = [torch.tensor(5.0, device='cpu'), torch.tensor(-3), torch.tensor(1)]
        for (base, exp) in product(xpu_tensors, scalar_tensors):
            self._test_pow(base, exp)

    @onlyCUDA
    def test_cpu_tensor_pow_xpu_scalar_tensor(self, device):
        xpu_tensors = [torch.tensor(5.0, device='xpu'), torch.tensor(-3, device='xpu')]
    ...
    def test_pow_xpu_complex_extremal_failing(self, device, dtype):
        t = torch.tensor(complex(-1.0, float('inf')), dtype=dtype, device=device)
        with self.assertRaises(AssertionError):
            xpu_out = t.pow(2)
            cpu_out = t.cpu().pow(2)
            self.assertEqual(cpu_out, xpu_out)

    @onlyNativeDeviceTypes
    @dtypes(*all_types_and_complex_and(torch.half))
    def test_complex_scalar_pow_tensor(self, device, dtype):
        complexes = [0.5j, 1.0 + 1j, -1.5j, 2.2 - 1.6j, 1 + 0j]
        ...
        for base in complexes:
            will_raise_error = torch.device(device).type == 'cpu' and dtype is torch.half and (base != 1 + 0j)
            if will_raise_error:
                with self.assertRaisesRegex(RuntimeError, "not implemented for 'ComplexHalf'"):
    ...
    def test_cross_device_binary_ops(self, devices):
        ...
        def do_test(op, a, b):
            ...
            with self.assertRaisesRegex(RuntimeError, 'Expected all tensors.+'):
                op(cpu_tensor, a)
        for op in (operator.add, torch.add, operator.sub, torch.sub, operator.mul, torch.mul, operator.truediv, torch.true_divide, operator.floordiv, torch.floor_divide):
    ...
    def test_binary_op_scalar_device_unspecified(self, devices):
        ...
        for default_device in devices:
            with torch.xpu.device(default_device):
    ...
    def test_binary_ops_with_scalars(self, device):
        for (python_op, torch_op) in ((operator.add, torch.add), (operator.sub, torch.sub), (operator.mul, torch.mul), (operator.truediv, torch.div)):
            for (a, b) in product(range(-10, 10), range(-10, 10)):
                for op in (lambda x: x * 0.5, lambda x: math.floor(x)):
                    a = op(a)
                    b = op(b)
    ...
    @dtypes(*product(all_types_and(torch.half, torch.bfloat16, torch.bool), all_types_and(torch.half, torch.bfloat16, torch.bool)))
    ...
    def test_maximum_minimum_int_and_bool(self, device, dtype):
        ops = ((torch.maximum, torch.max, np.maximum), (torch.minimum, torch.min, np.minimum), (torch.fmax, None, np.fmax), (torch.fmin, None, np.fmin))
    ...
    def test_maximum_minimum_complex(self, device, dtypes):
        for torch_op in (torch.maximum, torch.minimum, torch.max, torch.min, torch.fmax, torch.fmin):
            with self.assertRaisesRegex(RuntimeError, '.+not implemented for.+'):
                torch_op(torch.ones(1, device=device, dtype=dtypes[0]), torch.ones(1, device=device, dtype=dtypes[1]))
            with self.assertRaisesRegex(RuntimeError, '.+not implemented for.+'):
                torch_op(torch.ones(1, device=device, dtype=dtypes[1]), torch.ones(1, device=device, dtype=dtypes[0]))
    ...
    @dtypes(*product(floating_types_and(torch.half, torch.bfloat16), floating_types_and(torch.half, torch.bfloat16)))
    def test_maximum_and_minimum_subgradient(self, device, dtypes):
        ...
        run_test(torch.maximum, [0.0, 1.0, 2.0], [1.0, 1.0, 1.0], [0.0, 0.5, 1.0], [1.0, 0.5, 0.0])
        run_test(torch.minimum, [0.0, 1.0, 2.0], [1.0, 1.0, 1.0], [1.0, 0.5, 0.0], [0.0, 0.5, 1.0])
    ...
    def test_copysign(self, device, dtypes):
        ...
        def _test_copysign_numpy(a, b):
            ...
            if a.dtype != torch.float16 and b.dtype != torch.float16:
                self.assertEqual(torch.copysign(torch.tensor(1.0), torch_result), torch.copysign(torch.tensor(1.0), expected))
    ...
    def test_div(self, device, dtype):
        for (op, method, inplace) in ((torch.div, torch.Tensor.div, torch.Tensor.div_), (torch.true_divide, torch.Tensor.true_divide, torch.Tensor.true_divide_)):
            ...
            if dtype == torch.bfloat16:
                ...
                self.assertEqual(op(a1, a2), torch.tensor([2.1, 3.1], dtype=dtype, device=device), atol=0.01, rtol=0)
    ...
    def test_true_divide_out(self, device, dtype):
        ...
        self.assertEqual(torch.true_divide(a1, a2, out=res), torch.tensor([2.1, 3.1], dtype=dtype, device=device), atol=0.01, rtol=0)
    ...
    def test_divmul_scalar(self, device, dtype):
        ...
        x = torch.tensor(1e-05, device=device, dtype=dtype)
        x_ref = x.float()
        res = x.mul(scale)
    ...
    def test_floor_divide_scalar(self, device, dtype):
        ...
        z = x // 3
        z_alt = torch.tensor([math.floor(v.item() / 3.0) for v in x], dtype=x.dtype, device=device)
        self.assertEqual(z.dtype, x.dtype)
        self.assertEqual(z, z_alt)
    ...
    def test_fmod_remainder(self, device, dtype):
            ...
            for (fn, inplace_fn, ref_fn) in fns_list:
                ...
                if torch.is_tensor(x):
                    ...
                    try:
                        ...
                        self.assertRegex(str(e), "result type (Half|Float|Double) can't be cast to the desired output type (Byte|Char|Short|Int|Long)")
        x = make_tensor((10, 10), device=device, dtype=dtype, low=-9, high=9)
        ...
        for (dividend, mod) in product([x, x.t()], mods):
            _helper(dividend, mod, ((torch.fmod, torch.Tensor.fmod_, np.fmod), (torch.remainder, torch.Tensor.remainder_, np.remainder)))
        for (dividend, mod) in product([5, 3.14], mods):
            if torch.is_tensor(mod):
                _helper(dividend, mod, ((torch.remainder, torch.Tensor.remainder_, np.remainder),))
    ...
    def test_hypot(self, device, dtype):
        inputs = [(torch.randn(10, device=device).to(dtype), torch.randn(10, device=device).to(dtype)), (torch.randn((3, 3, 3), device=device).to(dtype), torch.randn((3, 3, 3), device=device).to(dtype)), (torch.randn((10, 1), device=device).to(dtype), torch.randn((10, 1), device=device).to(dtype).transpose(0, 1)), (torch.randint(100, (10,), device=device, dtype=torch.long), torch.randn(10, device=device).to(dtype))]
        for input in inputs:
    ...
    def test_nextafter_bfloat16(self, device, dtype):
        nan = float('nan')
        inf = float('inf')
        cases = ((0, 1, 9.183549615799121e-41), (0, -1, -9.183549615799121e-41), (1, -2, 0.99609375), (1, 0, 0.99609375), (1, 2, 1.0078125), (-1, -2, -1.0078125), (-1, 0, -0.99609375), (2, -1, 1.9921875), (2, 1, 1.9921875), (20, 3000, 20.125), (20, -3000, 19.875), (3000, -20, 2992.0), (-3000, 20, -2992.0), (65536, 0, 65280.0), (65536, inf, 66048.0), (-65536, 0, -65280.0), (-65536, -inf, -66048.0), (nan, 0, nan), (0, nan, nan), (nan, nan, nan), (nan, inf, nan), (inf, nan, nan), (inf, -inf, 3.3895313892515355e+38), (-inf, inf, -3.3895313892515355e+38), (inf, 0, 3.3895313892515355e+38), (0, inf, 9.183549615799121e-41), (-inf, 0, -3.3895313892515355e+38), (0, -inf, -9.183549615799121e-41))
    ...
    @dtypes(*tuple(itertools.combinations_with_replacement(all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool), 2)))
    def test_comparison_ops_type_promotion_and_broadcasting(self, device, dtypes):
        ...
        op_pairs = [(torch.lt, np.less), (torch.le, np.less_equal), (torch.gt, np.greater), (torch.ge, np.greater_equal), (torch.eq, np.equal), (torch.ne, np.not_equal), (torch.logical_and, np.logical_and), (torch.logical_or, np.logical_or), (torch.logical_xor, np.logical_xor)]
    ...
    @onlyNativeDeviceTypes
    @dtypes(*list(product(all_types_and(torch.half, torch.bfloat16, torch.bool), all_types_and(torch.half, torch.bfloat16, torch.bool))))
    def test_heaviside(self, device, dtypes):
        input_dtype = dtypes[0]
        values_dtype = dtypes[1]
        rng = np.random.default_rng()
        input = np.array(rng.integers(-10, 10, size=10), dtype=torch_to_numpy_dtype_dict[input_dtype if input_dtype != torch.bfloat16 else torch.float64])
        input[0] = input[3] = input[7] = 0
        values = np.array(rng.integers(-10, 10, size=10), dtype=torch_to_numpy_dtype_dict[values_dtype if values_dtype != torch.bfloat16 else torch.float64])
    ...
    @dtypes(*product(all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool), all_types_and_complex_and(torch.half, torch.bfloat16, torch.bool)))
    ...
    def test_add(self, device):
        ...
        expected = torch.tensor([2.4 + 3.5j, 4.2 + 5.3j], dtype=torch.complex64, device=device)
        self.assertEqual(res, expected)
        res = torch.add(m1, m2, alpha=complex(0.1, 0.2))
        expected = torch.tensor([1.4 + 4.3j, 3.6 + 5.7j], dtype=torch.complex64, device=device)
        self.assertEqual(res, expected)
        res = torch.add(m1, m2, alpha=2)
        expected = torch.tensor([10.0 + 13j, 8.0 + 11j], dtype=torch.complex64, device=device)
        self.assertEqual(res, expected)
        m1 = torch.tensor([1], dtype=torch.int8, device=device)
        m2 = torch.tensor([2], dtype=torch.int8, device=device)
        self.assertRaisesRegex(RuntimeError, 'Boolean alpha only supported for Boolean results\\.', lambda : torch.add(m1, m2, alpha=True))
        self.assertRaisesRegex(RuntimeError, 'For integral input tensors, argument alpha must not be a floating point number\\.', lambda : torch.add(m1, m2, alpha=1.0))
        msg = 'For non-complex input tensors, argument alpha must not be a complex number\\.'
        m1 = torch.tensor([3.0, 4.0], device=device)
        m2 = torch.tensor([4.0, 3.0], device=device)
        self.assertRaisesRegex(RuntimeError, msg, lambda : torch.add(m1, m2, alpha=complex(0.1, 0.2)))
        m1 = torch.tensor([3.0, 4.0], dtype=torch.double, device=device)
        m2 = torch.tensor([4.0, 3.0], dtype=torch.double, device=device)
        self.assertRaisesRegex(RuntimeError, msg, lambda : torch.add(m1, m2, alpha=complex(0.1, 0.2)))
        m1 = torch.tensor(4.0 + 4j, dtype=torch.complex64)
        m2 = torch.tensor(4.0, dtype=torch.float64)
        self.assertRaisesRegex(RuntimeError, "result type ComplexFloat can't be cast to the desired output type Double", lambda : torch.add(m1, m1, out=m2))

    @onlyCUDA
    def test_addsub_half_tensor(self, device):
        x = torch.tensor([60000.0], dtype=torch.half, device=device)
        for (op, y, alpha) in ((torch.add, torch.tensor([-60000.0], dtype=torch.half, device=device), 2), (torch.sub, torch.tensor([60000.0], dtype=torch.half, device=device), 2), (torch.add, -70000.0, 1), (torch.sub, 70000.0, 1)):
    ...
    def test_sub_typing(self, device):
        m1 = torch.tensor([True, False, False, True, False, False], dtype=torch.bool, device=device)
        m2 = torch.tensor([True, True, False, False, False, True], dtype=torch.bool, device=device)
        self.assertRaisesRegex(RuntimeError, 'Subtraction, the `\\-` operator, with two bool tensors is not supported. Use the `\\^` or `logical_xor\\(\\)` operator instead.', lambda : m1 - m2)
        self.assertRaisesRegex(RuntimeError, 'Subtraction, the `\\-` operator, with a bool tensor is not supported. If you are trying to invert a mask, use the `\\~` or `logical_not\\(\\)` operator instead.', lambda : 1 - m1)
        self.assertRaisesRegex(RuntimeError, 'Subtraction, the `\\-` operator, with a bool tensor is not supported. If you are trying to invert a mask, use the `\\~` or `logical_not\\(\\)` operator instead.', lambda : m2 - 1)
        m1 = torch.tensor([1], dtype=torch.int8, device=device)
        m2 = torch.tensor([2], dtype=torch.int8, device=device)
        self.assertRaisesRegex(RuntimeError, 'Boolean alpha only supported for Boolean results\\.', lambda : torch.sub(m1, m2, alpha=True))
        self.assertRaisesRegex(RuntimeError, 'For integral input tensors, argument alpha must not be a floating point number\\.', lambda : torch.sub(m1, m2, alpha=1.0))
    ...
    def test_bool_tensor_comparison_ops(self, device):
        ...
        self.assertEqual(a == torch.tensor(True, dtype=torch.bool, device=device), torch.tensor([1, 0, 1, 0, 1, 0], dtype=torch.bool, device=device))
        self.assertEqual(a == torch.tensor(0, dtype=torch.bool, device=device), torch.tensor([0, 1, 0, 1, 0, 1], dtype=torch.bool, device=device))
    ...
    def test_atan2(self, device):
        ...
        def _test_atan2_with_size(size, device):
            ...
            x = a.view(-1)
            y = b.view(-1)
            expected = torch.tensor([math.atan2(x[i].item(), y[i].item()) for i in range(x.numel())], device=device, dtype=torch.double)
    ...
    def test_trapezoid(self, device):
        ...
        with self.assertRaisesRegex(RuntimeError, 'There must be one `x` value for each sample point'):
    ...
    def test_cumulative_trapezoid(self, device):
        ...
        def test_dx(sizes, dim, dx, device):
            ...
            self.assertEqual(expected.shape, actual.shape)
            self.assertEqual(expected, actual, exact_dtype=False, atol=0.0001, rtol=0.0001)
        ...
        def test_x(sizes, dim, x, device):
            ...
            self.assertEqual(expected.shape, actual.shape)
            self.assertEqual(expected, actual.cpu(), exact_dtype=False, atol=0.0001, rtol=0.0001)
        ...
        with self.assertRaisesRegex(RuntimeError, 'There must be one `x` value for each sample point'):
            ...
            test_x((2, 3), 1, [1.0, 2.0, 3.0, 4.0], device)
        with self.assertRaisesRegex(RuntimeError, 'Currently, we only support dx as a real number'):
    ...
    def test_pow_scalar_overloads_mem_overlap(self, device, dtype):
        ...
        self.unary_check_input_output_mem_overlap(doubles, sz, lambda input, out: torch.pow(input, 42, out=out))
        self.unary_check_input_output_mem_overlap(doubles, sz, lambda input, out: torch.pow(42, input, out=out))

    @dtypes(*list(product(all_types_and_complex_and(torch.half, torch.bfloat16), all_types_and_complex_and(torch.half, torch.bfloat16))))
    def test_float_power(self, device, dtypes):
        ...
        complex_exponents = exponents + [-2.5j, -1j, 1j, 2.5j, 1.0 + 1j, -1.0 - 1.5j, 3.3j]
        ...
        def _promo_helper(x, y):
            for i in (x, y):
                if type(i) == complex:
                    ...
                    return torch.complex128
            return torch.double
        test_cases = ((torch.tensor([-2, -1, 0, 1, 2], device=device), -0.25), (torch.tensor([-1j, 0j, 1j, 1.0 + 1j, -1.0 - 1.5j], device=device), 2.0))
    ...
    @skipIf(not TEST_SCIPY, 'Scipy required for the test.')
    @dtypes(*product(all_types_and(torch.half, torch.bool), all_types_and(torch.half, torch.bool)))
    def test_xlogy_xlog1py(self, device, dtypes):
        ...
        t = torch.tensor([-1.0, 0.0, 1.0, 2.0, float('inf'), -float('inf'), float('nan')], device=device)
...
tensor_binary_ops = ['__lt__', '__le__', '__gt__', '__ge__', '__eq__', '__ne__', '__add__', '__radd__', '__iadd__', '__sub__', '__rsub__', '__isub__', '__mul__', '__rmul__', '__imul__', '__matmul__', '__rmatmul__', '__truediv__', '__rtruediv__', '__itruediv__', '__floordiv__', '__rfloordiv__', '__ifloordiv__', '__mod__', '__rmod__', '__imod__', '__pow__', '__rpow__', '__ipow__', '__lshift__', '__rlshift__', '__ilshift__', '__rshift__', '__rrshift__', '__irshift__', '__and__', '__rand__', '__iand__', '__xor__', '__rxor__', '__ixor__', '__or__', '__ror__', '__ior__']

def generate_not_implemented_tests(cls):

    class UnknownType:
        pass
    _types = [torch.half, torch.float, torch.double, torch.int8, torch.short, torch.int, torch.long, torch.uint8]
