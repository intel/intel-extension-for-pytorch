...
from torch.testing._internal.common_device_type import instantiate_device_type_tests, onlyCUDA, skipCUDAIfRocm, dtypes, OpDTypes
...
class TestPrims(TestCase):
    ...
    def test_nvfuser_impl_is_used(self, device):
        ...
        ops_without_nvfuser_impl = {name for name in prim_nvfuser_ops if getattr(torch.ops.nvprims, name, None) is None}
        assert len(ops_without_nvfuser_impl) == 0, (f"The following prims do not have 'impl_nvfuser' defined: {ops_without_nvfuser_impl} ", 'while there exists nvfuser implementations for them.')
    ...
    def test_skip_ops_nvfuser_prims_mode(self, device):
        ...
        with NvfuserPrimsMode(skip_ops=skip_ops):
            gm = make_fx(func)(a)
        includes_any_prims_sin = any((node.target == torch.ops.prims.sin.default for node in gm.graph.nodes))
        self.assertTrue(includes_any_prims_sin)
        include_any_nvprims_sin = any((node.target == torch.ops.nvprims.sin.default for node in gm.graph.nodes))
    ...
    def test_skip_ops_nvfuser_capability_mode(self, device):
        ...
        with TorchRefsNvfuserCapabilityMode(skip_ops=skip_ops):
            gm = make_fx(func)(a)
        includes_any_aten_sin = any((node.target == torch.ops.aten.sin.default for node in gm.graph.nodes))
    ...
    def test_nvprim_convert_element_type(self, device, dtype):
        ...
        includes_aten_to_copy = any((torch.ops.aten._to_copy.default == node.target for node in call_function_nodes))
        includes_nvprim_convert_element_type = any((torch.ops.nvprims.convert_element_type.default == node.target for node in call_function_nodes))
    ...
    def test_nvfuser_capability_context(self, device):
        ...
        with TorchRefsNvfuserCapabilityMode():
            gm = make_fx(func)(a)
        call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
        includes_aten_digamma = any((torch.ops.aten.digamma.default == node.target for node in call_function_nodes))
        includes_prims_digamma = any((torch.ops.prims.digamma.default == node.target for node in call_function_nodes))
        ...
        with TorchRefsNvfuserCapabilityMode():
            gm = make_fx(func)(a)
        call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
        includes_aten_sigmoid = any((torch.ops.aten.sigmoid.default == node.target for node in call_function_nodes))
        includes_prims_digamma = any((torch.ops.prims.digamma.default == node.target for node in call_function_nodes))
        includes_nvprims_exp = any((torch.ops.nvprims.exp.default == node.target for node in call_function_nodes))
    ...
    def test_native_batch_norm_nvprims(self, device, dtype):
        ...
        from torch._prims.executor import execute
        from torch.fx.experimental.proxy_tensor import make_fx
        from torch.testing._internal.common_methods_invocations import sample_inputs_native_batch_norm
        samples = sample_inputs_native_batch_norm(None, device, dtype, requires_grad=False)
        batch_norms = [torch.native_batch_norm, torch.ops.aten.native_batch_norm, torch.ops.aten.native_batch_norm.default, torch.ops.nvprims.native_batch_norm.default]
        for (sample, batch_norm) in product(samples, batch_norms):
            if sample.input.numel() == 0:
                continue

            def func(input, weight, bias, running_mean, running_var, training, momentum, eps):
                return batch_norm(input, weight, bias, running_mean, running_var, training, momentum, eps)
            with TorchRefsNvfuserCapabilityMode():
                gm = make_fx(func)(sample.input, *sample.args)
            call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
            includes_aten_batch_norm = any((torch.ops.aten.native_batch_norm.default == node.target for node in call_function_nodes))
            self.assertFalse(includes_aten_batch_norm)
            includes_nvprims_batch_norm = any((torch.ops.nvprims.native_batch_norm.default == node.target for node in call_function_nodes))
    ...
    def test_batch_norm_backward_nvprims(self, device, dtype):
        ...
        def func(grad, input, weight, rm, rv, eps, train):
            return torch.ops.aten.native_batch_norm_backward.default(grad, input, weight, rm, rv, rm, rv, train, eps, [True, True, True])
        args = sample.args
        ...
        call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
        includes_batch_norm_backward = any((torch.ops.aten.native_batch_norm_backward.default == node.target for node in call_function_nodes))
    ...
    def test_var_mean(self, device, dtype, correction, keepdim):
        ...
        call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
        includes_nvprims_var_mean = any((torch.ops.nvprims.var_mean.main == node.target for node in call_function_nodes))
    ...
    def test_cpu_tensor(self, device, dtype):
        ...
        self.assertEqual(expected, actual)
        call_function_nodes = list(filter(lambda n: n.op == 'call_function', gm.graph.nodes))
        includes_aten_add = any((torch.ops.aten.add.default == node.target for node in call_function_nodes))
