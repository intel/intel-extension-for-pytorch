...
class TestFX(JitTestCase):
    ...
    def test_args_kwargs_no_self(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "cannot be part of \\*args expansion"
        ):
    ...
    def test_trace_multiple_funcs(self):
        ...
        torch.testing.assert_close(
            GraphModule(f, tracer.trace(f))(x, y), f.minus_forward(x, y)
        )
        tracer.traced_func_name = "multiply_forward"
        torch.testing.assert_close(
            GraphModule(f, tracer.trace(f))(x, y), f.multiply_forward(x, y)
        )
        ...
        class CustomArgObject:
            ...
            def __fx_create_arg__(self, tracer: torch.fx.Tracer):
                return tracer.create_node(
                    "call_function",
                    CustomArgObject,
                    args=(tracer.create_arg(self.x), tracer.create_arg(self.y)),
                    kwargs={},
                )
    ...
    def test_custom_traceback_raised_when_exception_source_is_graphmodule(self):
        ...
        self.assertRegex(
            captured[0],
            "Call using an FX-traced Module, line .* of the traced Module's generated forward function:",
        )
    ...
    def test_submodule_manipulation_API(self):
        ...
        with a.graph.inserting_before(cat):
            ...
            with self.assertWarnsRegex(
                UserWarning,
                "Attempted to insert a get_attr Node with no underlying reference in the owning GraphModule",
            ):
    ...
    def test_pytree(self):
        ...
        pytree._register_pytree_node(
            Foo, lambda x: ([x.a, x.b], None), lambda x, _: Foo(x[0], x[1])
        )
...
class TestFXAPIBackwardCompatibility(JitTestCase):
    ...
    def _annotation_type_to_stable_str(self, t, sig_str):
        ...
        raise RuntimeError(
            f"Unrecognized type {t} used in BC-compatible type signature {sig_str}.Please add support for this type and confirm with the FX team that your signature change is valid."
        )
    ...
    def test_public_api_surface(self):
        ...
        if len(non_back_compat_strs) != 0:
            raise AssertionError(
                f"Public FX API(s) {non_back_compat_strs} introduced but not given a backwards-compatibility classification! Please decorate these API(s) with `@torch.fx._compatibility.compatibility` to specify BC guarantees."
            )
...
class TestVisionTracing(JitTestCase):
    ...
    INCONSISTENT_TYPE = (
        RuntimeError,
        "Return value was annotated as having type __torch__.torchvision.models[.\\w]+ but is actually of type Tensor",
    )
    UNTRACEABLE_MODELS = {
