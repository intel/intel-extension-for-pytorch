...
from torch.testing._internal.common_utils import IS_FBCODE, IS_MACOS, IS_WINDOWS, find_library_location, run_tests, skipIfSlowGradcheckEnv
...
from common.jit_utils import JitTestCase
...
class TestFX(JitTestCase):
    ...
    def test_example_shape_prop(self):
        ...
        self.assertEqual(opcodes, set(['placeholder', 'get_attr', 'call_function', 'call_method', 'call_module', 'output']))
    ...
    def test_profiler_ranges_side_effect(self):
        ...
        self.assertEqual(list(found_targets.keys()), [torch.ops.profiler._record_function_enter, torch.ops.profiler._record_function_exit])
    ...
    def test_pytree(self):
        ...
        def f_return_custom(x):
            return Foo(x.b, x.a)
        tests = [(f_sum, [PH, PH, PH]), (f_sum, []), (f_sum_dict, {'a': PH, 'b': PH, 'c': PH}), (f_dict_list_map, {'a': (PH, PH), 'b': [PH], 'c': []}), (f_dict_list_map, {5: (PH, PH, PH)}), (f_dict_add, {'a': PH, 'z': (PH, PH, PH)}), (f_dict_add, {'a': PH, 'z': []}), (f_custom, Foo(PH, PH)), (f_custom, Foo(PH, 3)), (f_custom_dict, Foo({'a': PH, 'b': PH}, PH)), (f_namedtuple_add, Point(PH, PH))]
...
class TestFunctionalTracing(JitTestCase):
    ...
    def generate_test_func(cls, func_name, fn):
        ...
        def functional_test(self):
            if func_name in self.UNTRACEABLE_FUNCTIONALS_PY38 and sys.version_info >= (3, 8) and (sys.version_info < (3, 11)):
