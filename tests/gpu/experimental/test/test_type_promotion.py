...
import unittest
import torch
from torch.testing._internal.common_utils import TestCase, run_tests, load_tests, make_tensor, TEST_NUMPY, torch_to_numpy_dtype_dict, numpy_to_torch_dtype_dict
...
from torch.testing._internal.common_dtype import all_types_and_complex_and, get_all_math_dtypes, floating_types, get_all_dtypes
...
class TestTypePromotion(TestCase):
    ...
    def test_many_promotions(self, device):
        dtypes1 = get_all_math_dtypes('xpu')
    ...
    def test_complex_assertraises(self, device):
        ...
        for op in comparison_ops:
            ...
            dtypes = get_all_dtypes(include_half=is_xpu, include_bfloat16=False, include_bool=False, include_complex32=True)
        ...
        for np_first in (True, False):
            for op in (operator.add, torch.add):
                ...
                if undesired_failure and same_result:
                    msg = 'Failure: {0} == {1}. torch type was {2}. NumPy type was {3}. np_first is {4} default type is {5}.'.format(actual, expected, torch_type, np_type, np_first, torch.get_default_dtype())
                    self.fail(msg)
    ...
    def test_clamp_type_promotion(self, device, dtypes):
        ...
        for (min_v, max_v) in itertools.product(mins, maxs):
            ...
            if exp_type != torch.bool:
                ...
                inps = list(map(lambda x: x.to(exp_type) if isinstance(x, torch.Tensor) else x, (inp, min_v, max_v)))
        ...
        for val in mins:
            ...
            if exp_type != torch.bool:
                ...
                inps = list(map(lambda x: x.to(exp_type) if isinstance(x, torch.Tensor) else x, (inp, val)))
