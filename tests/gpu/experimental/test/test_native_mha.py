...
class TestMHADeviceType(TestCase):
    ...
    def _test_multihead_attention_impl(self, device, dtype, mode, use_nt, need_weights, average_attn_weights, use_padding=False, pad_all=False):
        ...
        npt = NativeMHA(embed_dim=embed_dim, num_heads=num_heads, qkv=qkv, proj=proj).to(dtype)
        if device == 'xpu':
            pt = pt.xpu()
            npt = npt.xpu()
        ...
        (ynpt, weight_npt) = npt(q, k, v, key_padding_mask=mask if use_padding and (not use_nt) else None)
        if use_nt:
        ...
        if dtype == torch.half:
            torch.testing.assert_close(ypt, ynpt, atol=0.001, rtol=0.001)
        else:
