...
from torch.testing._internal.common_utils import IS_FBCODE, IS_SANDCASTLE, IS_WINDOWS, TestCase, run_tests, skipIfRocm, slowTest, parametrize, subtest, instantiate_parametrized_tests, dtype_name, TEST_WITH_ROCM
from torch.testing._internal.common_device_type import PYTORCH_TESTING_DEVICE_EXCEPT_FOR_KEY, PYTORCH_TESTING_DEVICE_ONLY_FOR_KEY, dtypes, get_device_type_test_bases, instantiate_device_type_tests, onlyCUDA, onlyNativeDeviceTypes, deviceCountAtLeast, ops, expectedFailureMeta
...
class TestTesting(TestCase):
    ...
    def test_isclose_float(self, device, dtype):
        tests = ((0, 0, True), (0, -1, False), (float('inf'), float('inf'), True), (-float('inf'), float('inf'), False), (float('inf'), float('nan'), False), (float('nan'), float('nan'), False), (0, float('nan'), False), (1, 1, True))
        self._isclose_helper(tests, device, dtype, False)
        eps = 0.01 if dtype is torch.half else 1e-06
        tests = ((0, 1, True), (0, 1 + eps, False), (1, 0, False), (1, 3, True), (1 - eps, 3, False), (-0.25, 0.5, True), (-0.25 - eps, 0.5, False), (0.25, -0.5, True), (0.25 + eps, -0.5, False))
        self._isclose_helper(tests, device, dtype, False, atol=0.5, rtol=0.5)
        tests = ((0, float('nan'), False), (float('inf'), float('nan'), False), (float('nan'), float('nan'), True))
        self._isclose_helper(tests, device, dtype, True)

    @unittest.skipIf(IS_SANDCASTLE, "Skipping because doesn't work on sandcastle")
    @dtypes(torch.complex64, torch.complex128)
    def test_isclose_complex(self, device, dtype):
        tests = ((complex(1, 1), complex(1, 1 + 1e-08), True), (complex(0, 1), complex(1, 1), False), (complex(1, 1), complex(1, 0), False), (complex(1, 1), complex(1, float('nan')), False), (complex(1, float('nan')), complex(1, float('nan')), False), (complex(1, 1), complex(1, float('inf')), False), (complex(float('inf'), 1), complex(1, float('inf')), False), (complex(-float('inf'), 1), complex(1, float('inf')), False), (complex(-float('inf'), 1), complex(float('inf'), 1), False), (complex(float('inf'), 1), complex(float('inf'), 1), True), (complex(float('inf'), 1), complex(float('inf'), 1 + 0.0001), False))
        self._isclose_helper(tests, device, dtype, False)
        eps = 1e-06
        tests = ((complex(0, 0), complex(1, 0), True), (complex(0, 0), complex(1 + eps, 0), False), (complex(1, 0), complex(0, 0), False), (complex(1, 0), complex(3, 0), True), (complex(1 - eps, 0), complex(3, 0), False), (complex(-0.25, 0), complex(0.5, 0), True), (complex(-0.25 - eps, 0), complex(0.5, 0), False), (complex(0.25, 0), complex(-0.5, 0), True), (complex(0.25 + eps, 0), complex(-0.5, 0), False), (complex(0, 0), complex(0, 1), True), (complex(0, 0), complex(0, 1 + eps), False), (complex(0, 1), complex(0, 0), False), (complex(0, 1), complex(0, 3), True), (complex(0, 1 - eps), complex(0, 3), False), (complex(0, -0.25), complex(0, 0.5), True), (complex(0, -0.25 - eps), complex(0, 0.5), False), (complex(0, 0.25), complex(0, -0.5), True), (complex(0, 0.25 + eps), complex(0, -0.5), False))
        self._isclose_helper(tests, device, dtype, False, atol=0.5, rtol=0.5)
        tests = ((complex(1, -1), complex(-1, 1), False), (complex(1, -1), complex(2, -2), True), (complex(-math.sqrt(2), math.sqrt(2)), complex(-math.sqrt(0.5), math.sqrt(0.5)), True), (complex(-math.sqrt(2), math.sqrt(2)), complex(-math.sqrt(0.501), math.sqrt(0.499)), False), (complex(2, 4), complex(1.0, 8.8523607), True), (complex(2, 4), complex(1.0, 8.8523607 + eps), False), (complex(1, 99), complex(4, 100), True))
        self._isclose_helper(tests, device, dtype, False, atol=0.5, rtol=0.5)
        tests = ((complex(1, 1), complex(1, float('nan')), False), (complex(1, 1), complex(float('nan'), 1), False), (complex(float('nan'), 1), complex(float('nan'), 1), True), (complex(float('nan'), 1), complex(1, float('nan')), True), (complex(float('nan'), float('nan')), complex(float('nan'), float('nan')), True))
        self._isclose_helper(tests, device, dtype, True)

    @dtypes(torch.bool, torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64, torch.float16, torch.float32, torch.float64)
    ...
    def test_make_tensor(self, device, dtype):
        ...
        def check(size, low, high, requires_grad, noncontiguous):
            if dtype not in [torch.float, torch.cfloat]:
                requires_grad = False
            t = make_tensor(size, dtype=dtype, device=device, low=low, high=high, requires_grad=requires_grad, noncontiguous=noncontiguous)
    ...
    @onlyCUDA
    @slowTest
    def test_xpu_assert_should_stop_common_utils_test_suite(self, device):
        stderr = TestCase.runWithPytorchAPIUsageStderr("#!/usr/bin/env python3\n\nimport torch\nfrom torch.testing._internal.common_utils import (TestCase, run_tests, slowTest)\n\nclass TestThatContainsXPUAssertFailure(TestCase):\n\n    @slowTest\n    def test_throw_unrecoverable_xpu_exception(self):\n        x = torch.rand(10, device='xpu')\n        # cause unrecoverable XPU exception, recoverable on CPU\n        y = x[torch.tensor([25])].cpu()\n\n    @slowTest\n    def test_trivial_passing_test_case_on_cpu_xpu(self):\n        x1 = torch.tensor([0., 1.], device='xpu')\n        x2 = torch.tensor([0., 1.], device='cpu')\n        self.assertEqual(x1, x2)\n\nif __name__ == '__main__':\n    run_tests()\n")
    ...
    @onlyCUDA
    @slowTest
    def test_xpu_assert_should_stop_common_device_type_test_suite(self, device):
        stderr = TestCase.runWithPytorchAPIUsageStderr("#!/usr/bin/env python3\n\nimport torch\nfrom torch.testing._internal.common_utils import (TestCase, run_tests, slowTest)\nfrom torch.testing._internal.common_device_type import instantiate_device_type_tests\n\nclass TestThatContainsXPUAssertFailure(TestCase):\n\n    @slowTest\n    def test_throw_unrecoverable_xpu_exception(self, device):\n        x = torch.rand(10, device=device)\n        # cause unrecoverable XPU exception, recoverable on CPU\n        y = x[torch.tensor([25])].cpu()\n\n    @slowTest\n    def test_trivial_passing_test_case_on_cpu_xpu(self, device):\n        x1 = torch.tensor([0., 1.], device=device)\n        x2 = torch.tensor([0., 1.], device='cpu')\n        self.assertEqual(x1, x2)\n\ninstantiate_device_type_tests(\n    TestThatContainsXPUAssertFailure,\n    globals(),\n    only_for='xpu'\n)\n\nif __name__ == '__main__':\n    run_tests()\n")
        self.assertIn('XPU error: device-side assert triggered', stderr)
        self.assertIn('errors=1', stderr)
    ...
    @onlyCUDA
    @slowTest
    def test_xpu_assert_should_not_stop_common_distributed_test_suite(self, device):
        stderr = TestCase.runWithPytorchAPIUsageStderr("#!/usr/bin/env python3\n\nimport torch\nfrom torch.testing._internal.common_utils import (run_tests, slowTest)\nfrom torch.testing._internal.common_device_type import instantiate_device_type_tests\nfrom torch.testing._internal.common_distributed import MultiProcessTestCase\n\nclass TestThatContainsXPUAssertFailure(MultiProcessTestCase):\n\n    @slowTest\n    def test_throw_unrecoverable_xpu_exception(self, device):\n        x = torch.rand(10, device=device)\n        # cause unrecoverable XPU exception, recoverable on CPU\n        y = x[torch.tensor([25])].cpu()\n\n    @slowTest\n    def test_trivial_passing_test_case_on_cpu_xpu(self, device):\n        x1 = torch.tensor([0., 1.], device=device)\n        x2 = torch.tensor([0., 1.], device='cpu')\n        self.assertEqual(x1, x2)\n\ninstantiate_device_type_tests(\n    TestThatContainsXPUAssertFailure,\n    globals(),\n    only_for='xpu'\n)\n\nif __name__ == '__main__':\n    run_tests()\n")
...
def make_assert_close_inputs(actual: Any, expected: Any) -> List[Tuple[Any, Any]]:
    ...
    """
    return [(actual, expected), ((actual,), (expected,)), ([actual], [expected]), ((actual,), [expected]), ({'t': actual}, {'t': expected}), (collections.OrderedDict([('t', actual)]), collections.OrderedDict([('t', expected)])), ({'t': actual}, collections.OrderedDict([('t', expected)])), ([(actual,)], ([expected],)), ([{'t': actual}], (collections.OrderedDict([('t', expected)]),)), ({'t': [actual]}, collections.OrderedDict([('t', (expected,))]))]
...
class TestAssertClose(TestCase):
    ...
    def test_matching_nan(self):
        nan = float('NaN')
        tests = ((nan, nan), (complex(nan, 0), complex(0, nan)), (complex(nan, nan), complex(nan, 0)), (complex(nan, nan), complex(nan, nan)))
...
instantiate_device_type_tests(TestAssertCloseMultiDevice, globals(), only_for='xpu')
...
class TestAssertCloseErrorMessage(TestCase):
    ...
    def test_not_close(self):
        actual = torch.tensor([1, 2, 3, 4], dtype=torch.float32)
        expected = torch.tensor([1, 2, 5, 6], dtype=torch.float32)
        for (fn, (rtol, atol)) in itertools.product(assert_close_with_inputs(actual, expected), ((1.3e-06, 0.0), (0.0, 1e-05), (1.3e-06, 1e-05))):
...
class TestAssertCloseQuantized(TestCase):
    ...
    def test_mismatching_qscheme(self):
        ...
        expected = torch.quantize_per_channel(t, scales=torch.tensor((1.0,)), zero_points=torch.tensor((0,)), axis=0, dtype=torch.qint32)
        for fn in assert_close_with_inputs(actual, expected):
    ...
    def test_matching_per_channel(self):
        actual = torch.quantize_per_channel(torch.tensor((1.0,)), scales=torch.tensor((1.0,)), zero_points=torch.tensor((0,)), axis=0, dtype=torch.qint32)
        expected = actual.clone()
        for fn in assert_close_with_inputs(actual, expected):
            fn()
...
def _get_test_names_for_test_class(test_cls):
    ...
    test_names = ['{}.{}'.format(test_cls.__name__, key) for key in test_cls.__dict__ if key.startswith('test_')]
...
class TestTestParametrization(TestCase):
    ...
    def test_subtest_names(self):
        ...
        class TestParametrized(TestCase):
            ...
            @parametrize('x,y', [subtest((1, 2), name='double'), subtest((1, 3), name='triple'), subtest((1, 4), name='quadruple')])
            def test_two_things_custom_names(self, x, y):
                pass
        instantiate_parametrized_tests(TestParametrized)
        expected_test_names = ['TestParametrized.test_custom_names_bias', 'TestParametrized.test_custom_names_no_bias', 'TestParametrized.test_two_things_custom_names_double', 'TestParametrized.test_two_things_custom_names_quadruple', 'TestParametrized.test_two_things_custom_names_triple']
        test_names = _get_test_names_for_test_class(TestParametrized)
        self.assertEqual(expected_test_names, test_names)
...
class TestTestParametrizationDeviceType(TestCase):
    ...
    def test_unparametrized_names(self, device):
        ...
        expected_test_names = [name.format(device_cls.__name__, device) for name in ('{}.test_device_dtype_specific_{}_float32', '{}.test_device_dtype_specific_{}_float64', '{}.test_device_specific_{}')]
    ...
    def test_subtest_names(self, device):
        ...
        instantiate_device_type_tests(TestParametrized, locals(), only_for=device)
        device_cls = locals()['TestParametrized{}'.format(device.upper())]
        expected_test_names = [name.format(device_cls.__name__, device) for name in ('{}.test_custom_names_bias_{}', '{}.test_custom_names_no_bias_{}', '{}.test_two_things_custom_names_double_{}', '{}.test_two_things_custom_names_quadruple_{}', '{}.test_two_things_custom_names_triple_{}')]
        test_names = _get_test_names_for_test_class(device_cls)
        self.assertEqual(expected_test_names, test_names)
        ...
        for op in op_db:
            for dtype in op.supported_dtypes(torch.device(device).type):
                for flag_part in ('flag_disabled', 'flag_enabled'):
                    expected_name = '{}.test_op_parametrized_{}_{}_{}_{}'.format(device_cls.__name__, op.formatted_name, flag_part, device, dtype_name(dtype))
    ...
    def test_dtypes_composition_valid(self, device):
        ...
        instantiate_device_type_tests(TestParametrized, locals(), only_for=device)
        device_cls = locals()['TestParametrized{}'.format(device.upper())]
        expected_test_names = [name.format(device_cls.__name__, device) for name in ('{}.test_parametrized_x_0_{}_float32', '{}.test_parametrized_x_0_{}_float64', '{}.test_parametrized_x_1_{}_float32', '{}.test_parametrized_x_1_{}_float64', '{}.test_parametrized_x_2_{}_float32', '{}.test_parametrized_x_2_{}_float64')]
        test_names = _get_test_names_for_test_class(device_cls)
        self.assertEqual(sorted(expected_test_names), sorted(test_names))
...
class TestImports(TestCase):
    ...
    def test_circular_dependencies(self) -> None:
        ...
        ignored_modules = ['torch.utils.tensorboard', 'torch.distributed.elastic.rendezvous', 'torch.backends._coreml', 'torch.contrib.', 'torch.testing._internal.distributed.', 'torch.ao.sparsity._experimental.', 'torch.xpu._dynamo_graphs']
...
class TestOpInfos(TestCase):
    ...
    def test_sample_input_metadata(self) -> None:
        ...
        s2 = s1.with_metadata(output_process_fn_grad=lambda x: a, broadcasts_input=True, name='foo')
        self.assertIs(s1, s2)
