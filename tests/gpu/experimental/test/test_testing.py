...
class TestTesting(TestCase):
    ...
    def test_assertEqual_longMessage(self):
        ...
        try:
            ...
            with self.assertRaisesRegex(
                AssertionError, re.escape(f"{default_msg} : {extra_msg}")
            ):
    ...
    def test_isclose_complex(self, device, dtype):
        tests = (
            (complex(1, 1), complex(1, 1 + 1e-08), True),
            ...
            (complex(float("inf"), 1), complex(float("inf"), 1 + 0.0001), False),
        )
        self._isclose_helper(tests, device, dtype, False)
        eps = 1e-06
        tests = (
...
class TestAssertCloseErrorMessage(TestCase):
    ...
    def test_not_close(self):
        ...
        for fn, (rtol, atol) in itertools.product(
            ...
            ((1.3e-06, 0.0), (0.0, 1e-05), (1.3e-06, 1e-05)),
        ):
...
class TestImports(TestCase):
    def test_circular_dependencies(self) -> None:
        ...
        ignored_modules = [
            ...
            "torch.ao.sparsity._experimental.",
            "torch.xpu._dynamo_graphs",
        ]
...
class TestOpInfos(TestCase):
    ...
    def test_sample_input_metadata(self) -> None:
        ...
        s2 = s1.with_metadata(
            output_process_fn_grad=lambda x: a, broadcasts_input=True, name="foo"
        )
        self.assertIs(s1, s2)
