...
class TestDataParallel(TestCase):
    ...
    def test_parallel_apply_passes_exception(self):
        ...
        with self.assertRaisesRegex(
            KeyError,
            "Caught KeyError in replica \\d on device 0.\nOriginal Traceback[\\s\\S]+wonderful",
        ):
    ...
    def _test_gather(self, output_device):
        inputs = (
            torch.randn(2, 4, device="xpu:0", requires_grad=True),
            torch.randn(2, 4, device="xpu:1", requires_grad=True),
        )
        ...
        inputs = (
            torch.randn((), device="xpu:0", requires_grad=True),
            torch.randn((), device="xpu:1", requires_grad=True),
        )
        result = dp.gather(inputs, output_device)
