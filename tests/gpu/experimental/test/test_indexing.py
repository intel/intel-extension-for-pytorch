...
class TestIndexing(TestCase):
    ...
    def test_index_put_accumulate_expanded_values(self, device):
        ...
        out_xpu = t_dev.index_put_(indices_dev, values0d.to(device), accumulate=True)
        out_cpu = t.index_put_(indices, values0d, accumulate=True)
        self.assertEqual(out_xpu.cpu(), out_cpu)
        out_xpu = t_dev.index_put_(indices_dev, values1d.to(device), accumulate=True)
        ...
        out_cpu = t.index_put_(indices, values1d, accumulate=True)
        self.assertEqual(out_xpu.cpu(), out_cpu)
        out_xpu = t_dev.index_put_(indices_dev, values2d.to(device), accumulate=True)
    ...
    def test_index_put_accumulate_non_contiguous(self, device):
        ...
        out_xpu = t1.index_put_(indices_dev, value.to(device), accumulate=True)
    ...
    def test_index_put_accumulate_with_optional_tensors(self, device):
        ...
        out_xpu = func(t_dev, indices_dev, value0d.xpu())
        out_cpu = func(t, indices, value0d)
        self.assertEqual(out_xpu.cpu(), out_cpu)
        out_xpu = func(t_dev, indices_dev, value1d.xpu())
