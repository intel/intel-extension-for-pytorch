...
def handle_case_skip(fname, clsname, case, logfile, status):
    ...
    if should_skip_case is True:
        if skip_case is not None:
            ...
            print(
                "Exited: Failed to find case should skip,"
                f" please check logfile {logfile} manually."
            )
            os._exit(1)  # marks fail and abort the process
    else:
        print(
            "Test ran to the end but no case can be skipped."
...
def run_test(fname, clsname, case=None, epoch=1):
    ...
    if case is None or case == "":
        logfile = os.path.join(
            ALL_PATHS["log_path"], f"{real_fname}.{clsname}.{epoch}.log"
        )
    else:
        logfile = os.path.join(
            ALL_PATHS["log_path"], f"{real_fname}.{clsname}.{case}.{epoch}.log"
        )
    ...
    try:
        ...
        if timeout > 0:
            ...
            proc = subprocess.run(
                cmd, capture_output=True, check=True, cwd=script_path, env=os.environ
            )
    ...
    print(
        f"{case_title} ... Finish (status {status}),"
            ...
            if isinstance(test_map[fname], list):
                for clsname in test_map[fname]:
                    ...
                    if matchLine is not None:
                        ...
                        print(
                            f"Skipped: won't run {fname}::{clsname},"
                            f' reason="{reason}"'
                        )
                    elif not run_in_single:
                        p = Process(
                            target=run_test,
                            args=(fname, clsname + "XPU", None, epoch + 1),
                        )
                        p.deamon = True
                        p.start()
...
def main():
    ...
    parser.add_argument("--clean", action="store_true", help="clean raw logs")
    ...
    if "PYTHONPATH" in os.environ.keys():
        ...
        os.environ["PYTHONPATH"] = ":".join(ALL_PATHS.values())
    os.environ["TORCH_TEST_DEVICES"] = os.path.join(
        ALL_PATHS["common_path"], "pytorch_test_base.py"
    )
