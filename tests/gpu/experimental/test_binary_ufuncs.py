...
class TestBinaryUfuncs(TestCase):
    ...
    def test_div_rounding_modes(self, device, dtype):
        ...
        rounding_unsupported = (
            dtype == torch.half and device != 'xpu' or
    ...
    def _test_pow(self, base, exponent, np_exponent=None):
        ...
        try:
            ...
            if isinstance(base, torch.Tensor):
                ...
                actual = base.clone()
                # When base is a 0-dim cpu tensor and exp is a xpu tensor, we exp `pow` to work but `pow_` to fail, since
                # `pow` will try to create the output tensor on a xpu device, but `pow_` needs to use the cpu tensor as the output
                if (isinstance(exponent, torch.Tensor) and base.dim() == 0 and base.device.type == 'cpu' and
                        exponent.device.type == 'xpu'):
    ...
    def test_cpu_tensor_pow_xpu_scalar_tensor(self, device):
        xpu_tensors = [torch.tensor(5.0, device='cuda'), torch.tensor(-3, device='cuda')]
        for exp in xpu_tensors:
            base = torch.randn((3, 3), device='cpu')
            regex = 'Expected all tensors to be on the same device, but found at least two devices, xpu.* and cpu!'
            self.assertRaisesRegex(RuntimeError, regex, torch.pow, base, exp)
        for exp in xpu_tensors:
            # Binary ops with a cpu + xpu tensor are allowed if the cpu tensor has 0 dimension
    ...
    def test_maximum_minimum_cross_device(self, device):
        ...
        # test xpu tensor and cpu scalar
    ...
    @onlyOnCPUAndXPU  # Check Issue https://github.com/pytorch/pytorch/issues/48130
