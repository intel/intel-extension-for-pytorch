...
from torch.testing._internal.common_utils import \
    (IS_FBCODE, IS_SANDCASTLE, IS_WINDOWS, run_tests, skipIfRocm, slowTest,
     parametrize, subtest, instantiate_parametrized_tests, dtype_name)
from torch.testing._internal.common_device_type import \
    (PYTORCH_TESTING_DEVICE_EXCEPT_FOR_KEY, PYTORCH_TESTING_DEVICE_ONLY_FOR_KEY, dtypes,
     get_device_type_test_bases, instantiate_device_type_tests, deviceCountAtLeast, ops)
...
class TestTesting(TestCase):
    ...
    def test__comparetensors_debug_msg(self, device):
        ...
        if self.device_type == 'xpu':
            ...
            expected_msg = ("Attempted to compare equality of tensors "
                            "on different devices! Got devices cpu and xpu:0.")
            self.assertEqual(debug_msg, expected_msg)
    ...
    def test_xpu_assert_should_stop_common_utils_test_suite(self, device):
        # test to ensure common_utils.py override has early termination for XPU.
...
""")
    ...
    def test_xpu_assert_should_stop_common_device_type_test_suite(self, device):
        # test to ensure common_device_type.py override has early termination for XPU.
        stderr = TestCase.runWithPytorchAPIUsageStderr("""\
#!/usr/bin/env python3
...
""")
    ...
    def test_xpu_assert_should_not_stop_common_distributed_test_suite(self, device):
        # test to ensure common_distributed.py override should not early terminate XPU.
        stderr = TestCase.runWithPytorchAPIUsageStderr("""\
#!/usr/bin/env python3
...
if __name__ == '__main__':
    run_tests()
""")
        # we are currently disabling XPU early termination for distributed tests.
