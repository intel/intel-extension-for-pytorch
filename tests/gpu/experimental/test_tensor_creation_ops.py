...
from torch.testing._internal.common_utils import (
    run_tests, do_test_empty_full, TEST_WITH_ROCM, suppress_warnings,
    torch_to_numpy_dtype_dict, skipIfTBB, slowTest,
    TEST_SCIPY, IS_MACOS, IS_PPC, IS_WINDOWS)
from torch.testing._internal.common_device_type import (
    instantiate_device_type_tests, deviceCountAtLeast,
    onlyCPU, precisionOverride, dtypes, skipCPUIf, dtypesIfCPU, skipMeta)
...
class TestTensorCreation(TestCase):
    ...
    def test_cat_out_memory_format(self, device):
        ...
        # - For the XPU variant it only propagates memory format if all the tensors have
        ...
        # a_xpu and b_xpu have different memory_format
        ...
        out_xpu = torch.empty((0), device=device).contiguous(memory_format=torch.contiguous_format)
        # a_xpu and c_xpu have same memory_format
    ...
    def test_cat_different_devices(self, devices):
        xpu0 = torch.randn((3, 3), device=devices[0])
        xpu1 = torch.randn((3, 3), device=devices[1])
        with self.assertRaisesRegex(RuntimeError,
    ...
    def test_float_to_int_conversion_finite(self, device, dtype):
        ...
        # Note: XPU max float -> integer conversion is divergent on some dtypes
    ...
    # Note: XPU will fail this test on most dtypes, often dramatically.
    @onlyCPU
    @dtypes(torch.bool, torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64)
    ...
    # TODO: update to work on XPU, too
    ...
    # TODO: udpate to work on XPU, too
    @onlyCPU
    ...
    @dtypesIfXPU(*get_all_dtypes(include_bool=False, include_half=True))
    ...
    # Our Linspace and logspace torch.half XPU kernels are not very precise.
    ...
    @dtypesIfXPU(*((get_all_int_dtypes() + [torch.float32, torch.float16, torch.bfloat16])
                    if TEST_WITH_ROCM
...
class TestRandomTensorCreation(TestCase):
    ...
    def test_randperm(self, device):
        if device == 'cpu' or device == 'meta':
            ...
            # TODO: This won't actually work for non-XPU device
            # see https://github.com/pytorch/pytorch/issues/54282
            rng_device = [device]

        # Test core functionality. On XPU, different value of n has different
        # code path
    ...
    def test_randperm_device_compatibility(self, device):
        ...
        for n in (1, 3, 100, 30000):
            ...
            # For xpu:0 to match xpu:1, we are making consistent device type matching
            ...
            self.assertRaisesRegex(RuntimeError, regex, lambda: torch.randperm(n, generator=xpu_gen))  # implicitly on CPU
