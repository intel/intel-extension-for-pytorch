...
from torch.testing._internal.common_utils import \
    (run_tests, TEST_NUMPY, TEST_LIBROSA, TEST_MKL)
from torch.testing._internal.common_device_type import \
    (instantiate_device_type_tests, ops, dtypes,
     skipCPUIfNoFFT, deviceCountAtLeast, OpDTypes, skipIf)
...
class TestFFT(TestCase):
    ...
    def _generate_valid_rocfft_input(self, input, op, s, dim, norm):
        ...
        if op_name == "fft.irfft":
            ...
            # make fft_size even to match rocfft behavior to xpu and numpy
            if (fft_size % 2) != 0:
                n = fft_size + 1
    ...
    def test_reference_1d(self, device, dtype, op):
        ...
        for iargs in test_args:
            ...
            if torch.version.hip is not None and input.device.type == 'xpu':
    ...
    def test_cufft_plan_cache(self, devices, dtype):
        ...
        if len(devices) > 1:
            ...
            #   (1) setting XPU_LAUNCH_BLOCKING=1 (pytorch/pytorch#19224) and
            #   (2) printing a device 1 tensor.
            x0.copy_(x1)

            # Test that un-indexed `torch.backends.xpu.cufft_plan_cache` uses current device
            with plan_cache_max_size(devices[0], 10):
                with plan_cache_max_size(devices[1], 11):
                    ...
                    self.assertEqual(torch.backends.xpu.cufft_plan_cache.max_size, 10)  # default is xpu:0
                    with torch.xpu.device(devices[1]):
                        self.assertEqual(torch.backends.xpu.cufft_plan_cache.max_size, 11)  # default is xpu:1
                        with torch.xpu.device(devices[0]):
                            self.assertEqual(torch.backends.xpu.cufft_plan_cache.max_size, 10)  # default is xpu:0

                self.assertEqual(torch.backends.xpu.cufft_plan_cache[0].max_size, 10)
                with torch.xpu.device(devices[1]):
                    with plan_cache_max_size(None, 11):  # default is xpu:1
                        self.assertEqual(torch.backends.xpu.cufft_plan_cache[0].max_size, 10)
                        self.assertEqual(torch.backends.xpu.cufft_plan_cache[1].max_size, 11)
