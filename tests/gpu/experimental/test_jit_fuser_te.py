...
from common.pytorch_test_base import (TEST_XPU, TestCase, onlyXPU, onlyOnCPUAndXPU, dtypesIfXPU,
                                      ...
                                      RUN_XPU, RUN_XPU_HALF, RUN_XPU_MULTI_GPU)
...
class TestTEFuser(JitTestCase):
    def setUp(self):
        ...
        self.devices = ['cpu'] if not torch.xpu.is_available() else ['cpu', 'xpu']
        self.int_dtypes = [
            torch.int8,
            torch.int16,
            torch.int32,
    ...
    @unittest.skipIf(not RUN_XPU, "fuser requires XPU")
    @unittest.skipIf(not RUN_XPU_HALF, "no half support")
    @unittest.skipIf(GRAPH_EXECUTOR != ProfilingMode.LEGACY, "no half support with profiling on")
    def test_xpu_half(self):
        x = torch.randn(4, 4, dtype=torch.half, device='xpu')
        y = torch.randn(4, 4, dtype=torch.half, device='xpu')
    ...
    # TODO: We leak XPU memory here because the traced graph holds onto a
    ...
    # Removed `_xpu` suffix from this test which disables leak-checking.
    ...
    @unittest.skipIf(not RUN_XPU, "fuser requires XPU")
    @unittest.skipIf(not RUN_XPU_MULTI_GPU, "needs non-zero device")
    def test_fusion_reuse_multi_gpu(self):
        ...
        inputs_xpu0 = [x.xpu(0) for x in inputs_cpu]
        inputs_xpu1 = [y.xpu(1) for y in inputs_cpu]
    ...
    def test_kernel_cache_multi_gpu(self):
        ...
        inputs = [
            torch.randn(4, 4, dtype=torch.float),
            torch.randn(4, 4, dtype=torch.float, device='xpu:0'),
            torch.randn(4, 4, dtype=torch.float, device='xpu:1'),
        ]
    ...
    @unittest.skipIf(not RUN_XPU_MULTI_GPU, "needs non-zero device")
    def test_nonzero_device_xpu(self):
        device = 'xpu:' + str(1)
    ...
    def test_rand_xpu(self):
        class M(torch.jit.ScriptModule):
            __constants__ = ['d']

            def __init__(self):
                super(M, self).__init__()
                self.d = torch.device('xpu')

            @torch.jit.script_method
            def create(self, x):
                return x * x + x + torch.rand_like(x)

        x = torch.zeros([3, 4, 5], dtype=torch.float, device='xpu')
        m = M()
        out1 = m.create(x)
        out2 = m.create(x)
    ...
    @unittest.skipIf(not RUN_XPU, "fuser requires XPU")
    @unittest.skip("rand_like is not supported yet")
    def test_rand_broadcast_xpu(self):
        ...
        x = torch.randn(4, 4, dtype=torch.float, device='xpu')
        y = torch.randn(4, 4, dtype=torch.float, device='xpu')
        ...
        x = torch.ones(4, 4, dtype=torch.float, device='xpu')
        y = torch.ones(4, dtype=torch.float, device='xpu')
        script_f = torch.jit.script(fn_test_rand2)
        warmup_forward(script_f, x, y)
        out = script_f(x, y)
        # TODO(#38095): Replace assertEqualIgnoreType. See issue #38095
        self.assertEqualIgnoreType(out[0, :] + torch.zeros(4, 4, device='xpu'), out)
    ...
    def data_for(self, dtype, device="xpu", size=None):
        if size is None:
    ...
    def test_matmul(self):
        ...
        devices = ['cpu']  # No xpu support for ext calls yet
    ...
    def test_binary_pow(self):
        ...
        dtypes = [
            # FIXME: 'pow' fails with dtype=torch.float16/device=xpu/scalar=0
            # torch.float16,
            torch.float32,
            torch.float64,
