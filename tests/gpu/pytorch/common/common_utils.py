...
def run_tests(argv=UNITTEST_ARGS):
    if TEST_IN_SUBPROCESS:
        ...
        if IN_CIRCLECI:
            ...
            unittest.main(
                argv=argv, testRunner=xmlrunner.XMLTestRunner(output=test_report_path)
            )
        else:
    ...
    if isinstance(obj, torch.Tensor):
        ...
        with torch.no_grad():
            ...
            if obj.dtype != torch.int64:
                ...
                ).requires_grad_()  # That makes the dpcpp tensor to be leaf
            else:
                res = res.to(
                    device="xpu"
                )  # keep input datatype int64 for some special case, like EmbeddingBag needs long input
        return res
    elif torch.is_storage(obj):
        return obj.new().resize_(obj.size()).copy_(obj)
    elif isinstance(obj, list):
        return [to_dpcpp(o, type_map) for o in obj]
    elif isinstance(obj, tuple):
        return tuple(to_dpcpp(o, type_map) for o in obj)
    else:
        return deepcopy(obj)
...
class TestCase(expecttest.TestCase):
    ...
    def setUp(self):
        ...
        print("In common_utils.py, we set seed = ", SEED)
        set_rng_seed(SEED)
...
def random_sparse_pd_matrix(matrix_size, density=0.01, **kwargs):
    ...
    return torch.sparse_coo_tensor(
        indices, values, (matrix_size, matrix_size), dtype=dtype, device=device
    )
