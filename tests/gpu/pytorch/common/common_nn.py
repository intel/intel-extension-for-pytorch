...
new_module_tests = [
    ...
    # SkipDPCPP, Can not access null pointer
    # dict(
...
class TestBase(object):
    ...
    def _unpack(self, value):
        if isinstance(value, torch.Tensor):
            print("In _unpack, return value = ", value)
            return value
        elif is_iterable(value):
            print("In _unpack,type(value), value = ", value)
            return type(value)(self._unpack(v) for v in value)
        else:
            print("In _unpack, else return value = ", value)
    ...
    def _get_arg(self, name, unpack):
        if name == "bfloat16_prec" and "bfloat16_prec" in self._extra_kwargs:
            return self._extra_kwargs[name]
        elif name == "bfloat16_prec":
            return 0
        else:
            pass
...
class ModuleTest(TestBase):
    ...
    def test_noncontig(self, test_case, module, input):
        ...
        with freeze_rng_state():
            ...
            output = output.clone()
            d_grad = test_case._backward(
                module, input, output, grad_output
            )  # dpcppTensor cannot call self storage(), so there is a workaround
            d_grad_copy = deepcopy(
                d_grad.cpu() if d_grad != None else d_grad
            )  # d_grad may sometime be None, for example, Embedding backward grad
        ...
        for contig_i, contig_g in product((True, False), repeat=2):
            ...
            if grad_output.device == torch.device(
                type="cpu"
            ):  # cpuTensor has storage method, so we don't need to make a workaround
                go = deepcopy(grad_output if contig_g else nc_grad_output)
            else:
                go = deepcopy(
                    grad_output.cpu() if contig_g else nc_grad_output.cpu()
                ).to(
                    "xpu"
                )  # dpcppTensor cannot call self storage(), so there is a workaround
            test_case._zero_grad_parameters(module)
    ...
    def test_dpcpp(self, test_case):  # DUDU
        ...
        try:
            ...
            print(
                "****************************** In test dpcpp, cpu input dtype = ",
                cpu_input.dtype,
            )
            gpu_input = to_dpcpp(cpu_input)
            print(
                "****************************** In test dpcpp, gpu input dtype = ",
                gpu_input.dtype,
            )
            ...
            if gpu_input.type() != "torch.dpcpp.HalfTensor":
                ...
                for _ in range(5):
                    ...
                    gpu_gradOutput = cpu_gradOutput.to(
                        device="xpu", dtype=torch.float32
                    )
...
class CriterionTest(TestBase):
    ...
    def __call__(self, test_case):
        ...
        if self.reference_fn is not None:
            ...
            print(
                "In CriterionTest, reference_fn has generated expected output=",
                expected_out.cpu(),
            )
...
class NewModuleTest(InputVariableMixin, ModuleTest):
    ...
    def _do_test(self, test_case, module, input):  # DUDU
    ...
    def test_dpcpp(self, test_case, dtype=None, extra_args=None):
        ...
        try:
            ...
            if not isinstance(cpu_input, tuple):
                ...
                print(
                    "************************************************ cpu input requires grad = ",
                    cpu_input.requires_grad,
                )
                print(
                    "************************************************ cpu target requires grad = ",
                    cpu_target.requires_grad,
                )
                ...
                print(
                    "************************************************ gpu input requires grad = ",
                    gpu_input.requires_grad,
                )
                print(
                    "************************************************ gpu target requires grad = ",
                    gpu_target.requires_grad,
                )
            ...
            prec = (
                ...
                else self._get_bfloat16_prec()
            )
            if prec == 0:
                prec = 1e-1  # dtype = BF16 & FP16
            # print('finally prec = ', prec)
            print(
                "************************************************ output cpu - output dpcpp = ",
                cpu_output - gpu_output.cpu().to(cpu_output.dtype),
            )
            print("************************************************ prec = ", prec)
            test_case.assertEqual(cpu_output, gpu_output, prec)

            if (
                dtype != torch.half and not self._get_skip_grad_check()
            ):  # some UT has gap between CPU grad and GPU grad caused by dtype, so we skip some UT.
                cpu_gradInput = test_case._backward_criterion(
                    cpu_module, cpu_input, cpu_target, extra_args=extra_args
                )
                gpu_gradInput = test_case._backward_criterion(
