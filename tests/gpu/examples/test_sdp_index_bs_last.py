...
class TestTorchMethod(TestCase):
    ...
    def test_fsdp_index_select(self, dtype=torch.float16):
        ...
        ref = torch.xpu.IpexSDP(q.permute(0, 2, 1, 3), k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3),
                                alibi, attn_mask_padded, head_mask,
                                alpha, beta, dropout, is_causal, seq_last)
        # sdp index fusion op is on SequenceLast by default
        res = torch.xpu.IpexSDP_Index(q.permute(0, 2, 1, 3), k_in_proj.permute(0, 2, 1, 3),
                                      v_in_proj.permute(0, 2, 1, 3), k_cache[0:t_out, :, :, :].permute(1, 2, 0, 3),
                                      v_cache[0:t_out, :, :, :].permute(1, 2, 0, 3), index, alibi, attn_mask_padded,
                                      head_mask, t_out, alpha, beta, dropout, is_causal)
