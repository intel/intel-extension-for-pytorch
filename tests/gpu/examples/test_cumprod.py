...
class TestTorchMethod(TestCase):
    def test_cumprod_1d_dim0(self, dtype=torch.float):
        ...
        self.assertEqual(
            torch.cumprod(x1, dim=0),
            torch.cumprod(x1_dpcpp, dim=0).to(cpu_device).to(torch.float),
            rtol=1.3e-6,
            atol=1e-5,
        )
    ...
    def test_cumprod_2d_dim0(self, dtype=torch.float):
        ...
        self.assertEqual(
            torch.cumprod(x2, dim=0),
            torch.cumprod(x2_dpcpp, dim=0).to(cpu_device).to(torch.float),
            rtol=1.3e-6,
            atol=1e-5,
        )
    ...
    def test_cumprod_2d_dim1(self, dtype=torch.float):
        ...
        x2_dpcpp = x2.to(dpcpp_device).to(dtype)
        self.assertEqual(
            torch.cumprod(x2, dim=1),
            torch.cumprod(x2_dpcpp, dim=1).to(cpu_device).to(torch.float),
            rtol=1.3e-6,
            atol=1e-5,
        )
    ...
    def test_cumprod_3d_dim2(self, dtype=torch.float):
        ...
        x2_dpcpp = x2.to(dpcpp_device).to(dtype)
        self.assertEqual(
            torch.cumprod(x2, dim=2),
            torch.cumprod(x2_dpcpp, dim=2).to(cpu_device).to(torch.float),
            rtol=1.3e-6,
            atol=1e-5,
        )
