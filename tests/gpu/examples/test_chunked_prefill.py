...
class TestChunkedPrefill(TestCase):
    ...
    def construct_local_mask(
        ...
        if window_size[0] < 0:
            ...
            right = (
                sk
                if window_size[1] < 0
                else torch.minimum(row_idx + sk - sq + window_size[1], sk)
            )
    ...
    def ref_chunked_prefill(
        ...
        attn_mask = torch.empty(
            [num_batch, 1, max_seqlen_q, max_seqlen_k], device="xpu"
        ).fill_(float("-inf"))
        ...
        if window_size != (-1, -1):
            for i in range(num_batch):
                ...
                qk_mask[i] = qk_mask[i].logical_and(local_mask.logical_not())
