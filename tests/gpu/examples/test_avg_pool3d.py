...
class TestNNMethod(TestCase):
    ...
    def test_avg_pool3d_non_overlapping(self, dtype=torch.float):
        x_cpu = torch.ones([1, 8, 8, 24, 24], device=cpu_device)
        grad_cpu = torch.ones([1, 8, 8, 24, 24], device=cpu_device)

        avg_pool = nn.AvgPool3d(kernel_size=3, stride=3, padding=1)

        # cpu
        x_cpu.requires_grad_(True)
        y_cpu = avg_pool(x_cpu)
        # print("y_cpu", y_cpu)
        y_cpu.backward(torch.ones([1, 8, 3, 8, 8], device=cpu_device))
        # print("y_cpu backward", x_cpu.grad)
        ...
        # grad_dpcpp = grad_cpu.to("xpu")
        y_dpcpp.backward(torch.ones([1, 8, 3, 8, 8], device=dpcpp_device))
        # print("y_dpcpp backward", x_dpcpp.grad.cpu())
        self.assertEqual(x_cpu.grad, x_dpcpp.grad.to(cpu_device))

    def test_avg_pool3d_stride1_pad0(self, dtype=torch.float):
        x_cpu = torch.ones([1, 8, 8, 24, 24], device=cpu_device)
        grad_cpu = torch.ones([1, 8, 8, 24, 24], device=cpu_device)
        ...
        y_cpu = avg_pool(x_cpu)
        # print("y_cpu", y_cpu)
        y_cpu.backward(torch.ones([1, 8, 6, 22, 22], device=cpu_device))
        # print("y_cpu backward", x_cpu.grad)
        ...
        # grad_dpcpp = grad_cpu.to("xpu")
        y_dpcpp.backward(torch.ones([1, 8, 6, 22, 22], device=dpcpp_device))
        # print("y_dpcpp backward", x_dpcpp.grad.cpu())
        self.assertEqual(x_cpu.grad, x_dpcpp.grad.to(cpu_device))
