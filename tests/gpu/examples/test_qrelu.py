...
class TestNNMethod(TestCase):
    ...
    def test_QReLU_u8(self, dtype=torch.float):
        ...
        self.assertEqual(
            torch.dequantize(q_inputs),
            torch.dequantize(q_inputs_gpu),
            "Input is not quantized equal",
        )
        ...
        self.assertEqual(
            torch.dequantize(q_filters),
            torch.dequantize(q_filters_gpu),
            "Weight is not quantized equal",
        )
        ...
        np.testing.assert_almost_equal(
            torch.dequantize(res_cpu).numpy(),
            torch.dequantize(res_gpu).cpu().numpy(),
            decimal=0,
        )
