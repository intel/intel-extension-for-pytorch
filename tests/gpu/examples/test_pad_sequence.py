...
class TestNNMethod(TestCase):
    def test_pad_sequence(self, dtype=torch.float):
        for batch_first in [True]:
            ...
            output = torch.nn.utils.rnn.pad_sequence((a, b, c), batch_first, padding_value=91)

            a_xpu = a.to(dpcpp_device)
            b_xpu = b.to(dpcpp_device)
            c_xpu = c.to(dpcpp_device)
            output_xpu = torch.nn.utils.rnn.pad_sequence((a_xpu, b_xpu, c_xpu), batch_first, padding_value=91)
