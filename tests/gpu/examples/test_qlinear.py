...
class TestTorchMethod(TestCase):
    ...
    def test_qlinear(self, dtype=torch.float):
        ...
        output_gpu_int8 = torch.ops.quantized.linear(q_inputs_gpu, packed_params_gpu, output_sc, 0)
        print(output_gpu_int8)
        # print(torch.round(output_gpu_int8 / output_sc))

        #  Align with FBGEMM, which output with UInt8.
        output_gpu_int8 = torch.quantize_per_tensor(output_gpu_int8, output_sc, zero_point, torch.quint8)
        print(output_gpu_int8.int_repr())

        self.assertEqual(output_int8.int_repr(), output_gpu_int8.int_repr(), rtol=2, atol=2)
