    ...
    def test_qlinear(self, dtype=torch.float):
        ...
        for shape in input_shape:
            for zp_in in zp_vec:
                ...
                q_filters = torch.quantize_per_tensor(filters, 0.5, zp_filter, dtype_filters)

                packed_params = torch.ops.quantized.linear_prepack(q_filters, bias)
                output_int8 = torch.ops.quantized.linear(q_inputs, packed_params, output_sc, output_zp)
                ...
                q_inputs_gpu = torch.quantize_per_tensor(inputs_gpu, 0.4, zp_in, dtype_inputs)
                q_filters_gpu = torch.quantize_per_tensor(filters_gpu, 0.5, zp_filter, dtype_filters)
                ...
                self.assertEqual(torch.dequantize(output_gpu_int8), torch.dequantize(output_int8))
