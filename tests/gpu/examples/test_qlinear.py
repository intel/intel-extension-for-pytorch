...
class TestTorchMethod(TestCase):
    ...
    def test_qlinear_perchannel(self, dtype=torch.float):
        print(
            "Please open FBGEMM (PyTorch CPU INT8 default engine ) when build PyTorch, "
            "which will be referenced by GPU. Or you may meet runtime error like "
            "'Didn't find engine for operation quantized::linear_prepack'."
        ...
        for shape in input_shape:
            for zp_in in zp_vec:
                inputs = torch.randn(shape)
                filters = torch.randn(5, 5)
                filter_scale = torch.empty((5), dtype=torch.float).fill_(0.5)
                filter_zero_point = torch.empty((5), dtype=torch.int32).fill_(0)
                # bias = torch.randn(5)
                bias = None
                output_sc = 0.5
                output_zp = 2
                ...
                inputs_gpu = inputs.to("xpu")
                filters_gpu = filters.to("xpu")
                filters_scale_gpu = filter_scale.to("xpu")
                filters_zero_point_gpu = filter_zero_point.to("xpu")
                # bias_gpu = bias.to("xpu")
                bias_gpu = None
