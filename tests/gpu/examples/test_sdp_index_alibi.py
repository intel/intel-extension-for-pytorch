...
class TestTorchMethod(TestCase):
    def test_fsdp_index_alibi(self, dtype=torch.float16):
        ...
        ref_cpu = torch.bmm(attn_probs_reshaped, v.cpu().float().permute(0, 2, 1, 3).contiguous().view(-1, t, h)).view(b, n, f, h)
        ...
        ref = torch.xpu.IpexSDP(q.permute(0, 2, 1, 3), k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3), alibi_sdp, attn_mask_padded, head_mask, alpha, beta, dropout, is_causal, seq_last)
        ref_no_alibi = torch.xpu.IpexSDP(q.permute(0, 2, 1, 3), k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3), None, attn_mask_padded, head_mask, alpha, beta, dropout, is_causal, seq_last)
        # sdp index fusion op is on SequenceLast by default
        res = torch.xpu.IpexSDP_Index(q.permute(0, 2, 1, 3), k_in_proj.permute(0, 2, 1, 3), v_in_proj.permute(0, 2, 1, 3), k_cache[0:t_out,:,:,:].permute(1, 2, 0, 3), v_cache[0:t_out,:,:,:].permute(1, 2, 0, 3), index, alibi_sdp, attn_mask_padded, head_mask, t_out, alpha, beta, dropout, is_causal)
