...
def expected_fn(in_data, rois, pool_h, pool_w, spatial_scale=1, sampling_ratio=-1,
                aligned=False,
                device=None,
                dtype=torch.float64,
                ):
    ...
    out_data = torch.zeros(rois.size(0), n_channels,
                           pool_h, pool_w, dtype=dtype, device=device)
    ...
    for r, roi in enumerate(rois):
        ...
        for i in range(0, pool_h):
            ...
            grid_h = sampling_ratio if sampling_ratio > 0 else int(
                np.ceil(bin_h))
