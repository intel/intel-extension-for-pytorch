...
class TestTorchMethod(TestCase):
    def test_activation(self, dtype=torch.float):
        conv1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False).to(
            "xpu"
        )
        conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False).to(
            "xpu"
        )
    ...
    def test_weight_channels_last(self, dtype=torch.float):
        ...
        ref = (
            conv1.weight.detach().clone().contiguous(memory_format=torch.channels_last)
        )
        conv1.weight.data = conv1.weight.data.contiguous(
    ...
    def test_group_conv(self, dtype=torch.float):
        conv1 = nn.Conv2d(
            64, 64, kernel_size=3, stride=1, padding=1, groups=2, bias=False
        ).to("xpu")
