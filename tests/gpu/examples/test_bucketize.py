    ...
    def test_searchsorted(self):
        ...
        for dtype in dtypes:
            ...
            searchsorted_result_xpu = torch.searchsorted(
                sorted_sequence.to("xpu").to(dtype), values.to("xpu").to(dtype)
            )
            ...
            bucketize_result_xpu = torch.bucketize(
                v.to("xpu").to(dtype), boundaries.to("xpu").to(dtype)
            )
            ...
            searchsorted_result_xpu = torch.searchsorted(
                x.to("xpu").to(dtype), y.to("xpu").to(dtype)
            )
            self.assertEqual(searchsorted_result_cpu, searchsorted_result_xpu)

            bucketize_result_cpu = torch.bucketize(y, x)
            bucketize_result_xpu = torch.bucketize(
                y.to("xpu").to(dtype), x.to("xpu").to(dtype)
            )
