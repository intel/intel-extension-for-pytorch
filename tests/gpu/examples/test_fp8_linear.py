...
from intel_extension_for_pytorch.xpu.fp8.recipe import DelayedScaling, Format
from torch.testing._internal.common_utils import TestCase


class TestFP8GEMM(TestCase):
    def test_fp8_linear_E5M2(self):
        ...
        # print("gw = ", gw)
        # print("gw_ref = ", gw_ref)
        self.assertEqual(output.bfloat16(), output_ref, rtol=1e-1, atol=1e-2)
        self.assertEqual(gd, gd_ref, rtol=1e-1, atol=1e-2)
        self.assertEqual(gw, gw_ref, rtol=1e-1, atol=1e-2)

    def test_fp8_linear_E4M3(self):
        dtype = torch.bfloat16
        input = torch.ones(
            [8, 2], requires_grad=True, dtype=dtype, device=torch.device("xpu")
        )
