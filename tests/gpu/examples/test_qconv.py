...
class TestTorchMethod(TestCase):
    ...
    def test_qconv_simple_channels_last(self, dtype=torch.float):
        ...
        output_int8 = torch.ops.quantized.conv2d_relu(
            q_inputs, packed_params, _pair(1), _pair(0), _pair(1), 1, scale_out, zero_point)
        ...
        q_inputs_gpu = torch.quantize_per_tensor(
            inputs_gpu, scale_in, zero_point, dtype_inputs).to(memory_format=torch.channels_last)
    ...
    def test_qconv(self, dtype=torch.float):
        ...
        output_int8_2 = torch.ops.quantized.conv2d_relu(
            q_inputs, packed_params, _pair(1), _pair(0), _pair(1), 1, scale_out_2, zero_point)
