...
def _get_analytical_jacobian_forward_ad_backward(func, inputs, tupled_grad_outputs
                                                 ) -> Tuple[Tuple[torch.Tensor, ...], ...]:
    ...
    return _get_analytical_jacobian_forward_ad(new_func, new_inputs, _as_tuple(new_func(*new_inputs)))
...
class TestNNMethod(TestCase):
    ...
    def test_glu_jvp(self, dtype=torch.float):
        ...
        cpu_out = _get_analytical_jacobian_forward_ad(model, _as_tuple(input_cpu), _as_tuple(model(input_cpu)))
        xpu_out = _get_analytical_jacobian_forward_ad(model, _as_tuple(input_xpu), _as_tuple(model(input_xpu)))
        ...
        test_cpu = _get_analytical_jacobian_forward_ad_backward(model, input_cpu, tupled_grad_outputs)
        test_xpu = _get_analytical_jacobian_forward_ad_backward(model, input_xpu, tupled_grad_outputs_xpu)
        self.assertEqual(cpu_out[0][0], xpu_out[0][0].cpu())
