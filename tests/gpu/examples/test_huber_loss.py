...
class TestNNMethod(TestCase):
    def test_huber_loss(self, dtype=torch.float):
        ...
        def _test_cpu(input, target, reduc):
            loss = nn.HuberLoss(reduction=reduc)
        ...
        def _test_dpcpp(input, target, reduc):
            ...
            if reduc == "none":
                output.backward(
                    torch.ones_like(input, dtype=torch.float).to(dpcpp_device)
                )
            else:
                output.backward(torch.tensor((1.0), dtype=torch.float).to(dpcpp_device))
        ...
        print("xpu")
        input_dpcpp, output_dpcpp = _test_dpcpp(input_dpcpp, target_dpcpp, "none")
        ...
        print("xpu")
        input_dpcpp, output_dpcpp = _test_dpcpp(input_dpcpp, target_dpcpp, "sum")
        self.assertEqual(input_cpu, input_dpcpp.to(cpu_device))
        self.assertEqual(output_cpu, output_dpcpp.to(cpu_device))
        ...
        print("xpu")
        input_dpcpp, output_dpcpp = _test_dpcpp(input_dpcpp, target_dpcpp, "mean")
        self.assertEqual(input_cpu, input_dpcpp.to(cpu_device))
        self.assertEqual(output_cpu, output_dpcpp.to(cpu_device))
