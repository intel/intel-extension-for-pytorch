...
class TestNNMethod(TestCase):
    def test_vallina_transducer_loss(self):
        ...
        alpha, beta, grad_ref, loss_ref = transducer_loss_reference(x=x_ref,
                                                                    label=y,
                                                                    f_len=f_len,
                                                                    y_len=y_len,
                                                                    blank_idx=blank_idx,
        ...
        loss_xpu = my_loss(x=x_xpu, label=y_xpu, f_len=f_len_xpu, y_len=y_len_xpu, blank_idx=blank_idx)
    ...
    def test_conv_vallina_transducer_loss(self):
        ...
        x_tst = torch.randn([B, T_max, U_max, V], dtype=torch.float, device="cpu", requires_grad=True)
        x_ref = x_tst.data.clone()
        x_ref.requires_grad = True
        conv_cpu = torch.nn.Conv2d(T_max, T_max, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        # Generate reference
        loss_grad = torch.ones(x_ref.size(0), dtype=x_ref.dtype, device=x_ref.device) / x_ref.size(0)
        alpha, beta, grad_ref, loss_ref = transducer_loss_reference(x=output_cpu,
                                                                    label=y,
                                                                    f_len=f_len,
                                                                    y_len=y_len,
                                                                    blank_idx=blank_idx,
        ...
        y_len_xpu = y_len.xpu()
        my_loss = torch.xpu.TransducerLoss()
        loss_xpu = my_loss(x=output_xpu, label=y_xpu, f_len=f_len_xpu, y_len=y_len_xpu, blank_idx=blank_idx)
        loss_xpu.mean().backward()
