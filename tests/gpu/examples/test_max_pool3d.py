...
class TestNNMethod(TestCase):
    def test_max_pool3d(self, dtype=torch.float):
        x_cpu = torch.randn([12, 16, 16, 12, 12], device=cpu_device, dtype=dtype)
        grad_cpu = torch.randn([12, 16, 16, 12, 12], device=cpu_device, dtype=dtype)
    ...
    def test_max_pool3d_5D_blk_format(self, dtype=torch.float):
