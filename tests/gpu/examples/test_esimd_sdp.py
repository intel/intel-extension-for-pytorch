...
f = 1
t_in = 3000  # history tokens n_kvs
t_out = 1
t = t_in + t_out  # n_kvs
# t_max = 2176
beam = 1
...
class TestTorchMethod(TestCase):
    ...
    #     )  # 1, 32, 32, 128
    #     v = (
    #         torch.randn([b, t, n, h], dtype=dtype, device=torch.device("xpu"))
    #     )  # 1, 32, 32, 128
    ...
    #     naive, _ = naive_sdp(
    #         q.permute(0, 2, 1, 3).to(torch.float).to("cpu"),
    #         k.permute(0, 2, 1, 3).to(torch.float).to("cpu"),
    #         v.permute(0, 2, 1, 3).to(torch.float).to("cpu"),
    ...
    #     print("naive: ", naive.to(torch.float).to("cpu"))
    ...
    #     print("esimd: ", ref.to(torch.float).to("cpu"))

    #     print(
    #         "esimd sdp vs naive: ", torch.max(torch.abs(ref.cpu() - naive.cpu())).item()
    #     )

    #     print("ref[0][22][0][63] = ", ref[0][22][0][63].to(torch.float).to("cpu"))
    #     print("naive[0][22][0][63] = ", naive[0][22][0][63].to(torch.float).to("cpu"))

    #     self.assertEqual(
    #         ref.to(torch.float).to("cpu"),
    #         naive.to(torch.float).to("cpu"),
    #         atol=1e-2,
    #         rtol=1e-3,
    #     )
    ...
    # def test_esmid_sdp_perf(self, dtype=torch.float16):
    #     print("\nf(n_q) t(n_kv) b(beam) n(head) h(hid dim) are:")
    #     print(f)
    #     print(t)
    #     print(b)
    ...
    #         q = torch.randn(
    #             [b, f, n, h], dtype=dtype, device=torch.device("xpu")
    #         )  # 1, 1, 32, 128
    #         k = torch.randn(
    #             [b, t, n, h], dtype=dtype, device=torch.device("xpu")
    #         )  # 1, 32, 32, 128
