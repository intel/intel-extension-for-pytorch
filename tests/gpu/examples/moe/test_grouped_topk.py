...
class TestTorchMethod:
    ...
    @pytest.mark.parametrize("n_topk", [4, 8])
    @pytest.mark.parametrize("n_topk_group", [4, 8])
    ...
    def test_grouped_topk_sigmoid(
        ...
        ref_topk_weights, ref_topk_indices, ref_max_buf, ref_score_buf = (
            self.ref_grouped_topk_sigmoid(
                gating_output, n_topk, renormalize, n_expert_group, n_topk_group, bias
            )
        )
        topk_weights, topk_indices = torch.ops.torch_ipex.grouped_topk_sigmoid(
            hidden_states,
            gating_output,
            n_topk,
            renormalize,
        ...
        ref_topk_weights_sorted, _ = torch.sort(ref_topk_weights, -1)
        topk_weights_sorted, _ = torch.sort(topk_weights, -1)
        ref_topk_indices_sorted, _ = torch.sort(ref_topk_indices, -1)
        topk_indices_sorted, _ = torch.sort(topk_indices, -1)
        # Compare the results
