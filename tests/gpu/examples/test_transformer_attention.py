...
class TestTransformers(NNTestCase):
    def test_scaled_dot_product_attention(self, dtype=torch.float):
        for (input_dim,attn_mask_dim,is_causal) in [(3, None, False), (3, 2, False), (3, 3, False),(4, None, False), (3, None, True), (4, None, True)]:
            ...
            if attn_mask_dim is not None:
                ...
                attn_mask_cpu = (torch.ones(mask_size, dtype=torch.bool).tril() if is_causal
                                else torch.randint(0, 2, size=mask_size, dtype=torch.bool))
            
            attn_mask_xpu = None if attn_mask_dim is None else attn_mask_cpu.to(xpu_device)
            
            dropout_p = 0
            need_attn_weights = True

            output_cpu, attn_cpu = torch.ops.aten._scaled_dot_product_attention(
            query_cpu, key_cpu, value_cpu, attn_mask_cpu, dropout_p, need_attn_weights, is_causal)

            output_xpu, attn_xpu = torch.ops.aten._scaled_dot_product_attention(
            query_xpu, key_xpu, value_xpu, attn_mask_xpu, dropout_p, need_attn_weights, is_causal)
