...
class TestFloat8(TestCase):
    ...
    def test_e4m3fn_casts(self):
        for dtype in (torch.float32, torch.float16):
            x = torch.randn(16, dtype=torch.float).xpu()
            x_fp8 = x.to(torch.float8_e4m3fn).xpu()
            x_orig_dtype = x_fp8.to(torch.float).xpu()
    ...
    def test_e4m3fn_numerics(self):
        ...
        def _compare(bits_str, expected_fp32, comp_name):
            ...
            tensor_int = torch.tensor([bits_int], dtype=torch.uint8).xpu()
            tensor_fp8 = tensor_int.view(torch.float8_e4m3fn)
            tensor_fp32 = tensor_fp8.float()  # .xpu()
    ...
    def test_e5m2fn_casts(self):
        for dtype in (torch.float32, torch.float16):
            x = torch.randn(16, dtype=torch.float).xpu()
            x_fp8 = x.to(torch.float8_e5m2).xpu()
            x_orig_dtype = x_fp8.to(torch.float).xpu()
