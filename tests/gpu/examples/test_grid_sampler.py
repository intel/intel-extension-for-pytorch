...
class TestTorchMethod(TestCase):
    ...
    def test_gridSampler(self, dtype=torch.float):
        ...
        inp_xpu = inp.to("xpu")
        outp_xpu = F.grid_sample(
            inp_xpu, grid=grid_xpu, mode="bilinear", align_corners=True
        )
    ...
    def test_gridSampler_3d(self, dtype=torch.float):
        ...
        out_cpu = F.grid_sample(
            input_cpu, grid_cpu, mode="bilinear", align_corners=True
        )
    ...
    def test_gridSampler_2d_Bicubic(self, dtype=torch.float):
        ...
        inp_xpu = input_cpu.to("xpu")
        out_xpu = F.grid_sample(
            inp_xpu, grid=grid_xpu, mode="bicubic", align_corners=True
        )
        self.assertEqual(out_cpu.cpu(), out_xpu.cpu())
