...
w = torch.tensor([1, 10, 3, 2], dtype=torch.float) # create a tensor of weights
...
print("x[0] replacement=True", x[0].cpu().bincount().unsqueeze(0).float().renorm(1, 0, 1))
print("x[200] replacement=True", x[200].cpu().bincount().unsqueeze(0).float().renorm(1, 0, 1))
print("x[999] replacement=True", x[999].cpu().bincount().unsqueeze(0).float().renorm(1, 0, 1))

x = torch.multinomial(weights_dpcpp, 4, replacement=False)
x = x.transpose(0, 1).to(cpu_device)
print("x.transpose(0,1)[0] replacement=False", x[0].bincount().unsqueeze(0).float().renorm(1, 0, 1))
