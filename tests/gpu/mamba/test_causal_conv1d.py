...
@pytest.mark.parametrize("silu_activation", [True])
@pytest.mark.parametrize("has_bias", [True])
@pytest.mark.parametrize("has_initial_state", [False])
...
def test_causal_conv1d(batch, dim, seqlen, width, has_bias, silu_activation,
    ...
    out, _ = ipex.llm.modules.MambaMixer.causal_conv1d_fn(x,
                           weight,
                           bias,
                           initial_states,
...
def test_causal_conv1d_update(dim, width, seqlen, has_bias, silu_activation,
    ...
    activation = None if not silu_activation else "silu"
    out = ipex.llm.modules.MambaMixer.causal_conv1d_update(x,
                               conv_state,
                               weight,
                               bias,
                               activation=activation)
