...
y_cpu = x_cpu.masked_fill(mask=torch.ByteTensor([1, 1, 0, 0]), value=torch.tensor(-1e9))
y_sycl = x_sycl.masked_fill(mask=torch.ByteTensor([1, 1, 0, 0]).to("dpcpp"), value=torch.tensor(-1e9))
