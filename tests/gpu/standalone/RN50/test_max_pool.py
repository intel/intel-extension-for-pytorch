...
class TestNNMethod(TestCase):
    def test_max_pool(self, dtype=torch.float):
        ...
        conv1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def test_channels_last_global_max_pool(self, dtype=torch.float):
        x_cpu = torch.randn([1, 64, 112, 112], device="cpu", dtype=dtype)
        grad_cpu = torch.randn([1, 64, 112, 1], device="cpu", dtype=dtype)
        x_dpcpp = x_cpu.to("xpu").to(memory_format=torch.channels_last)
        grad_dpcpp = grad_cpu.to("xpu")

        for stride in [(1, 112), []]:
            max_pool = nn.MaxPool2d(
                kernel_size=(1, 112), stride=stride, padding=0, return_indices=True
            )
    ...
    def test_max_pool_blk_format(self, dtype=torch.float):
        x = torch.randn([1, 64, 112, 112])
        grad = torch.randn([1, 64, 112, 112])
        conv_cpu1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)
        pool_cpu = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        conv_cpu2 = nn.Conv2d(64, 64, kernel_size=1, stride=1, bias=False)
