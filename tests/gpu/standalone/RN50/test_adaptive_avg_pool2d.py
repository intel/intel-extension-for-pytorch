...
class TestNNMethod(TestCase):
    ...
    def test_adative_avg_pool2d_blk_format(self, dtype=torch.float):
        x = torch.randn([1, 2048, 7, 7])
        grad = torch.randn([1, 2048, 2, 2])
        conv_cpu1 = nn.Conv2d(2048, 2048, kernel_size=3, stride=1, padding=1, bias=False)
        pool_cpu = nn.AdaptiveAvgPool2d((2, 2))
        conv_cpu2 = nn.Conv2d(2048, 2048, kernel_size=1, stride=1, bias=False)
    ...
    def test_adaptive_avg_pool2d_general_fwd(self, dtype=torch.float):
        x_cpu = torch.randn([1, 2048, 7, 7], device=cpu_device)
        output_sz = [2, 2]
        ...
        x_cpu = torch.randn([1, 2048, 7, 7], device=cpu_device).to(
    ...
    def test_adaptive_avg_pool2d_general_bwd(self, dtype=torch.float):
        x_cpu = torch.randn([1, 2048, 7, 7], device=cpu_device).requires_grad_(True)
        ...
        x_cpu = (
            torch.randn([1, 2048, 7, 7], device=cpu_device)
