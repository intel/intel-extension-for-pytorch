...
class TestParamSharing(TestCase):
    def test_param_shared(self):
        ...
        def check_shared_in_model(model, dtype):
            self.assertEqual(model.shared.weight.data_ptr(), model.encoder.weight.data_ptr())
            self.assertEqual(model.shared.weight.data_ptr(), model.decoder.weight.data_ptr())
            self.assertEqual(model.shared.weight.data_ptr(), model.linear.weight.data_ptr())
            self.assertEqual(model.shared.weight.dtype, dtype)

        def check_shared_in_state_dict(state_dict):
            self.assertEqual(state_dict['shared.weight'].data_ptr(), state_dict['encoder.weight'].data_ptr())
            self.assertEqual(state_dict['shared.weight'].data_ptr(), state_dict['decoder.weight'].data_ptr())
            self.assertEqual(state_dict['shared.weight'].data_ptr(), state_dict['linear.weight'].data_ptr())
        ...
        def test_inference(model):
            ...
            for dtype, level, inplace in list(itertools.product(*params_dict.values())):
                ...
                opt_M = ipex.optimize(test_model, dtype=dtype, level=level, inplace=inplace)
        ...
        def test_training(model):
            ...
            for dtype, level, inplace, opt in list(itertools.product(*params_dict.values())):
                test_model = copy.deepcopy(model)
                if opt == SGD:
                    optimizer = opt(model.parameters(), lr=10.01, momentum=0.1)
                else:
                    optimizer = opt(model.parameters(), lr=10.01)
                opt_M, _ = ipex.optimize(test_model, optimizer=optimizer, dtype=dtype, level=level, inplace=inplace)
                check_shared_in_model(opt_M, dtype)
                check_shared_in_state_dict(opt_M.state_dict())
        ...
        for level in ['O0', 'O1']:
            for train in [True, False]:
                ...
                if train:
                    ...
                    opt_M, _ = ipex.optimize(test_model, optimizer=optimizer, dtype=torch.bfloat16, level=level)
                else:
                    opt_M = ipex.optimize(test_model.eval(), dtype=torch.bfloat16, level=level)
                self.assertEqual(opt_M.param.data_ptr(), opt_M.no_cast_linear.weight.data_ptr())
    ...
    def test_noprepack_since_shared(self):
        ...
        for level in ['O0', 'O1']:
            for train in [True, False]:
                ...
                if train:
                    optimizer = SGD(model.parameters(), lr=10.01, momentum=0.1)
                    opt_M, _ = ipex.optimize(test_model, weights_prepack=True, optimizer=optimizer, dtype=torch.bfloat16, level=level)
                else:
                    opt_M = ipex.optimize(test_model.eval(), weights_prepack=True, dtype=torch.bfloat16, level=level)
                self.assertEqual(opt_M.shared.weight.data_ptr(), opt_M.no_prepack_linear.weight.data_ptr())
