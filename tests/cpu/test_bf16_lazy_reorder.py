...
class TestRNN(TestCase):
    ...
    def test_gru(self):
        ...
        with AutoDNNL(True), AutoMixPrecision(True):
            ...
            y_dpcpp.sum().backward(retain_graph=True)
            self.assertEqual(input_dpcpp.grad.to('cpu'), input.grad, 0.05)
            self.assertEqual(model_dpcpp.bias_ih_l0.grad.to('cpu'), model.bias_ih_l0.grad, 0.2)
            self.assertEqual(model_dpcpp.bias_hh_l0.grad.to('cpu'), model.bias_hh_l0.grad, 0.2)
            self.assertEqual(model_dpcpp.weight_ih_l0.grad.to('cpu'), model.weight_ih_l0.grad, 0.2)
            self.assertEqual(model_dpcpp.weight_hh_l0.grad.to('cpu'), model.weight_hh_l0.grad, 0.2)
            hy.sum().backward(retain_graph=True)
            hy_dpcpp.sum().backward(retain_graph=True)
            self.assertEqual(h0_dpcpp.grad.to('cpu'), h0.grad, 0.05)
