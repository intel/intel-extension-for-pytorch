...
class MaskedMHA(torch.nn.Module):
    ...
    def forward(
        ...
        attention_mask,
        beam_idx,
        indirect_access_kv_cache=True,
        offset=0,
    ):
        head_size = self.head_dim
        # self.query_key_value(input_t)
        # linear_res = torch.randn(
        #     input_t.shape[0],
        #     input_t.shape[1],
        #     (self.num_heads + self.num_kv * 2) * head_size,
...
batch_size = 1
input_t = torch.randn(
    batch_size * beam_size, 1, head_num * head_size, dtype=torch.bfloat16
)
key_cache_iakv = torch.randn(
    max_seq_len, beam_size * batch_size, head_num, head_size, dtype=torch.bfloat16
)
...
attention_mask = torch.zeros(
    batch_size * beam_size, 1, 1, offset + 1, dtype=torch.bfloat16
)
count = 10000
total_time = 0
