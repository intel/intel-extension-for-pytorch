...
class CoreInfo():
    def __init__(self, cpu=-1, core=-1, socket=-1, node=-1, is_physical_core=True, maxmhz=0.0, is_p_core=True):
        self.cpu = cpu
        self.core = core
        self.socket = socket
        self.node = node
...
def construct_numa_config(n_nodes, n_phycores_per_node, enable_ht=True, n_e_cores=0, numa_mode=0, show_node=True):
    cores = []
    for i in range(n_nodes):
        for j in range(n_phycores_per_node):
            ...
            if enable_ht:
                cores.append(CoreInfo(core=core_id, socket=i, node=i, is_physical_core=False, maxmhz=5000.0))
        for j in range(n_e_cores):
            core_id = i * (n_phycores_per_node + n_e_cores) + n_phycores_per_node + j
            cores.append(CoreInfo(core=core_id, socket=i, node=i, maxmhz=3800.0, is_p_core=False))
    if numa_mode == 0:
        cores.sort(key=lambda x: (x.node, 1 - int(x.is_p_core), 1 - int(x.is_physical_core), x.core))
    if numa_mode == 1:
        cores.sort(key=lambda x: (1 - int(x.is_p_core), 1 - int(x.is_physical_core), x.node, x.core))
    if numa_mode == 2:
        cores.sort(key=lambda x: (x.node, 1 - int(x.is_p_core), x.core, 1 - int(x.is_physical_core)))
...
if __name__ == '__main__':
    lscpu_txt = construct_numa_config(n_nodes=1, n_phycores_per_node=8, enable_ht=True, n_e_cores=8, numa_mode=2, show_node=True)
