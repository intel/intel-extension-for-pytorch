...
try:
    ...
    import sys
    import subprocess
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'transformers==4.11.0'])
    import transformers
...
class TPPOPsTester(TestCase):
    ...
    def _test_backward(self, hf_res, tpp_res, hf_model, tpp_model, prec=0.0001):
        ...
        for param_hf, param_tpp in zip(hf_model.parameters(), tpp_model.parameters()):
            if param_tpp.is_blocked():
                self.assertEqual(param_hf.grad, self._unblock_grad(param_tpp), prec=prec)    
            else:
    ...
    def test_tpp_bert_self_attention(self):
        ...
        tpp_res = tpp_self_att(hidden_states.view(self.batch * self.max_seq_len, self.config.hidden_size), \
            ...
            seq_sqr_offsets=self.seq_spr_offsets)[0].unblocked_tensor(). view(self.batch, self.max_seq_len, -1)            
    ...
    def test_tpp_bert_output(self):
        ...
        tpp_res = tpp_self_out(hidden_states.view(self.batch * self.max_seq_len, self.config.hidden_size),  \
            input_tensor.view(self.batch * self.max_seq_len, self.config.hidden_size))\
            .unblocked_tensor(). view(self.batch, self.max_seq_len, -1)                 
    ...
    def test_tpp_bert_intermediate(self):     
        ...
        tpp_res = tpp_intermediate(hidden_states.view(self.batch * self.max_seq_len, self.config.hidden_size))\
            .unblocked_tensor(). view(self.batch, self.max_seq_len, -1)                 
        self.assertEqual(hf_res, tpp_res, prec=0.001)    
        self._test_backward(hf_res, tpp_res, hf_intermediate, tpp_intermediate, prec=0.01)
