...
since the chat_format in generation_config is not "chatml". If you are directly using the model downloaded from Huggingface, 
...
Pass argument `stream` to model.chat() is buggy, deprecated, and marked for removal. 
Please use model.chat_stream(...) instead of model.chat(..., stream=True).
向model.chat()传入参数stream的用法可能存在Bug，该用法已被废弃，将在未来被移除。请使用model.chat_stream(...)代替model.chat(..., stream=True)。
"""

_ERROR_INPUT_CPU_QUERY_WITH_FLASH_ATTN_ACTIVATED = """\
We detect you have activated flash attention support, but running model computation on CPU. 
Please make sure that your input data has been placed on GPU. If you actually want to run CPU computation, 
please following the readme and set device_map="cpu" to disable flash attention when loading the model
 (calling AutoModelForCausalLM.from_pretrained).
检测到您的模型已激活了flash attention支持，但正在执行CPU运算任务。如使用flash attention，请您确认模型输入已经传到GPU上。
如果您确认要执行CPU运算，请您在载入模型（调用AutoModelForCausalLM.from_pretrained）时，按照readme说法，指定device_map="cpu"以禁用flash attention。
"""
...
class FlashSelfAttention(torch.nn.Module):
    ...
    def pad_input(self, hidden_states, indices, batch, seqlen):
        ...
        return output.reshape(batch, -1, output.shape[1:])
    ...
    def forward(self, q, k, v, attention_mask=None):
        ...
        k = k.view(-1, k.shape[2], k.shape[3])
        v = v.view(-1, v.shape[2], v.shape[3])
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if autoset_precision:
            if SUPPORT_BF16:
                logger.warn(
                    ...
                    "If you want to disable the automatic precision, please manually add "
                    'bf16/fp16/fp32=True to "AutoModelForCausalLM.from_pretrained".'
                )
                config.bf16 = True
            elif SUPPORT_FP16:
                logger.warn(
        ...
        if config.bf16 and SUPPORT_CUDA and not SUPPORT_BF16:
            logger.warn(
                "Your device does NOT seem to support bf16, you can switch to fp16 or fp32 by"
                'passing fp16/fp32=True in "AutoModelForCausalLM.from_pretrained".'
            )
