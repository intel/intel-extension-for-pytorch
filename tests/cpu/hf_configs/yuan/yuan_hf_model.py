...
class YuanAttention(nn.Module):
    ...
    def forward(
        ...
        past_key_value = (
            (key_states, value_states, inference_hidden_states_memory)
            if use_cache
            else None
        )
...
class YuanDecoderLayer(nn.Module):
    def __init__(self, config: YuanConfig):
        ...
        self.post_attention_layernorm = YuanRMSNorm(
...
class YuanModel(YuanPreTrainedModel):
    ...
    def __init__(self, config: YuanConfig):
        ...
        self.layers = nn.ModuleList(
            [YuanDecoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def _prepare_decoder_attention_mask_training(
        ...
        attention_mask = torch.tril(
            torch.ones(
                (micro_batch_size, seq_length, seq_length), device=inputs_embeds.device
            )
