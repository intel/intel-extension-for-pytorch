...
class Phi4MMImageEmbedding(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # we use the token embedding layer from the huggingface model,
        # this is REQUIRED to make sure we are using the loaded weights.
...
class Phi4MMImageAudioEmbedding(nn.Module):
    ...
    def forward(
        ...
        #               actually, in the debug code above, the non-image-audio tokens
        # from image_hidden_states and audio_hidden_states should be the same
...
class Phi4MMSdpaAttention(Phi4MMAttention):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Phi4MMModel is using Phi4MMSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` \
        ...
        # an inline conditional assignment in SDPA to support both torch.compile's dynamic shapes and full graph options.
...
class Phi4MMModel(Phi4MMPreTrainedModel):
    ...
    # ._prepare_4d_causal_attention_mask_with_cache_position with Mistral->Phi3
...
class Phi4MMForCausalLM(Phi4MMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        'This is an example script .\n Certainly! Below is a sample script that demonstrates a simple task,
        ...
        if (
            ...
            logger.warning(
                ...
                + f"{self.config.original_max_position_embeddings}th token, as the KV cache needs to be recomputed."
            )
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->Phi,
# LLAMA->PHI, self.transformer->self.model, transformer_outputs->model_outputs
...
# Copied from transformers.models.mpt.modeling_mpt.MptForTokenClassification with Mpt->Phi,MPT->PHI,
# self.transformer->self.model,transformer_outputs->model_outputs
