...
class TestPrepackCases(TestCase):
    def _test_convolution_inference_base(self, dim):
        ...
        if dim == 1:
            options = itertools.product(
                [True, False], [1, 2], [1, 4], [True, False], [torch.contiguous_format]
            )
        else:
            options = itertools.product(
                [True, False],
        ...
        for bias, dilation, groups, feed_sample_input, memory_format in options:
            ...
            if feed_sample_input:
                ipex_model = ipex.optimize(
                    model, dtype=torch.float32, level="O1", sample_input=x
                )
            else:
    ...
    def _test_convolution_training_base(self, dim, dtype, rtol=None, atol=None):
        ...
        if dim == 1:
            options = itertools.product(
                [True, False], [1, 2], [1, 4], [torch.contiguous_format], [True, False]
            )
        else:
            options = itertools.product(
                [True, False],
        ...
        for bias, dilation, groups, memory_format, feed_sample_input in options:
            ...
            if bias:
                self.assertEqual(
                    origin_model1.bias.grad,
                    ipex_model1.bias.grad.float(),
                    rtol=rtol,
                    atol=atol,
                )
                self.assertEqual(
                    origin_model1.bias.grad,
                    ipex_model2.bias.grad.float(),
                    rtol=rtol,
                    atol=atol,
                )
    ...
    def test_conv1d_training(self):
        ...
        if core.onednn_has_bf16_support():
            self._test_convolution_training_base(
                dim=1, dtype=torch.bfloat16, rtol=1e-2, atol=1e-03
            )
    ...
    def test_conv2d_training(self):
        ...
        if core.onednn_has_bf16_support():
            self._test_convolution_training_base(
                dim=2, dtype=torch.bfloat16, rtol=1e-2, atol=1e-03
            )
    ...
    def test_linear_training(self):
        ...
        options = itertools.product(
            out_feature, [True, False], input_shapes, [torch.bfloat16], [True, False]
        )
