...
class TestLauncher(TestCase):
    ...
    def test_memory_allocator_setup(self):
        ...
        tcmalloc_enabled = (
            "libtcmalloc.so" in os.environ["LD_PRELOAD"] if ld_preload_in_os else False
        )
        ...
        jemalloc_enabled = (
            "libjemalloc.so" in os.environ["LD_PRELOAD"] if ld_preload_in_os else False
        )
        ...
        launcher.set_memory_allocator(
            enable_tcmalloc=False, enable_jemalloc=True, benchmark=True
        )
        if jemalloc_enabled:
            self.assertTrue("MALLOC_CONF" in os.environ)
            self.assertTrue(
    ...
    def test_mpi_pin_domain_and_ccl_worker_affinity(self):
        ...
        proc_per_node = 2
        ccl_worker_count = 4
        pin_doamin = launcher.get_mpi_pin_domain(
            proc_per_node, ccl_worker_count, total_cores
        )
        expect_pin_domain = "[0xffffff0,0xffffff00000000,]"
        self.assertEqual(pin_doamin, expect_pin_domain)
        ccl_worker_affinity = launcher.get_ccl_worker_affinity(
            proc_per_node, ccl_worker_count, total_cores
        )
    ...
    def test_numactl_core_affinity(self):
        ...
        if numactl_available:
            expected_core_affinity = "numactl -C {}-{}".format(
                str(0), str(num_physical_cores - 1)
            )
    ...
    def test_taskset_core_affinity(self):
        ...
        expected_core_affinity = "taskset -c {}-{}".format(
            str(0), str(num_physical_cores - 1)
        )
        for launch_script in self.launch_scripts:
    ...
    def test_core_affinity_with_skip_cross_node_cores(self):
        ...
        if num_nodes > 1:
            ...
            for i in range(num_nodes):
                ...
                expected_node_i_core_affinity = "-c {}-{}".format(
                    str(node_i_start_core),
                    str(node_i_start_core + ncore_per_instance - 1),
                )
    ...
    def test_core_affinity_with_skip_cross_node_cores_and_use_logical_core(self):
        ...
        num_threads_per_core = int(
            cpuinfo.logical_core_nums() / cpuinfo.physical_core_nums()
        )
        ...
        if num_nodes > 1 and num_threads_per_core > 1:
            ...
            for i in range(num_nodes):
                ...
                expected_node_i_physical_core_affinity = "-c {}-{}".format(
                    str(node_i_physical_start_core),
                    str(node_i_physical_start_core + ncore_per_instance - 1),
                )
                expected_node_i_logical_core_affinity = "-c {}-{}".format(
                    str(node_i_logical_start_core),
                    str(node_i_logical_start_core + ncore_per_instance - 1),
                )
                assert (
                    expected_node_i_physical_core_affinity
                    in str(r.stdout, "utf-8").lower()
                )
                assert (
                    expected_node_i_logical_core_affinity
                    in str(r.stdout, "utf-8").lower()
                )
    ...
    def test_core_affinity_with_skip_cross_node_cores_and_node_id_use_logical_core(
        ...
        if num_nodes > 1 and num_threads_per_core > 1:
            ...
            expected_node_0_physical_core_affinity = "-c {}-{}".format(
                str(node_0_physical_start_core),
                str(node_0_physical_start_core + ncore_per_instance - 1),
            )
            expected_node_0_logical_core_affinity = "-c {}-{}".format(
                str(node_0_logical_start_core),
                str(node_0_logical_start_core + ncore_per_instance - 1),
            )

            assert (
                expected_node_0_physical_core_affinity in str(r.stdout, "utf-8").lower()
            )
            assert (
                expected_node_0_logical_core_affinity in str(r.stdout, "utf-8").lower()
            )
