...
class TestCompileCases(TestCase):
    def test_conv_inference(self):
        for dim in [1, 2, 3]:
            ...
            options = itertools.product([True, False], [1, 2], [1, 4], [torch.float32], ['ipex', 'inductor'], [True, False], [True, False])
            for bias, dilation, groups, dtype, backend, dynamic, ipex_optimize in options:
    ...
    def test_deconv_inference(self):
        for dim in [2, 3]:
            ...
            # TODO: add bfloat16 data type tests when 'inductor' backend supports bfloat16.
            options = itertools.product([True, False], [1, 2], [1, 2], [1, 2], [1, 2], [torch.contiguous_format, channels_last], [torch.float32], ['ipex', 'inductor'], [True, False], [True, False])
            for bias, stride, padding, groups, dilation, memory_format, dtype, backend, dynamic, ipex_optimize in options:
    ...
    def test_linear_inference(self):
        ...
        # TODO: add bfloat16 data type tests when 'inductor' backend supports bfloat16.
        options = itertools.product([True, False], input_shapes, [torch.float32], ['ipex', 'inductor'], [True, False], [True, False])
    ...
    def test_lstm_inference(self):
        # TODO: add bfloat16 data type tests when 'inductor' backend supports bfloat16.
        options = itertools.product([torch.float32], ['ipex', 'inductor'], [True, False], [True, False])
