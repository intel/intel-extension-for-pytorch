...
from intel_extension_for_pytorch.nn.utils._weight_prepack import may_import_deepspeed_modules, _IPEXLinear, _IPEXLinearAllreduce
from intel_extension_for_pytorch.quantization import prepare, convert
from intel_extension_for_pytorch.quantization._quantize import DynamicQuantizedLinearLayer, DynamicQuantizedLinearAllreduce
...
class DeepspeedTester(TestCase):
    def _get_ds_model(self, m_linear):
        ...
        assert ds_world_size > 1, "expect ds_world_size > 1, you could try launching the script with: deepspeed --num_gpus 2 --bind_cores_to_rank tests/cpu/test_deepspeed.py"
        engine = deepspeed.init_inference(model=m_linear, mp_size=ds_world_size, dtype=torch.float32,
                                        replace_method="auto")
    ...
    def test_dynamic_quantization(self):
        ...
        if deepspeed_modules is not None:
            ...
            prepared_model = prepare(ds_model, dynamic_qconfig, example_inputs=(x), inplace=True, bn_folding=False)
