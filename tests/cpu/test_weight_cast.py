...
from intel_extension_for_pytorch.nn.utils._weight_cast import (
    IPEX_WEIGHT_CAST_MODULE as IPEX_WEIGHT_CAST_MODULE,
)
        ...
        for optimizer in [
            ...
            for cast_dtype in [torch.bfloat16, torch.float16]:
                for split_master_weight_for_bf16 in [True, False]:
                    if (
                        type(optimizer) not in IPEX_FUSED_OPTIMIZER_LIST
                    ...
                    for _, sub_m in model.named_children():
                        if self.is_master_weight_solution(
                            ...
                            self.assertEqual(sub_m.weight.dtype, cast_dtype)
                            self.assertEqual(sub_m.master_weight.dtype, torch.float)
                            self.assertTrue(
                                sub_m.master_weight is optimizer_params_list[param_id]
                            )
                            self.assertTrue(
                                sub_m.weight is not optimizer_params_list[param_id]
                            )
                            param_id += 1
                            if hasattr(sub_m, "bias"):
                                self.assertTrue(hasattr(sub_m, "master_bias"))
                                self.assertEqual(sub_m.bias.dtype, cast_dtype)
                                self.assertEqual(sub_m.master_bias.dtype, torch.float)
                                self.assertTrue(
                                    sub_m.master_bias is optimizer_params_list[param_id]
                                )
                                self.assertTrue(
                                    sub_m.bias is not optimizer_params_list[param_id]
                                )
                                param_id += 1
                            ...
                            self.assertEqual(sub_m.weight.dtype, cast_dtype)
                            self.assertTrue(
                                sub_m.weight is optimizer_params_list[param_id]
                            )
                            param_id += 1
                            if hasattr(sub_m, "bias"):
                                ...
                                self.assertEqual(sub_m.bias.dtype, cast_dtype)
                                self.assertTrue(
                                    sub_m.bias is optimizer_params_list[param_id]
                                )
                                param_id += 1
                        else:
                    ...
                    for var_name in ipex_model_state:
                        ...
                        if hasattr(getattr(model, module_name), "master_" + param_name):
                            output_param = getattr(
                                getattr(model, module_name), "master_" + param_name
                            )
                        elif hasattr(
                            getattr(model, module_name), param_name + "_trail"
                        ):
                            trail = getattr(
                                getattr(model, module_name), param_name + "_trail"
                            )
                            top_half = getattr(getattr(model, module_name), param_name)
                            output_param = torch.ops.torch_ipex.cat_bfloat16_float(
                                top_half, trail
                            )
                        else:
                            output_param = getattr(
                                getattr(model, module_name), param_name
                            )
