...
from intel_extension_for_pytorch.nn.utils._weight_cast import (
    IPEX_WEIGHT_CAST_MODULE_BF16 as IPEX_WEIGHT_CAST_MODULE_BF16,
)
from intel_extension_for_pytorch.nn.utils._weight_cast import (
    IPEX_WEIGHT_CAST_MODULES as IPEX_WEIGHT_CAST_MODULES,
)
...
class TestWeightCastCases(TestCase):
    def is_master_weight_solution(self, module, dtype, split_master_weight):
        return (
            type(module) in IPEX_WEIGHT_CAST_MODULES[dtype] and not split_master_weight
        )
    ...
    def master_weight_test(self, m, param_id, cast_dtype, optimizer_params_list):
        for name, param in m.named_parameters():
            if hasattr(m, name):
                ...
                self.assertEqual(
                    getattr(m, "master_{}".format(name)).dtype, torch.float
                )
                self.assertTrue(
                    getattr(m, "master_{}".format(name))
                    is optimizer_params_list[param_id]
                )
                    ...
                    for _, sub_m in model.named_children():
                        if self.is_master_weight_solution(
                            ...
                            param_id = self.master_weight_test(
                                sub_m, param_id, cast_dtype, optimizer_params_list
                            )
