...
if [[ ${IMAGE_TYPE} = "xpu" ]]; then
    IMAGE_NAME=intel/intel-extension-for-pytorch:2.7.10-$IMAGE_TYPE
    docker build --build-arg http_proxy=$http_proxy \
                 ...
                 --build-arg IPEX_VERSION=2.7.10+xpu \
                 --build-arg TORCHVISION_VERSION=0.22.0 \
                 --build-arg TORCHAUDIO_VERSION=2.7.0 \
                 --build-arg ONECCL_BIND_PT_VERSION=2.7.0+xpu \
