...
launcher_hyperparam_default_search_space = {'hp': ['ncore_per_instance', 'ncores_per_instance', 'ninstances', 'use_all_nodes', 'use_logical_core', 'use_logical_cores', 'disable_numactl', 'disable_iomp', 'malloc'],
...
launcher_schema = Schema({
                          ...
                          Optional('ncore_per_instance', default='all_logical_cores'): And(Or(str, list), Use(input_str_to_list_int), lambda s: all(isinstance(i, int) for i in s)),
                          Hook('ncores_per_instance', handler=_valid_launcher_schema): object,
                          Optional('ncores_per_instance', default='all_logical_cores'): And(Or(str, list), Use(input_str_to_list_int), lambda s: all(isinstance(i, int) for i in s)),

                          Hook('ninstances', handler=_valid_launcher_schema): object,
                          Optional('ninstances', default='all_logical_cores'): And(Or(str, list), Use(input_str_to_list_int), lambda s: all(isinstance(i, int) for i in s)),

                          Optional('use_all_nodes', default=[True, False] if len(set([c.node for c in cpuinfo])) > 1 else [True]):And(list, lambda s: all(isinstance(i, bool) for i in s)),
                          Optional('use_logical_core', default=[True, False] if is_hyperthreading_enabled else [False]): And(list, lambda s: all(isinstance(i, bool) for i in s)),
                          Optional('use_logical_cores', default=[True, False] if is_hyperthreading_enabled else [False]): And(list, lambda s: all(isinstance(i, bool) for i in s)),
                          Optional('disable_numactl', default=[True, False]): And(list, lambda s: all(isinstance(i, bool) for i in s)),
...
#reference: https://github.com/intel/neural-compressor/blob/15477100cef756e430c8ef8ef79729f0c80c8ce6/neural_compressor/conf/config.py
class Conf(object):
    def __init__(self, conf_fpath, program_fpath, program_args):
        assert Path(conf_fpath).exists(), f'{conf_fpath} does not exist'
        self.execution_conf = DotDict(schema.validate(self._convert_conf(self._read_conf(conf_fpath), copy.deepcopy(schema.validate(dict())))))
    ...
    def _extract_usr_objectives(self, program_fpath):
        # e.g. [{'name': 'latency', 'higher_is_better': False, 'target_val': 0}, {'name': 'throughput', 'higher_is_better':True, 'target_val': 100}]
        ...
        def _parse_hypertune_token(line):
            ...
            try:
                ...
                raise RuntimeError(f'Parsing @hypertune failed for line {line} of {program_fpath} file')
            return objective
