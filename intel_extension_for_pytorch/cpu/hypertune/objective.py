...
class MultiObjective(object):
    ...
    def deprecate_config(self, cfg, deprecated, new, default):
        ...
        if new in cfg.keys():
            v_new = cfg[new]
        assert v_deprecated == default or v_new == default, f'Configurations {deprecated} and {new} cannot be set at the same time.'
        ret = default
    ...
    def decode_launcer_cfg(self, cfg):
        ncores_per_instance = self.deprecate_config(cfg, 'ncore_per_instance', 'ncores_per_instance', -1)
        ninstances = cfg['ninstances']
        use_all_nodes = cfg['use_all_nodes']
        use_logical_cores = self.deprecate_config(cfg, 'use_logical_core', 'use_logical_cores', False)
        ...
        for i, s in enumerate(output):
            if HYPERTUNE_TOKEN in s:
                try:
                    ...
                    raise RuntimeError(f'Extracting objective {output[i]} failed for {self.program} file. Make sure to print an int/float value after the @hypertune token as the objective value to be minimized or maximized.')
        return objectives
