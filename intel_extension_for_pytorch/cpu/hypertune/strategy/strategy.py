...
class TuneStrategy(object):
    def __init__(self, conf):
        ...
        # objective
        self.multiobjective = MultiObjective(self.program, self.program_args, tune_launcher)
        ...
        self.tune_result_record.writerow(list(self.hyperparam2searchspace.keys()) + [objective['name'] for objective in self.usr_objectives]) # noqa B950
    ...
    def traverse(self):
        ...
        for tune_cfg in self.next_tune_cfg():
            ...
            click.secho(f"\nTune ", fg='green', nl=False) # noqa F541
            click.secho(f"{trials_count}", fg='blue', nl=False) # noqa F541

            click.secho(f"\nCurrent configuration is: ", fg='green', nl=False) # noqa F541
        ...
        click.secho("\nFinished traversing the entire search space, but didn't find configuration meeting the objective goal", fg='red') # noqa B950
        self._print_best_result()
        return
    ...
    def _update_best_tune_result(self, curr_tune_result, curr_tune_cfg):
        if self.best_tune_result is None and self.best_tune_cfg is None:
            ...
            # multi objective
            if all([self._compare(higher_is_better, curr_val, best_val) for higher_is_better, curr_val, best_val in zip([objective['higher_is_better'] for objective in self.usr_objectives], curr_tune_result, self.best_tune_result)]): # noqa B950
                self.best_tune_result = curr_tune_result
                self.best_tune_cfg = curr_tune_cfg
    ...
    def _record_tune_result(self, curr_tune_result, curr_tune_cfg):
        ...
        click.secho(f"Best configuration is: ", fg='green', nl=False) # noqa F541
        click.secho(f"{self.best_tune_cfg}", fg='blue')
        for objective, val in zip(self.usr_objectives, self.best_tune_result):
            click.secho(f"{objective['name']}: {val}", fg='blue')

        curr_tune_cfg_val = [_ for _ in curr_tune_cfg.values()] # noqa C416
        self.tune_result_record.writerow(curr_tune_cfg_val + curr_tune_result)

    def _stop(self, trials_count):
        if all([self._compare(higher_is_better, best_val, target_val) \
            for higher_is_better, best_val, target_val in zip([objective['higher_is_better'] \
                for objective in self.usr_objectives], self.best_tune_result, [objective['target_val'] \
                    for objective in self.usr_objectives])]): # noqa E128
            click.secho("\nFound configuration meeting the target values.", fg='red')
            return True 
        elif trials_count == self.max_trials:
            click.secho("\nMax trials is reached, but didn't find configuration meeting the objective goal.", fg='red')
            return True 
        return False 

    def _print_best_result(self):
        click.secho(f"Best configuration found is: ", fg='green', nl=False) # noqa F541
        click.secho(f"{self.best_tune_cfg}", fg='blue')
        for objective, val in zip(self.usr_objectives, self.best_tune_result):
            click.secho(f"{objective['name']}: {val}", fg='blue') # noqa W292