...
class CPUinfo:
    ...
    def numa_aware_check(self, core_list):
        ...
        if len(numa_ids) > 1:
            logger.warning(
                "Numa Aware: cores:{} on different NUMA nodes:{}".format(
...
class Launcher:
                ...
                if len(matches) > 0:
                    if "LD_PRELOAD" in os.environ:
                        os.environ["LD_PRELOAD"] = (
                            matches[0] + ":" + os.environ["LD_PRELOAD"]
                        )
                    else:
    ...
    def set_memory_allocator(
        ...
        if enable_tcmalloc:
            ...
            if not find_tc:
                logger.warning(
                    "Unable to find the {} library file lib{}.so in $CONDA_PREFIX/lib or $VIRTUAL_ENV/lib"
    ...
    def set_multi_thread_and_allocator(
        ...
        self.set_memory_allocator(
            enable_tcmalloc, enable_jemalloc, use_default_allocator, benchmark
        )
    ...
    def launch(self, args):
        ...
        if args.core_list:  # user specify what cores will be used by params
            ...
            if (
                ...
                if not args.skip_cross_node_cores:
                    ...
                    if args.ncore_per_instance > ncore_per_node:
                        ...
                        logger.warning(
                            "there are {} core(s) per socket, but you specify {} ncore_per_instance and skip_cross_node_cores. \
            ...
            if args.latency_mode:
                logger.warning(
                    "--latency_mode is exclusive to --ninstances, --ncore_per_instance, --node_id and \
                        --use_logical_core. They won't take effect even if they are set explicitly."
                )
            ...
            if args.throughput_mode:
                logger.warning(
                    "--throughput_mode is exclusive to --ninstances, --ncore_per_instance, \
                        --node_id and --use_logical_core. They won't take effect even if they are set explicitly."
                )
        ...
        if args.ninstances > 1 and args.instance_idx != -1:
            logger.info(
                "assigning {} cores for instance {}".format(
        ...
        for i in range(args.ninstances):
            ...
            if not args.disable_numactl or enable_taskset:
                ...
                if (
                    ...
                    core_list = cores[
                        args.instance_idx *
                        args.ncore_per_instance : (args.instance_idx + 1) *
                        args.ncore_per_instance
                    ]
                ...
                for r in core_ranges:
                    cur_process_cores = cur_process_cores + "{}-{},".format(
                        r["start"], r["end"]
                    )
            ...
            log_name = (
                ...
                "_instance_{}_cores_".format(i) +
                cur_process_cores.replace(",", "_") +
                ".log"
            )
    ...
    def launch(self, args):
        ...
        if args.nnodes > 1 and not os.path.exists(args.hostfile):
            ...
            if len(ip_list) < args.nnodes:
                logger.error(
                    "The number of IP {} should greater than nnodes parameters {}".format(
...
def add_multi_instance_params(parser):
    ...
    group.add_argument(
        ...
        help="Specify instance index to assign ncores_per_instance for instance_idx; \
            otherwise ncore_per_instance will be assigned sequentially to ninstances. \
                Please refer to \
                    https://github.com/intel/intel-extension-for-pytorch/blob/master\
    ...
    group.add_argument(
        "--disable_taskset", action="store_true", default=False, help="Disable taskset"
    )
    group.add_argument(
        "--core_list",
        metavar="\b",
    ...
    group.add_argument(
        ...
        help="Enable benchmark config. JeMalloc's MALLOC_CONF has been tuned for low latency. \
            Recommend to use this for benchmarking purpose; for other use cases, this MALLOC_CONF may cause Out-of-Memory crash.",
    )
    group.add_argument(
        "--log_path",
        metavar="\b",
...
def run_main_with_args(args):
    ...
    if args.log_path:
        path = os.path.dirname(
            args.log_path if args.log_path.endswith("/") else args.log_path + "/"
        )
