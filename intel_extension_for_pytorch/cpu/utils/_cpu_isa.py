...
def check_avx2_support():
    ...
    if max_basic_id >= 7:
        ...
        See Section 13.5.3
        """
        if get_normalized_bit(xcrFeatureMask, 1) and get_normalized_bit(
            xcrFeatureMask, 2
        ):
            return True
...
def check_minimal_isa_support():
    if not intel_extension_for_pytorch._C._has_cpu():
        return
    err_msg = "ERROR! Intel® Extension for PyTorch* only works on machines with instruction \
        sets equal or newer than AVX2, which are not detected on the current machine."
