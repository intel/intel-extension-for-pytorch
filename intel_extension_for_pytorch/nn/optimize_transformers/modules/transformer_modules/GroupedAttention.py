...
class IPEXTransformerAttnOptimizedFp16Grouped(IPEXTransformerAttnOptimizedFp16):
    ...
    def prepare_kv_cache(self, hidden_states):
        ...
        if self.runtime_cache.key_cache is None or self.runtime_cache.value_cache is None or batch_size != self.batch_size:
            if self.is_beam_search():
                cache_shape = [self.max_out_position, bs_beam, self.num_kv_head, self.head_dim]
            else:
                cache_shape = [self.max_position, bs_beam, self.num_kv_head, self.head_dim]
    ...
    def sdp_kv_preprocess_2nd2last(self, key, value):
        ...
        value_prompt = self.repeat_kv(self.runtime_cache.value_prompt, self.num_kv_group)
        return key, value, key_prompt, value_prompt
