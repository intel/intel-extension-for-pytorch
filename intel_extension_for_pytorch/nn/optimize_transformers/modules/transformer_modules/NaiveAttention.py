...
# ################################## post qkv #######################################################
    def post_qkv(self, query, key, value, position_ids, layer_past=None):
        ...
        key, query = self.position_embed(key, query, position_ids, self.layer_id, self.beam_size, seq)
        if self.is_beam_search():
            query, key, value = self.combine_kv_cache_interface(query, key, value, layer_past)
        else:
...
# ################################################################ sdp #######################################################################################
    def sdp(self, query, key, value, attention_mask, head_mask, alibi):
        if self.is_beam_search():
            ...
            attn_output, attn_weights = self.naive_self_attention(query, key, value, attention_mask=attention_mask, head_mask=head_mask, alibi=alibi, first_token=self.is_1st_token())
...
# ########################################################################### post sdp #######################################################################
    ...
    def repeat_kv(self, kv, n_rep):
        ...
        if self.timestamp == 0 and self.beam_size > 1:
