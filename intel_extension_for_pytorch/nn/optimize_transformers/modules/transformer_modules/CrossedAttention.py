...
class IPEXTransformerAttnOptimizedFp16Crossed(IPEXTransformerAttnOptimizedFp16):
    ...
    def qkv_gemm(self,
        ...
        if is_cross_attention and layer_past is not None:
            query = torch.ops.torch_ipex.matmul_bias_out(hidden_states, self.q_proj.weight, self.q_proj.bias)
            key = layer_past[0]
            value = layer_past[1]
        else:
            query, key, value = super().qkv_gemm(hidden_states, key_value_state, layer_past)
