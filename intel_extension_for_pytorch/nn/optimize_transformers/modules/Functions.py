...
def ipex_prepare_model_inputs(model):
    if hasattr(model, "_prepare_model_inputs"):
        setattr(model, "_prepare_model_inputs", partial(_ipex_prepare_model_inputs, model))
...
def ipex_build_bloom_alibi_tensor(model):
    if hasattr(model, 'transformer'):
        if hasattr(model.transformer, 'build_alibi_tensor'):
            setattr(model.transformer, 'build_alibi_tensor', build_bloom_alibi_tensor)
...
def ipex_beam_search(model):
    if hasattr(model, "beam_search"):
        setattr(model, "beam_search", partial(_ipex_beam_search, model))
...
def ipex_beam_search_without_optimize(model):
    if hasattr(model, "beam_search"):
        setattr(model, "beam_search", _ipex_beam_search_without_optimize)
