...
class TransformersModelCapture(ModelCapture):
    def __init__(self, model, dtype, is_jit_absolute=False, weights_prepack=None):
        ...
        self.is_jit_absolute = True if is_jit_absolute or self.model_jit_absolute() or os.getenv("CAPTURE_JIT_ABSOLUTE", default=None) else False
        ...
        if change_past_key_values:
            ...
            if "attention_mask" in inputs:
                ...
                inputs["attention_mask"] = torch.cat(
                    [attention_mask.new_zeros((attention_mask.shape[0], 1)), attention_mask], dim=-1
                )
    ...
    def jit_output_wrapper(self, outputs, change_past_key_values):
        if change_past_key_values == True:
                ...
                with self.lock:
                    if self.is_jit_absolute:
                        ...
                        trace_model = torch.jit.trace(
                            self.model.eval(), example_kwarg_inputs=args, strict=False, check_trace=True
                        ).eval()
                        trace_model = torch.jit.freeze(trace_model)
                        outputs = trace_model(**args)
                        outputs = self.jit_output_wrapper(outputs, change_past_key_values)
                        ...
                        attention_mask_change = args["attention_mask"] if change_past_key_values == True else None
                        use_cache_change = args["use_cache"] if change_past_key_values == True else None
                        token_type_ids_change = args["token_type_ids"] if change_past_key_values == True else None
                        return_dict_change = args["return_dict"] if change_past_key_values == True else None
                        output_attentions_change = args["output_attentions"] if change_past_key_values == True else None
                        output_hidden_states_change = args["output_hidden_states"] if change_past_key_values == True else None
                        ...
                        try:
                            ...
                            try:
                                ...
                                if change_past_key_values == True:
                                    ...
                                    args["output_hidden_states"] = output_hidden_states_change
                                    args["past_key_values"] = None
                                #dynamo_model = torch._dynamo.optimize(
                                #    compiler, dynamic=True
                                #)(self.model)
                                dynamo_model = torch.compile(self.model, backend=compiler, dynamic=True)
