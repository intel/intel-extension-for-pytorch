        ...
        if len(target_list):
            for i, target in enumerate(target_list):
                if target[0] == type(module):
                    ...
                    target_list[i] = tuple([type(module), slicing_list])
                    return target_list
        target_list.append(([type(module), slicing_list]))
        return target_list

    if not supported():
        print("We are not support this model for tensor parallel, original model will be returned for next phase of optimization. If you want this model runing on tensor parallel, please provide your own rule")
        return model
...
class TensorSlicer:
    def __init__(self, tp_size, tp_group, slicing_rule = None, dtype=torch.float) -> None:
        ...
        self.slicing_rule = {nn.Linear: self.linear_slicing_rules,
                            #  nn.Embedding: self.embedding_slicing_rule
                             }
    
    def linear_slicing_rules(self, module: nn.Linear, name):
        if getattr(module, "replaced", False) == True:
            return
        w_shape = module.weight.shape
        if name in self.slicing_target_list[0][1]:
            slicing_weight = torch.empty((w_shape[0], w_shape[1] // self.tp_size), dtype=self.dtype, device='meta')
            setattr(module, "replaced", True)
            setattr(module, "all_reduce", True)
            setattr(module, "tp_weight", slicing_weight)
        else:
            slicing_weight = torch.empty((w_shape[0] // self.tp_size, w_shape[1]), dtype=self.dtype, device='meta')
            slicing_bias = torch.empty((w_shape[0] // self.tp_size), dtype=self.dtype, device='meta')
            setattr(module, "replaced", True)
            setattr(module, "all_reduce", False)
            setattr(module, "tp_weight", slicing_weight)
            setattr(module, "tp_bias", slicing_bias)
        return
    ...
    def update_parallel_params(self, child):
        ...
        for param in [
            ...
            if hasattr(child, param):
                ...
                assert val % self.tp_size == 0, f"{param} ({val}) must be divisible by mp_size ({self.tp_size})"
                para_name = "tp_" + param
