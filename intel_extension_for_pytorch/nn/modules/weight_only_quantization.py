...
class WeightOnlyQuantizedLinear(nn.Module):
    ...
    def from_float_and_qweight(
        ...
        Args:
            ...
            g_idx: Indices of groups for each input channel of weight. Generated by
                GPTQ with act-order.
            weight_format: Int4 weight format, plain format or GPTQ format or AWQ format
        """
        float_modules = [torch.nn.Linear]
        deepspeed_modules = may_import_deepspeed_modules()
        if deepspeed_modules is not None:
        ...
        ), "Data type of quantized weight should be in {}, but got: {}".format(
        ...
        qlinear = cls._init_cls(
            ...
            act_quant_mode,
            cache_weight_for_large_batch,
            dtype == WoqWeightDtype.INT4,  # is_from_int4_weight
            weight_format,
        )
        del qweight
        mod.weight = torch.nn.Parameter()
    ...
    def from_float_and_int4_weight(
        ...
        Args:
            ...
            g_idx: Indices of groups for each input channel of weight. Generated by
                GPTQ with act-order.
            weight_format: weight format, plain format or GPTQ format or AWQ format
        """
        print(
            "[WARNING] from_float_and_int4_weight() is deprecated. Please use from_float_and_qweight() instead."
        )
        return cls.from_float_and_qweight(
            mod,
            qweight,
            WoqWeightDtype.INT4,
            scales,
        ...
        if qconfig is not None and hasattr(qconfig, "global_qconfig"):
            ...
            if hasattr(qconfig.global_qconfig, "cache_weight_for_large_batch"):
                cache_weight_for_large_batch = (
                    ...
                    and lowp_mode in (WoqLowpMode.BF16, WoqLowpMode.INT8)
                )
