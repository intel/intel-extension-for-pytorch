...
class IpexWoqLinear(nnq.Linear):
    ...
    def __init__(self, in_features, out_features, bias_=True, dtype=torch.qint8):
        ...
        qweight = torch.quantize_per_channel(weight, torch.ones(out_features), torch.zeros(out_features), 0, dtype)
        bias = torch.rand(out_features)
        self._op_context = torch.ops.ipex_prepack.weight_only_qlinear_prepack(qweight, bias, None)
    ...
    def from_float(cls, mod):
        ...
        qlinear._op_context = torch.ops.ipex_prepack.weight_only_qlinear_prepack(qweight, mod.bias, None)
    ...
    def from_reference(cls, ref_qlinear):
        ...
        qlinear._op_context = torch.ops.ipex_prepack.weight_only_qlinear_prepack(qweight, bias, None)
        qlinear.weight_qscheme = qlinear.weight().qscheme()
        return qlinear
