...
def weight_dtype_convert_with_ipex(module, optimizer, params_attr, master_weight_split):
    ...
    def cast_attr(m, attr, master_weight_split, params_attr, optimizer):
        ...
        # create new element in params_attr
        params_attr[float_param] = {}
        if master_weight_split:
            # TODO: split master weight is not implemented for now.
            raise RuntimeError("not implemented for master_weight_split")
            ...
            # params_attr: {'layer.weight(fp32)': {'bf16_param': 'layer.weight(bf16)'}}
    ...
    if optimizer is not None:
        ...
        if not master_weight_split:
            # cast customer step to optimizer, including fp32 updated master weight to bf16 weight
            _optimizer_utils.patch_step_for_master_weight_training(casted_optimizer)
            # cast customer zero_grad to clean/set none for grad on bf16 parameter
