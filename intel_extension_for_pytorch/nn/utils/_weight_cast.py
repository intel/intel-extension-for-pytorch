...
def weight_dtype_convert_with_ipex(
    module, optimizer, params_attr, master_weight_split, convert_dtype=torch.bfloat16
):
    assert convert_dtype in [
        torch.bfloat16,
        torch.float16,
    ...
    def cast_attr(m, attr, master_weight_split, params_attr, optimizer):
        ...
        if master_weight_split:
            if not hasattr(m, attr + "_trail"):
                ...
                top_half, bot_half = torch.ops.torch_ipex.split_float_bfloat16(
                    float_param.data
                )
                ...
                setattr(
                    ...
                    attr,
                    nn.Parameter(
                        top_half.detach(), requires_grad=float_param.requires_grad
                    ),
                )
                params_attr[float_param]["trail"] = getattr(m, attr + "_trail")
        else:
            if not hasattr(m, "master_" + attr):
                assert float_param.dtype == torch.float32, (
                    "The original "
                    + attr
                    + " of the "
                    + str(m)
                    + " should be kept float and the associated float master "
                    + attr
                ...
                if convert_dtype == torch.bfloat16:
                    setattr(
                        ...
                        attr,
                        nn.Parameter(
                            float_param.detach().bfloat16(),
                            requires_grad=float_param.requires_grad,
                        ),
                    )
        ...
        _optimizer_utils.refresh_optimizer_params_after_cast(
        ...
        if type(m) in IPEX_WEIGHT_CAST_MODULES[convert_dtype]:
            if not hasattr(m, "master_weight_split"):
                ...
                setattr(
                    ...
                    types.MethodType(_load_from_state_dict, m),
                )
                for name, sub_m in m.named_children():
                    if isinstance(sub_m, torch.nn.ParameterList):
                        ...
                        setattr(
                            sub_m,
                            "_save_to_state_dict",
                            types.MethodType(_save_to_state_dict, sub_m),
                        )
                        setattr(
                            sub_m,
                            "_load_from_state_dict",
                            types.MethodType(_load_from_state_dict, sub_m),
                        )
    ...
    if optimizer is not None:
        ...
        if not master_weight_split:
            ...
            _optimizer_utils.patch_zero_grad_for_master_weight_training(
                casted_optimizer
            )
