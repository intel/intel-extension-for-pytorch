...
elt_wise_noq_ops = [str(torch.relu_), str(torch.sigmoid_), str(nn.ReLU), str(torch.Tensor.relu_), str(torch.Tensor.sigmoid_), \
    str(torch.nn.Hardtanh), str(F.hardtanh), str(F.hardtanh_),str(torch.nn.ELU), str(F.elu), str(F.elu_), \
...
s8_s8_symmetric_ops = [str(interaction), str(torch.ops.torch_ipex.interaction_forward), str(torch.embedding_bag), \
    str(F.embedding_bag), str(torch.nn.EmbeddingBag)]
conv_gemm_fs = [str(F.conv2d), str(F.conv3d), str(F.conv_transpose2d), str(F.conv_transpose3d), str(torch.conv2d), str(torch.conv3d), \
    ...
    if pre_node is not None:
        if pre_node.type in ops:
            if len(pre_node.post_nodes) == 1:
                return pre_node
            elif len(node.post_nodes) == 1 and _find_conv_or_gemm_swish_fusion_node(node.post_nodes[0]):
                # conv+sigmoid+mul
                return pre_node
            else:
                return None
        elif pre_node.type in ([str(nn.Identity)] + elt_wise_q_ops + elt_wise_noq_ops) and \
            len(pre_node.post_nodes) == 1 and len(pre_node.pre_nodes) > 0:
...
def _find_fused_node_with_cur_add(node, ops):
    ...
    if len(node.pre_nodes) == 0:
        return None
    if len(node.pre_nodes) > 0:
        if node.pre_nodes[0].type in ops and len(node.pre_nodes[0].post_nodes) == 1 and node.pre_nodes[0].qconfig is not None:
            return node.pre_nodes[0]
        elif node.pre_nodes[0].type == str(nn.Identity) and \
            len(node.pre_nodes[0].post_nodes) == 1 and len(node.pre_nodes[0].pre_nodes) > 0:
        ...
        if len(node.pre_nodes) == 2:
            if node.pre_nodes[1].type in ops and len(node.pre_nodes[1].post_nodes) == 1 and node.pre_nodes[1].qconfig is not None:
                return node.pre_nodes[1]
            elif node.pre_nodes[1].type == str(nn.Identity) and \
                len(node.pre_nodes[1].post_nodes) == 1 and len(node.pre_nodes[1].pre_nodes) > 0:
...
def _find_conv_or_gemm_swish_fusion_node(node):
    ...
    sigmoid_ops = [str(torch.Tensor.sigmoid), str(torch.Tensor.sigmoid_), str(torch.sigmoid), str(torch.sigmoid_),\
        str(F.sigmoid), str(torch.nn.Sigmoid)]
    if node.type in mul_ops and len(node.pre_nodes) == 2:
        if node.pre_nodes[0].type in conv_gemm_ops and node.pre_nodes[1].type in sigmoid_ops:
            if len(node.pre_nodes[0].post_nodes) == 2 and len(node.pre_nodes[1].post_nodes) == 1 and \
                node.pre_nodes[1] in node.pre_nodes[0].post_nodes:
                return node.pre_nodes[0]
        elif node.pre_nodes[1].type in conv_gemm_ops and node.pre_nodes[0].type in sigmoid_ops:
            if len(node.pre_node[1].post_nodes) == 2 and len(node.pre_nodes[0].post_nodes) == 1 and \
                node.pre_nodes[0] in node.pre_node[1].post_nodes:
                return node.pre_nodes[1]
    return None
    ...
    if node.type == str(nn.Identity):
        ...
        if node.qconfig is None:
            ...
            if node.type in elt_wise_noq_ops:
                fused_elt_wise_node = _find_fused_node_with_cur_elt_wise(node, conv_gemm_ops + add_ops + add_inplace_ops)
                if fused_elt_wise_node is not None:
                    ...
                    if fused_elt_wise_node.type in add_inplace_ops:
                        fused_add_node = _find_fused_node_with_cur_add(node, conv_gemm_ops)
                        if  fused_add_node is not None and fused_add_node.qconfig is not None:
                            return True
                        else:
                            return False
                    else:
                ...
                if fused_add_wise_node is not None and fused_add_wise_node.qconfig is not None:
                    return True
            # conv+sigmoid+mul(_)
            fused_conv_or_gemm_swish_node = _find_conv_or_gemm_swish_fusion_node(node)
            if fused_conv_or_gemm_swish_node is not None and fused_conv_or_gemm_swish_node.qconfig is not None:
                return True
            return False
        else:
            if node.type in s8_s8_symmetric_ops:
                if node.type in [str(interaction), str(torch.ops.torch_ipex.interaction_forward)]:
...
def _add_recipe(node):
    ...
    if conv_gemm_node is None:
        ...
        if len(node.pre_nodes) > 0:
            add_1_has_pre_quantizable_op = _check_has_quantizable_node_before_node(node.pre_nodes[0])
            add_2_has_pre_quantizable_op = False
            if len(node.pre_nodes) == 2:
                add_2_has_pre_quantizable_op = _check_has_quantizable_node_before_node(node.pre_nodes[1])
...
def get_default_recipe(nodes):
    ...
    embedding_bag_ops = [str(torch.embedding_bag), str(F.embedding_bag), str(torch.nn.EmbeddingBag)]
    for node in nodes:
        if isinstance(node, ParentNode):
            continue
        if node.qconfig is not None and not node.type in (conv_gemm_ops + rnn_ops + embedding_bag_ops):
            if node.type in add_ops:
                ...
                if len(node.pre_nodes) > 0:
                    if _find_fused_node_with_cur_elt_wise(node, conv_gemm_ops + add_ops + add_inplace_ops) is not None:
                        has_pre_quantized_node = False
                    else:
                        has_pre_quantized_node = _check_has_quantizable_node_before_node(node.pre_nodes[0])
                else:
                    has_pre_quantized_node = False
