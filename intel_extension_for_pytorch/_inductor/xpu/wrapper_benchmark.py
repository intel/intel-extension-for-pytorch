...
def parse_profile_event_list(benchmark_name, event_list, wall_time_ms, nruns):
    def get_self_xpu_time(ev):
        """
        ev.self_xpu_time_total is in microsecond. Convert to millisecond.
        """
        return ev.self_xpu_time_total / 1000 / nruns
    ...
    def add_event(ev, category):
        profile_ev = ProfileEvent(
            category=category,
            key=ev.key,
            self_xpu_time_ms=get_self_xpu_time(ev),
    ...
    def report_category(category, profile_events):
        ...
        profile_events.sort(key=lambda ev: ev.self_xpu_time_ms, reverse=True)

        rows = []
        total_time = 0.0
        print(f"\n  == {category} category kernels == ")
        for ev in profile_events:
            total_time += ev.self_xpu_time_ms
            percent = f"{ev.self_xpu_time_ms / wall_time_ms * 100:.2f}%"
            rows.append([ev.key[:120], ev.self_xpu_time_ms, ev.count, percent])
        rows.append(
            ["Total", total_time, "", f"{total_time / wall_time_ms * 100:.2f}%"]
        )
        print(
            tabulate(rows, headers=["Kernel", "Self XPU TIME (ms)", "Count", "Percent"])
        )
        return total_time
    ...
    def report():
        ...
        gpu_busy_percent = f"{total_xpu_ms / wall_time_ms * 100:.2f}%"
...
def compiled_module_main(benchmark_name, benchmark_compiled_module_fn):
    ...
    if args.benchmark_kernels:
        ...
        # Note: Currently Level Zero Tracer does NOT work on multi processing.
        ...
        print(event_list.table(sort_by="self_xpu_time_total", row_limit=10))
