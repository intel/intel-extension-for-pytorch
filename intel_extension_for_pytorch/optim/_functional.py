...
# TODO: merge the lars with cpu and xpu
@torch.no_grad()
def lars_xpu_step(self, closure=None):
    """Performs a single optimization step.
    Args:
        closure (callable, optional): A closure that reevaluates the model
            and returns the loss.
    ...
    for group in self.param_groups:
        ...
        for i, param in enumerate(params_with_grad):
            # TODO: merge the lars fused function with cpu and xpu
            momentum_buffer_list[i] = torch.ops.torch_ipex.lars_xpu_fused_step(
                param,
                params2[i],
                d_p_list[i],
                momentum_buffer_list[i],
