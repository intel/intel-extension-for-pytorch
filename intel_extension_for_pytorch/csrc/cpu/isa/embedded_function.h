...
#define BIT_M_TO_N_64(x, m, n)  ((uint64_t)(x << (63-(n))) >> ((63 - (n)) + (m)))
...
__forceinline void read_cpuid(uint32_t func_id, uint32_t * p_eax, uint32_t * p_ebx, uint32_t * p_ecx, uint32_t * p_edx)
{
    int reg_data[4] = {0};
#if defined(__GNUC__)
...
__forceinline void read_cpuidex(uint32_t func_id, uint32_t sub_func_id, uint32_t * p_eax, uint32_t * p_ebx, uint32_t * p_ecx, uint32_t * p_edx)
{
    int reg_data[4] = {0};
#if defined(__GNUC__)
    __cpuid_count(func_id, sub_func_id, reg_data[0], reg_data[1], reg_data[2], reg_data[3]);
#elif defined(_MSC_VER)
...
__forceinline bool read_xcr(uint32_t ext_ctrl_reg, uint64_t * p_xcr)
{
    ...
    */
    ...
    ECX:
    BIT 27: OSXSAVE, A value of 1 indicates that the OS has set CR4.OSXSAVE[bit 18] to enable XSETBV/XGETBV
    instructions to access XCR
    */
