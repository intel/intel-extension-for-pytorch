...
class tensor : public memory {
 public:
  ...
  // Reorder data from *this to dst if dst's memory desc(size, stride, format,
  // etc) is different from *this;
  void reorder_to_if_differ_from(tensor& dst, const attr_t& aattr = attr_t())
      const {
