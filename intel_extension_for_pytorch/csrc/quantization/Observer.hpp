...
struct Observer {
  ...
  std::vector<std::vector<std::vector<float>>>
      weights_min_max_values; // per_channel or per_tensor
  ...
  // "int8" means input will be quantized to int8, otherwise quantized to
  // "uint8".
 ...
 public:
  Indicator(
      ...
      : id(i),
        name(n),
        algorithm(alg),
        weight_granularity(granu),
        input_params(i_params),
        weights_scales(std::move(w_scales)),
        output_params(o_params),
        input_quantized_dtypes(i_quantized_dtypes),
        output_quantized_dtypes(o_quantized_dtypes),
        inputs_quantized(inputs_quant),
        outputs_quantized(outputs_quant),
        inputs_flow(i_flow),
        outputs_flow(o_flow) {}
  ...
  get_indicator_insert_quantized_status() {
    return std::make_tuple(inputs_quantized, outputs_quantized);
  }

  void set_indicator_scales(
      std::vector<TensorQuantizationParams> new_input_params,
  ...
  void set_indicator_quantized_dtypes(
      std::vector<std::string> new_input_quantized_dtypes,
