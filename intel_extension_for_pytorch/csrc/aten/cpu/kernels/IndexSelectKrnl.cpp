...
#include <csrc/aten/cpu/TensorAdvancedIndexing.h>
...
void neg_kernel(at::TensorIteratorBase& iter) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      kComplexHalf, kBFloat16, kHalf, iter.dtype(), "neg_cpu", [&]() {
        at::native::cpu_kernel_vec(
            iter,
            [=](scalar_t a) -> scalar_t { return -a; },
            [=](at::vec::Vectorized<scalar_t> a) { return a.neg(); });
      });
}

void conj_kernel(at::TensorIteratorBase& iter) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
      ...
      [&]() {
        at::native::cpu_kernel_vec(
            iter,
            [=](scalar_t a) -> scalar_t { return at::native::conj_impl(a); },
            [=](at::vec::Vectorized<scalar_t> a) { return a.conj(); });
      });
}
...
void direct_copy_kernel(at::TensorIteratorBase& iter) {
  ...
  if (isQIntType(dtype)) {
    ...
    at::native::cpu_kernel(
        iter,
        [=](c10::complex<at::Half> a) -> c10::complex<at::Half> { return a; });
  } else {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
...
void neg_conj_kernel(at::TensorIteratorBase& iter) {
  // fused a = b.neg().conj_physical()
  AT_DISPATCH_COMPLEX_TYPES(iter.common_dtype(), "neg_conj_cpu", [&] {
    at::native::cpu_kernel_vec(
        iter,
        [=](scalar_t a) -> scalar_t { return -at::native::conj_impl(a); },
        [=](at::vec::Vectorized<scalar_t> a) -> at::vec::Vectorized<scalar_t> {
          return a.neg().conj();
        });
  });
}
