...
#include <csrc/aten/cpu/Normalization.h>
...
void batch_norm_cpu_collect_stats_kernel_impl(
  ...
  if (input.is_contiguous()) {
    ...
    TORCH_CHECK(
        false,
        "batch_norm_cpu_collect_stats_kernel_impl: expecting input "
        "to be contiguous.");
  }
}
...
void batch_norm_cpu_backward_kernel_impl(
  ...
  if (input.is_contiguous()) {
    ...
    TORCH_CHECK(
        false,
        "batch_norm_cpu_backward_kernel_impl: expecting input to be contiguous.");
  }
}
...
REGISTER_DISPATCH(
    ...
    &batch_norm_cpu_collect_stats_kernel_impl);
