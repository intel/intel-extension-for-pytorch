...
void cpu_channel_shuffle(
  ...
  at::parallel_for(
      0, nbatch * /* oc*g */ channels, 0, [&](int64_t begin, int64_t end) {
        int64_t n = 0;
        int64_t oc = 0;
        int64_t g = 0;
        at::native::data_index_init(
            begin, n, nbatch, oc, channels_per_group, g, groups);
        ...
        for (int64_t i = begin; i < end; i++) {
          ...
          at::vec::map(
              [](Vec x) { return x; }, output_ptr, input_ptr, image_size);

          // move on to next output index
          at::native::data_index_step(
              n, nbatch, oc, channels_per_group, g, groups);
        }
      });
}

// abstract parallel routine so as to avoid condition checks on perf critical
// paths.
...
void cpu_channel_shuffle_cl(
  ...
  bool is_float32 = input.scalar_type() == at::kFloat;
  ...
  if (G == 2 && is_float32) {
    parallel_nd(
        input_data, output_data, M, channels, [&](scalar_t* in, scalar_t* out) {
          int64_t k = 0;
          for (; k < inner_size; k += Vec::size()) {
            ...
            Vec ab0, ab1;
            std::tie(ab0, ab1) = at::vec::interleave2(a, b);
    ...
    parallel_nd(
        input_data, output_data, M, channels, [&](scalar_t* in, scalar_t* out) {
          ...
          for (; k < inner_size; k += Vec::size()) {
            ...
            Vec ac0, ac1, bd0, bd1;
            std::tie(ac0, ac1) = at::vec::interleave2(a, c);
            std::tie(bd0, bd1) = at::vec::interleave2(b, d);
            ...
            std::tie(abcd0, abcd1) = at::vec::interleave2(ac0, bd0);
            std::tie(abcd2, abcd3) = at::vec::interleave2(ac1, bd1);
        ...
        input_data, output_data, M, channels, [&](scalar_t* in, scalar_t* out) {
          for (int64_t grp = 0; grp < G; grp += Vec::size()) {
            ...
            for (; k < inner_size; k += Vec::size()) {
              ...
              Vec ae0, ae1, bf0, bf1;
              std::tie(ae0, ae1) = at::vec::interleave2(a, e);
              std::tie(bf0, bf1) = at::vec::interleave2(b, f);
              ...
              Vec cg0, cg1, dh0, dh1;
              std::tie(cg0, cg1) = at::vec::interleave2(c, g);
              std::tie(dh0, dh1) = at::vec::interleave2(d, h);
              ...
              std::tie(aceg0, aceg1) = at::vec::interleave2(ae0, cg0);
              std::tie(aceg2, aceg3) = at::vec::interleave2(ae1, cg1);
              ...
              std::tie(bdfh0, bdfh1) = at::vec::interleave2(bf0, dh0);
              std::tie(bdfh2, bdfh3) = at::vec::interleave2(bf1, dh1);

              // y_i = {a_i, b_i, c_i, d_i, e_i, f_i, g_i, h_i} : where
              // i = 0:7
              Vec y0, y1, y2, y3, y4, y5, y6, y7;
              std::tie(y0, y1) = at::vec::interleave2(aceg0, bdfh0);
              std::tie(y2, y3) = at::vec::interleave2(aceg1, bdfh1);
              std::tie(y4, y5) = at::vec::interleave2(aceg2, bdfh2);
              std::tie(y6, y7) = at::vec::interleave2(aceg3, bdfh3);
...
at::Tensor channel_shuffle(const at::Tensor& self, int64_t groups) {
  ...
  if (self.device().type() == c10::DeviceType::CPU &&
    ...
    channel_shuffle_kernel(output, input, groups);
