...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> batch_score_nms_cpu(
    const at::Tensor& dets,
    const at::Tensor& scores,
    const float threshold,
    const int max_output) {
...
std::tuple<std::vector<at::Tensor>, std::vector<at::Tensor>> rpn_nms_cpu(
    const at::Tensor& batch_dets,
    const at::Tensor& batch_scores,
    const std::vector<std::tuple<int64_t, int64_t>>& image_shapes,
    const int min_size,
