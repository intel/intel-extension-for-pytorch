...
class PagedAttention:
    ...
    - slot_mapping (torch.Tensor):  It stores the position to store the key/value in the pre-allocated buffers.
                                    The shape should be the number of sequences. For sequence _i_, the slot_mapping[i]//block_number
    ...
    Args:
    - out (torch.Tensor): The output tensor with shape of [num_seqs, num_heads, head_size]. where the num_seqs
                          is the number of the sequence in this batch. The num_heads means the number of query
                          head. head_size means the head dimension.
