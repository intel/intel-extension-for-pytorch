...
from .cpu.launch import init_parser as cpu_init_parser, run_main_with_args as cpu_run_main_with_args
from .xpu.launch import init_parser as xpu_init_parser, run_main_with_args as xpu_run_main_with_args
...
def init_parser():
    ...
    cpu_parser = subparsers.add_parser('cpu', help="Run with CPU Backend", 
    ...
    xpu_parser = subparsers.add_parser('xpu',help="Run with XPU Backend",
...
def main():
    ...
    parser.print_help = partial(mixed_print_help,
    ...
    if len(sys.argv) > 1 and '-h' not in sys.argv and '--help' not in sys.argv \
            and sys.argv[1] != 'cpu' and sys.argv[1] != 'xpu':
        warnings.warn('Backend is not specified, it will automatically default to cpu.', UserWarning)
