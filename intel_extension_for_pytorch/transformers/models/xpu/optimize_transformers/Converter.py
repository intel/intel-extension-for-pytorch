...
class Converter:
    ...
    def ds_autotp_enabled(self, model, enable_ds=False):
        ...
        if isinstance(model, deepspeed.InferenceEngine):
            ...
            if (
                self.module_replacer.tp_size > 1
                and self.module_replacer.tp_size is not None
            ):
                return True
        return False
    ...
    def create_model_parallel_group(self):
        if self.tp_size < 2:
            ...
            return
        if (
            not hasattr(EnvParam, "tp_group")
            or getattr(EnvParam, "tp_group", None) is None
        ):
            # init_distributed()
            ranks = range(self.tp_size)
    ...
    def convert_model(self, model, dtype):
        try:
            ...
            print(
                "Warning: we didn't find Huggingface's transformers package in your environments, please install it first, "
                "otherwise no feature of optimize_transformers will work"
            )
            return model
        enable_ds = False
        _ = transformers.FlavaModel  # to avoid flake8 issue
        try:
            import deepspeed
        except ImportError as e:
            print(
        ...
        if self.ds_kernel_injection_enabled(model, enable_ds):
            assert False, (  # noqa B011
                "Deepspeed's kernel injection is not supported in IPEX, please turn off the kernel"
                " injection option in Deepspeed and re-run the script."
            )
        dtype_tag, dtype = self.mapping_dtype(dtype)
        setattr(model, "dtype_tag", dtype_tag)  # noqa
