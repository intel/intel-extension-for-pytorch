...
class WeightLoader:
    ...
    def _copy(self, dst, src, prefix, allocate_tensor=False):
        ...
        for i in range(len(src_shape)):
            ...
            ), "src shape should be larger or equal than dst shape in dim {}".format(i)
        assert (
            len(src_shape) <= 2 and len(dst_shape) <= 2
        ), "copy in weightload only support the two dim tensor,"
        " {} is not follow this rule".format(prefix)
        if len(src_shape) == 2:
    ...
    def _qkv_copy(self, dst, src, num_splits, name, allocate_tensor):
        if src is None:
            return src
        src_shape = src.shape
        dst_shape = dst.shape
        ...
        if len(src_shape) == 2 and len(dst_shape) == 2:
            if src_shape[1] == dst_shape[1]:
                dst = (
                    dst.reshape(-1).data.copy_(src.data.reshape(-1)).reshape(src.shape)
                )
                dst = torch.nn.parameter.Parameter(dst, requires_grad=False)
                return dst
            assert (
                src_shape[1] > dst_shape[1]
            ), "src shape should larger than dst shape when copy"
            qkv_size = dst_shape[1] // num_splits
            qkv_split = [torch.split(src_s, qkv_size, dim=1) for src_s in src_split]
            weight_split = [
                torch.cat([qkv_s[i] for qkv_s in qkv_split], axis=1)
                for i in range(len(qkv_split[0]))
            ]
            dst = (
                dst.reshape(-1)
                .data.copy_(weight_split[self.gpu_idex].contiguous().reshape(-1))
                .reshape(weight_split[self.gpu_idex].shape)
            )
        else:
    ...
    def _weight_load_scheme(self, module, name):
        if getattr(module, "replaced", False) is False:
        ...
        assert (
            tp_weight is not None
        ), "Found no tp weight in the replaced module named {}".format(name)
        ...
        if model_weight is not None and tp_weight is not None:
            ...
            for i in range(len(tp_shape)):
                if tp_shape[i] != weight_shape[i]:
                    assert (
                        weight_shape[i] // self.tp_size == tp_shape[i]
                    ), "mp size is not consistant"
                    " between WeightLoader and TensorSlicer, please check your config"
                    data = model_weight.data.split(tp_shape[i], dim=i)[
                        self.gpu_idex
                    ].to(self.device)
                    param_weight = torch.nn.parameter.Parameter(
                        data, requires_grad=False
                    )
                    module.weight.data = param_weight
            setattr(module, "tp_weight", None)  # noqa

        # load bias
        if model_bias is not None and tp_bias is not None:
            if model_bias.shape != tp_bias.shape:
                assert (
                    model_bias.shape // self.tp_size == tp_bias.shape
                ), "mp size is not consistant"
                "between WeightLoader and TensorSlicer, please check your config"
                data = model_bias.data.split(tp_bias.shape, dim=0)[self.gpu_idex].to(
                    self.device
                )
                para_bias = torch.nn.parameter.Parameter(data, requires_grad=False)
                module.bias.data = para_bias
            setattr(module, "tp_bias", None)  # noqa
        return
        ...
        if hasattr(module, "weight"):
            ...
            if meta_weight is not None:
                data = torch.nn.parameter.Parameter(
                    torch.empty_like(meta_weight, device="cpu")
                )
                module.weight = data
            ...
            if hasattr(module, "norm") and hasattr(module.norm, "weight"):
                if module.norm.weight.data.is_meta:
                    module.norm.weight = torch.nn.parameter.Parameter(
                        torch.empty_like(module.norm.weight.data, device="cpu")
                    )
                sd_key = prefix + "weight"
                sd_weight = state_dict[sd_key]
        ...
        if prefix + "bias" in state_dict.keys():
            if hasattr(module, "bias"):
                ...
                if meta_bias is not None:
                    data = torch.nn.parameter.Parameter(
                        torch.empty_like(meta_bias, device="cpu")
                    )
                    module.bias = data
                sd_key = prefix + "bias"
                ...
                if hasattr(module, "norm") and hasattr(module.norm, "bias"):
                    if module.norm.bias.data.is_meta:
                        module.norm.bias = torch.nn.parameter.Parameter(
                            torch.empty_like(module.norm.bias.data, device="cpu")
                        )
                    sd_key = prefix + "bias"
                    sd_bias = state_dict[sd_key]
    ...
    def _load_weight_from_ckpt_recursive(self, model, sd, recursive_depth, prefix=""):
        for name, child in model.named_children():
            ...
            check_name = (
                ""
                if recursive_depth == 0 and self.hidden_prefix_0_if_necessary(model, sd)
                else check_name
            )
            module_cls = [nn.Linear, nn.LayerNorm, nn.Embedding]
            if (
                type(child) in module_cls
                and check_name != ""
                and any(check_name in item for item in sd.keys())
            ):
                self._load_weight_from_state_dict(child, sd, check_name)
                if (
                    "word_embeddings." in check_name
                    or "embed_tokens." in check_name
                    or "wte." in check_name
                ):
                    self.weight_embedding = child
            else:
                # self._load_attr_from_sliced_block(child)
                self._load_weight_from_ckpt_recursive(
                    child, sd, recursive_depth + 1, check_name
                )
            if len(child._buffers) != 0 and any(
                check_name in item for item in sd.keys()
            ):
    ...
    def _load_weight_from_ckpt(self, model, ckpt):
        ...
        if embedding_weight is not None and hasattr(model, "lm_head"):
            if hasattr(model.lm_head, "weight") and model.lm_head.weight.is_meta:
    ...
    def _load_weight_from_model(self, model):
        ...
        if device is not None:
            assert (
                device == "meta"
            ), "Can not port data from a meta device model to our optimized model, "  # noqa
            "please provide checkpoint path or load the model with real data type"
        for name, child in model.named_children():
    ...
    def load_weight_if_necessary(self, model):
        if self.ckpt_dict is not None:
            ...
            progress = tqdm.tqdm(
                total=len(ckpt), desc=f"Loading {len(ckpt)} checkpoint shards"
            )
            for elem in ckpt:
            ...
            assert (
                model.device != "meta"
            ), "This model is runing on tensor parallel mode on meta device"
            "but with no ckpt provided, please provide ckpt for running !"
            print(
                "this model have been properly sliced in slicer, load weight from original model"
            )
