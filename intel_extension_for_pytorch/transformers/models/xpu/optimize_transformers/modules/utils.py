...
def pad_for_gptj_lm_head(model, is_int4=False):
    ...
    if is_int4 and isinstance(model.lm_head, WeightOnlyQuantizedLinear):
        ...
        lm_head_new = WeightOnlyQuantizedLinear(
            in_features=model.lm_head.in_features, out_features=n
        )
        ...
        if hasattr(model.lm_head, "qzeros") and model.lm_head.qzeros is not None:
