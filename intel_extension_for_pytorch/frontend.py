...
class _O1:
    def __call__(self, properties):
        ...
        # TODO: implement split master weight later. so now set False
        properties.split_master_weight_for_bf16 = False
...
def optimize(
    ...
    Args:
        ...
        TODO: temp default true for xpu because copy model will add memory pressure
        ...
        fuse_update_step (bool): Whether to use fused params update for training
            ...
            The default value is ``None``. Explicitly setting this knob
            overwrites the configuration set by ``level`` knob.         
        sample_input (tuple or torch.Tensor): Whether to feed sample input data 
    ...
    Returns:
        ...
        parameters of convolution and linear will be casted to associated dtype.
    ...
    if optimize_lstm is not None:
        ...
        raise RuntimeError("for now, no implementation for LSTM")
    # TODO: master weight is ready while the split master weight is pending
    ...
    # sample input only for inference weight prepack
    if sample_input is not None and not model.training:
        if isinstance(sample_input, torch.Tensor):
            sample_input = (sample_input,)
        utils._weight_prepack.record_input_shape_for_prepack(optimized_model, sample_input)

    if not model.training:
        # conv bn folding can be done on both imperative model and jit model
        ...
        if dtype == torch.bfloat16:
            ...
            optimized_model = utils._model_convert.convert_module_data_type(optimized_model, torch.float16)
        else:
            pass
    ...
    # TODO: split master weight is low priority for us now
    ...
    if opt_properties.weights_prepack and not model.training:
        optimized_model = utils._weight_prepack.weight_prepack_with_ipex(optimized_model)
