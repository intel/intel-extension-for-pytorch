...
class _IPEXDynamicQuantizedLinear(nnqd.Linear):
    ...
    def from_float(cls, mod):
        assert type(mod) in cls._float_module(), \
            'DynamicQuantizedLinearLayer.from_float only works for one of' + \
            str([float_mod.__name__ for float_mod in cls._float_module()])
        ...
        assert dtype in [torch.qint8], "The only supported dtypes for " \
...
class DynamicQuantizedLinearAllreduce(_IPEXDynamicQuantizedLinear):
    @classmethod
    def _init_cls(cls, mod, dtype, qweight):
        qlinear = cls(mod.weight.size()[1], mod.weight.size()[0], mod.mp_group, mod.bias, dtype=dtype)
    ...
    def _float_module(self):
        deepspeed_modules = may_import_deepspeed_modules()
        assert deepspeed_modules is not None, "DynamicQuantizedLinearAllreduce requires deepspeed to be installed"
        LinearAllreduce, _ = deepspeed_modules
        _FLOAT_MODULE = [LinearAllreduce]
        return _FLOAT_MODULE

    def __init__(self, in_features, out_features, mp_group, bias_value, bias_=True, dtype=torch.qint8):
...
def convert(
    ...
    if isinstance(convert_model.q_config.activation(), PlaceholderObserver):
        ...
        return torch.quantization.quantize_dynamic(convert_model, qconfig_spec=qconfig_spec, mapping = module_mappings, inplace=True)
