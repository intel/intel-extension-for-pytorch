...
class DataArguments:
    data_path: str = field(
        default=None, metadata={"help": "Path to the specified training data, by default, use alpaca data"})
...
def train():
    ...
    # this is a WA for torch adamw fuse only support devices of cuda, xpu, privateuseone
    model = model.to('cuda')
    ...
    # User can specify the dataset, by default, we support alpaca dataset; alternatively, user could use the datasets from huggingface datasets.Dataset
    if data_args.data_path is not None:
        ...
        trainer = SFTTrainer(
            ...
            dataset_text_field="text",
            # formatting_func=globals()[f"{data_args.dataset_type}_formatter"],
            packing=True,
