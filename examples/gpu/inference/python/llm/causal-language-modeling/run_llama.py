...
parser.add_argument("--dataset_path", nargs="?", default="lambada", const="lambada")
...
if args.dtype == "bfloat16" or args.dtype == "float32" or args.dtype == "float16":
    ...
    if args.disable_optimize_transformers:
        user_model = ipex.optimize(user_model.eval(), dtype=amp_dtype)
    else:
        user_model = ipex.optimize_transformers(user_model.eval(), dtype=amp_dtype)
tokenizer = transformers.LlamaTokenizer.from_pretrained(args.model)
print("Data type of the model:", user_model.dtype, args.dtype, amp_dtype, amp_enabled)
...
class Evaluator:
    ...
    def evaluate(self, model):
        ...
        for i, (
            ...
            key_values = list(past_key_values)
            for idx1, key_value1 in enumerate(key_values):
                key_value1 = list(key_value1)
                for idx2, key_value2 in enumerate(key_value1):
                    key_value1[idx2] = key_value2.to("xpu")
                key_values[idx1] = tuple(key_value1)
            past_key_values = tuple(key_values)

            for idx, attn in enumerate(attention_mask):
                attention_mask[idx] = attn.to("xpu")
            ...
            last_token_logits = output[torch.arange(len(last_ind)), -2 - pad_len, :]
