...
def load_imdb_dataset(batch_size=16, max_samples=5000):
    ...
    def tokenize_function(examples):
        return tokenizer(examples['text'], padding="max_length", truncation=True, max_length=512, return_token_type_ids=True)
...
def quantize_model(model, test_loader):
    ...
    prepared_model = ipex.quantization.prepare(model, qconfig, example_inputs=example_data, inplace=False)
    ...
    example_data = example_data['input_ids'], example_data['attention_mask'], example_data['token_type_ids']
