...
if [ $((${MODE} & 0x02)) -ne 0 ]; then
    ...
    echo "python -m pip install cpuid accelerate datasets==2.21.0 sentencepiece mkl protobuf==${VER_PROTOBUF} transformers==${VER_TRANSFORMERS} neural-compressor==${VER_INC} transformers_stream_generator tiktoken unzip librosa soundfile" >> ${AUX_INSTALL_SCRIPT}
