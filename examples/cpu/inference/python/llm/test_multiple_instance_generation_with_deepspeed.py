...
for i in range(args.request_number):    
    ...
    tokenizer = tokenizer_list[model_id]
    total_time = 0.0
    ...
    if args.profile:
        with torch.profiler.profile(
            ...
            for i in range(5):
                gen_ids, outputs = generate(model, tokenizer_list)
                prof.step()
    # latency
    ...
    print_rank0("Model_{} ".format(model_id), "\n", "-" * 10, "Summary:", "-" * 10)
    print_rank0("Model_{} ".format(model_id), "Inference latency: %.3f sec." % latency)
    if args.token_latency:        
