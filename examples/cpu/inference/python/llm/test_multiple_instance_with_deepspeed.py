...
parser.add_argument(
    "--request-number", default=10, type=int, help="The number of request."
)
...
generated, _ = generate(model, tokenizer)
...
for i in range(args.request_number):
    idx = np.random.randint(0, args.instance_number)
    model_idx_tensor = torch.tensor([idx], dtype=torch.long)
    deepspeed.comm.broadcast(model_idx_tensor, src=0)
    time_m_0 = time.time()
    model_id = model_idx_tensor.item()
    ...
    time_m_1 = time.time()
    if i != 0:
         print_rank0("Model preparation time: changed from model_id: {} to {} in %.3f ms" % pre_model_id, model_id, (time_m_0 - time_m_1) * 1000)
    pre_model_id = model_id
    # latency
    for i in range(cycles):
        t0 = time.time()
        gen_ids, outputs = generate(model, tokenizer)
        t1 = time.time()
    ...
    if args.token_latency:
        ...
        print_rank0("Model_{} ".format(model_id), "First token average latency: %.3f sec." % first_latency)
        print_rank0("Model_{} ".format(model_id), "Average 2... latency: %.3f sec." % average_2n_latency)
        print_rank0("Model_{} ".format(model_id), "P90 2... latency: %.3f sec." % p90_latency)
        print_rank0("Model_{} ".format(model_id), "P99 2... latency: %.3f sec." % p99_latency)
