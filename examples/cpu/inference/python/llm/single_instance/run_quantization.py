...
if args.ipex_smooth_quant:
    ipex_smooth_quant_model_list = ["meta-llama/Llama-2-7b-hf", "meta-llama/Llama-2-13b-hf", "meta-llama/Llama-2-70b-hf", "EleutherAI/gpt-j-6b"]
    assert args.model_id in ipex_smooth_quant_model_list, args.model_id + " does not support ipex_smooth_quant"
    class Evaluator:
        def __init__(self, dataset, tokenizer, args, batch_size=1, pad_val=1, pad_max=512):
        ...
        def collate_batch(self, batch):
            ...
            return (
                (
                    ...
                    tuple(global_past_key_value) if model.use_global_past_key_value else tuple(past_key_value),
                ),
                torch.tensor(last_ind),
            )

    calib_dataset = load_dataset(args.dataset if args.dataset else model.default_dataset, split="train")
    user_model.eval()
