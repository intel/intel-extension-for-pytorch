...
parser = argparse.ArgumentParser("T5 generation script (int8 path)", add_help=False)
parser.add_argument(
    "-m",
    "--model-id",
    type=str,
...
if args.benchmark and args.jit and not args.ipex_weight_only_quantization:
    try:
        with ipex._IPEXOnDevice(dtype=torch.float, device="meta"):
            user_model = T5ForConditionalGeneration.from_config(config)
    except:
        user_model = T5ForConditionalGeneration.from_pretrained(
            args.model_id, config=config, low_cpu_mem_usage=True
        )
else:
    user_model = T5ForConditionalGeneration.from_pretrained(
...
kv_cache_dtype = (
    torch.bfloat16 if args.kv_cache_dtype == "None" else args.kv_cache_dtype
)
        ...
        (
            ...
            torch.zeros(
                [
                    ...
                    user_model.config.num_heads,
                    user_model.config.d_kv,
                ],
...
class Evaluator:
    ...
    def collate_batch(self, batch):
            ...
            [
                (
                    ...
                    .view(
                        int(args.batch_size * num_beams),
                        -1,
                        user_model.decoder.block[i].layer[1].EncDecAttention.n_heads,
                        user_model.decoder.block[i]
                        .layer[1]
        ...
        decoder_input_ids = (
            torch.zeros(int(args.batch_size * num_beams)).to(torch.long).unsqueeze(1)
        )
        return (
            (
                decoder_input_ids,
    ...
    def evaluate(self, model):
        ...
        for i, (
            (input_ids, attention_mask, past_key_values, encoder_outputs),
            last_ind,
...
if args.lambada:
    ...
    calib_dataset = full_dataset["validation"]

    user_model.eval()
    evaluator = Evaluator(
        dataset, tokenizer, args.batch_size, pad_max=int(args.input_tokens)
    )
    calib_evaluator = Evaluator(
        calib_dataset, tokenizer, args.batch_size, pad_max=int(args.input_tokens)
    )
...
if args.ipex_smooth_quant:
    ...
    prepared_model = prepare(
        user_model.eval(), qconfig, example_kwarg_inputs=example_inputs, inplace=True
    )
    with torch.no_grad():
        for i, (
            (input_ids, attention_mask, past_key_values, encoder_outputs),
...
if args.benchmark:
    ...
    if args.prompt is not None:
        ...
        prompt = prompt_pool["t5"]["8192"] * int(int(args.input_tokens) / 8192)
    elif args.input_tokens in prompt_pool["t5"]:
        prompt = prompt_pool["t5"][args.input_tokens]
    else:
