...
if [[ "${PRECISION}" == "bf16" ]]; then
    ...
    echo "Supported precisions are: fp32, fp16, bf16, int8-bf16, int8-fp32"
    exit 1
fi
...
if [[ "${TEST_MODE}" == "THROUGHPUT" ]]; then
    LOG_PREFIX="LCM_${PRECISION}_inference_throughput"
    ...
    if [[ "${DISTRIBUTED}" == "True" || "${DISTRIBUTED}" == "TRUE" ]]; then
        LOG_PREFIX="LCM_${PRECISION}_dist_inference_accuracy"
        ...
        LOG_PREFIX="LCM_${PRECISION}_inference_accuracy"
