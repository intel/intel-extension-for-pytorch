...
# TODO: `TrainingArguments` users rely on it being fully mutable. In the future see if we can narrow
# this to a few keys: https://github.com/huggingface/transformers/pull/25903
@dataclass
class TrainingArguments:
    ...
    int8: bool = field(
        default=False,
        metadata={"help": ("Whether to use int8 (mixed) precision instead of 32-bit")},
    )
    int8_fp32: bool = field(
        default=False,
        metadata={
            "help": ("Whether to use int8_fp32 (mixed) precision instead of 32-bit")
        },
    )
    int8_bf16: bool = field(
        default=False,
        metadata={
            "help": ("Whether to use int8_bf16 (mixed) precision instead of 32-bit")
        },
    )
    fp8: bool = field(
        default=False,
        metadata={"help": ("Whether to use fp8 (mixed) precision instead of 32-bit")},
    )
    fp8_fp32: bool = field(
        default=False,
        metadata={
            "help": ("Whether to use fp8_fp32 (mixed) precision instead of 32-bit")
        },
    )
    fp8_bf16: bool = field(
        default=False,
        metadata={
            "help": ("Whether to use fp8_bf16 (mixed) precision instead of 32-bit")
        },
    )
    use_share_weight: bool = field(
        default=False,
        metadata={"help": "Enable weight sharing for real time mode inference"},
    )
    total_cores: int = field(
        default=56,
        metadata={"help": "Total cores one socket for use_share_weight"},
    )
    cores_per_instance: int = field(
        default=4,
        metadata={"help": "cores per instance for use_share_weight"},
    )
    do_calibration: bool = field(
        default=False,
        metadata={"help": "Enable calibration process for ipex int8"},
    )
    calibration_iters: int = field(
        default=200,
        metadata={"help": "The iterations for calibration"},
    )
    smooth_quant: bool = field(
        default=False,
        metadata={"help": ("Whether to use smoothQuant for int8 (mixed) precision")},
    )

    int8_config: str = field(
        default="", metadata={"help": "The calibration result for int8 config"}
    )
    fp8_config: str = field(
        default="", metadata={"help": "The calibration result for fp8 config"}
    )
    auto_kernel_selection: bool = field(
        default=False,
        metadata={"help": "Enable mkldnn for ipex fp32"},
    )
    ...
    perf_run_iters: int = field(
        default=100,
        metadata={"help": "The iterations number for benchmark"},
    )
    perf_begin_iter: int = field(
        default=10,
        metadata={"help": "The iteration to start the benchmark iterations"},
    )
    fp16: bool = field(
        default=False,
    ...
    fp16_cpu: bool = field(
        default=False,
        metadata={
            "help": "Whether to use fp16 (mixed) precision instead of 32-bit on cpu using IPEX"
        },
    )
    inductor: bool = field(
        default=False,
    ...
    profile: bool = field(default=False, metadata={"help": "enable profile"})
    ...
    # optim (`str` or [`training_args.OptimizerNames`], *optional*, defaults to `"adamw_torch_fused"`
    # (for torch<2.1.0 `"adamw_torch"`):
    ...
    dataloader_persistent_workers: bool = field(
        default=False,
        metadata={
            "help": "If True, the data loader will not shut down the worker processes "
            "after a dataset has been consumed once. This allows to maintain the workers "
            "Dataset instances alive. Can potentially speed up training, but will increase RAM usage."
        },
    )
    ...
    gradient_checkpointing_kwargs: Optional[dict] = field(
        ...
        metadata={
            ...
            "Will be passed to `torch.utils.checkpoint.checkpoint` through `model.gradient_checkpointing_enable`."
        },
    )
    ...
    include_num_input_tokens_seen: Optional[bool] = field(
        default=False,
        metadata={
            "help": "If set to `True`, will track the number of input tokens seen "
            "throughout training. (May be slower in distributed training)"
        },
    )
    ...
    def __post_init__(self):
        ...
        if len(self.fsdp) > 0 and not self.fsdp_config["xla"]:
            ...
            prefetch_policy = self.fsdp_config.get(
                "fsdp_backward_prefetch", "NO_PREFETCH"
            )
            os.environ[f"{prefix}BACKWARD_PREFETCH"] = prefetch_policy.upper()
            os.environ[f"{prefix}FORWARD_PREFETCH"] = self.fsdp_config.get(
                "forward_prefect", "false"
            )
            os.environ[f"{prefix}SYNC_MODULE_STATES"] = self.fsdp_config.get(
                "sync_module_states", "true"
            )
            os.environ[f"{prefix}USE_ORIG_PARAMS"] = self.fsdp_config.get(
