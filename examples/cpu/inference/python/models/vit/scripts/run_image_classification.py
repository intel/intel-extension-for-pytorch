...
def main():
    ...
    if data_args.dataset_name is not None and data_args.dataset_name == "dummy":
        ...
        class DummyImageDataset(torch.utils.data.Dataset):
            def __init__(
                self, num_samples=1000, image_size=(3, 224, 224), num_classes=1000
            ):
                self.num_samples = num_samples
                self.image_size = image_size
                self.num_classes = num_classes
                self.data = [
                    torch.randn(*self.image_size) for i in range(self.num_samples)
                ]
                self.labels = [
            ...
            def features(self):
                return {
                    "label": ClassLabel(names=[str(i) for i in range(self.num_classes)])
                }
        ...
        dataset = {
            "train": DummyImageDataset(),
            "validation": DummyImageDataset(training_args.per_device_eval_batch_size),
        }
    else:
