...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="doing calibration step for int8/fp8",
    )
    parser.add_argument(
        "--compile_inductor",
        action="store_true",
...
def main():
    ...
    if args.precision == "fp32":
        ...
        args.dtype = torch.float32
    elif args.precision == "fp8-bf16":
        print("Running fp8-bf16 ...")
        args.dtype = torch.bfloat16
    elif args.precision == "fp8-fp32":
        print("Running fp8-fp32 ...")
        args.dtype = torch.float32
    else:
        raise ValueError(
    ...
    if args.model_name_or_path == "stabilityai/stable-diffusion-2-1":
        inputs = (torch.randn(2, 4, 96, 96), torch.tensor(981))
        extra_kargs = {
        ...
        raise ValueError(
            "This script currently only supports stabilityai/stable-diffusion-2-1."
        )
    ...
    if args.compile_inductor:
        ...
        if args.precision == "fp32":
            ...
            if args.calibration:
                ...
                if args.precision == "int8-fp32":
                    ...
                    with torch.autocast("cpu"), torch.no_grad():
                        ...
                        pipe.text_encoder = torch.compile(pipe.text_encoder)
                        pipe.vae.decode = torch.compile(pipe.vae.decode)
        elif args.precision == "fp8-fp32" or args.precision == "fp8-bf16":
            if args.calibration:
                from util import prepare, finalize_calib

                pipe.unet = prepare(pipe.unet.eval())
                pipe(args.prompt)
                finalize_calib()
                print(".........calibration step done..........")
                exit(0)
            ...
            raise ValueError(
                "If you want to use torch.compile with inductor backend, --precision needs "
                "to be the following: fp32, bf16, int8-bf16, int8-fp32, fp8-fp32, fp8-bf16"
            )
