...
if [[ "${PRECISION}" == "int8-bf16" || "${PRECISION}" == "int8-fp32" ]]; then
  ARGS="$ARGS --quantized_model_path=${INT8_MODEL} --precision=${PRECISION}"
elif [[ "${PRECISION}" == "fp8-bf16" || "${PRECISION}" == "fp8-fp32" ]]; then
  ARGS="$ARGS --precision=${PRECISION}"
else
  echo "The specified precision '${PRECISION}' is unsupported."
  echo "Supported precisions are: int8-bf16, int8-fp32, fp8-bf16, fp8-fp32"
  exit 1
fi
