...
elif [[ "${PRECISION}" == "int4-bf16" ]]
then
    precision="int4-bf16"
    ARGS="$ARGS --dtype int8-bf16 --weight-dtype INT4"
    echo "### running int4-bf16 mode"
elif [[ "${PRECISION}" == "int4-int8" ]]
then
    precision="int4-int8"
    ARGS="$ARGS --dtype int8 --weight-dtype INT4"
    echo "### running int4-int8 mode"
else
    echo "The specified precision '${PRECISION}' is unsupported."
    echo "Supported precisions are: fp32, bf32, bf16, fp16, int8-fp32, int8-bf16, int8-int8, int4-bf16, int4-int8"
    exit 1
fi
...
if [[ "$TEST_MODE" != "ACCURACY" ]]; then
    ...
    python -m torch.backends.xeon.run_cpu --disable-numactl --log_path=${OUTPUT_DIR} \
        ${EVAL_SCRIPT} $ARGS \
        --inductor \
        --model-name-or-path ${FINETUNED_MODEL} > ${OUTPUT_DIR}/GPTJ_${PRECISION}_accuracy.log
