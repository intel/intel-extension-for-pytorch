...
class VersionAnalyzer:
    def __init__(self):
        ...
        try:
            ...
            self.THAtenRegistDeclare = os.path.join(
                torch_path,
                "include",
                "torch",
                "csrc",
            ...
            self.ExtAtenRegistDeclare = os.path.join(
                cwd, "reference", "RegistrationDeclarations.h"
            )
        except Exception:
    ...
    def analysis(self):
        diff = difflib.unified_diff(
            open(self.THAtenRegistDeclare).readlines(),
            open(self.ExtAtenRegistDeclare).readlines(),
        )
...
def main():
    ...
    if len(op_list) > 0:
        print(
            "Warning: The Pytorch Extension has conflicting operation declarations with currently installed Pytorch, you should consider upgrading them."
        )
        print(
            "Warning: The following Op have conflicting definition: ",
            ", ".join(op_list),
        )
    if len(new_op) > 0:
    ...
    if len(removed_op) > 0:
        print(
            "Warning:", ", ".join(removed_op), " is not supported in Pytorch Extension!"
        )
