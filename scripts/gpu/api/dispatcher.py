    ...
    if local.use_c10_dispatcher() is UseC10Dispatcher.full:
        return list(
            map(
                argument,
                itertools.chain(
                    func.out_arguments, func.arguments, func.kwarg_only_arguments
                ),
            )
        )
    else:
...
def cppargument_exprs(
    a: CppArgument, *, tensor_options: Optional[CppArgument]
    ...
    if isinstance(a.argument, TensorOptionsArguments):
        if local.use_c10_dispatcher() is UseC10Dispatcher.full:
            ...
            return [
                ...
                DispatcherExpr(
                    type=argument_type(ta.layout), expr=f"{a.name}.layout_opt()"
                ),
                DispatcherExpr(
                    type=argument_type(ta.device), expr=f"{a.name}.device_opt()"
                ),
                DispatcherExpr(
                    type=argument_type(ta.pin_memory),
                    expr=f"{a.name}.pinned_memory_opt()",
                ),  # weird discrep
            ]
        else:
        ...
        if (
            a.name == "memory_format"
            and tensor_options is not None
            and local.use_c10_dispatcher() is UseC10Dispatcher.full
        ):
            return [
                DispatcherExpr(
...
def cpparguments_exprs(args: Sequence[CppArgument]) -> Sequence[DispatcherExpr]:
    tensor_options = next(
        (a for a in args if isinstance(a.argument, TensorOptionsArguments)), None
    )
    return [
        r for a in args for r in cppargument_exprs(a, tensor_options=tensor_options)
    ]


# I don't think this is entirely sound, but it should be reasonably
# close
def legacydispatcherarguments_exprs(
    args: Sequence[LegacyDispatcherArgument],
) -> Sequence[DispatcherExpr]:
    return cpparguments_exprs(
        [
            CppArgument(type=a.type, name=a.name, default=None, argument=a.argument)
            for a in args
        ]
    )
