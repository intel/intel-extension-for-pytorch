...
def gpu_gen_and_should_copy(install_dir):
    ...
    files_name = [
        "aten_ipex_type_default.cpp.in",
        "aten_ipex_type_default.h.in",
        "aten_ipex_type_dpcpp.h.in",
    ]
    generated_files = [os.path.join(os.path.abspath(cwd), file) for file in files_name]
    source_files = [
        os.path.join(os.path.abspath(install_dir), os.path.splitext(file)[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--install_dir", help="output directory", default=".")
