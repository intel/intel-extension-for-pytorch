...
def load_aten_declarations(path):
    ...
    for declaration in declarations:
        ...
        declaration["schema_order_args"] = [
            arg["name"] for arg in declaration["schema_order_arguments"]
        ]
        if has_tensoroptions_argument(declaration):
            declaration["schema_order_args"] = [
                process_schema_order_arg(arg)
                for arg in declaration["schema_order_args"]
            ]
...
def handle_outputs_taken_as_arguments(options):
    ...
    def is_nullable(arg):
        return arg["type"] in {"THIntegerTensor*", "THTensor*"} and arg.get(
            "default", ""
        ) in {None, "NULL", "nullptr"}
...
def native_get_formals(option, schema_order, use_optional_tensor):
    ...
    def native_translate_formals(argument, option):
        ...
        if (
            use_optional_tensor
            and argument.get("is_nullable")
            and argument["type"] == "const c10::optional<Tensor>&"
        ):
            ...
            if backend in option["backend_types"]:
                ...
                if option["overload_name"]:
                    option["native_type_method_dispatch"] = (
                        native_dispatch + "_" + option["overload_name"]
                    )
                else:
                ...
                candidate_args = itertools.chain(
                    self_args, func.out_arguments, func.arguments
                )
                ...
                has_tensor_options = any(
                    isinstance(a.argument, TensorOptionsArguments) for a in args
                )
...
def get_lazy_reorder(schema_name, argument):
    ...
    if schema_name not in lazy_reorder_block_list:
        if cptype == "TensorList":
            return (
                LAZY_REORDER_TENSORLIST.substitute(change_dict),
            ...
            if argument["type"] == "const c10::optional<Tensor>&":
                ...
                return (
                    LAZY_REORDER_CONST_TENSOR.substitute(change_dict),
                    change_dict["temp_name"],
                )
    ...
    for declaration in declarations:
        ...
        for option in declaration["options"]:
            ...
            if declaration["use_c10_dispatcher"] == "full":
                ...
                lazy_reorder = [
                    get_lazy_reorder(option["name"], argument)
                ...
                native_order_cpp_signature = "{} ( {} )".format(
                    option["return_type"],
                    ", ".join([f["type"] for f in native_formals]),
                )
                option["native_order_cpp_signature"] = native_order_cpp_signature
            elif (
                declaration["use_c10_dispatcher"]
                == "with_codegenerated_unboxing_wrapper"
            ):
...
def process_dpcpp_declarations(declarations, script_path):
    ...
    for declaration in declarations:
        if declaration["schema_string"] in schemas:
            declaration["type_method_definition_dispatch"]["SparseXPU"] = declaration[
                "name"
            ]
            schemas.remove(declaration["schema_string"])
...
def main():
    ...
    parser.add_argument("--declarations-path", help="path to Declarations.yaml")
    parser.add_argument("--out", help="path to output directory")
    parser.add_argument(
        "--source-path", help="path to source directory for ATen", default="."
    )
