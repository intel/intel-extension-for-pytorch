...
def get_opvector(args, backend=""):
    if args.key == "CUDA":
        cmd = (
            "python -c 'import torch; torch._C._dispatch_print_registrations_for_dispatch_key(\""
            + backend
            + "\")' > /tmp/log.txt"
        )
    else:
        cmd = (
            "python -c 'import torch; import intel_extension_for_pytorch; torch._C._dispatch_print_registrations_for_dispatch_key(\""
            + backend
            + "\")' > /tmp/log.txt"
        )
...
def main():
    ...
    native_yaml_path = os.path.join(
        scripts_root, "scripts/tools/torchgen/yaml/native_functions.yaml"
    )
    ...
    output_data = pd.DataFrame(
        columns=col_name, index=row_name, data=op_registration_backend
    )
    ...
    merged_file = reduce(
        lambda left, right: pd.merge(left, right, on=[op_label, op_label]), merged_data
    )
    ...
    op_category = [[""] * 3 for _ in range(op_size + 2)]
    ...
    op_category_df = pd.DataFrame(
        columns=["structured_delegate", "register", "register or not"], data=op_category
    )
