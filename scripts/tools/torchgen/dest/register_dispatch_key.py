...
class RegisterDispatchKey:
    ...
    lazy_reorder_no_variable_list = dict(
        {
            "resize_": ["self"],
            "as_strided": ["self"],
            "as_strided_": ["self"],
...
"""
    ...
    def get_lazy_reorder(self, schema_name, arg, exprs_symint):
        cptype = str(arg.argument.type)
        argtype = str(arg.type)
        if argtype in [
            "c10::SymIntArrayRef",
            "c10::SymInt",
            "c10::optional<c10::SymInt>",
        ...
        if schema_name not in self.lazy_reorder_block_list:
            if cptype == "Tensor[]":
                return (
                    self.LAZY_REORDER_TENSORLIST.substitute(change_dict),
                    change_dict["temp_name"],
                )
            elif cptype == "Tensor":
                if arg.type == "const c10::optional<Tensor>&":
                    return (
                        self.LAZY_REORDER_OPTIONAL_TENSOR.substitute(change_dict),
                        change_dict["temp_name"],
                    )
                elif not arg.type.startswith("const"):
                    return (
                        self.LAZY_REORDER_TENSOR.substitute(change_dict),
                        change_dict["name"],
                    )
                elif (
                    schema_name in self.lazy_reorder_no_variable_list.keys()
                    and arg.name in self.lazy_reorder_no_variable_list[schema_name]
                ):
                    return (
                        self.LAZY_REORDER_CONST_NO_VARIABLE_TENSOR.substitute(
                            change_dict
                        ),
                        change_dict["name"],
                    )
                    ...
                    return (
                        self.LAZY_REORDER_CONST_TENSOR.substitute(change_dict),
                        change_dict["temp_name"],
                    )
...
"""
  ...
  const DeviceGuard device_guard(device_or_default(device));"""
                ...
                if self.simple_trace:
                    simple_trace_code = (
                        f'xpu::dpcpp::SimpleTrace trace("{name} -> {impl_name}");'
                    )

                # lazy_reorder
                lazy_reorder = "  // no lazy reorder"
                lazy_reorder_info = [
                    self.get_lazy_reorder(metadata.kernel, arg, exprs_symint)
                    for arg, exprs_symint in zip(args, args_exprs_list_symint)
                ]

                lazy_reorder_list = [
                    elem[0] for elem in lazy_reorder_info if len(elem[0]) > 0
                ]
