...
python -m pip install torch==${VER_PYTORCH}+cpu torchvision==${VER_TORCHVISION}+cpu torchaudio==${VER_TORCHAUDIO}+cpu --extra-index-url https://download.pytorch.org/whl/cpu
...
mkdir build
cd build
cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD=X86 -DLLVM_ENABLE_TERMINFO=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_INCLUDE_EXAMPLES=OFF ../llvm/
cmake --build . -j $(nproc)
