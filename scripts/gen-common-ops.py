...
def generate_aten_to_ipex(ctx, tree, rwxtree, fname, sig, rwsig, params, fnopts):
    ...
    for p in params:
        ...
        if cptype == 'TensorList':
            how_to_check_device = ('(check_device_by_tensor_list({}, DPCPPSubDev::CPU))').format(pname)
            break
        elif cptype == 'TensorOptions':
            how_to_check_device = ('(check_device_by_options({}, DPCPPSubDev::CPU))').format(pname)
            break
        elif cptype == 'Tensor':
            how_to_check_device = ('(check_device_by_tensor({}, DPCPPSubDev::CPU))').format(pname)
            break
    ...
    cpu_ops_class_name = "AtenIpexCPUAlias" if fn_with_inplace_semantic(fname) else "AtenIpexCPUDefault"
    # Check device type
