...
quantizer_xpu_module = ipex.deepspeed
ipex_device = 'xpu:0'
...
def run_quant_dequant(inputs, groups, bits):
    return quantizer_xpu_module.ds_quantize_fp16(inputs, groups, bits)
...
def test_fake_quant_dequant(tensor_shape, groups):
    ...
    input_tensor = torch.rand((tensor_shape), dtype=torch.float16).to(ipex_device)
