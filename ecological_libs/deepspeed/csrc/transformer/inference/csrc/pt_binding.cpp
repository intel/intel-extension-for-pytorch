...
std::vector<at::Tensor> padd_add_transform(
  ...
  int padded_head_size = add_padding
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
      "gated_activation", &ds_gated_activation, "DeepSpeed Bias GEGLU (SYCL)");
  ...
  m.def(
      ...
      &apply_rotary_pos_emb,
      "DeepSpeed mlp with fp16 (SYCL)");
  m.def(
      "moe_res_matmul",
      &moe_res_matmul,
      "DeepSpeed moe residual matmul (SYCL)");
  m.def("reset_cache", &reset_cache, "Reset Cache for generation tasks");
  m.def(
      "release_workspace",
      &ds_release_workspace,
...
#define DEF_OPS(_name, _dtype)                                                 \
  ...
  m.def(                                                                       \
      "bias_gelu_" #_name,                                                     \
      &ds_bias_gelu<_dtype>,                                                   \
      "DeepSpeed Gelu with " #_name " (SYCL)");                                \
  m.def(                                                                       \
      "bias_add_" #_name,                                                      \
      &ds_bias_add<_dtype>,                                                    \
      "DeepSpeed Bias Add with " #_name " (SYCL)");                            \
  m.def(                                                                       \
      "bias_relu_" #_name,                                                     \
      &ds_bias_relu<_dtype>,                                                   \
      "DeepSpeed ReLU with " #_name " (SYCL)");                                \
  m.def(                                                                       \
